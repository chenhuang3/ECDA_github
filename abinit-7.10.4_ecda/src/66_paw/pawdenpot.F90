...
!!
!! CHILDREN
!!      free_my_atmtab,get_my_atmtab,pawdensities,pawdijhartree,pawdijso
!!      pawrad_deducer0,pawuenergy,pawxc,pawxc3,pawxcm,pawxcm3,pawxcmpositron
!!      pawxcpositron,pawxenergy,pawxpot,poisson,setnoccmmp,timab,wrtout
!!      xmpi_sum
!!
!! SOURCE
...
!scalars
 ...
 integer,optional,intent(in) :: mpi_comm_atom !,usefock
...
!scalars
 ...
 integer :: jrhoij,kklmn,klmn,lm_size,lmn2_size,mesh_size,my_comm_atom,nkxc1,nspdiag,nsppol,opt_compch
...
!arrays
 ...
 real(dp) :: rdum2(0,0),rdum3(0,0,0),rdum4(0,0,0,0)
...
! *************************************************************************
 ...
 if (my_natom>0) then
   if(paw_ij(1)%has_dijhartree==0.and. .not.(ipert==natom+1.or.ipert==natom+5)) then
...
&   .not.(ipert==natom+1.or.ipert==natom+5)) then
...
&     mpi_comm_atom=my_comm_atom,mpi_atmtab=mpi_atmtab)
   else
     call setnoccmmp(1,0,rdum4,0,0,idum3,my_natom,natom,0,1,nsppol,0,ntypat,&
&     paw_ij,pawang,pawprtvol,pawrhoij,pawtab,rdum2,idum1,idum1,0,usepawu)
   end if
...
&   (option/=2.or.pawrhoij(iatom)%cplex==2)) then
     call pawdijso(paw_ij(iatom)%cplex_dij,paw_ij(iatom)%dijso,paw_ij(iatom)%ndij,&
&     nspden,pawang,pawrad(itypat),pawtab(itypat),pawxcdev,spnorbscl,&
...
!   if (present(usefock).and.usefock==1) then 
!     if (ipositron/=1) then
!       call pawdijfock(cplex,paw_ij(iatom)%cplex_dij,paw_ij(iatom)%dijfock, &
!&       paw_ij(iatom)%ndij,nspden,nsppol,pawrhoij(iatom),pawtab(itypat))
!     else
!       paw_ij(iatom)%dijfock(:,:)=zero
!     end if
!     paw_ij(iatom)%has_dijfock=2

!  Fock energy computation
!     if (option/=1) then
!       if ((cplex==1).or.(ipert==0)) then
...
!             klmn=pawrhoij(iatom)%rhoijselect(irhoij)
!             ro(1)=pawrhoij(iatom)%rhoijp(jrhoij,ispden)*pawtab(itypat)%dltij(klmn)
!             eexex=eexex+half*ro(1)*paw_ij(iatom)%dijfock(klmn,ispden)
!             jrhoij=jrhoij+pawrhoij(iatom)%cplex
!           end do
!         end do
!       else
...
!             klmn=pawrhoij(iatom)%rhoijselect(irhoij);kklmn=2*klmn-1
!             ro(1:2)=pawrhoij(iatom)%rhoijp(jrhoij:jrhoij+1,ispden)*pawtab(itypat)%dltij(klmn)
!             eexex=eexex+half*ro(1)*paw_ij(iatom)%dijfock(kklmn,ispden)+half*ro(2)*paw_ij(iatom)%dijfock(kklmn+1,ispden)
!             jrhoij=jrhoij+pawrhoij(iatom)%cplex
!           end do
!         end do
!       end if
