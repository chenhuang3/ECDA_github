...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (FJ,MT,BA)
...
!!
!! CHILDREN
!!      free_my_atmtab,get_my_atmtab,mat_mlms2jmj,mat_slm2ylm,paw_ij_destroy
!!      paw_ij_gather,paw_ij_nullify,pawio_print_ij,pawrhoij_destroy
!!      pawrhoij_gather,pawrhoij_nullify,setnoccmmp,wrtout,xcomm_group
!!      xgroup_free,xmpi_group_translate_ranks
!!
!! SOURCE
...
&                 mpi_atmtab,mpi_comm_atom) ! optional arguments (parallelism)
 ...
 use m_paw_ij,           only : paw_ij_type, paw_ij_destroy, paw_ij_nullify, paw_ij_gather
 use m_pawrhoij,         only : pawrhoij_type, pawrhoij_destroy, pawrhoij_gather, pawrhoij_nullify
 use m_pawio,            only : pawio_print_ij
 use m_sphharm,          only : mat_mlms2jmj, mat_slm2ylm
...
!Set up parallelism over atoms
 paral_atom=(present(mpi_comm_atom).and.(my_natom/=dtset%natom))
 ...
 me_atom=xcomm_rank(my_comm_atom)

!Continue only if mpi_comm_atom contains the master of the output comm
 if (paral_atom) then
   call xcomm_group(abinit_comm_output,group1,ierr)
   call xcomm_group(my_comm_atom    ,group2,ierr)
   call xmpi_group_translate_ranks(group1,1,(/0/),group2,idum,ierr)
   call xgroup_free(group1)
   call xgroup_free(group2)
