...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, MF, AR, MM, MT, FJ, MB, MT)
...
!!  usecprj=1 if cprj datastructure has to be output
...
&           ucvol,usecprj,wffnew,vtrial,wvl,xred,ylm,ylmgr,ylmdiel,vxctau)
 ...
 use m_pawrhoij,           only : pawrhoij_type, pawrhoij_alloc, pawrhoij_destroy, pawrhoij_io
 use m_pawcprj,            only : pawcprj_type, pawcprj_alloc, pawcprj_diskinit_w, pawcprj_destroy
...
&                                 load_paw_hamiltonian,finalize_hamiltonian,gs_hamiltonian_type
 ...
 use BigDFT_API,           only : cprj_clean,cprj_paw_alloc,nullify_cprj_objects,&
&                                 last_orthon, evaltoocc, write_energies
#endif
...
!Arguments -------------------------------
 ...
 integer, intent(in) :: atindx(natom),atindx1(natom),dimcprj(natom*psps%usepaw),gbound_diel(2*mgfftdiel+8,2)
...
!Local variables-------------------------------
 ...
 integer :: bdtot_index,counter
 integer :: cplex,dimffnl,enunit
 integer :: iband,iband1,ibdkpt
 integer :: ibg,icg,ider,idir
 integer :: ierr,ifft,ifor,ifor1,ii,ikg,ikpt
 integer :: ikpt_loc,ikpt1,ikpt_this_proc,ikxc,ilm,imagn,index1,iorder_cprj,ipert,iplex
 integer :: iscf,ispden,isppol,istwf_k
 integer :: mband_cprj,mbdkpsp,mb2dkpsp
 ...
 integer :: nband_k,nband_cprj_k,nbuf,ndatarecv,neglect_pawhat,nfftot,nkpg,nkpt1,nnn,nnsclo_now
 integer :: nproc_distrb,npw_k,nspden_rhoij,option,prtvol
 integer :: spaceComm_distrb,usecprj_local,usetimerev
 logical :: berryflag,computesusmat,fixed_occ,orbmagflag
...
!Initializations for PAW
 n4=dtset%ngfft(4) ; n5=dtset%ngfft(5) ; n6=dtset%ngfft(6)
 my_nspinor=max(1,dtset%nspinor/mpi_enreg%nproc_spinor)
 paral_atom=(my_natom/=natom)
 compch_fft=-1.d5

 usecprj_local=0; mcprj_local=0 ; mband_cprj=0 ; !cprj_ptr => cprj
 if (psps%usepaw==1) then
   usecprj_local=1 ; iorder_cprj=0
   ...
   if (usecprj==1) then
     ...
     !this is a check but swhould always be true since scfcv allocated cprj anyway
     ...
     !ABI_DATATYPE_ALLOCATE(cprj_ptr,(dtset%natom,mcprj_local))
...
! In case of parallelized HF calculation, modify me_distrb
 if ((xmpi_paral==1).and.(mpi_enreg%paral_hf==1)) me_distrb=mpi_enreg%me_kpt
...
!wvlbigdft - Branching with a separate vtorho procedure in wavelet.
 if (dtset%usewvl == 1) then
#if defined HAVE_DFT_BIGDFT
   ABI_ALLOCATE(xcart,(3, dtset%natom))
   call xred2xcart(dtset%natom, rprimd, xcart, xred)

!  PAW: Copy objects to BigDFT ones (this is not yet implemented for wvlbigdft case).
   if(dtset%usepaw==1 .and. .not. wvlbigdft) then
     call paw2wvl_ij(my_natom,1,paw_ij,wvl%descr)
     ABI_DATATYPE_ALLOCATE(wvl%descr%paw%cprj,(dtset%natom,mcprj_local))
!    allocate cprj
     call cprj_paw_alloc(wvl%descr%paw%cprj,0,dimcprj)
   end if
...
!    This might accelerate convergence:
     wvl%wfs%ks%diis%energy_min=1.d0
     wvl%wfs%ks%diis%alpha=2.d0
...
!  For PAW: we will compute R_ij at the end.
   if(wvlbigdft) return
#else
...
!  Magnetization: set flag to turn on various behaviors
   orbmagflag = (dtset%orbmag > 0) 
...
!    Also take into account the spin.
     if(dtset%nspden/=4)then
       if (psps%usepaw==0.or.pawfgr%usefinegrid==0) then
         ...
         if(with_vxctau) then
           do ii=1,4
             call fftpac(isppol,mpi_enreg,dtset%nspden,n1,n2,n3,n4,n5,n6,dtset%ngfft,vxctau(:,:,ii),vxctaulocal(:,:,:,:,ii),2)
           end do
         end if
       else
...
!    in berryphase_new.F90, ikpt_loc ALSO includes info about value of isppol.
     ...
     do while (ikpt_loc < nkpt1)
       ...
       if ( (.not.berryflag) .and. (.not.orbmagflag) ) then
         ikpt_loc = ikpt_loc + 1
         ikpt = ikpt_loc
       else
       ...
       if(xmpi_paral==1)then
         ...
         if ( (.not.berryflag) .and. (.not.orbmagflag) ) then
           ...
           if ((proc_distrb_cycle(mpi_enreg%proc_distrb,ikpt,1,nband_k,isppol, &
&           me_distrb)).and.(ikpt_loc <= dtset%mkmem)) then
             eigen(1+bdtot_index : nband_k+bdtot_index) = zero
             resid(1+bdtot_index : nband_k+bdtot_index) = zero
             bdtot_index = bdtot_index + nband_k
             cycle
           ...
           if(orbmagflag) then
             mcgq = dtset%mpw*my_nspinor*nband_k*dtbfield%nneigh(ikpt)
             ikg = dtbfield%kgindex(ikpt)
           end if
           mkgq = 6*dtset%mpw
         end if ! berryflag, orbmagflag

       end if ! parallel kpt

       if (berryflag .or. orbmagflag) ikpt_loc = ikpt_loc + 1
...
!      corresponding phase factors (in case of tnons)
       ...
       if ( (berryflag .or. orbmagflag) .and. xmpi_paral==1 ) then

         call cgq_builder(berryflag,cg,cgq,dtbfield,dtefield,dtset,ikpt,ikpt_loc,isppol,mcg,mcgq,&
&         me_distrb,mkgq,mpi_enreg,my_nspinor,nband_k,nproc_distrb,&
&         npwarr,orbmagflag,pwnsfac,pwnsfacq,pwind_alloc,spaceComm_distrb)
       ...
       if (orbmagflag) dtbfield%sflag(:,ikpt + dtset%nkpt*(isppol - 1),1:2,1:3) = 0
...
&         kpoint,psps%lmnmax,gs_hamk%matblk,dtset%mgfft,dtset%mkmem,mpi_enreg,nkpg,npw_k,ntypat,1,ph3d)
       ...
       if(psps%usepaw == 1 .and. dtset%wfoptalg == 1 .and. istep <= 1) then
         ...
         call make_invovl(ikpt, gs_hamk, dimffnl, ffnl, ph3d, mpi_enreg)
       end if
       ...
       if(gemm_nonlop_use_gemm) then
         if(mpi_enreg%paral_kgb == 1) then
           ...
           gemm_nonlop_ikpt_this_proc_being_treated = ikpt_this_proc
           if(istep <= 1) then
             ! Init the arrays
             call make_gemm_nonlop(ikpt_this_proc, gs_hamk, dimffnl, bandfft_kpt(ikpt_this_proc)%ffnl_gather,&
&             bandfft_kpt(ikpt_this_proc)%ph3d_gather, bandfft_kpt(ikpt_this_proc)%ndatarecv)
           end if
         else
           gemm_nonlop_ikpt_this_proc_being_treated = ikpt
           if(istep <= 1) then
             call make_gemm_nonlop(ikpt, gs_hamk, dimffnl, ffnl, ph3d, npw_k)
           end if
         end if
       end if
...
&         dtset,eig_k,ek_k,ek_k_nd,enl_k,fixed_occ,ffnl,grnl_k,gs_hamk,fock,&
&         ibg,icg,ikpt,iscf,isppol,kg_k,kinpw,kpg_k,mband_cprj,mcg,mcgq,mcprj_local,mkgq,&
&         mpi_enreg,dtset%mpw,natom,nband_k,nkpg,dtset%nkpt,nnsclo_now,npw_k,npwarr,ntypat,&
&         occ_k,optforces,ph3d,prtvol,pwind,pwind_alloc,pwnsfac,pwnsfacq,resid_k,&
&         rhoaug,paw_dmft,usecprj_local,vlocal,dtset%wtk(ikpt),zshift,vxctaulocal=vxctaulocal)
       else
         call vtowfk(cg,cgq,cprj,cpus,dimcprj,dimffnl,dphase_k,dtefield,dtfil,&
&         dtset,eig_k,ek_k,ek_k_nd,enl_k,fixed_occ,ffnl,grnl_k,gs_hamk,fock,&
&         ibg,icg,ikpt,iscf,isppol,kg_k,kinpw,kpg_k,mband_cprj,mcg,mcgq,mcprj_local,mkgq,&
&         mpi_enreg,dtset%mpw,natom,nband_k,nkpg,dtset%nkpt,nnsclo_now,npw_k,npwarr,ntypat,&
&         occ_k,optforces,ph3d,prtvol,pwind,pwind_alloc,pwnsfac,pwnsfacq,resid_k,&
&         rhoaug,paw_dmft,usecprj_local,vlocal,dtset%wtk(ikpt),zshift)
       end if

       call timab(985,1,tsec)

!      update magnetizations at this k point do this here BEFORE ffnl, kpg_k, ph3d are deallocated
       if(orbmagflag) then
         call update_orbmag(cg,cgq,cprj,dimffnl,dtbfield,ffnl,dtfil%filstat,gs_hamk,&
&         icg,ikpt,isppol,kg,kinpw,dtset%mband,mcg,mcgq,mcprj_local,mkgq,dtset%mkmem,&
&         mpi_enreg,dtset%mpw,natom,dtset%nkpt,npw_k,npwarr,dtset%nsppol,ntypat,&
&         dtset%paral_kgb,pawtab,ph3d,prtvol,pwind,pwind_alloc,pwnsfac,pwnsfacq,vlocal)
       end if
...
!wavelets are treated in wvl_occ()
 if(dtset%usewvl==0)then
   if((.not.fixed_occ) .and. (iscf>0.or.iscf==-3)) then
...
&       rhog,rhor,rprimd,tim_mkrho,ucvol,wvl%wfs,wvl%den)
     else
       call mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,paw_dmft,phnons,&
&       rhowfg,rhowfr,rprimd,tim_mkrho,ucvol,wvl%wfs,wvl%den)
     end if
...
!    Compute the highest occupied eigenenergy
     if(iscf/=-1 .and. iscf/=-2)then
       ...
       if(mpi_enreg%paral_kgb/=1) then
         call xmpi_max(energies%e_fermie,emax,spaceComm_distrb,ierr)
         energies%e_fermie=emax
       else
         if ((xmpi_paral==1) .and. (mpi_enreg%paral_kgb==1)) then
           call xmpi_max(energies%e_fermie,emax,mpi_enreg%comm_kpt,ierr)
           energies%e_fermie=emax
         end if
       end if
...
!Compute the kinetic energy density
 if(dtset%usewvl==0) then
   if(dtset%usekden==1 .and. (iscf > 0 .or. iscf==-3 ) )then
     call mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,paw_dmft,phnons,&
&     taug,taur,rprimd,tim_mkrho,ucvol,wvl%wfs,wvl%den,option=1)
   end if
...
!In the non-self-consistent case, print eigenvalues and residuals
 if(iscf<=0 .and. dtset%usewvl==0)then
...
&       ucvol,dtfil%unpaw,dtfil%unkg,dtfil%unylm,0,wffnew,xred,ylm,ylmgr_dum)
       call pawmkrhoij(atindx,atindx1,cprj_tmp,dimcprj,dtset%istwfk,dtset%kptopt,dtset%mband,mband_cprj,&
&       mcprj_tmp,dtset%mkmem,mpi_enreg,natom,dtset%nband,dtset%nkpt,dtset%nspinor,dtset%nsppol,&
&       occ,dtset%paral_kgb,paw_dmft,dtset%pawprtvol,pawrhoij_unsym,&
...
!      In WVL: copy density to BigDFT object:
       call wvl_rho_abi2big(nfftf,dtset%nspden,1,rhor,wvl%den)
     end if
     if (paral_atom) then
       call pawrhoij_destroy(pawrhoij_unsym)
...
!  &       computesusmat       )then
   ...
   if(computesusmat) then
     ...
     !call suscep_stat(atindx,atindx1,cg,cprj_ptr,dielar,&
...
!This is done in scfcv since cprj has first been allocated in scfcv.
! vtorho just change the size if necessary
!!PAW: deallocate <p|c> (cprj)
! if (psps%usepaw==1) then
!   if (usecprj==0.and.usecprj_local==1) then
!     if(dtset%mkmem/=0) then
!       call pawcprj_destroy(cprj)
!       !call pawcprj_destroy(cprj_ptr)
!     end if
!     usecprj_local=0
...
!Deallocations for WVL+PAW:
 if(dtset%usepaw==1 .and. dtset%usewvl==1) then
#if defined HAVE_DFT_BIGDFT
   call cprj_clean(wvl%descr%paw%cprj)
   ABI_DATATYPE_DEALLOCATE(wvl%descr%paw%cprj)
#endif
   call paw2wvl_ij(my_natom,2,paw_ij,wvl%descr)
 end if
...
&       istep,inonsc, dtset%iscf, dtset%ixc,mpi_enreg%me_wvl,dtset%natom,&
...
!      Direc minimization technique: no diagonalization
       call wvl_psitohpsi(dtset%diemix,energies%e_exactX,energies%e_xc,energies%e_hartree,&
&       energies%e_kinetic, energies%e_localpsp, eproj, energies%e_sicdc,&
&       istep,ii, dtset%iscf, dtset%ixc,mpi_enreg%me_wvl,dtset%natom,&
&       nfftf,mpi_enreg%nproc_wvl,dtset%nspden,&
&       raux,do_scf,energies%e_xcdc,wvl,wvlbigdft,xcart,strsxc)
...
&     dtset%ixc, mpi_enreg%me_wvl, dtset%natom, nfftf, mpi_enreg%nproc_wvl,&
...
!!
!! COPYRIGHT
!!  Copyright (C) 2013-2014 ABINIT group (D.Caliste, T. Rangel)
!!  This file is distributed under the terms of the
!!  GNU General Public License, see ~abinit/COPYING
!!  or http://www.gnu.org/copyleft/gpl.txt .
!!
