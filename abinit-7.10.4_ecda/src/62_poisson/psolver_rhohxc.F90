...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR,TRangel).
...
! use defs_basis,only: std_out,std_out_default
 ...
 use m_xmpi, only: xcomm_rank,xcomm_size,xmpi_sum

#if defined HAVE_DFT_BIGDFT
 use poisson_solver, only : H_potential !,psolver
! use Poisson_Solver
 use BigDFT_API, only : CHARGE_DENSITY,HARTREE_POTENTIAL,&
& xc_potential,ELECTRONIC_DENSITY,deallocate_coulomb_operator,&
...
!  Pending, next line should follow the same logic that the rest
   ...
   if(usepaw==1) then
     ...
     call wvl_rhov_abi2big(nfft,1,1,xccc3d,rhocore)
...
!  rhohat => nhat !do not know how to point 4index to 2 index
!  here we have to copy since convention for spin changes.
   ABI_ALLOCATE(rhohat,(n1i,n2i,n3d,nspden))
   call wvl_rhov_abi2big(nfft,nspden,1,nhat,rhohat)
 else
   nullify(rhohat)
 end if
...
!  copy rhor again into rhonow following the BigDFT convention:  
   call wvl_rhov_abi2big(nfft,nspden,1,rhor,rhonow) 
...
!  write(message, '(a,a,a,a)' ) ch10, ' rhohxc_wvlpaw : but why are you copying me :..o('
   call wvl_vhartr_abi2big(nfft,1,vhartr,wvl_den)
!  
!  (this can be commented out, since we do not use denspot%v_xc
   call wvl_vxc_abi2big(nfft,nspden,1,vxc,wvl_den)
!  
 end if
