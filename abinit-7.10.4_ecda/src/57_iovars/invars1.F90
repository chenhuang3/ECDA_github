...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, AR, MKV)
...
!real(dp) :: field(3)
 real(dp),allocatable :: amu(:),dprarr(:),kpt(:,:),mixalch(:,:)
...
!---------------------------------------------------------------------------
 ...
 response=0
 if(rfddk/=0.or.rfelfd/=0.or.rfphon/=0.or.rfstrs/=0.or.rfuser/=0)response=1
...
!  Call ingeo for each image in turn, with the possible default values
   ...
   call ingeo(acell,amu,dtset,bravais,dtset%genafm(1:3),iatfix,&  !!HONG
...
&   dtset%nsym,ntypalch,dtset%ntypat,dtset%pawspnorb,dtset%ptgroupma,ratsph,&
...
&   kpt,dtset%kptopt,kptnrm,dtset%kptrlatt,kptrlen,lenstr,msym,&
