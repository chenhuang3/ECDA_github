...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, MM)
...
!scalars
 ...
 integer :: lpawu,lpawu1,narr,multi_lpawu,multi_natpawu,mxnsp
 integer :: natom,natpawu,nimfrqs,nimage
...
&       (/ narrm(idtset) /) )
   ...
   call prttagm(dprarr,intarr,iout,jdtset_,5,marr,narr,narrm,ncid,ndtset_alloc,'dmatpawu','DPR',&
&   multi_lpawu+multivals%nsp+multi_natpawu)
 end if
...
&   mxvals%nimage,nimagem,ndtset,prtimg,strimg)
 ...
 !firstchar_fftalg=firstchar_gpu
 !if (firstchar_fftalg==' '.and.any(dtsets(1:ndtset_alloc)%ngfft(7)==314)) then
 !  firstchar_fftalg='-';if (any(mod(dtsets(:)%wfoptalg,10)==4)) firstchar_fftalg='_'
 !end if
 firstchar_fftalg = "_"
