...
!!
!! COPYRIGHT
!! Copyright (C) 2010-2014 ABINIT group (GG,MT)
...
! Arrays
  ...
  real(dp),pointer :: zeta_prev(:,:,:,:)
  real(dp),pointer :: zeta     (:,:,:,:)
  real(dp),pointer :: zeta_next(:,:,:,:)
  real(dp),pointer :: dzeta    (:,:,:,:)
 end type pimd_type
!!***
...
subroutine pimd_init(dtset,pimd_param)
...
!************************************************************************
 ...
 if((dtset%imgmov==9).or.(dtset%imgmov==13))then
   ...
   pimd_param%traj_unit=-1;if (dtset%useria==37) pimd_param%traj_unit=4321
...
!************************************************************************
 ...
 nullify(pimd_param%zeta_prev)
 nullify(pimd_param%zeta)
 nullify(pimd_param%zeta_next)
 nullify(pimd_param%dzeta)
...
!************************************************************************
 ...
 if (associated(pimd_param%zeta_prev)) then
   ABI_DEALLOCATE(pimd_param%zeta_prev)
 end if
 if (associated(pimd_param%zeta)) then
   ABI_DEALLOCATE(pimd_param%zeta)
 end if
 if (associated(pimd_param%zeta_next)) then
   ABI_DEALLOCATE(pimd_param%zeta_next)
 end if
 if (associated(pimd_param%dzeta)) then
...
&                        temperature,trotter,vel,volume,xcart)
...
!scalars
 integer,intent(in) :: natom,trotter
 real(dp),intent(in) :: temperature,volume
!arrays
...
!scalars
 ...
 real(dp) :: stress_tmp(3,3,3),omega2,kt
 character(len=500) :: msg
