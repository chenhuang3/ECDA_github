...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, FD)
...
!scalars
 ...
 real(dp) :: rrd,rrf,rrp,rrs,yp1,ypn, ehomo, radfine
...
!Additionnal wavelet parameters
 if (dtset%usewvl == 1) then
   read (tmp_unit, '(a100)', iostat = ios) line
   if (ios /=0) then
     line=''
   end if
!  We try to read the values from the pseudo.
   read (line, *, iostat = ios) rad_long, rad_core, rad_cov
   if (ios /= 0 ) then
     read (line, *, iostat = ios) rad_long, rad_core
     rad_cov = rad_core
   end if
   if (ios /= 0 .or. rad_long == zero) then
#if defined HAVE_DFT_BIGDFT
...
!    the maximum radii is useful only for projectors
   ...
   if (maxrad == zero) then
     ...
     rad_cov = max(min(dtset%wvl_crmult * rad_long, &
&     real(15.0, dp) * maxrad) / dtset%wvl_frmult, rad_core)
   end if
   write(message, '(a,f12.7,a,f12.7,a,f12.7)' )&
&   '  radii_cf(1)=', rad_long,'; radii_cf(2)=', rad_core, "; rad_cov=", rad_cov
 ...
 psps%gth_params%radii_cf(ipsp, 1:3) = (/ rad_long, rad_core, rad_cov /)
 psps%gth_params%radii_cov(ipsp)     = rad_cov
 if (rad_core < zero .or. rad_long < zero) then
