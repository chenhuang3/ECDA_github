...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2014 ABINIT group (XG,GMR,MF)
...
!!
!! CHILDREN
!!      getfreqsus,getmpw,getng,hdr_free,init_distribfft,inwffil3,ioarr,kpgio
!!      metric,mkrdim,newocc,setsym,sphereboundary,status,suscep_dyn
!!      suscep_stat,timab,wffclose,wrtout,xcacfd
!!
!! SOURCE
...
!Get diecut, and the fft grid to be used for the susceptibility computation
 ...
 call getng(dtset%boxcutmin,ecutsus,gmet,k0,mpi_enreg%me_fft,mgfftdiel,nfftdiel,ngfftdiel,&
& mpi_enreg%nproc_fft,nsym,dtset%fft_opt_lob,mpi_enreg%paral_kgb,dtset%symrel,&
& use_gpu_cuda=dtset%use_gpu_cuda)
!Update the fft distribution
 call init_distribfft(mpi_enreg%distribfft,'f',mpi_enreg%nproc_fft,ngfftdiel(2),ngfftdiel(3))
...
!Compute new occupation numbers if needed
 ...
 if(dtset%occopt>=3.and.dtset%occopt<=8) then
   ...
   call newocc(doccde,eigen,entropy,fermie,dtset%spinmagntarget,mband,dtset%nband,&
...
!  Compute the static susceptibility matrix
   lmax_diel=0
   ABI_ALLOCATE(atindx_dum,(dtset%natom))
   usetimerev=1;neglect_pawhat=0
   call suscep_stat(atindx_dum,atindx_dum,cg,cprj,&
...
&   0,dtfil%unpaw,0,usetimerev,dtset%wtk,ylmdiel)
   ABI_DEALLOCATE(atindx_dum)
...
&   nsym,occ,phnonsdiel,rhor,rprimd,ucvol,wght_freq)
...
&   dtset%symrel,dtset%tnons,ucvol,dtset%wtk)
