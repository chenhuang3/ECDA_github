...
!Check arguments
 if ( nfreqs == 0 ) then
   message = ' freqs and weights must be of nonzero size, however nfreqs = 0'
   MSG_ERROR(message)
 end if
...
&         '  Using preassigned mesh for Be/Be2 with 24 frequencies',ch10
       ...
       case default
         write(message,'(4a,i0)' )' No preassigned mesh for nfreqs=',nfreqs
         MSG_ERROR(message)
     end select
...
!    Check arguments
     if ( mod(nfreqs,2) == 1 ) then
       write(message, '(a,i0)' )' freqs and weights must be of even size, however nfreqs =',nfreqs
       MSG_ERROR(message)
     end if
...
!    First build generic mesh for [-1,1] interval
     ...
     if ( i > gl_maxiter ) then
       write(message, '(a,i0,a)' )' Too many iterations (',i,') while building Gauss-Legendre mesh'
       MSG_ERROR(message)
     end if
...
!    Any other option raises an error
   case default
     ...
     write(message, '(a,i0)' )' Unrecognized value for optfreq: ',optfreq
     MSG_ERROR(message)
 end select
