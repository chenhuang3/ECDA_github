...
!!
!! COPYRIGHT
!! Copyright (C) 2008-2014 ABINIT group (MG, GMR, VO, LR, RWG, MT)
...
!! SOURCE
 ...
 type,public :: kmesh_t
  ...
  integer :: has_tetra=0
    ! 0 if tetrahedra are not computed
    ! 1 if tetra tabs are stored in memory
    ! -1 if tetra tab cannot be computed (e.g we have a k-path instead of a k-mesh)
 ...
 !arrays
  ...
  ! Table for the tetrahedron method
 ...
 public :: init_tetra_tabs       ! Initialize tables for the tetrahedron integration scheme. 
...
!!
!! PARENTS
!!      cchi0q0_intraband,m_bz_mesh,mlwfovlp_qp,mrgscr,setup_bse
!!      setup_bse_interp,setup_screening,setup_sigma
!!
!! CHILDREN
!!
!! SOURCE
...
!!
!! PARENTS
!!      bethe_salpeter,cchi0q0_intraband,m_shirley,mlwfovlp_qp,mrgscr,screening
!!      sigma
!!
!! CHILDREN
!!
...
!types
 call destroy_tetra(Kmesh%Tetra)
...
!!
!! PARENTS
!!      mrgscr,setup_bse,setup_bse_interp,setup_screening,setup_sigma
!!
!! CHILDREN
!!
!! SOURCE
...
!!      exc_build_block,exc_build_ham,m_bz_mesh,m_dyson_solver,m_eet,m_ppmodel
!!      m_screen,m_screening,m_shirley,m_vcoul,m_wfs,paw_symcprj
!!      prep_calc_ucrpa,random_stopping_power,read_plowannier,setup_bse
...
!! [want_tetra]= True if the tables needed for integration with tetrahedron methods are wanted. default: False
...
&  want_tetra,vacuum,break_symmetry)  ! Optional
...
!scalars
 ...
 logical,optional,intent(in) :: want_tetra,break_symmetry
 type(kmesh_t),intent(inout) :: Kmesh
...
!scalars
 ...
 integer :: timrev,iscf,ierr,nkbz,nkibz,nkpt_computed,my_nshiftk
 real(dp) :: kptrlen
 character(len=500) :: msg
 logical :: my_break_symmetry,do_tetra
!arrays
 integer :: my_vacuum(3)
 real(dp),allocatable :: kibz(:,:),wtk(:),my_shiftk(:,:)
...
&  Cryst%nsym,Cryst%rprimd,my_shiftk,Cryst%symafm,Cryst%symrel,my_vacuum,wtk)
 ...
 do_tetra = .False.; if (present(want_tetra)) do_tetra = want_tetra
 if (do_tetra) then
   call init_tetra_tabs(Kmesh,Cryst,ierr)
 endif
...
subroutine init_tetra_tabs(Kmesh,Cryst,ierr)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'init_tetra_tabs'
 use interfaces_14_hidewrite
...
! *************************************************************************
 ...
 if (Kmesh%has_tetra==1) then 
   MSG_WARNING("Tetrahedrons already calculated, returning")
   ierr = ierr + 1
   return
 end if
 ...
 ABI_MALLOC(bz2ibz,(Kmesh%nbz))
 ABI_MALLOC(my_fullbz,(3,Kmesh%nbz))
...
&  Kmesh%nshift,Cryst%nsym,Kmesh%shift,Cryst%symrel)
 ...
 if (.not. ALL(ABS(my_fullbz-Kmesh%bz)<tol6)) then
   MSG_WARNING("Mismatch between my_fullbz and Kmesh%bz")
   ierr = ierr + 1
 end if

 if (ierr == 0) then
   call init_tetra(bz2ibz, Cryst%gprimd, qlatt, Kmesh%bz, Kmesh%nbz,Kmesh%Tetra, ierr, errstr)
   if (ierr/=0) MSG_WARNING(errstr)
   write(msg,'(a,i0)')' Number of irreducible tetrahedrons= ',Kmesh%Tetra%ntetra
   call wrtout(std_out,msg,"COLL")
 end if

 Kmesh%has_tetra = 1; if (ierr /= 0) Kmesh%has_tetra = -1

 ABI_FREE(my_fullbz)
 ABI_FREE(bz2ibz)

end subroutine init_tetra_tabs
!!***
...
!!  It is allocated inside the routine. When the subroutine returns, path(3,npts) will
!!  contain the path in reduced coordinates.
!! 
!! PARENTS
!!      m_bz_mesh,m_gamma,m_nesting,m_phonons,mkph_linwid
!!
!! CHILDREN
!!
!! SOURCE
...
!Local variables ------------------------------
!scalars
 integer :: ifound,ik,isym,iq,memory_exhausted,nqall,nqallm,itim
!arrays
 integer :: g0(3)
...
!************************************************************************
 ...
 do 
   memory_exhausted=0
   ABI_MALLOC(qall,(3,nqallm))
   ABI_CHECK_ALLOC('out-of-memory qall')
   nqall=0
...
!!
!! PARENTS
!!      calc_sigc_me,calc_sigx_me,cohsex_me,exc_build_block,gw_tools,m_bz_mesh
!!      prep_calc_ucrpa
!!
!! CHILDREN
!!
...
subroutine findq_count(Kmesh,Qmesh,mg0)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'findq_count'
!End of the abilint section
...
!Local variables-------------------------------
!scalars
 integer :: ikp_bz,ik_bz !,ik_ibz,itim_k,ikp_ibz,itim_kp,isym_k,isym_kp
...
!arrays
 integer :: g0(3)
 integer,pointer :: qibz_count(:),iperm(:)
 real(dp) :: qbz(3),kmkp(3) !kbz(3),kpbz(3),

! *************************************************************************

 ABI_CALLOC(qibz_count, (Qmesh%nibz))

 ! Loop over kp  
 do ikp_bz=1,Kmesh%nbz 
   ...
   do ik_bz=1,ikp_bz 
     ...
     call findqg0(iq_bz,g0,kmkp,Qmesh%nbz,Qmesh%bz,mg0)
     !
     ! * Get iq_ibz, and symmetries from iq_bz
     call get_BZ_item(Qmesh,iq_bz,qbz,iq_ibz,isym_q,itim_q)
     qibz_count(iq_ibz) = qibz_count(iq_ibz) + 1
   end do
 end do

 ! Sort qibz_count in ascending order.
 ABI_MALLOC(iperm, (qmesh%nibz))
 iperm = [(iq_ibz, iq_ibz=1,qmesh%nibz)]

 !call sort_int(qmesh%nibz,qibz_count,iperm,ord="<")

end subroutine findq_count
!!***
...
!Compute reciprocal space metric.
 ...
 call make_path(Kpath%nbounds,bounds,Kpath%gmet,"G",ndiv_small,Kpath%ndivs,Kpath%npts,pts_ptr,unit=dev_null)

 ABI_MALLOC(Kpath%bounds, (3,Kpath%nbounds))
 Kpath%bounds = bounds

 ABI_MALLOC(Kpath%points, (3,Kpath%npts))
 Kpath%points = pts_ptr
 ABI_FREE(pts_ptr)
