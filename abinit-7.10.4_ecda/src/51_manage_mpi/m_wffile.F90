...
!!
!! COPYRIGHT
!! Copyright (C) 2009-2014 ABINIT group (MT,MB,MVer,ZL,MD)
...
!Local variables-------------------------------
!scalars
 integer*2  :: delim_record2(1)
 integer*4  :: delim_record4(1)
 integer*8  :: delim_record8(1)
#if defined HAVE_FC_INT_QUAD
...
!!
!! PARENTS
!!      conducti_nc,conducti_paw,conducti_paw_core,cut3d,emispec_paw,inwffil
!!      inwffil3,ioarr,kss2wfk,linear_optics_paw,loper3,m_wfk,nstdy3,nstpaw3
!!      optic,optics_paw,optics_paw_core,optics_vloc,outwf,uderiv
!!
!! CHILDREN
!!      mpi_file_write_at
!!
...
!  Define all type values
   ...
   write(fildata, "(A,A)") trim(filename), "-etsf.nc"    !MG Why this?
   call etsf_io_low_open_modify(wff%unwff, fildata, lstat, error_data = error)
   if (.not. lstat) then ! Try my_file
     write(fildata, "(A,A,I0,A)") trim(filename), "_", me, "-etsf.nc"
     call etsf_io_low_open_modify(wff%unwff, fildata, lstat, error_data = error)
   end if
   ETSF_CHECK_ERROR(lstat, error)
   write(message, '(3A,I0)' ) ' WffOpen: opening ', trim(wff%fname),"-etsf.nc on unit ", wff%unwff
   call wrtout(std_out, message, 'COLL')
#endif
 else
   write(message, '(10a,i0,3a)' ) ch10, &
&   ' WffOpen : ERROR -',ch10,&
...
!!      kss2wfk,linear_optics_paw,loop3dte,loper3,m_wfk,nonlinear,nstdy3
!!      nstpaw3,optic,optics_paw,optics_paw_core,optics_vloc,outwf,respfn,rrho
!!      scfcv3,suscep,uderiv
!!
!! CHILDREN
!!      mpi_file_write_at
!!
...
! *************************************************************************
 ...
 if (wff%accesswff==IO_MODE_FORTRAN.or.(wff%accesswff==IO_MODE_FORTRAN_MASTER.and.wff%master==wff%me)) then
   ...
   write(msg,'(a,i0)')"Wrong accesswff: ",wff%accesswff
   MSG_ERROR(msg)
 end if
...
! *************************************************************************
 ...
 if (wff%accesswff==IO_MODE_FORTRAN.or.(wff%accesswff==IO_MODE_FORTRAN_MASTER.and.wff%master==wff%me)) then
   ...
   write(msg,'(a,i0)')"Wrong acesswff: ",wff%accesswff
   MSG_ERROR(msg)
 end if
...
!!
!! PARENTS
!!      nstdy3,nstpaw3,nstwf3,rwwf,vtowfk3
!!
!! CHILDREN
!!      mpi_file_write_at
!!
...
!!
!! PARENTS
!!      gstate,nstdy3,nstpaw3,nstwf3,randac,rwwf,vtowfk3
!!
!! CHILDREN
!!      mpi_file_write_at
!!
