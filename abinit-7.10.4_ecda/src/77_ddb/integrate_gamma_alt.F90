...
!!
!! COPYRIGHT
!! Copyright (C) 2004-2014 ABINIT group (MVer)
...
!scalars
 ...
 integer :: irec, qtor, rtoq, ibranch,nbranch,ngkkband
 integer :: symrankkpt
...
! *************************************************************************
 ...
 if (xcomm_size(xmpi_world) > 1) then
...
!allocations
 ABI_ALLOCATE(elph_ds%gamma_qpt,(2,nbranch**2,elph_ds%nsppol,elph_ds%k_fine%nkpt))
 ABI_CHECK_ALLOC('integrate_gamma_alt trying to allocate array elph_ds%gamma_qpt')
 elph_ds%gamma_qpt(:,:,:,:) = zero

!transport case
 if (elph_tr_ds%ifltransport==1) then
   ABI_ALLOCATE(elph_tr_ds%gamma_qpt_trin,(2,9,nbranch**2,elph_ds%nsppol,elph_ds%k_fine%nkpt))
   ABI_CHECK_ALLOC('trying to allocate array elph_tr_ds%gamma_qpt_trin')
   elph_tr_ds%gamma_qpt_trin = zero
   
   ABI_ALLOCATE(elph_tr_ds%gamma_qpt_trout,(2,9,nbranch**2,elph_ds%nsppol,elph_ds%k_fine%nkpt))
...
!temp variables
 ABI_ALLOCATE(tmp_gkk_1q ,(2,ngkkband**2,nbranch**2,8,elph_ds%nsppol))
 ABI_CHECK_ALLOC('trying to allocate tmp_gkk_1q')

 sz1=elph_ds%ngkkband*elph_ds%ngkkband
 sz2=elph_ds%nbranch*elph_ds%nbranch
 ABI_ALLOCATE(tmp_gkk ,(2,sz1,sz2,elph_ds%nsppol,elph_ds%nqpt_full))
 ABI_CHECK_ALLOC('trying to allocate tmp_gkk')

 ABI_ALLOCATE(tmp_gkr ,(2,elph_ds%nbranch*elph_ds%nbranch,nrpt))
 ABI_CHECK_ALLOC('trying to allocate array tmp_gkr')
