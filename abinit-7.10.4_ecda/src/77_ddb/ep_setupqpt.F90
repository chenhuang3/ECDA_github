...
!!
!! COPYRIGHT
!! Copyright (C) 2009-2014 ABINIT group (MVer)
...
&   -qptrlatt(1,1)*qptrlatt(2,3)*qptrlatt(3,2)
   ...
   ABI_ALLOCATE(qpt_full,(3,mqpt))
   ABI_CHECK_ALLOC('allocating qpt_full')
   iout = 6
...
!  save the q-grid for future reference
   ...
   ABI_CHECK_ALLOC("allocating elph_ds%qpt_full")
...
!=================================================================
 ...
 ABI_CHECK_ALLOC("allocating indqpt1 ,wtq_folded ,wtq")
...
& timrev,wtq,wtq_folded)
 ...
 ABI_CHECK_ALLOC("allocating elph_ds%wtq")
