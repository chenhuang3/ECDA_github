...
!!
!! COPYRIGHT
!!  Copyright (C) 2013-2014 ABINIT group (BXu)
...
!!
!! CHILDREN
!!      dgemm,ep_el_weights,ep_ph_weights,ftgam,ftgam_init,gam_mult_displ
!!      ifc_fourq,matrginv,mkqptequiv,phdispl_cart2red,prtbltztrp_tau_out
!!      spline,splint,update_occ,wrtout,xmpi_sum,zgemm
!!
!! SOURCE
...
!scalars
 ...
 real(dp) :: diagerr
 real(dp) :: chu_mfp,chu_cond,chu_cth,chu_sbk
...
! *************************************************************************
 ...
 ABI_CHECK_ALLOC('out of memory in array fermie')

 ABI_ALLOCATE(tmp_gkk_qpt ,(2,nbranch**2,nqpt))
 ABI_CHECK_ALLOC('out of memory in array tmp_gkk_qpt')

 ABI_ALLOCATE(tmp_gkk_rpt ,(2,nbranch**2,nrpt))
 ABI_CHECK_ALLOC(' out of memory in array tmp_gkk_rpt')

 ABI_ALLOCATE(tmp_gkk_kpt ,(2,nbranch**2))
 ABI_CHECK_ALLOC('out of memory in array tmp_gkk_kpt')

 ABI_ALLOCATE(tmp_gkk_kpt2 ,(2,nbranch,nbranch))
 ABI_CHECK_ALLOC('out of memory in array tmp_gkk_kpt2')

 ABI_ALLOCATE(gkk_kpt ,(2,nbranch,nbranch))
 ABI_CHECK_ALLOC('out of memory in array gkk_kpt')

 ABI_ALLOCATE(a2f_2d, (nene))
 ABI_CHECK_ALLOC('out of memory in array a2f_2d')

 ABI_ALLOCATE(a2f_2d2, (nene))
 ABI_CHECK_ALLOC('out of memory in array a2f_2d2')

 ABI_ALLOCATE(inv_tau_k, (ntemper,nsppol,nkpt,nband))
 ABI_CHECK_ALLOC('out of memory in array inv_tau_k')

 ABI_ALLOCATE(tau_k, (ntemper,nsppol,nkpt,nband))
 ABI_CHECK_ALLOC('out of memory in array tau_k')

 ABI_ALLOCATE(tmp_tau_k ,(ntemper,nsppol,new_nkptirr,nband))
 ABI_CHECK_ALLOC('out of memory in array tmp_tau_k')
...
!for dense k grid
 ...
 ABI_CHECK_ALLOC("allocating FSfullpktofull")
...
!kpttokpt(itim,isym,iqpt) = kpoint index which transforms to ikpt under isym and with time reversal itim.
 ...
 ABI_CHECK_ALLOC("allocating kpttokpt")
...
!=========================================================    
 ...
 ABI_CHECK_ALLOC('out of memory in array tmp_wtk ')

 ABI_ALLOCATE(tmp2_wtk,(nene_all))
 ABI_CHECK_ALLOC('out of memory in array tmp2_wtk ')

 ABI_ALLOCATE(ff2,(nene_all))
 ABI_CHECK_ALLOC('out of memory in array ff2 ')

 ABI_ALLOCATE(ene_pt,(nene_all))
 ABI_CHECK_ALLOC('out of memory in array ene_pt ')

 ABI_ALLOCATE(ene_ptfine,(nene_all*nspline))
 ABI_CHECK_ALLOC('out of memory in array ene_ptfine ')

 ABI_ALLOCATE(tmp_wtk1,(nene_all*nspline))
 ABI_CHECK_ALLOC('out of memory in array tmp_wtk1')

 ABI_ALLOCATE(tmp_wtk2,(nene_all*nspline))
 ABI_CHECK_ALLOC('out of memory in array tmp_wtk2')

 ABI_ALLOCATE(dos_e,(nsppol,nene_all))
 ABI_CHECK_ALLOC('out of memory in array dos_e')
...
!get fermie for itemp
 ...
 do itemp=1,ntemper  ! runs over termperature in K
   ...
   if (abs(elph_ds%elph_fermie) < tol10) then
...
!no need to *two_pi due to the missing prefactor in gkk (see mka2f_tr_lova)
       ...
       write(unit_invtau,'(100D16.8)') (inv_tau_k(itemp,isppol,iFSkpt,iband)/chu_tau,iband=1,nband)
       write(unit_tau,'(a,i8,a,3f12.6)') '# kpt# ', iFSkpt, '   kpt=', elph_ds%k_phon%kptirr(:,iFSkpt)
       write(unit_tau,'(100D16.8)') (tau_k(itemp,isppol,iFSkpt,iband)*chu_tau,iband=1,nband)
     end do ! nkptirr
     write(unit_invtau,*) ' '
     write(unit_tau,*) ' '
   end do ! nsppol
...
! Only use the irred k for eigenGS and tau_k
 ...
 ABI_CHECK_ALLOC(' out of memory in array tmp_eigenGS ')
...
!print header to relaxation time file
 ...
 write (unit_taue,*) '# ene[Ha] tau [SI] DOS[au]    '
...
&           tmp_wtk(jband,iFSkpt,isppol,iene)
       ...
       write(unit_taue,"(3D16.8)") enemin+(iene-1)*deltaene*nspline, rate_e/chu_tau, dos_e(isppol,iene)
...
!&                          elph_ds%k_phon%new_wtkirr(iFSqpt)
         ...
         write(unit_mfp,"(2D16.8)") enemin+(iene-1)*deltaene*nspline, mfp_e*chu_mfp
       end do ! number of energies
       write(unit_mfp,*) ' '
     end do ! icomp
     write(unit_mfp,*) ' '
 ...
 ABI_CHECK_ALLOC(' out of memory in array cond_e ')
...
!print header to relaxation time file
 ...
 ABI_CHECK_ALLOC(' out of memory in array cond ')

 ABI_ALLOCATE(cth ,(ntemper,nsppol,3,3))
 ABI_CHECK_ALLOC(' out of memory in array cth ')

 ABI_ALLOCATE(sbk ,(ntemper,nsppol,3,3))
 ABI_CHECK_ALLOC(' out of memory in array sbk ')

 ABI_ALLOCATE(seebeck ,(ntemper,nsppol,3,3))
 ABI_CHECK_ALLOC(' out of memory in array seebeck ')
