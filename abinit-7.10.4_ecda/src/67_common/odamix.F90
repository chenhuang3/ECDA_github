...
!scalars
 ...
 real(dp) :: enonlocal,fp0,gammp1,ro_dlt,ucvol_local,eenth
 logical :: with_vxctau
 character(len=500) :: message
!arrays
 real(dp) :: gmet(3,3),gprimdlc(3,3),rmet(3,3), qpt(3),tsec(2)
 real(dp) :: A(3,3),A1(3,3),A_new(3,3),efield_new(3)   !!HONG
...
!Compute Hartree energy energies%e_hartree
 call dotprod_vn(1,rhor,energies%e_hartree,doti,nfft,nfftot,1,1,vhartr,ucvol_local,mpi_comm_sphgrid=mpi_enreg%comm_fft)
 energies%e_hartree=half*energies%e_hartree


!Compute local psp energy energies%e_localpsp
 call dotprod_vn(1,rhor,energies%e_localpsp,doti,nfft,nfftot,1,1,vpsp,ucvol_local,mpi_comm_sphgrid=mpi_enreg%comm_fft)

!Compute double-counting XC energy energies%e_xcdc
 call dotprod_vn(1,rhor,energies%e_xcdc,doti,nfft,nfftot,dtset%nspden,1,vxc,ucvol_local,mpi_comm_sphgrid=mpi_enreg%comm_fft)
 if (with_vxctau) then
...
&   nzlmopt,option,paw_an,paw_an,paw_ij,pawang,dtset%pawprtvol,pawrad,pawrhoij,dtset%pawspnorb,&
...
&   dtset%nspden,ntypat,nzlmopt,option,paw_an,paw_an,paw_ij,pawang, &
