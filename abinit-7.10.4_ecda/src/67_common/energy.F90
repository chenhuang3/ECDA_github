...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, AR, MB, MT, EB)
...
!!
!! INPUTS
!!  atindx(natom)=index table for atoms (see gstate.F90)
!!  atindx1(natom)=index table for atoms, inverse of atindx (see gstate.F90)
...
!!   | e_vdw_dftd2(IN)=VdW DFT-D2 energy
...
!!
!! CHILDREN
!!      destroy_hamiltonian,dotprod_vn,fftpac,finalize_hamiltonian,fourdp
!!      gpu_finalize_ffnl_ph3d,gpu_update_ffnl_ph3d,init_hamiltonian
!!      load_paw_hamiltonian,mag_constr,make_gemm_nonlop,meanvalue_g,metric
!!      mkffnl,mkkin,mkresi,mkrho,nonlop,pawaccrhoij,pawcprj_alloc
!!      pawcprj_destroy,pawcprj_gather_spin,pawmknhat,pawrhoij_alloc
!!      pawrhoij_destroy,pawrhoij_destroy_unpacked,pawrhoij_init_unpacked
!!      pawrhoij_mpisum_unpacked,ph1d3d,psolver_rhohxc,rhohxc,rhohxcpositron
!!      sphereboundary,symrhoij,timab,transgrid,xmpi_sum
!!
!! SOURCE
...
subroutine energy(atindx,atindx1,cg,compch_fft,dimcprj,dtfil,dtset,electronpositron,&
& energies,eigen,etotal,gsqcut,indsym,irrzon,kg,mcg,mpi_enreg,my_natom,nattyp,nfftf,ngfftf,nhat,&
...
& vxctau) ! optional argument
 ...
 use m_pawrhoij,         only : pawrhoij_type, pawrhoij_alloc, pawrhoij_destroy, pawrhoij_init_unpacked, &
&                               pawrhoij_mpisum_unpacked, pawrhoij_destroy_unpacked, symrhoij
 use m_pawcprj,          only : pawcprj_type, pawcprj_alloc, pawcprj_destroy, pawcprj_gather_spin
...
!Local variables-------------------------------
!scalars
 integer :: bdtot_index,choice,cplex,cplex_rf,cpopt,dimffnl
 integer :: iband,icg,ider,idir,ierr,ifft,ig,ikg,ikpt,ilm
 integer :: indx,ipert,ipositron,iresid,ispden,isppol,istwf_k,izero
 integer :: me_distrb,mpi_comm_sphgrid,my_nspinor,n1,n2,n3,n4,n5,n6
 integer :: nband_k,nfftotf,nkpg,nkxc,nk3xc,nnlout,npw_k,nspden_rhoij,option
 integer :: option_rhoij,paw_opt,signs,spaceComm,tim_mkrho,tim_nonlop
 logical :: paral_atom,usetimerev,with_vxctau
 logical :: wvlbigdft=.false.
 real(dp) :: doti,evxc,e_xcdc_vxctau,eeigk,ekk,enlk,ucvol,ucvol_local,vxcavg
 !character(len=500) :: message
 type(gs_hamiltonian_type) :: gs_hamk
!arrays
 integer,allocatable :: kg_k(:,:)
 real(dp) :: enlout(1),gmet(3,3),gprimd(3,3),kpoint(3),nonlop_in(1,1),nonlop_out(1,1)
 real(dp) :: qpt(3),rhodum(1),rmet(3,3),tsec(2),ylmgr_dum(1,1,1), dum(1)
 real(dp) :: vzeeman(4)
 real(dp),allocatable :: buffer(:),cgrvtrial(:,:)
 real(dp),allocatable :: cwavef(:,:),eig_k(:),ffnl(:,:,:,:),kinpw(:)
 real(dp),allocatable :: kxc(:,:),occ_k(:)
 real(dp) :: kpg_dum(0,0)
...
&         vxctau=vxctau,exc_vdw_out=energies%e_xc_vdw)
       else
         call rhohxc(dtset,energies%e_xc,gsqcut,psps%usepaw,kxc, &
&         mpi_enreg,nfftf,ngfftf,nhat,psps%usepaw,nhatgr,nhatgrdim, &
&         nkxc,nk3xc,dtset%nspden,n3xccc,option,rhog,rhor,rprimd,strsxc, &
&         usexcnhat,vhartr,vxc,vxcavg,xccc3d,taug=taug,taur=taur, &
&         exc_vdw_out=energies%e_xc_vdw)
       end if
     else
       if(with_vxctau)then
         call rhohxc(dtset,energies%e_xc,gsqcut,psps%usepaw,kxc, &
...
&   nfftf,nfftotf,1,1,electronpositron%vha_ep,ucvol,mpi_comm_sphgrid=mpi_comm_sphgrid)
...
!EB add the vzeeman pot in the trial pot
...
& dtset%natom,dtset%ntypat,dtset%typat,xred,dtset%nfft,&
...
!PAW: additional initializations
 if (psps%usepaw==1) then
   ...
   if (mpi_enreg%paral_spinor==1) then
     ...
     call pawcprj_alloc(cwaveprj_gat,0,dimcprj)
   else
...
!    Allocate the array ph3d, and eventually compute it
     ABI_ALLOCATE(ph3d,(2,npw_k,gs_hamk%matblk))
     if(dtset%nloalg(1)>0) then
!      Here, precomputation of ph3d
       call ph1d3d(1,dtset%natom,kg_k,gs_hamk%matblk,dtset%natom,npw_k,n1,n2,n3,&
&       gs_hamk%phkxred,ph1d,ph3d)
     ...
     if(dtset%use_gpu_cuda==1) then
       call gpu_update_ffnl_ph3d(ph3d,ffnl,npw_k,dimffnl,psps%lmnmax,psps%ntypat,dtset%natom)
     end if
#endif
...
!      Select occupied bands
       ...
       if (abs(occ_k(iband))>tol8) then
         ...
         cwavef(:,1:npw_k*my_nspinor) = cg(:,1+(iband-1)*npw_k*my_nspinor+icg:iband*npw_k*my_nspinor+icg)
...
&         gs_hamk%matblk,dtset%mgfft,mpi_enreg,psps%mpsang,&
&         psps%mpssoang,dtset%natom,nattyp,1,dtset%ngfft,nkpg,nkpg,dtset%nloalg,nnlout,npw_k,npw_k,&
&         my_nspinor,dtset%nspinor,psps%ntypat,0,paw_opt,gs_hamk%phkxred,gs_hamk%phkxred,ph1d,&
&         ph3d,ph3d,signs,nonlop_in,nonlop_out,tim_nonlop,ucvol,&
...
&             mpi_enreg%comm_spinor,ierr)
             call pawaccrhoij(atindx,cplex,cwaveprj_gat,cwaveprj_gat,0,isppol,dtset%natom,dtset%natom,&
&             dtset%nspinor,occ_k(iband),option_rhoij,pawrhoij_unsym,usetimerev,dtset%wtk(ikpt))
           else
             call pawaccrhoij(atindx,cplex,cwaveprj,cwaveprj,0,isppol,dtset%natom,dtset%natom,&
&             dtset%nspinor,occ_k(iband),option_rhoij,pawrhoij_unsym,usetimerev,dtset%wtk(ikpt))
           end if
...
&     gs_hamk,icg,kg_k,kinpw,mcg,mpi_enreg,dtset%natom,&
&     nband_k,npw_k,my_nspinor,dtset%paral_kgb,ph3d,dtset%prtvol,&
...
&     energies%e_xcdc + energies%e_corepsp - energies%e_corepspdc + energies%e_pawdc 
 ...
 etotal = etotal + energies%e_ewald + energies%e_vdw_dftd2
...
&   npwarr,occ,paw_dmft,phnons,rhog,rhor,rprimd,tim_mkrho,ucvol,wfs, wvl_den)
   if(dtset%usekden==1)then
     call mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,&
&     npwarr,occ,paw_dmft,phnons,taug,taur,rprimd,tim_mkrho,ucvol,wfs,wvl_den,option=1)
   end if
 else
...
&   mpi_comm_atom=mpi_enreg%comm_atom,mpi_atmtab=mpi_enreg%my_atmtab)
   call pawrhoij_destroy_unpacked(pawrhoij_unsym)
   if (paral_atom) then
...
&   mpi_comm_fft=mpi_enreg%comm_fft,paral_kgb=dtset%paral_kgb,me_g0=mpi_enreg%me_g0,&
&   mpi_comm_atom=mpi_enreg%comm_atom,mpi_atmtab=mpi_enreg%my_atmtab,&
&   distribfft=mpi_enreg%distribfft,mpi_comm_wvl=mpi_enreg%comm_wvl)
   ...
   rhowfr(:,:)=zero
   call mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,&
&   npwarr,occ,paw_dmft,phnons,rhowfg,rhowfr,rprimd,tim_mkrho,ucvol_local,wfs,wvl_den)
 ...
 if (psps%usepaw==1) then
   ...
   if (mpi_enreg%paral_spinor==1) then
     call pawcprj_destroy(cwaveprj_gat)
