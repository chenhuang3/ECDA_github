...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (MVer,MB)
...
!!
!! CHILDREN
!!      getkpgnorm,getph,init_bess_spl,initylmg,kpgio,metric,ph1d3d,recip_ylm
!!      sort_dp,splint,xmpi_sum
!!
!! SOURCE
...
!scalars
 ...
 integer :: mgfft,my_nspinor,n1,n2,n3,natsph_tot,nfit,npw_k,nradintmax,oldkpt,prtsphere
 integer :: spaceComm,unkg_dum,unylm
 integer :: nk_and_spin
...
!initialize mpi_enreg_dummy
 ABI_ALLOCATE(mpi_enreg_dummy%proc_distrb,(dtset%nkpt,dtset%mband,dtset%nsppol))
 mpi_enreg_dummy%proc_distrb=0
...
!    kg array is the same for both sppol ?????
     ...
     do ikpt=1,dtset%nkpt
       if (mpi_enreg%proc_distrb(ikpt,1,isppol)/=mpi_enreg%me) cycle
...
!  Gather all contributions from different processors
   ...
   ABI_DEALLOCATE(mpi_enreg_dummy%proc_distrb)
