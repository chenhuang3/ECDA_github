...
!!
!! COPYRIGHT
!! Copyright (C) 2003-2014 ABINIT  group (MVeithen)
...
!!
!! PARENTS
!!      elpolariz,update_eb_field_vars
!!
!! CHILDREN
!!      appdig,dsdr_k_paw,outwf,pawcprj_alloc,pawcprj_copy,pawcprj_destroy
!!      pawcprj_get,pawcprj_getdim,pawcprj_mpi_allgather,pawcprj_mpi_recv
!!      pawcprj_mpi_send,pawcprj_put,pawpolev,polcart,rhophi,smatrix
!!      smatrix_k_paw,sym_pawcprj_kn,wrtout,xmpi_allgather,xmpi_bcast,xmpi_recv
...
&                       pawcprj_mpi_send, pawcprj_destroy, pawcprj_getdim
...
!Local variables -------------------------
 integer :: count,count1,ddkflag,dest,fdir
...
&             proc_distrb=mpi_enreg%proc_distrb)
             ...
             if ( ikpt1i /= ikpt1 ) then
               ...
               call sym_pawcprj_kn(cprj_fkn,cprj_ikn,dtefield%atom_indsym,dimlmn,-1,indlmn,&
           ...
           if(lexist)then
             if(idir==1)then
               if(mpi_enreg%me==0)then
                 open(unit=709,file='POLSAVE',status='OLD')
                 read(709,*)pol0
...
&       nxfh,occ_dum,resid,response,dtfil%unwff2,wfs,wvl,xfhist)
...
!!REC start
 if (dtset%berrysav == 1) then
   if(mpi_enreg%me==0)then
     open(unit=709,file='POLSAVE',status='UNKNOWN')
     write(709,'(3F20.12)') pol0
     close(709)
   end if
   first=.false.
 end if
