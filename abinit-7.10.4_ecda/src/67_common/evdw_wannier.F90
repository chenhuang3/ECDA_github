...
!!         P. L. Silvestrelli in ArXiv:13057035v1 vdw_xc=14.
!!
!! COPYRIGHT
!!  Copyright (C) 2010-2014 ABINIT group (CE, TR and AR)
...
!Local variables-------------------------------
 integer  :: ier,igr,ii,inx,iny,inz,isppol,iwan,jwan,jj,ll,mwan,ngr,tmp_mwan,mwan_half
 integer, allocatable:: amagr(:,:,:),inwan(:,:),nw(:,:),nwan(:),npwf(:),ord(:,:) 
 integer, allocatable:: tmp_nwan(:)
 real(dp) ::fij,rij,rij_c(3),fu,shift,erfValue
...
!vdW-QHO-WF method. 
 ...
 if(vdw_xc==14) then
   ...
   ABI_ALLOCATE(c_QHO,(3*mwan,3*mwan))
   ABI_ALLOCATE(Tij_dip,(3*mwan,3*mwan))
...
!DEBUG
     write(std_out,*) 'Non interacting energy (fu):',ch10
     ...
     do iwan=1,nwan(isppol)
       do jwan=1,nwan(isppol)
         do ii=1,3
           ...
           do jj=1,3
             iny=3*jwan-(3-jj)
             if (iwan==jwan) then !This should be corrected for proper treatment of spin polarized MLWFs.
               if (ii==jj) then
                 c_QHO(inx,iny)=omega(jwan)*omega(jwan)
               else
                 c_QHO(inx,iny)=zero
               end if
             else
               rij_c=wanncent(:,iwan,isppol)-wanncent(:,jwan,isppol)
...
!DEBUG
!In the current stage of development there is no interaction between MLWFs of different spins.
!Also, it is not clear how to deal with periodic systems...vdw_supercell should be used... 
!Writing c_QHO real matrix:      
!    write(std_out,*) 'c_QHO Matrix:',ch10  
!    do ii=1,3*mwan 
!       write(std_out,*) (c_QHO(ii,jj),jj=1,3*mwan) 
!    enddo
!ENDDEBUG

! here we diagonalize the matrix c_QHO and the eigenvalues come back in vector eigv
     ABI_ALLOCATE(matrx,((3*mwan*(3*mwan+1))/2))
     ABI_ALLOCATE(eigv,(3*mwan))
     ABI_ALLOCATE(eigvec,(3*mwan,3*mwan))
     ABI_ALLOCATE(zhpev1,(3*2*mwan-1))
     ABI_ALLOCATE(zhpev2,(3*3*mwan-2))
...
!    do jj=1,3*mwan*(3*mwan+1)/2
!     write(std_out,*) real(matrx(jj))
!    enddo
!ENDDEBUG    
     call ZHPEV ('N','U',3*mwan,matrx,eigv,eigvec,3*mwan,zhpev1,zhpev2,ier)
!DEBUG
...
!DEBUG
   write(std_out,*) 'Half the sum of eigenvalues square roots:',ch10
...
!!
!! COPYRIGHT
!!  Copyright (C) 2010-2014 ABINIT group (CE and TR)
!!  This file is distributed under the terms of the
!!  GNU General Public License, see ~abinit/COPYING
!!  or http://www.gnu.org/copyleft/gpl.txt .
!!
...
!!  
!! COPYRIGHT
!!  Copyright (C) 2011-2014 ABINIT group (CE)
!!  This file is distributed under the terms of the
!!  GNU General Public License, see ~abinit/COPYING
!!  or http://www.gnu.org/copyleft/gpl.txt .
!!
...
!!
!! COPYRIGHT
!!  Copyright (C) 2012-2014 ABINIT group (CE)
!!  This file is distributed under the terms of the
!!  GNU General Public License, see ~abinit/COPYING
!!  or http://www.gnu.org/copyleft/gpl.txt .
!!
