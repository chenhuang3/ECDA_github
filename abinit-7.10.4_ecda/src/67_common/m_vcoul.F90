...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (MG, FB)
...
! *************************************************************************
 ...
 rank   = xcomm_rank(comm); nprocs = xcomm_size(comm)
...
&    ' integrate q->0 : numerical BZ geometry factor = ',bz_geometry_factor*q0_vol**(2./3.)
 ...
 if (TRIM(Vcp%mode)/='CRYSTAL') then
   ABI_MALLOC(Vcp%ctab,(Vcp%nfft))
   !JB: Not sure what happens next but valgrind complains later that ctab is not
   !initilized
   Vcp%ctab=0
   call cutoff_table(Vcp)
 else 
   ABI_MALLOC(Vcp%ctab,(1))
