...
!!  [xc_tb09_c]=parameter for the TB09 functional
!!
!! OUTPUT
...
!arrays
 ...
 real(dp),intent(out),optional :: dvxc(npts,ndvxc),d2vxc(npts,nd2vxc),vxcgrho(npts,nvxcgrho)
...
!Checks input parameters
 if (mgga==1) then
   if (.not.present(lrho)) then
     MSG_BUG('lrho argument must be present in case of metaGGA!')
   end if
   if (.not.present(tau)) then
     MSG_BUG('tau argument must be present in case of metaGGA!')
   end if
   if (.not.present(vxclrho)) then
     MSG_BUG('vxclrho argument must be present in case of metaGGA!')
   end if
   if (.not.present(vxctau)) then
     MSG_BUG('vxctau argument must be present in case of metaGGA!')
   end if
 end if
