...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (PMA)
...
!!      ftfvw2__init,hartre,mean_fftr,metric,mkffnl,nonlop,ph1d3d,rhohxc,rhotov
!!      sphereboundary,timab,xmpi_sum
!!
!! SOURCE
...
!End of the abilint section
 ...
 implicit none
  ...
  real(dp),intent(in) :: spinmagntarget,gsqcut
  ...
  integer :: nk3xc,istep_dumm
  integer :: i1,i2,i3,ia,iatom,iband,icg,ider,idir,ierr,ifft
  integer :: ikg,ikpt,ilm,index,ispden,isppol
  integer :: istwf_k,matblk,n1,n2,n3,n4,n5,n6,nband_k
  ...
  real(dp) :: vtrialold(nfftf,nspden),ylmgr_dum(1,1,1),dummy_lambda(1)
  real(dp),allocatable :: buffer(:),cwavef(:,:)
  real(dp),allocatable :: dummyt2(:,:),dummyt3(:,:,:)
  real(dp),allocatable :: eig_k(:),ffnl(:,:,:,:),g2cart(:),kpg_dum(:,:)
  real(dp),allocatable :: kxc(:,:),laplacerhor(:,:),lavnl(:,:,:),lavnlfft(:,:)
  real(dp),allocatable :: newvout(:,:),newvoutfourier(:,:,:),occ_k(:)
...
!Test size of FFT grids (1 grid in norm-conserving, 2 grids in PAW)
 ...
 if ((psps%usepaw==1.and.pawfgr%nfft/=nfftf).or.(psps%usepaw==0.and.nfft/=nfftf)) then
   message = '  wrong values for nfft, nfftf !'
   MSG_ERROR(message)
 end if
...
!    Compute nonlocal psp energy - Norm-conserving only
     usepaw_lavnl: if (psps%usepaw==0) then
       iband_lavnl: do iband=1,nband_k
         if(nspinor > 1) then 
           MSG_ERROR('prctfw does not handle nspinor>1')
         end if
...
&        dummy_lambda,&       ! Lambda in Vnl-lambda.S - not used here
...
!    WARNING this may work only for nspden == 1
     call fftpac(ispden,mpi_enreg,nspden,n1,n2,n3,n4,n5,n6,ngfftf,lavnlfft,lavnl,option)
     do ifft=1,nfftf
...
!    end do
 ...
 message = '  New local average of non local potential made from  wfs and Vnl'
 MSG_COMMENT(message)
...
&   + (vpsp(:) &                  ! potential associated to the kernel.
&  + vhartr(:))*identity(ispden)& ! Hartree potential
...
!call random_number(opB)
 ...
 phi1(:,:)=zero
 call cgpr(nfftf,nspden,ftfvw2__pf,ftfvw2__dpf,ftfvw2__newphi2,tol12,&
...
!optres=0
 ...
 call rhotov(dtset,energies,gprimd,gsqcut,istep_dumm,kxc,mpi_enreg,nfftf,ngfftf,&
...
& vres_mean,vres2,newvout,vxcavg,vxc,wvl,xccc3d,xred)
 call mean_fftr(newvout,vmean,nfftf,nfftotf,min(nspden,2),mpi_comm_sphgrid=mpi_enreg%comm_fft)
!shift the newvout to remove the constant part. Don't know if this is OK
!WARNING
 do ispden=1,min(nspden,2)
   if(nspden/=2 .or. &
&   ( occopt>=3 .and. abs(spinmagntarget+real(99.99,dp))<real(1.0d-10,dp) ))then
     if(nspden==1)then
       vme=vmean(1)
     else
