...
!!
!! CHILDREN
!!      fftpac,fourwf,metric,mkffnl,nonlop,ph1d3d,sphereboundary,timab,wrtout
!!      xmpi_sum
!!
!! SOURCE
...
integer :: bdtot_index,choice,cpopt,dimffnl,i1,i2,i3,ia,iatom,iband
 ...
 integer :: matblk,me,n1,n2,n3,n4,n5,n6,nband_k,nkpg
...
!arrays
 ...
 real(dp) :: ylmgr_dum(1,1,1),qphon(3),nonlop_dum(1,1),nonlop_dum2(1,1),dummy_lambda(1)
 real(dp),allocatable :: buffer(:),cwavef(:,:),dummyt2(:,:)
 real(dp),allocatable :: dummyt3(:,:,:),eig_k(:),ffnl(:,:,:,:)
 real(dp),allocatable :: g2cart(:),kpg_dum(:,:),lavnl3d(:,:,:)
...
!    Skip this k-point if not the proper processor
     if(proc_distrb_cycle(mpi_enreg%proc_distrb,ikpt,1,nband_k,isppol,mpi_enreg%me)) then
