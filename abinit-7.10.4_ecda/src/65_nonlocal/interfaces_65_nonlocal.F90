...
interface
 subroutine getcprj(choice,cpopt,cwavef,cwaveprj,dimekb1,dimekb2,dimffnl,ekb,ffnl,&  
  ...
  real(dp),intent(in) :: ekb(dimekb1,dimekb2*(1-usepaw))
...
interface
 subroutine nonlop(atindx1,choice,cpopt,cprjin,dimenl1,dimenl2,dimffnlin,dimffnlout,&  
  ...
  real(dp),intent(inout),target :: vectout(2,npwout*nspinor*ndat)
...
interface
 subroutine opernlc_ylm(atindx1,cplex,cplex_dgxdt,cplex_enl,cplex_fac,dgxdt,dgxdtfac,dgxdtfac_sij,&  
...
interface
 subroutine opernld_ylm(choice,cplex,cplex_fac,d2gxdt,dgxdt,dgxdtfac,dgxdtfac_sij,enlk,enlout,fnlk,&  
  &  gx,gxfac,gxfac_sij,ia3,natom,nd2gxdt,ndgxdt,ndgxdtfac,nincat,&  
  &  nlmn,nnlout,nspinor,paw_opt,strnlk)
  use defs_basis
  implicit none
  integer,intent(in) :: choice
  integer,intent(in) :: cplex
