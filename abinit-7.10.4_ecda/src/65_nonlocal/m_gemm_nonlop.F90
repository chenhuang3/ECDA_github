...
! handle the case where nloalg(1) < 0, ie no precomputation of ph3d
...
!!
!! SOURCE
 subroutine make_gemm_nonlop(ikpt, ham, dimffnl, ffnl, ph3d, npw)
  use defs_basis
...
!End of the abilint section
  ...
  real(dp),intent(in) :: ffnl(npw,dimffnl,ham%lmnmax,ham%ntypat)
  real(dp),intent(in) :: ph3d(2,npw,ham%matblk)

  integer :: nprojs

  real(dp) :: atom_projs(2, npw, ham%lmnmax)
  real(dp) :: temp(npw)
...
!End of the abilint section
  ...
  real(dp),intent(out) :: enlout(nnlout*ndat)
  ...
  integer :: iatm, ndgxdt, ndgxdtfac, optder, itypat, ilmn
  integer :: cplex_dgxdt(1)
  real(dp) :: dgxdt_dum_in(1,1,1,1,1), dgxdt_dum_out(1,1,1,1,1)
  real(dp) :: sij_typ(((paw_opt+1)/3)*lmnmax*(lmnmax+1)/2)
  real(dp), allocatable :: projections(:,:,:), s_projections(:,:,:), vnl_projections(:,:,:)
  real(dp) :: temp_realvec(MAX(npwout,npwin)*nspinor*ndat)   ! MG: possible problem with the stack as the array may be large, could you use an allocatable?
  ...
  if(choice > 0) then
    ...
    if(choice /= 7) then
      ...
      do itypat=1, ntypat
        ...
        do idat = 1,ndat
          call opernlc_ylm(atindx1,cplex,cplex_dgxdt,cplex_enl,cplex_fac,dgxdt_dum_in,dgxdt_dum_out,dgxdt_dum_out,&
&         dimenl1,dimenl2,enl,projections(:, ibeg:iend, idat),vnl_projections(:, ibeg:iend, idat),&
&         s_projections(:, ibeg:iend, idat),&
&         iatm,indlmn(:,:,itypat),itypat,lambda(idat),mpi_enreg,natom,ndgxdt,ndgxdtfac,&
