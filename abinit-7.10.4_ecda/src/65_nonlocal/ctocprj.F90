...
!!
!! PARENTS
!!      extrapwf,loper3,scfcv,update_eb_field_vars,vtorho
!!
!! CHILDREN
!!      getcprj,mkffnl,mkkpg,pawcprj_alloc,pawcprj_destroy,pawcprj_put
!!      pawcprj_set_zero,ph1d3d,xmpi_allgather,xmpi_allgatherv,xmpi_alltoallv
!!
!! SOURCE
...
& unylm,useylmgr,wffnow,xred,ylm,ylmgr)
 ...
 use m_pawcprj,  only : pawcprj_type, pawcprj_diskinit_w, pawcprj_alloc, pawcprj_put, pawcprj_destroy, &
...
!Init parallelism
 if (paral_kgb==1) then
   ...
   if (mpi_enreg%paralbd==1) then
     ...
     spaceComm_band=xmpi_self;npband_bandfft=1
   end if
 end if
