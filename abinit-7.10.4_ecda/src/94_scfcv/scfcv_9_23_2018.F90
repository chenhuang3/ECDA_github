...
! pbar(3) and dbar(3) are reduced polarization and displacement field, defined by Eq.(27) and (29) Nat. Phys. suppl. (2009)
 ...
 integer  :: n1, n2, n3, io, band_index, resume, stat
...
do while(.true.)
   ...
   write(message,'(a,i6)')'   counter_run : ',counter_run;              call c_wrtlog(message)
   write(message,'(a)')' ============================================'; call c_wrtlog(message)
   write(message,'(a)')' '; call c_wrtlog(message)
   ...
   !
   write(message,*)'waiting for file message_from_dfet ...'; call c_wrtlog(message)
   file_exist = .false.
   do while ( .NOT. file_exist) 
     inquire( file='message_from_dfet', exist=file_exist ) 
     call sleep(1)
   enddo
   write(message,*)'waiting for file message_from_dfet2 ...'; call c_wrtlog(message)
   file_exist = .false.
   do while (.not. file_exist) 
   ...
   if (file_exist) then 
     ...
     call c_wrtlog(message)
     write(message,'(a,i3)')"calc_mode:      ",dtset%calc_mode
     call c_wrtlog(message)
     write(message,'(a,i3)')'do_exx:         ',do_exx
     call c_wrtlog(message)
     call c_wrtlog("")
     !
     xcep = .false.
...
&               rhog,rhor,rprimd,0,ucvol,wvl%wfs,wvl%den,0) 
     ...
     close(111)
     !!
     !! write kinetic energy density tau
     !! tau is defined as \tau=\sum_i 1/2*(\grad \phi)^2
     !!
     !call mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,paw_dmft,phnons,&
&    !           chen_taug,chen_taur,rprimd,0,ucvol,wvl%wfs,wvl%den,1) 
     !open(file='taur.dat',unit=111,action='write',form='unformatted')
     !write(111)chen_taur(:,2)
     !write(111)chen_taur(:,1)-chen_taur(:,2)
     !close(111)
     !!!
     !! compute ebar = sum e_i*rho_i / rho_total
     !!
     !call chen_ebar(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,eigen,paw_dmft,phnons,&
&    !               chen_taug,chen_taur,rprimd,0,ucvol,wvl%wfs,wvl%den) 
     !open(file='ebar.dat',unit=111,action='write',form='unformatted')
     !write(111)chen_taur(:,2)
     !write(111)chen_taur(:,1)-chen_taur(:,2)
     !1close(111)
     !
...
&     ylm,ylmgr,ylmdiel,vxctau=vxctau)
     ...
     do isp=1,dtset%nspden 
        ...
        ! check 
        if (abs(sum(rho_perb(:,isp))*dvol)>1e-3) then 
          call c_wrtlog('sum of rho_perb is >1e-3, BUG in DFPT code, FIX IT')
          stop
        endif 
     enddo 
   ...
   !  calc_mode = 502
   !
   !  subsystem is treated by OF-DFT, we 
   !  comptue the perturbation of the system's density for v_perb
   !  system is treated by TF+vW 
   !==================================================================
   if (dtset%calc_mode==505) then 

     call c_wrtlog('compute the perturbation using orbital-free DFPT.')
     ! load the OF-DFT solution from XCEP program 
     ! load v_perb  
     ! solve sternheimer equation to get the perturation 
   ...
   select case (type_of_system)
   case(1)
     !
     ! subsytem: load localizing potential for DFET
     !
...
!   endif
   ...
   !
   ! some information for running vtorho() 
   !
   dtset%tolwfr = 1e-10 ! set the stop criterion for cg solver: cgwf.F90 and lobpcgwf.F90
   dtset%nline  = 10    ! restart CG each 10 steps
   !
...
!!! DEBUG
!   open(file='../cg.tmp',unit=111,action='write',form='unformatted')
!   write(111) cg,eigen,rhor,rhog,occ
!   close(111)
!   stop
!
!777 continue 
!   write(6,*)'reading cg.tmp'; call flush(6)
!   open(file='../cg.tmp',unit=111,action='read',form='unformatted')
!   read(111) cg,eigen,rhor,rhog,occ
!   close(111)
!!!!!!!!!! END OF DEBUG 
...
&             chen_taug,chen_taur,rprimd,0,ucvol,wvl%wfs,wvl%den,1) 
   if (dtset%nsppol==2) then 
     print *,'min/max tau_up: ',minval(chen_taur(:,2)),maxval(chen_taur(:,2))
     print *,'min/max tau_dn: ',minval(chen_taur(:,1)-chen_taur(:,2)),maxval(chen_taur(:,1)-chen_taur(:,2))
     print *,'integrated tau_up: ',sum(chen_taur(:,2))*dvol
     print *,'integrated tau_dn: ',sum(chen_taur(:,1)-chen_taur(:,2))*dvol
     open(file='taur.dat',unit=111,action='write',form='unformatted')
     write(111)chen_taur(:,2)
     write(111)chen_taur(:,1)-chen_taur(:,2)
     close(111)
   else
     print *,'min/max tau_up: ',minval(chen_taur(:,1)),maxval(chen_taur(:,1))
     print *,'integrated tau: ',sum(chen_taur(:,1))*dvol
     open(file='taur.dat',unit=111,action='write',form='unformatted')
     write(111)chen_taur(:,1)-chen_taur(:,2)
     close(111)
   endif 
...
&     rhor,rprimd,strsxc,usexcnhat,vhartr,vxc,vxcavg,xccc3d,taug=taug,taur=taur)
   ...
   !
   ! compute ebar = \sum_i 1/e_i * phi_i**2
   !
   ! make sure that all the eigenvalues are negative
   counter = 0
   do isp=1,dtset%nspden
     do ib=1,dtset%nband(1)
        counter = counter + 1
        shift_eigen(counter) = eigen(counter) - chen_fermi(isp) - 0.2d0
     enddo 
   enddo 
   call c_wrtlog('ebar is based on shift_eigen.')
   call chen_ebar_inv_eigen(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,shift_eigen,paw_dmft,phnons,&
&                 chen_taug,chen_taur,rprimd,0,ucvol,wvl%wfs,wvl%den) 
   !!print *,sum(chen_taur)*dvol
   if (maxval(chen_taur)>0.0d0) then 
     call c_wrtlog("chen_taur>0.d0, to use Wang-Parr method it must <0.0, stop")
     print *,maxval(chen_taur)
     print *,'shift_eigen: ',shift_eigen
     stop
   endif 
   !
   !if (dtset%nsppol==2) then
   !  write(message,'(a,2es12.4)') & 
   !    'min/max ebar_up: ',minval(chen_taur(:,2)),maxval(chen_taur(:,2))
   !  call c_wrtlog(message)
   !  write(message,'(a,2es12.4)') & 
   !    'min/max ebar_dn: ',minval(chen_taur(:,1)-chen_taur(:,2)),maxval(chen_taur(:,1)-chen_taur(:,2))
   !  call c_wrtlog(message)
   !  open(file='ebar.dat',unit=111,action='write',form='unformatted')
   !  write(111)chen_taur(:,2)
   !  write(111)chen_taur(:,1)-chen_taur(:,2)
   !  close(111)
   !else 
   !  write(message,'(a,2es12.4)')'min/max ebar_up: ',minval(chen_taur(:,1)),maxval(chen_taur(:,1))
   !  call c_wrtlog(message)
   !  open(file='ebar.dat',unit=111,action='write',form='unformatted')
   !  write(111)chen_taur(:,1)
   !  close(111)
   !endif 
   ...
   if (do_exx>0 .and. mpi_enreg%me==0 .and. (.not. do_hse_oep)) then 
     ...
     !===================================================
     ! computing dExc_dVks, Slater potential, and uxc_bar
     !===================================================
     ...
     do isp = 1,dtset%nspden
        ...
        do ib1 = 1,n_homo(isp)  
          ...
          do ib2=1,dtset%nband(isp) 
             ...
             if (dtset%nsppol==1) then 
               dExx_dvks(:,isp) = dExx_dvks(:,isp) + 2.d0*dExx_docc(isp,ib1)*docc_dEigen*phir(:,isp,ib2)**2
             else 
        ...
        write(message,'(a,2f12.4)')NEW_LINE('a')//'final dExx_dVks: ',  & 
           minval(dExx_dvks(:,isp)),maxval(dExx_dvks(:,isp))
        call c_wrtlog(message)
   ...
   !
   ! signal the XCEP code that we are done
...
subroutine compute_HSE_oep(fix_vxc)
     ...
     do isp=1,dtset%nspden 
        ...
        !! check if we have enough bands, note that for bands from 
        !if (n_homo(isp)>=dtset%nband(1)-dtset%nbdbuf) then 
        !   print *,'increase your nband! n_homo(isp)>=dtset%nband(1)-dtset%nbdbu , stop'
        !   call flush(6)
        !   stop
        !endif 
        write(message,'(a,i4,a,i3)')'n_HOMO => ',n_homo(isp),' for spin:',isp
