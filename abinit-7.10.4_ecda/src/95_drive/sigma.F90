...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (GMR, VO, LR, RWG, MT, MG, RShaltaf)
...
!!
!! CHILDREN
!!      abi_etsf_electrons_put,abi_etsf_geo_put,calc_sigc_me,calc_sigx_me
!!      calc_ucrpa,calc_vhxc_me,check_sym_ug,chkpawovlp,classify_bands
!!      cohsex_me,crystal_free,crystal_ncwrite,cutoff_density,denfgr
!!      destroy_epsilonm1_results,destroy_mpi_enreg,destroy_paw_pwff
!!      destroy_sigma_parameters,ebands_copy,ebands_free,ebands_ncwrite
!!      energies_init,esymm_free,fourdp,get_gftt,get_rhor,getem1_from_ppm,getph
!!      gsph_free,hdr_free,init_distribfft_seq,init_paw_pwff,initmpi_seq,ioarr
!!      kmesh_free,kpath_free,kpath_init,kxc_ada,kxc_driver,littlegroup_free
!!      littlegroup_init,melements_free,melements_print,melements_zero
!!      melflags_reset,metric,mkdump_er,mkrdim,nhatgrid,paw_an_destroy
!!      paw_an_init,paw_an_nullify,paw_check_symcprj,paw_dijhf,paw_gencond
!!      paw_ij_destroy,paw_ij_init,paw_ij_nullify,paw_ij_print
!!      paw_mkdijexc_core,paw_pwaves_lmn_free,paw_pwaves_lmn_init,paw_qpscgw
!!      pawcprj_alloc,pawcprj_destroy,pawdenpot,pawdij,pawfgr_destroy
!!      pawfgr_init,pawfgrtab_destroy,pawfgrtab_init,pawfgrtab_print,pawinit
!!      pawmknhat,pawprt,pawpuxinit,pawrhoij_alloc,pawrhoij_copy
!!      pawrhoij_destroy,pawtab_print,ppm_free,ppm_init,prep_calc_ucrpa
!!      print_ngfft,prtrhomxmn,pspini,psps_print,rdgw,rdqps
!!      recalculate_epsm1_freq_grid,reportgap,setsymrhoij,setup_ppmodel
!!      setup_sigma,setvtr,shirley_bands,shirley_interp,shirley_window,show_qp
!!      sigma_bksmask,sigma_free,sigma_init,sigma_ncwrite,sigma_tables
!!      solve_dyson,symdij,symdij_all,test_charge,timab,update_occ
!!      updt_m_lda_to_qp,use_wfk_file,vcoul_free,wfd_change_ngfft,wfd_copy
!!      wfd_free,wfd_get_cprj,wfd_init,wfd_mkrho,wfd_plot_ur,wfd_print
!!      wfd_read_kss,wfd_read_wfk,wfd_reset_ur_cprj,wfd_rotate,wfd_test_ortho
...
subroutine sigma(acell,codvsn,Dtfil,Dtset,Pawang,Pawrad,Pawtab,Psps,rprim,converged)
 ...
 use m_psps,          only : psps_print
 use m_crystal,       only : crystal_free, crystal_t
 use m_crystal_io,    only : crystal_ncwrite
 use m_ebands,        only : update_occ, ebands_copy, reportgap, get_valence_idx, get_bandenergy, &
&                            ebands_free, ebands_init, ebands_ncwrite, ebands_3dprint, edos_t, edos_init, edos_free, edos_print
 use m_energies,      only : energies_type, energies_init
 use m_bz_mesh,       only : kmesh_t, findqg0, kmesh_free, littlegroup_t, littlegroup_init, &
&                            littlegroup_free, kpath_t, kpath_init, kpath_free, make_mesh
 ...
 use m_wfs,           only : wfd_init, wfd_free, wfd_reset_ur_cprj, wfd_print, wfd_t,&
&                            wfd_rotate, wfd_get_cprj, wfd_iam_master, wfd_ihave_ug, wfd_change_ngfft, &
&                            wfd_plot_ur, wfd_test_ortho, wfd_read_wfk, check_sym_ug, wfd_copy, wfd_barrier, wfd_write_wfk
...
&                            write_sigma_header, write_sigma_results
 ...
 use m_paw_ij,        only : paw_ij_type, paw_ij_init, paw_ij_destroy, paw_ij_nullify, paw_ij_print
 use m_pawfgrtab,     only : pawfgrtab_type, pawfgrtab_init, pawfgrtab_destroy, pawfgrtab_print
 use m_pawrhoij,      only : pawrhoij_type, pawrhoij_alloc, pawrhoij_copy, pawrhoij_destroy, symrhoij
 use m_pawcprj,       only : pawcprj_type, pawcprj_alloc, pawcprj_destroy, paw_overlap
 ...
 use m_shirley,       only : shirley_interp, shirley_window, shirley_bands
...
!Local variables-------------------------------
!scalars
 integer,parameter :: level=40,tim_fourdp=5,nshiftk=1,master=0
 integer :: accessfil,approx_type,b1gw,b2gw,choice,cplex,cplex_dij,band
 integer :: dim_kxcg,fformr,gnt_option,has_dijU,has_dijso,iab,bmin,bmax,irr_idx1,irr_idx2
 integer :: iat,ib,ib1,ib2,ic,id_required,ider,idir,ii,ik,ierr,ount,iomode
 integer :: ik_bz,ikcalc,ik_ibz,ikxc,ipert,npw_k
 integer :: isp,is_idx,istep,itypat,itypatcor,izero,jj
 integer :: first_band,last_band
 integer :: ks_iv,lcor,lmn2_size_max,mband
 integer :: mgfftf,mod10,mod100,moved_atm_inside!,mgfft
 integer :: moved_rhor,n3xccc
 ...
 integer :: iomega,ppm_unt,temp_unt
 ...
 real(dp) :: ex_energy,gsqcutc_eff,gsqcutf_eff,gsqcut_shp,nelect,norm,oldefermi
 ...
 type(hdr_type) :: Hdr_wfk,Hdr_sigma
...
!arrays
 ...
 integer,allocatable :: ks_vbik(:,:),nband(:,:)
 integer,allocatable :: l_size_atm(:),qp_vbik(:,:)
...
&   'Some BLAS interfaces are not generated by abilint '
 ...
 comm    = xmpi_world
 my_rank = xcomm_rank(comm); nprocs  = xcomm_size(comm)
 converged = .FALSE.
 !
...
& gsqcutc_eff=gsqcutc_eff,gsqcutf_eff=gsqcutf_eff,gmet=gmet,k0=k0)
 ...
 if (psp_gencond==1) then
   call psps_print(Psps,std_out,0,'COLL')
 end if
...
!* spinmagntarget is passed to fermi.F90 to fix the problem with newocc in case of magnetic metals
 ...
 !call update_occ(KS_BSt,Dtset%spinmagntarget,prtvol=0)
...
!  write(std_out,*)"using nq_spl",nq_spl,"qmax=",qmax
   ...
   do iat=1,Cryst%natom
     l_size_atm(iat)=Pawtab(Cryst%typat(iat))%lcut_size
   end do
   cplex=1
...
&   Cryst%ntypat,nzlmopt,option,KS_Paw_an,KS_Paw_an,KS_paw_ij,&
...
!  Output the pseudopotential strengths Dij and the augmentation occupancies Rhoij.
 ...
 if (Dtset%userie==-666 .or. Dtset%userie==-667) then
   ! Energy window
...
&  [3,nbounds]) 
   ...
   intp_mband=Sigp%nbnds; min_bsize=intp_mband; sh_coverage=dtset%userra
   ...
   if (Dtset%userie==-667) then
     ...
     ! Bands on path
     call shirley_bands(Wfd,Dtset,Cryst,Kmesh,KS_Bst,Psps,Pawtab,Pawang,Pawrad,Pawfgr,KS_Pawrhoij,KS_Paw_ij,&
&     sh_coverage,ewin,ngfftc,ngfftf,nfftf,ks_vtrial,intp_mband,Kpath%points,[(zero, ii=1,Kpath%npts)],sh_fname,oBands,oWsh)

     ! Energies on k-Mesh
     kptrlatt_ = reshape([30,0,0,0,30,0,0,0,30], [3,3])
     !write(std_out,*)"Kmesh with kptrlatt: ",kptrlatt_
     !call make_mesh(newKmesh,Cryst,Dtset%kptopt,kptrlatt_,Dtset%nshiftk,Dtset%shiftk)

     !call shirley_bands(Wfd,Dtset,Cryst,Kmesh,KS_Bst,Psps,Pawtab,Pawang,Pawrad,Pawfgr,KS_Pawrhoij,KS_Paw_ij,&
     !& sh_coverage,ewin,ngfftc,ngfftf,nfftf,ks_vtrial,intp_mband,newKmesh%ibz,newKmesh%wt,sh_fname,oBands,oWsh)

     !if (ebands_3dprint(oBands,Cryst,kptrlatt_,dtset%nshiftk,dtset%shiftk,"fs.bxsf","bxsf") /= 0) then
     !  MSG_ERROR("Cannot compute FS")
     !end if

     !call edos_init(Edos,oBands,newKmesh,"gaussian",zero,zero,ierr)
     !if (Wfd%my_rank==master) call edos_print(Edos, 77)
     !call edos_free(Edos)

     !call edos_init(Edos,oBands,newKmesh,"tetra",zero,zero,ierr)
     !if (Wfd%my_rank==master) call edos_print(Edos, 88)
     !call edos_free(Edos)

     !call wfd_write_wfk(Wfd,Hdr,Bands,wfk_fname)
     !call wfd_read_wfk(Wfd,wfk_fname,accesswff)

     call kmesh_free(newKmesh)

   else if (Dtset%userie==-666) then
     ! Use energy window, then interpolate bands.
     call wrtout(std_out," Calling shirley_window","COLL")

     call shirley_window(Wfd,Dtset,Cryst,Kmesh,KS_Bst,Psps,Pawtab,Pawang,Pawrad,Pawfgr,KS_Pawrhoij,KS_Paw_ij,&
...
&     KS_Pawrhoij,KS_Paw_ij,ngfftc,ngfftf,nfftf,ks_vtrial,&
...
!#endif
 ...
 if (mod100<10) then  ! one-shot GW, just do a copy of the KS density.
   ...
   ABI_MALLOC(m_lda_to_qp,(Sigp%nbnds,Sigp%nbnds,Kmesh%nibz,Sigp%nsppol))
   m_lda_to_qp=czero
   do ib=1,Sigp%nbnds
...
!  end do
   ...
   call update_occ(QP_BSt,Dtset%spinmagntarget,prtvol=0)
...
!  Allocate these arrays anyway, since they are passed to subroutines.
!  JB : This is done just before
   !if (.not.allocated(qp_nhat  ))  then
   !  ABI_MALLOC(qp_nhat,(nfftf,0))
   !end if
   !if (.not.allocated(qp_nhatgr))  then
   !  ABI_MALLOC(qp_nhatgr,(nfftf,Dtset%nspden,0))
   !end if
...
&   Dtset%usepaw,usexcnhat,Pawfgr%usefinegrid,compch_sph,compch_fft,drude_plsmf)
   ...
   if(Dtset%usekden==1) then
     call prtrhomxmn(std_out,MPI_enreg_seq,nfftf,ngfftf,&
&     Dtset%nspden,1,qp_taur,optrhor=1,ucvol=ucvol)
   end if
   ...
   if (Psps%usepaw==1.and.nscf>0) then
     call pawrhoij_destroy(prev_pawrhoij)
   end if
...
&     MPI_enreg_seq,nfftf,Pawrhoij_dum,rdwr,rdwrpaw,Wvl%den)
     !call rhor_write(Dtfil%fnameabo_qp_den, qp_rhor,Hdr_sigma)
     !Complete the missing geometry and electronic information.
     !if (io_mode_from_file(Dtfil%fnameabo_qp_den) == IO_MODE_ETSF)
...
&       Hdr_sigma,MPI_enreg_seq,nfftf,Pawrhoij_dum,rdwr,rdwrpaw,Wvl%den)
       !call rhor_write(Dtfil%fnameabo_qp_pawden, qp_rhor_paw,Hdr_sigma)
       !Complete the missing geometry and electronic information.
       !if (io_mode_from_file(Dtfil%fnameabo_qp_pawden) == IO_MODE_ETSF)
       !NCF_CHECK(ncfile_create(ncf,fname,NF90_CLOBBER),"Creating GSR file")
       !call crystal_ncwrite(Crystal,ncf%ncid)
       !call ebands_ncwrite(Bands,dtset%nshiftk_orig,dtset%shiftk_orig,dtset%nshiftk,dtset%shiftk,dtset%ngkpt,dtset%kptrlatt,ncf%ncid)
       !NCF_CHECK(ncfile_close(ncf),"Closing GSR file")
   ...
   ! ===========================================
   if (.FALSE.) then 
     ! plot QP amplitudes. Change bks_mask to select particular states.
     ABI_MALLOC(bks_mask,(Wfd%mband,Wfd%nkibz,Wfd%nsppol))
     bks_mask=.FALSE.
!    bks_mask(1,1,1)=.TRUE.
     call wfd_plot_ur(Wfd,Cryst,Psps,Pawtab,Pawrad,ngfftf,bks_mask)
...
!    TODO terminate the implementation of this routine.
     call paw_ij_print(QP_Paw_ij,unit=std_out,pawprtvol=Dtset%pawprtvol,pawspnorb=Dtset%pawspnorb,&
&     mode_paral="COLL")
     call pawprt(Dtset,Cryst%natom,QP_paw_ij,QP_Pawrhoij,Pawtab)
   end if
...
!      write(std_out,*) 'getpawden: ',Dtset%getpawden,' irdpawden: ',Dtset%irdpawden
       ...
       if (ios/=0) then
         msg = sjoin(Dtfil%filpawdensin,' exists but iostat returned nonzero value!')
         MSG_ERROR(msg)
       end if
...
!      write(std_out,*) 'getpawden: ',Dtset%getpawden,' irdpawden: ',Dtset%irdpawden
       ...
       inquire(file=TRIM(Dtfil%filpawdensin),iostat=io)
       if (ios/=0) then
         msg = sjoin(Dtfil%filpawdensin,' exists but iostat returns nonzero value!')
         MSG_ERROR(msg)
       end if
       call get_rhor(Dtfil%filpawdensin,Dtset%accesswff,Wfd%nspden,nfftf_tot, &
&       ngfftf,Dtset%paral_kgb,MPI_enreg_seq,ks_aepaw_rhor,get_pawden=.TRUE.)
...
!  Set plasma frequency
   my_plsmf = drude_plsmf
   if (Dtset%ppmfrq>tol6) my_plsmf = Dtset%ppmfrq
...
&     Dtset%freqremax,my_plsmf,Dtset%freqim_alpha)
...
!   end if
 ...
 if (sigma_needs_ppm(Sigp)) then
   ...
   if (Wfd%usepaw==1 .and. Ppm%userho==1) then
     ...
     call wrtout(std_out,sjoin('Checking for existence of: ',pawden_fname),"COLL")
     inquire(file=pawden_fname,iostat=ios,exist=pawden_exists)
     if (ios/=0) then
       msg = sjoin(pawden_fname,' exists but iostat returns nonzero value!')
       MSG_ERROR(msg)
     end if
...
!   Wannier basis: rhot1_q_m will need an atom index in the near future.
   ...
   ABI_ALLOCATE(rhot1_q_m,(cryst%nattyp(itypatcor),ndim,ndim,sigp%npwx,Qmesh%nibz))
   ABI_ALLOCATE(M1_q_m,(cryst%nattyp(itypatcor),ndim,ndim,sigp%npwx,Qmesh%nibz))
...
!   do ikcalc=1,Qmesh%nibz
!       write(6,*) "rhot1_q_m",ikcalc,rhot1_q_m(1,1,1,1,ikcalc)+&
!&       rhot1_q_m(1,2,2,1,ikcalc)+   &
!&       rhot1_q_m(1,3,3,1,ikcalc)+   &
!&       rhot1_q_m(1,4,4,1,ikcalc)+   &
!&       rhot1_q_m(1,5,5,1,ikcalc)+   &
!&       rhot1_q_m(1,6,6,1,ikcalc)+   &
!&       rhot1_q_m(1,7,7,1,ikcalc)
!   end do
!   do ikcalc=1,Qmesh%nibz
!       write(6,*) "rhot1_q_m",ikcalc,rhot1_q_m(1,1,1,2,ikcalc)+&
!&       rhot1_q_m(1,2,2,2,ikcalc)+   &
!&       rhot1_q_m(1,3,3,2,ikcalc)+   &
!&       rhot1_q_m(1,4,4,2,ikcalc)+   &
!&       rhot1_q_m(1,5,5,2,ikcalc)+   &
!&       rhot1_q_m(1,6,6,2,ikcalc)+   &
!&       rhot1_q_m(1,7,7,2,ikcalc)
!   end do
...
&       Sigp%minbnd(ikcalc,spin) <= ks_vbik(ik_ibz,spin)  ) then
   ...
   if (Sigp%nkptgw==Kmesh%nibz) then
     ...
     call update_occ(QP_BSt,Dtset%spinmagntarget,prtvol=Dtset%prtvol) 
     qp_vbik(:,:) = get_valence_idx(QP_BSt)
...
&       Dtset%nspden,nscf,nfftf,ngfftf,Sr,QP_BSt,m_lda_to_qp,qp_rhor)
   ...
   if (wfd_iam_master(Wfd)) then
...
#ifdef HAVE_TRIO_ETSF_IO
     NCF_CHECK(ncfile_create(ncf,fname,NF90_CLOBBER),"NC_create")
     ...
     call sigma_ncwrite(Sigp,Sr,ncf%ncid)
     NCF_CHECK(ncfile_close(ncf),"NC_CLOSE")
#endif
   end if
 ...
 if (Dtset%usepaw==1) then 
   call pawrhoij_destroy(KS_Pawrhoij)
   ABI_DT_FREE(KS_Pawrhoij)
   call pawfgrtab_destroy(Pawfgrtab)
   call paw_ij_destroy(KS_paw_ij)
   call paw_an_destroy(KS_paw_an)
   call destroy_paw_pwff(Paw_pwff)
   if (mod100>=10) then
     call pawrhoij_destroy(QP_pawrhoij)
     call paw_ij_destroy(QP_paw_ij)
     call paw_an_destroy(QP_paw_an)
   end if
   if (Dtset%pawcross==1) then
