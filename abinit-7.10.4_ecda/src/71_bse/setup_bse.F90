...
!!      ebands_copy,ebands_init,ebands_print,find_qmesh,free_scrhdr,get_bz_item
!!      get_ng0sh,gsph_extend,gsph_init,hdr_check,hdr_init,hdr_update
!!      hdr_vs_dtset,init_transitions,kmesh_init,kmesh_print,make_mesh,matrginv
!!      metric,mkrdim,pawrhoij_alloc,pawrhoij_copy,pawrhoij_destroy
!!      print_bs_files,print_bs_parameters,print_gsphere,print_ngfft,rdgw
!!      reportgap,scr_hdr_io,scrhdr_comm,setmesh,update_occ,vcoul_init
!!      wfk_read_eigenvalues,wrtout,xmpi_barrier,xmpi_bcast,xmpi_max
!!      xmpi_split_work
!!
!! SOURCE
...
& Cryst,Kmesh,Qmesh,KS_BSt,QP_bst,Hdr_wfk,Gsph_x,Gsph_c,Vcp,Hdr_bse,w_fname,comm,Wvl)
 ...
 use m_gwdefs,        only : GW_Q0_DEFAULT, gw_uses_wfk_file
 use m_fstrings,      only : toupper
 use m_io_tools,      only : file_exists, open_file
 use m_geometry,      only : normv
 use m_header,        only : hdr_init, hdr_get_nelect_byocc, hdr_update, hdr_check
...
&                            update_occ, get_valence_idx, apply_scissor, ReportGap
 ...
 use m_pawrhoij,      only : pawrhoij_type, pawrhoij_alloc, pawrhoij_copy, pawrhoij_destroy
...
!scalars
 ...
 integer :: bantot,enforce_sym,ib,ibtot,ik_ibz,isppol,ii,jj,method,iat,ount
 integer :: tot_nreh,work_size,neh_per_proc
 integer :: mband,io,nfftot_osc,spin,hexc_size,nqlwl,iq
 integer :: restart,restartpaw,timrev,iq_bz,isym,iq_ibz,itim
 integer :: my_rank,nprocs,w_unt,rdwr,fform,npwe_file,ierr,my_k1, my_k2,my_nbks
 integer :: first_dig,second_dig,it
 real(dp) :: ucvol,nelect_hdr,qnorm
...
!************************************************************************
 ...
 nelect_hdr = hdr_get_nelect_byocc(Hdr_wfk)

 if (ABS(nelect_hdr)>tol6 .and. ABS(Dtset%nelect-nelect_hdr)>tol6) then
   write(msg,'(2(a,f8.2))')&
&   "File contains ", nelect_hdr," electrons but nelect initialized from input is ",Dtset%nelect
 ...
 if (w_fname /= ABI_NOFILE) then ! Read dimensions from the external file
   !
   if (my_rank==master) then ! Master node read npw and nqlwl from SCR file.
     call wrtout(std_out,' Testing file: '//TRIM(w_fname),"COLL")
     if (open_file(w_fname,msg,newunit=w_unt,form="unformatted", status="old",action="read") /= 0) then
       MSG_ERROR(msg)
     end if
...
! if (.FALSE.) then
 ...
 ! Hard-coded values
 BSp%rl_nb = 0 ! Don't use Rohlfing & Louie
...
!TODO add new dim for exchange part and consider the possibility of having npwsigx > npwwfn (see setup_sigma).
 ...
 if (Gsph_x%ng > Gsph_c%ng ) then
   call vcoul_init(Vcp,Gsph_x,Qmesh,Kmesh,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,Gsph_x%ng,&  
&    nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 else
   call vcoul_init(Vcp,Gsph_c,Qmesh,Kmesh,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,Gsph_c%ng,&  
&    nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 end if
...
&  Kmesh%nibz,npwarr,Dtset%nsppol,Dtset%nspinor,Dtset%tphysel,Dtset%tsmear,Dtset%occopt,occfact,Kmesh%wt)
 ...
 call update_occ(KS_BSt,Dtset%spinmagntarget,prtvol=Dtset%prtvol)
...
&  Cryst%rprimd,occfact,Pawrhoij,Dtset%usepaw,Cryst%xred)
 ...
 call hdr_check(1002,1002,Hdr_bse,Hdr_wfk,'COLL',restart,restartpaw)
...
&    "This is not allowed, sym_rhotwgq0 should be changed."
 ...
 if (dtset%max_ncpus /=0 .and. dtset%autoparal /=0 ) then
   ...
   do ii=1,dtset%max_ncpus
     ...
     neh_per_proc = work_size / ii
     neh_per_proc = neh_per_proc + MOD(work_size, ii)
     eff = (one * work_size) / (ii * neh_per_proc)
