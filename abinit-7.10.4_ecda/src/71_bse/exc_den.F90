...
subroutine exc_den(BSp,BS_files,ngfft,nfftot,Kmesh,ktabr,Wfd)
 ...
 use m_wfs,             only : wfd_t, wfd_get_ur
...
!allocate and load wavefunctions in real space
 ABI_MALLOC(wfr,(nfftot*Wfd%nspinor,BSp%nbnds,Wfd%nkibz))
 ABI_CHECK_ALLOC("out of memory: exc_den, wfr")
 ...
 ABI_MALLOC(wfrk,(nfftot,BSp%nbnds))
 ABI_CHECK_ALLOC('out of memory: exc_den, wfrk')
