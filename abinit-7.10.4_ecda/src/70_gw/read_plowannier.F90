...
!! 
!! COPYRIGHT
!! Copyright (C) 2006-2013 ABINIT group (BAmadon)
...
!scalars
 ...
 integer, intent(in) :: nsppol,prtvol
 integer, intent(in) :: ucrpa_bands(2)
!Local variables-------------------------------
 complex(dpc), allocatable :: coeffW_IBZ(:,:,:,:,:)
 character(len=500) :: message,msg
 integer :: at_indx,ik_ibz,band1,m1,m2,spin,ik_bz,dummy,isym,itim,iat,indx
 real(dp) :: xx,yy 
 real(dp) :: kbz(3)
! *********************************************************************
 ...
 rewind(2012)
 read(2012,*) message, lcor,itypatcor
 read(2012,*) message, bandinf,bandsup
...
!Do not dead the bandinf, bandinf redondance information
 ...
 ABI_ALLOCATE(coeffW_IBZ,(Cryst%nattyp(itypatcor),nsppol,bandinf:bandsup,Kmesh%nibz,2*lcor+1))
 coeffW_IBZ=czero
 do spin=1,nsppol
   do ik_ibz=1,Kmesh%nibz
     ...
     !read projection
        do iat=1,Cryst%nattyp(itypatcor)
          do m1=1,2*lcor+1
            read(2012,*) dummy,dummy,dummy,xx,yy
            !write(message,*)  dummy,dummy,dummy,x,y
            coeffW_IBZ(iat,spin,band1,ik_ibz,m1)=cmplx(xx,yy)
          end do
        end do
      end do
   end do
 ...
 ABI_ALLOCATE(coeffW_BZ,(Cryst%nattyp(itypatcor),nsppol,bandinf:bandsup,Kmesh%nbz,2*lcor+1))
 coeffW_BZ=czero
   ...
   !write(6,*) ik_bz,Kmesh%nbz
     ...
     do indx=1,cryst%nattyp(itypatcor)
       ...
       do spin=1,nsppol
         do m1=1,2*lcor+1
           do m2=1,2*lcor+1
           coeffW_BZ(indx,spin,:,ik_bz,m1)= coeffW_BZ(indx,spin,:,ik_bz,m1)&
&                   +coeffW_IBZ(at_indx,spin,:,ik_ibz,m2)*pawang%zarot(m2,m1,lcor+1,isym)
           enddo
