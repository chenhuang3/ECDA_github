...
!!
!! COPYRIGHT
!!  Copyright (C) 2008-2014 ABINIT group (MG, GMR, VO, LR, RWG, RS)
...
#if 1
   ABI_MALLOC(PPm%bigomegatwsq(iq_ibz)%vals, (PPm%npwc,PPm%dm2_botsq))
   ABI_CHECK_ALLOC("out of memory bigomegatwsq")
   !
   ABI_MALLOC(PPm%omegatw(iq_ibz)%vals, (PPm%npwc,PPm%dm2_otq))
   ABI_CHECK_ALLOC("out of memory omegatw")
   !
   ABI_MALLOC(PPm%eigpot(iq_ibz)%vals, (PPm%dm_eig,PPm%dm_eig))
   ABI_CHECK_ALLOC("out of memory eigpot")
   !%endif
#else
...
!scalars  
 real(dp) :: qpt(3)
 !complex(gwpc),pointer :: bigomegatwsq(:,:),omegatw(:,:),omegatw_1(:),epsm1_gg(:,:)
! *************************************************************************
 ...
 CASE (PPM_GODBY_NEEDS) 
   ...
   do iq_ibz=1,nqiA
     ...
     !bigomegatwsq => PPm%bigomegatwsq(iq_ibz)%vals
     !omegatw      => PPm%omegatw(iq_ibz)%vals

     call cppm1par(npwe,nomega,omega,PPm%drude_plsmf,&
&       epsm1(:,:,:,iq_ibz),&
&       PPm%omegatw(iq_ibz)%vals,&
&       PPm%bigomegatwsq(iq_ibz)%vals)
   end do
...
&      PPm%omegatw(iq_ibz)%vals)
 ...
 CASE (PPM_LINDEN_HORSH) ! TODO Check better double precision, this routine is in a messy state
   ...
   do iq_ibz=1,nqiA
     ...
     !epsm1_gg     => epsm1(:,:,1,iq_ibz)
     !bigomegatwsq => PPm%bigomegatwsq(iq_ibz)%vals
     !omegatw_1    => PPm%omegatw(iq_ibz)%vals(:,1)
     !eigpot       => PPm%eigpot(iq_ibz)%vals

     call cppm3par(qpt,npwe,epsm1(:,:,1,iq_ibz),ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,&
&      PPm%bigomegatwsq(iq_ibz)%vals,&
&      PPm%omegatw(iq_ibz)%vals(:,1),&
&      PPm%eigpot(iq_ibz)%vals) 
   end do
...
!Local variables-------------------------------
!scalars
 integer :: info,lwork,negw,ig1,ig2,idx,sdim,iomega
 character(len=500) :: msg
!arrays
...
! *************************************************************************
 ...
 do iomega=1,nomega
   ...
   if (ABS(REAL(omega(iomega)))>0.00001) then
     ...
     ABI_MALLOC(Adpp,(PPm%npwc*(PPm%npwc+1)/2))
...
!*************************************************************************
 ...
 ABI_MALLOC(rhogg,(npwc,npwc))
 ABI_CHECK_ALLOC("out of memory rhogg")
...
&   'Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_MALLOC(omegatwsq,(npwc,npwc))
 ABI_CHECK_ALLOC('out of memory in omegatwsq')
...
!*************************************************************************
 ...
 ABI_MALLOC(rhogg,(npwc,npwc))
 ABI_CHECK_ALLOC("out of memory in rhogg")
 !
...
&   'Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_MALLOC(mm,(npwc,npwc))
 ABI_CHECK_ALLOC('out of memory in mm')
...
&        (gppq(1)*b1(ii)+gppq(2)*b2(ii)+gppq(3)*b3(ii))
 ...
 ABI_MALLOC(eigvec,(npwc,npwc))
 ABI_CHECK_ALLOC('eigvec out of memory')
 ...
 ABI_MALLOC(matr,(npwc*(npwc+1)/2))
 ABI_CHECK_ALLOC('matr of memory')
...
!*************************************************************************
 ...
 ABI_MALLOC(rhogg,(npwc,npwc))
 ABI_CHECK_ALLOC('rhogg out of memory')
 !
...
&   'Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_MALLOC(mm,(npwc,npwc))
 ABI_CHECK_ALLOC('mm out of memory')
...
&        (gppq(1)*b1(ii)+gppq(2)*b2(ii)+gppq(3)*b3(ii))
 ...
 ABI_MALLOC(chitmp,(npwc,npwc))
 ABI_CHECK_ALLOC('chitmp out of memory')

 ABI_MALLOC(chi,(npwc,npwc))
 ABI_CHECK_ALLOC('chi out of memory')
 ...
 ABI_MALLOC(mtemp,(npwc,npwc))
 ABI_CHECK_ALLOC("mtemp out-of-memory")

 ABI_MALLOC(chitmps,(npwc,npwc))
 ABI_CHECK_ALLOC("chitmps out-of-memory")
 !
 ...
 ABI_MALLOC(zz2,(npwc,npwc))
 ABI_CHECK_ALLOC("zz2 out-of-memory")
 !
 ! good check:
...
! *********************************************************************
 ...
 if (q_isirred) then 
   ...
   if (PPm%bigomegatwsq_qbz_stat==PPM_ISPOINTER) then 
      nullify(PPm%bigomegatwsq_qbz)
      ABI_MALLOC(PPm%bigomegatwsq_qbz%vals, (PPm%npwc,PPm%dm2_botsq))
      ABI_CHECK_ALLOC("out of memory bigomegatwsq")
   ...
   if (PPm%omegatw_qbz_stat==PPM_ISPOINTER) then 
      nullify(PPm%omegatw_qbz)
      ABI_MALLOC(PPm%omegatw_qbz%vals,(PPm%npwc,PPm%dm2_otq))
      ABI_CHECK_ALLOC("out of memory omegatw")
   ...
   if (PPm%eigpot_qbz_stat==PPM_ISPOINTER) then 
     nullify(PPm%eigpot_qbz)
     ABI_MALLOC(PPm%eigpot_qbz%vals,(PPm%dm_eig,PPm%dm_eig))
     ABI_CHECK_ALLOC("out of memory eigpot")
   ...
   call ppm_get_qbz(PPm,Gsph,Qmesh,iq_bz,&
&    PPm%bigomegatwsq_qbz%vals,&
&    PPm%omegatw_qbz%vals,&
&    PPm%eigpot_qbz%vals) 

   ! Release the table in the IBZ if required.
   if (.not.PPm%keep_q(iq_ibz)) then
     call ppm_table_free(PPm,iq_ibz)
   end if
 end if
...
!scalars  
 real(dp) :: qpt(3)
 !complex(gwpc),pointer :: bigomegatwsq(:,:),omegatw(:,:),omegatw_1(:)
! *************************************************************************
