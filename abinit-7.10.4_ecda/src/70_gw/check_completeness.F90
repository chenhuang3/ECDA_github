...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (GMR, VO, LR, RWG, MG, RShaltaf, GKA)
...
!! Gsph_epsG0<gsphere_t data type> The G-sphere used to describe deltaI/eps. (including umklapp G0 vectors)
...
!!      destroy_paw_pwij,get_bz_diff,get_bz_item,get_gftt,gsph_fft_tabs
!!      gsph_free,gsph_in_fftbox,init_paw_pwij,littlegroup_print
!!      paw_cross_rho_tw_g,paw_rho_tw_g,paw_symcprj,pawcprj_alloc
!!      pawcprj_destroy,rho_tw_g,symmetrize_afm_chi0,timab,wfd_barrier
...
& Pawtab,Pawang,Paw_pwff,Pawfgrtab,Paw_onsite,ngfft_gw,nfftot_gw,ngfftf,nfftf_tot,deltaI,ktabr,ktabrf,Ltg_q,Wfd,Wfdf)
 ...
 use m_wfs,           only : wfd_get_ur, wfd_t, wfd_distribute_kb_kpbp, wfd_get_cprj, wfd_barrier, &
&                            wfd_change_ngfft, wfd_paw_get_aeur
...
!scalars
 ...
 type(wfd_t),intent(inout) :: Wfd,Wfdf
!arrays
...
!Local variables ------------------------------
!scalars
 integer,parameter :: tim_fourdp1=1,two_poles=2,one_pole=1,fake_nomega=1,ndat1=1
...
!************************************************************************
 ...
 if (Psps%usepaw==1) then  ! * Prepare the onsite contributions on the GW FFT mesh.   
   ...
   ABI_DATATYPE_ALLOCATE(Pwij_fft,(Psps%ntypat))
 ...
 if (Psps%usepaw==1) then
   ABI_DATATYPE_ALLOCATE(Pwij,(Psps%ntypat))
 ...
 do is=1,nsppol
   ...
   if (Psps%usepaw==1) then
     ABI_DATATYPE_ALLOCATE(Cprj2_k  ,(Cryst%natom,nspinor))
     call pawcprj_alloc(Cprj2_k,  0,Wfd%nlmn_atm)
     ABI_DATATYPE_ALLOCATE(Cprj1_kmq,(Cryst%natom,nspinor))
   ...
   do ik_bz=1,Kmesh%nbz ! Loop over k-points in the BZ.
     ...
     call get_BZ_diff(Kmesh,kbz,qpoint,ikmq_bz,G0,nfound)
     if (nfound==0) then 
       msg = "Cannot find kbz - qpoint in Kmesh"
       MSG_ERROR(msg)
     end if
     ...
     do ib1=1,nbmax ! Loop over all states.
       ...
       do ib2=1,nbmax ! Loop over "valence" states.
         ...
         if (QP_BSt%occ(ib2,ik_ibz,is) < GW_TOL_DOCC) CYCLE
...
&             dim_rtwg,rhotwg)
   ...
   if (Psps%usepaw==1) then
     ...
     ABI_DATATYPE_DEALLOCATE(Cprj1_kmq)
     if (Dtset%pawcross==1) then
 ...
 ! * Collective sum of the contributions of each node.
   call xmpi_sum(deltaI(:,:,1),comm,ierr)
 !
