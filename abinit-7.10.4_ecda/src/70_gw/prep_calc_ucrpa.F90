...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (FB, GMR, VO, LR, RWG, MG, RShaltaf,TApplencourt,BAmadon)
...
!!
!! CHILDREN
!!      destroy_paw_pwij,findqg0,flush_unit,get_bz_item,gsph_fft_tabs
!!      init_paw_pwij,paw_cross_rho_tw_g,paw_rho_tw_g,paw_symcprj,pawcprj_alloc
!!      pawcprj_copy,pawcprj_destroy,pawmknhat_psipsi,read_plowannier,rho_tw_g
...
& prtvol,pawcross,rhot1_q_m)
 ...
 use m_wfs,           only : wfd_t, wfd_get_ur, wfd_get_cprj, wfd_change_ngfft, wfd_paw_get_aeur
...
!arrays
 complex(dpc), intent(out) :: rhot1_q_m(cryst%nattyp(itypatcor),2*lpawu+1,2*lpawu+1,sigp%npwx,Qmesh%nibz)
 complex(dpc), intent(out) :: M1_q_m(cryst%nattyp(itypatcor),2*lpawu+1,2*lpawu+1,sigp%npwx,Qmesh%nibz)
...
!scalars
 ...
 integer :: ik_bz,ik_ibz,isym_q,iq_bz,iq_ibz,spin,isym,itypatcor_read,jb,iat,iatom
...
!    close(2011)
   ...
   if (prtvol>10.and.jk_bz==1) then ! probably just to print one time.
         ...
         open(unit=2022,file='normeG',form='formatted',status='unknown')            
...
!   write(6,*) "AA",Wfd%my_rank,spin
   ...
   ABI_CHECK_ALLOC("out of memory in wfr_bdgw")
   do jb=ib1,ib2
...
!  ************************************8
         ...
         if (ib_sum.GE.ib1.AND.ib_sum.LE.ib2) then
           ...
           do iat=1, cryst%nattyp(itypatcor)
             do m1=1,2*lcor+1
               do m3=1,2*lcor+1
                  M1_q_m(iat,m1,m3,:,iq_ibz)=M1_q_m(iat,m1,m3,:,iq_ibz)+&
&                  rhotwg_ki(:,jb)*coeffW_BZ(iat,spin,jb,jk_bz,m3)*conjg(coeffW_BZ(iat,spin,ib_sum,ik_bz,m1))
               enddo
             enddo
           enddo
         end if
             ...
             do m1=1,2*lcor+1
               do m3=1,2*lcor+1
                 if(m1==2.and.m3==2) then
                  ...
                  rhot1_q_m(iat,m1,m3,:,iq_ibz)=rhot1_q_m(iat,m1,m3,:,iq_ibz)+&
&                  rhotwg_ki(:,jb)*coeffW_BZ(iat,spin,jb,jk_bz,m3)*conjg(coeffW_BZ(iat,spin,ib_sum,ik_bz,m1))*weight
               enddo
             enddo
           enddo
         end if
