...
!! Gsph_wfn<gsphere_t data type> The G-sphere used to describe the wavefunctions.
...
!!      calc_wfwfg,chi0_bbp_mask,completechi0_deltapart,cwtime,destroy_paw_pwij
!!      flush_unit,get_bz_diff,get_bz_item,get_gftt,get_uug,gsph_fft_tabs
!!      gsph_free,gsph_in_fftbox,gw_eet_chi0,hilbert_transform,init_paw_pwij
!!      littlegroup_print,make_transitions,paw_cross_rho_tw_g,paw_rho_tw_g
!!      paw_symcprj,pawcprj_alloc,pawcprj_destroy,read_plowannier,rho_tw_g
!!      setup_spectral,symmetrize_afm_chi0,timab,wfd_change_ngfft
!!      wfd_distribute_kb_kpbp,wfd_get_cprj,wfd_get_ur,wfd_paw_get_aeur
!!      wfd_sym_ur,wrtout,xmpi_sum
!!
!! SOURCE
...
& chi0,ktabr,ktabrf,Ltg_q,chi0_sumrule,Wfd,Wfdf)
 ...
 use m_wfs,           only : wfd_get_ur, wfd_t, wfd_distribute_kb_kpbp, wfd_get_cprj, wfd_barrier, wfd_change_ngfft,&
...
!scalars
 ...
 integer :: bandinf,bandsup,dim_rtwg,band1,band2,idle,ierr
 integer :: ig1,ig2,iat,ik_bz,ik_ibz,ikmq_bz,ikmq_ibz
 integer :: io,iomegal,iomegar,isym_k,itypatcor,nfft
...
&    0,Ep%zcut,zero,my_wl,my_wr,kkweight)
   ...
   ABI_CHECK_ALLOC('out-of-memory in sf_chi0')
   sf_chi0=czero_gw
...
&                       nfft,ngfft_gw,use_padfft,igfftepsG0,gw_gbound,gw_mgfft,spin, &
...
&                       qp_occ,nspinor,tim_fourdp1,bbp_ks_distrb_EET,nbopt)
       nbmax=nbopt
     end if
...
&                                 coeffW_BZ(iat,spin,band2,ikmq_bz,m2)*conjg(coeffW_BZ(iat,spin,band2,ikmq_bz,m2)))
                     enddo
                   enddo
                 enddo
               ...
               if (band2<=ucrpa_bands(2).AND.band2>=ucrpa_bands(1)) then
                 do iat=1, cryst%nattyp(itypatcor)
                   do m1=1,2*lcor+1
                     fac3=fac3-real(coeffW_BZ(iat,spin,band2,ikmq_bz,m1)*conjg(coeffW_BZ(iat,spin,band2,ikmq_bz,m1)))
                   enddo
                 enddo
...
&            Ltg_q,Gsph_epsG0,Ep%npwepG0,rhotwg,chi0_sumrule)
   ...
   if (Psps%usepaw==1) then
     call pawcprj_destroy(Cprj2_k  )
     ABI_DT_FREE(Cprj2_k)
     call pawcprj_destroy(Cprj1_kmq)
