...
!!
!! COPYRIGHT
!! Copyright (C) 2008-2014 ABINIT group (FB, MG)
...
&  history, Error, with_etsf_header=.TRUE., overwrite=.FALSE.)
 ...
 cplex=2 ! Needed to store complex quantities
...
& lstat,Error_data=Error)
 ...
 ABI_MALLOC(full_mtmp,(Sigp%nbnds,Sigp%nbnds,Kmesh%nibz,Sigp%nsppol))
 do is=1,Sigp%nsppol
  do ik=1,Kmesh%nibz
 ...
 ABI_MALLOC(rdata5,(cplex,Sigp%nbnds,Sigp%nbnds,Kmesh%nibz,Sigp%nsppol))
...
!#if defined HAVE_TRIO_ETSF_IO
  ...
  ABI_MALLOC(full_rmtmp,(2,nbandR,nbandR,nkibzR,nsppolR))
