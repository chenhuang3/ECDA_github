...
!!
!! NOTES
!!  This is a module. It is made of two functions and one init subroutine
!!
!! NOTES
!!
!! PARENTS
...
module ftfvw1
  ...
  integer,save,private                  :: intxc,ixc,izero
  integer,save,private                  :: n3xccc
  integer,save,private                  :: ngfft(18),nfftf
  integer,save,private                  :: nhatgrdim,usepaw,usexcnhat
  integer,save,private                  :: nkxc
  integer,save,private                  :: nspden
  type(dataset_type),save,private       :: dtset
  ! MG: Why this? 
  type(MPI_type),save,private           :: mpi_enreg

  real(dp),save,allocatable,private     :: deltaW(:,:)
  real(dp),save,private                 :: gprimd(3,3)
  real(dp),save,private                 :: gsqcut
  real(dp),save,allocatable,private     :: lavnlfft(:,:)
  real(dp),save,allocatable,private     :: nhat(:,:),nhatgr(:,:,:)
  real(dp),save,private                 :: rprimd(3,3)
  real(dp),save,private                 :: ucvol
  real(dp),save,allocatable,private     :: vpion(:,:)
  real(dp),save,allocatable,private     :: xccc3d(:)
  real(dp),save,private                 :: Z
  !! common variables computed
  logical,save,private :: ok=.false.
  integer,save,private :: nfftot
  real(dp),save,private:: alpha
  real(dp),save,allocatable,private     :: rhog(:,:),rhogsp(:,:,:),kxc(:,:)
...
!   real(dp)             :: dummy_strsxc(6)
    ...
    if(.not.ok) then
       ...
       mpi_enreg=mpi_enreg_in !FDM : copy mpi_enre befor fourdp to have fft_distrib tabs
...
!End of the abilint section
    ...
    if(Z*dotr.lt.zero) then 
      MSG_ERROR('the density renormalisation is negative...')
    end if
