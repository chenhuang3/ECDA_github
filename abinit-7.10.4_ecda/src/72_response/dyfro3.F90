...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2014 ABINIT group (XG,MT)
...
! *************************************************************************
 ...
 if(nspden==4)then
   MSG_ERROR('dyfro3 : does not work yet for nspden=4')
 end if
...
&     mpi_comm_fft=mpi_enreg%comm_fft,distribfft=mpi_enreg%distribfft)
   ...
   call atm2fft(atindx1,dum_atmrho,dum_atmvloc,dyfrxc,dyfrlo,eei,dum_eltfrxc,&
&   dum_gauss,gmet,gprimd,dum_grn,dum_grv,gsqcut,mgfft,mqgrid,natom,nattyp,nfft,ngfft,ntypat,&
&   optatm,optdyfr,opteltfr,optgr,optn,optn2,optstr,optv,pawtab,ph1d,qgrid,qprtrb,rhog,&
...
!Symmetrize local part of the dynamical matrix dyfrlo:
 ...
 call sydy3(1,dyfrsym_tmp,indsym,natom,0,nsym,qphon,dyfrlo_tmp2,symq,symrec)
...
!atindx1 is used to reestablish the correct order of atoms
 ...
 call sydy3(dyfr_cplex,dyfrsym_tmp,indsym,natom,dyfr_nondiag,nsym,qphon,dyfrnl,symq,symrec)
 ABI_DEALLOCATE(dyfrsym_tmp)
