...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (PB, XG)
...
!!            <k+Q,n'|H(1)|k,n> (hartree) (calculated in cgwf3).
...
!!
!! CHILDREN
!!      distrb2,dotprod_g,kptfine_av,smeared_delta,timab,xmpi_sum
!!
!! SOURCE
...
!Init interpolation method
 ...
 if(nsppol==2)then
   message = 'nsppol=2 is not yet treated in eig2stern. Sorry for this ...'
   MSG_ERROR(message)
 end if        
...
!Add scissor shift to eigenenergies
 if (dtset%soenergy > tol6 ) then
   write(msg,'(a,f7.3,2a)')&
&   ' A scissor operator of ',dtset%soenergy*Ha_eV,' [eV] has been applied to the eigenenergies',ch10
     ...
     if (dtset%soenergy > tol6 ) then
       do iband=1,mband
         if (occ(iband+bandtot_index) < tol6) then
           eigen0(iband+bandtot_index) = eigen0tmp(iband+bandtot_index) + dtset%soenergy
           eigenq(iband+bandtot_index) = eigenqtmp(iband+bandtot_index) + dtset%soenergy
         else
...
!                Add the contribution of non-active bands, if DFPT calculation (= Sternheimer)
                 if(ieig2rf == 1 .or. ieig2rf ==3 .or. ieig2rf ==4  ) then
!                  if(ieig2rf == 1   ) then
