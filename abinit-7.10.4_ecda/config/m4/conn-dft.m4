...
AC_DEFUN([_ABI_DFT_CHECK_BIGDFT],[
  dnl Init
  abi_dft_bigdft_default_libs="-lbigdft -lpoissonsolver -labinit"
  ...
  tmp_saved_LIBS="${LIBS}"
  FCFLAGS="${FCFLAGS} ${abi_dft_bigdft_incs}"
...
#include "xc.h"
  ...
  dnl Check whether LibXC proposes xc_f90_functional_get_number
  if test "${abi_dft_libxc_has_incs}" = "yes" -a \
          "${abi_dft_libxc_version}" = "yes"; then
    AC_MSG_CHECKING([whether LibXC has xc_f90_functional_get_number()])
    AC_LANG_PUSH([Fortran])
    AC_LINK_IFELSE([AC_LANG_PROGRAM([],
      [[
        use xc_f90_lib_m
      ...
      ]])], [abi_dft_libxc_has_get_number="yes"], [abi_dft_libxc_has_get_number="no"])
    AC_LANG_POP([Fortran])
    AC_MSG_RESULT([${abi_dft_libxc_has_get_number}])
  fi
    ...
    for abi_dft_flavor in ${abi_dft_iter}; do
      ...
      case "${abi_dft_flavor}" in
        ...
        libxc)
          ...
          if test "${abi_dft_libxc_has_get_number}" = "yes"; then
            AC_DEFINE([HAVE_DFT_LIBXC_GET_NUMBER],1,
              [Define to 1 if LibXC has the xc_f90_functional_get_number function.])
          fi
          ;;
      ...
      if test "${tmp_dft_fallback}" = "yes"; then
        ...
        if test "${tmp_dft_serial}" = "no"; then
          if test "${tmp_dft_libs}" = "" -a "${tmp_dft_prereqs}" != "no"; then
