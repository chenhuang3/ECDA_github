...
! pbar(3) and dbar(3) are reduced polarization and displacement field, defined by Eq.(27) and (29) Nat. Phys. suppl. (2009)
 ...
 logical :: localize_rho=.false. , xc_patch
 ...
 integer  :: do_exx, & 
             ...
             type_of_system,  & 
             i, j, k, isp, myicount, & 
             c_nsym, tmp_int, calc_mode
 integer  :: n1, n2, n3, eEvalcount_c,io, band_index, resume
 logical  :: tigger_mode,ex,file_exist ,file_exist2
 real(dp) :: histW(4),net_value,el_renorm,nalpha,nbeta, & 
             entropy_tmp,total_zion,tmp_occ(dtset%nsppol), & 
             dfet_fermi_level(dtset%nsppol)
...
! penalty funciton related
 real(dp) :: penFun, penLambda, lapvfg(dtset%nfft), kepot(dtset%nfft)
...
!! =================== for KLI-EXX ===================
 integer  :: iter_kli, nband, ib1, ib2, mix_type
 integer  :: counter, ib, ipoint, exx_file=1973
 integer  :: ik, nband_k, iband, icg_shift, icg, ipw
 !
 real(dp) :: dtmp, dvol, hgx, hgy, hgz, alpha, mix_beta, d_resid, & 
             my_resid(dtset%nfft,dtset%nspden,2), & 
             den_orb(dtset%nfft), & 
             V_slater(dtset%nfft,dtset%nspden), & 
             V_second(dtset%nfft,dtset%nspden), & 
             kpot(dtset%nfft), & 
             old_vxc(dtset%nfft,dtset%nspden,2), & 
             vxc_kli(dtset%nfft,dtset%nspden), &      ! KLI spin polarized potential vxc
             uxc(dtset%nfft), resid_up, resid_dn,  & 
             chen_rhor(dtset%nfft,dtset%nspden), & 
             eps_exx(dtset%nfft), &                   ! XC energy per particle
             exx, vxc_kli_bar, uxc_bar, &             ! <phi_i|vxc_kli|phi_i>
...
!  Compute the density from the trial potential
   ...
   if (mpi_enreg%me==0) then 
     call system("rm invKS_log")
     open(file='oep.out',unit=exx_file,form='formatted',action='write')
     write(message,'(a)')'oep files are written to oep.out'
...
!!   call load_param(bLoadDen,bLoadVr,maxcount,bAllowsym,wtol,structure,ektol,penLambda,el_renorm)
   ...
   call c_wrtlog(message);
   write(message,'(a,es10.2)') 'penalty coef = ',penLambda
   call c_wrtlog(message)
...
do while(.true.)
   ...
   !
   if (file_exist) then 
     ! save wave function for restart
     open(file='control.dat',action='read',unit=111)
     read(111,*)type_of_system
     read(111,*)calc_mode 
     read(111,*)do_exx
     close(111)
     call c_wrtlog("")
     call c_wrtlog("found control.dat file, read it in.")
     write(message,*)"type_of_system:     ",type_of_system; call c_wrtlog(message)
     write(message,*)"calc_mode:          ",calc_mode;      call c_wrtlog(message)
     write(message,*)'do_exx:             ',do_exx;         call c_wrtlog(message)
     call c_wrtlog("")
     !
     xc_patch = .false.
     if (calc_mode==4) then 
     ...
     !
     ! ======== for localizing KS subsystems =======
     ! we simply just do not load bare vks
     !
     localize_rho = .false.
     if (calc_mode==12) then 
       calc_mode=2 
       localize_rho = .true.
     endif
     if (calc_mode==11) then 
       calc_mode=1
       localize_rho = .true.
     endif
     write(message,*)"localize_rho: ",localize_rho
     call c_wrtlog(message)
   endif
   
   !
   ! new density, energies from new occupation numbers 
   !
   if (calc_mode==2  .and. type_of_system==1) then 
     call c_wrtlog("")
     call c_wrtlog("==== Update electron density with new occupation numbers =======")
     call c_wrtlog("")
     !
     ! load new_occ.tmp file from DFET code
     !
     occ = 0.d0
     open(file='new_occ.tmp',action='read',unit=111)
     do isp=1,dtset%nsppol
       read(111,*)stemp,dfet_fermi_level(isp)
     enddo
     ! read in eigen values
     ii=0
     do isp=1,dtset%nsppol
       do ikpt=1,dtset%nkpt
         do iband=1,dtset%nband(ikpt+dtset%nkpt*(isp-1))
            ii=ii+1
            read(111,*)tmp_int,occ(ii)
            print *,'new_occ: ',occ(ii)
         enddo
       enddo
     enddo
     close(111)
     write(message,'(a)')'new_occ.tmp from DFET is loaded.'
     call c_wrtlog(message)

     !
     ! obtain total electron number and magnetic moment 
     ! based on the incoming new_occ array
     !
     tmp_nelect=zero; 
     index=0
     tmp_spinup_nelect=0.0d0
     tmp_spindn_nelect=0.0d0
     maxocc = 2.d0/(dtset%nsppol*dtset%nspinor)
     !
     do isp=1,dtset%nsppol
       do ikpt=1,dtset%nkpt
         do iband=1,dtset%nband(ikpt+dtset%nkpt*(isp-1))
          ...
          ! entropy=entropy+dtset%wtk(ikpt)*ent(index)
           tmp_nelect=tmp_nelect+dtset%wtk(ikpt)*occ(index)
           if (isp==1) tmp_spinup_nelect = tmp_spinup_nelect + dtset%wtk(ikpt)*occ(index)
           if (isp==2) tmp_spindn_nelect = tmp_spindn_nelect + dtset%wtk(ikpt)*occ(index)
         end do
       end do
     end do
     !
     ! set dtset%nelect and dtset%spinmagntarget
     !
     dtset%nelect         = tmp_nelect
     dtset%spinmagntarget = tmp_spinup_nelect - tmp_spindn_nelect
     print *,' dtset%nelect: ',dtset%nelect
     print *,' spinmagntarget: ',dtset%spinmagntarget
     !
     call c_wrtlog("")
     write(message,'(a,f12.6,a,f12.6)') & 
       'calc_mode==2. Reset the nelect and spinmagntarget to ', & 
       dtset%nelect,' and ',dtset%spinmagntarget
     call c_wrtlog(message)
     call c_wrtlog("")
   ...
   ! ----------------------------------------------------
   if (localize_rho) then 
     write(message,*)' localize_rho = .true. subvks is just set to vlocal (vpsp)'
     call c_wrtlog(message)
     do isp=1,dtset%nsppol
       bare_vks(:,isp) = vpsp
     enddo
...
!   open(file='rho.dat',action='read',unit=111,form='unformatted')
!   close(111)
!   call fourdp(1,rhog,rhor(:,1),-1,mpi_enreg,nfftf,ngfftf,dtset%paral_kgb,0)
!   ! xc pot
!   optxc = 1
...
!!! end of debug
   ...
   if ( (localize_rho .eqv. .false.) .and. (xc_patch .eqv. .false.)) then 
     !
     ! subsystem 
     !
     ...
     if (type_of_system==2) then 
       open(unit=111,action='read',file='vhxc.dat',form='unformatted')
       read(111)bare_vks
       close(111)
       write(message,'(a,2e12.4)')'min/max(vks_alpha):',minval(bare_vks(:,1)),maxval(bare_vks(:,1))
       call c_wrtlog(message)
   ...
   !
   ! final vks for global system 
   !
   if (calc_mode==1) then  
      if (type_of_system == 2) then 
         do isp=1,dtset%nspden
           vtrial(:,isp) = bare_vks(:,isp) + vpsp
         enddo
         write(message,'(a,2f18.6)')'vpsp: ',minval(vpsp),maxval(vpsp)
         call c_wrtlog(message)
         call c_wrtlog("")
   ...
   !
   if (calc_mode==2  .and. type_of_system==1 ) then
     ! since v_KS is not changed, wf solve show return immediately 
     dtset%tolwfr = 10.0  ! set the stop criterion for cg solver: cgwf.F90 and lobpcgwf.F90
     dtset%nline  = 1     ! restart CG each 10 steps
   endif
...
!&      istep_mix,istep_mix,kg,kg_diel,kxc,lmax_diel,mgfftdiel,mpi_enreg,&
!&      psps%mpsang,dtset%natom,nattyp,nfftf,nfftdiel,ngfftdiel,nhat,nkxc,&
!&      npwarr,npwdiel,res2,dtset%nspinor,psps%ntypat,nvresid,occ,&
...
&     ylm,ylmgr,ylmdiel,vxctau=vxctau)
   ...
   if (mpi_enreg%me==0) then 
     ...
     write(111,'(a,i5,a,i5,a,f12.6)') & 
       "nband: ",dtset%nband(1), & 
       " nkpt  ",dtset%nkpt, & 
       " tsmear_eV: ",dtset%tsmear*27.2114d0
     ! 
     ii = 0
     do isp=1,dtset%nsppol
       do ikpt=1,dtset%nkpt
...
!   call c_wrtlog(message)
   ...
   if (do_exx>0 .and. mpi_enreg%me==0) then 
     ...
     do isp=1,dtset%nspden 
       ...
       enddo
     enddo
     write(message,'(a,2f14.6)')'q_alpha,q_beta: ', & 
       sum(chen_rhor(:,1))*dvol,sum(chen_rhor(:,2))*dvol
     call c_wrtlog(message)
     ! 
     ! Solving the KLI OEP equation 
     !
     hgx = rprimd(1,1)/dble(ngfft(1))
     hgy = rprimd(2,2)/dble(ngfft(2))
     hgz = rprimd(3,3)/dble(ngfft(3))
     !
     write(1973,*)'iter |vxc_ch|[alpha]  |vxc_ch|[beta]'
     call flush(1973)
...
!!   phir(:,1,:) = phir(:,2,:)
     ...
     do iter_kli=1,10000
        ...
        print *,''
        print *,'   ========= KLI iter: ',iter_kli,' =========='
        print *,''
        do isp=1,dtset%nspden
          write(*,'(a,2es12.4)') '(rhohxc) vxc_kli: ', & 
        ...
        V_slater = 0.d0 
        !
        ! compute vxc_kli_bar and uxc_bar 
        ! compute v_slater, and other terms for KLI potenital 
        !
        exx= 0.0d0
        eps_exx=0.d0
        !
        do isp = 1,dtset%nspden
           ...
           do ib1 = 1, dtset%nband(1)
             uxc = 0.d0
             if (chen_occ(isp,ib1)<1e-4) cycle
             !
             do ib2=1,dtset%nband(1)
                if (chen_occ(isp,ib2)<1e-4) cycle
                !
                ! kpot
                !
                ...
                write(*,'(a,2es12.4)') '(rhohxc) kpot: ',minval(kpot),maxval(kpot)
                dtset%icoulomb = 0
                !
                ! compute uxc  
                !
                ...
                ! exx energy 
                eps_exx = eps_exx - 0.5d0*chen_occ(isp,ib1)*chen_occ(isp,ib2) & 
                                    * phir(:,isp,ib1)*phir(:,isp,ib2)*kpot
             enddo
             !
             ! slater exchange potenital 
             V_slater(:,isp) = V_slater(:,isp) - uxc * chen_occ(isp,ib1)*phir(:,isp,ib1)
             !
             uxc_bar = -sum(phir(:,isp,ib1)*uxc)*dvol
             !
             den_orb = chen_occ(isp,ib1)*phir(:,isp,ib1)**2
             !
             vxc_kli_bar = sum(phir(:,isp,ib1)*vxc_kli(:,isp)*phir(:,isp,ib1))*dvol
             !
             V_second(:,isp) = V_second(:,isp) + den_orb * (vxc_kli_bar - uxc_bar)
           enddo
        enddo
        !
        exx = sum(eps_exx)*dvol
        ! 
        ! done EXX and EXX potential 
        !
        V_slater = V_slater / (chen_rhor+1e-5)
        V_second = V_second / (chen_rhor+1e-5)
        !
        print *,''
        print *,'v_slater: ',minval(v_slater(:,1)),maxval(v_slater(:,1)),' spin up'
        print *,'v_slater: ',minval(v_slater(:,2)),maxval(v_slater(:,2)),' spin dn'
        print *,''
        print *,'v_second: ',minval(v_second(:,1)),maxval(v_second(:,1)),' spin up'
        print *,'v_second: ',minval(v_second(:,2)),maxval(v_second(:,2)),' spin dn'
        print *,''
        !
        old_vxc(:,:,2) = old_vxc(:,:,1)  ! older 
        old_vxc(:,:,1) = vxc_kli         ! store new
        ! 
        ! new vxc_kli
        vxc_kli = v_slater + v_second
        !
        resid_up = sum(dvol*(vxc_kli(:,1)-old_vxc(:,1,1))**2)
        resid_dn = sum(dvol*(vxc_kli(:,2)-old_vxc(:,2,1))**2)
         !
        write(message,'(a,i3,a,es10.3,a,es10.3,a,2es12.4,a,2es12.4,a,f14.8)') & 
         'iter: ',iter_kli,' resid_up: ', resid_up, ' resid_dn: ',resid_dn, & 
         ' vxc_up: ',minval(vxc_kli(:,1)),maxval(vxc_kli(:,1)), & 
         ' vxc_dn: ',minval(vxc_kli(:,2)),maxval(vxc_kli(:,2)), ' exx: ', exx
        call c_wrtlog(message)
        d_resid =max( resid_up, resid_dn) 
        if (d_resid<1e-5) then 
          write(message,*)'d_resid is less than 5e-4, done!'
          call c_wrtlog(message)
          exit
        endif 
        !
        ...
        !
       ! if (iter_kli==1) then 
          my_resid(:,:,1) = vxc_kli - old_vxc(:,:,1)
          vxc_kli = vxc_kli*0.5d0 + old_vxc(:,:,1)*0.5d0
       ! else 
          ! anderson mixing 
       !   my_resid(:,:,2) = my_resid(:,:,1) 
       !   my_resid(:,:,1) = vxc_kli - old_vxc(:,:,1)
       !   alpha = - sum( my_resid(:,:,2)*(my_resid(:,:,1)-my_resid(:,:,2))) / & 
       !             sum( (my_resid(:,:,1)-my_resid(:,:,2))**2)
       !   !write(exx_file,'(a,f12.4,a,f12.4)')'anderson_alpha: ',alpha,' mix_beta: ',mix_beta
       !   !call flush(exx_file)
       !   ! mixing 
       !   mix_beta = 0.70d0
       !   vxc_kli = (old_vxc(:,:,1) + mix_beta*my_resid(:,:,1)) * alpha  & 
       !           + (old_vxc(:,:,2) + mix_beta*my_resid(:,:,2)) * (1.d0-alpha)
       ! endif
        !
     enddo
   endif 
   ...
   if (mpi_enreg%me==0) then 
     if (do_exx>0) then 
       write(message,'(a)')'write exx files '
       call c_wrtlog(message)
       open(file='exx.dat',unit=111,action='write',form='unformatted')
       write(111) vxc_kli
       write(111) exx
       write(111) eps_exx
       close(111)
     ...
     ! dump vpsp for dfet to unscreening 
    ! open(file='vxc.dat',action='write',form='unformatted',unit=111)
    ! write(111)energies%e_xc
    ! write(111)vxc
    ! close(111)
    ! call c_wrtlog("exc and vxc are computed from rhohxc() and dumpped to vxc.dat file ")
     write(message,'(a,2f12.5)')'min/max vxc:',minval(vxc),maxval(vxc)
     call c_wrtlog(message)

    ! ! dump vpsp for dfet to unscreening 
    ! open(file='vhartr.dat',action='write',form='unformatted',unit=111)
    ! write(111)energies%e_hartree
    ! write(111)vhartr
    ! close(111)
