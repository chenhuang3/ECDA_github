...
class TestBot(object):
    ...
    def run(self):
        ...
        if self.runmode == "static":
            ...
            np_list = [2, 4, 10, 24]
            (nfailed, npassed) = self.run_tests_with_np(1, suite_args=suite_args, runmode=self.runmode)
        else:
            # New mode: run all available tests with 2 MPI nodes here, 
            # then enter the mp_suites with [4, 10, 24] CPUs
            np_list = [4, 10, 24]
            nfailed, npassed = self.run_tests_with_np(2, suite_args=suite_args, runmode=self.runmode)
        ...
        if self.has_mpi:
            ...
            for np in np_list:
                ...
                (para_nfailed, para_npassed) = self.run_tests_with_np(np, runmode="static", suite_args=suite_args)
                #
        ...
        #self.summary.cpkl_dump("testbot_summary.cpkl")
