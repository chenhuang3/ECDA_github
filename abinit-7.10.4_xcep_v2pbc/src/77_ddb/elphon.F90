...
!!
!! COPYRIGHT
!! Copyright (C) 2004-2014 ABINIT group (MVer,BXu,MG)
...
!!
!! CHILDREN
!!      a2f_dump,a2f_free,a2f_init,complete_gamma,complete_gamma_tr
!!      copy_kptrank,d2c_weights,ebands_free,ebands_from_hdr,eliashberg_1d
!!      elph_ds_clean,elph_k_procs,elph_tr_ds_clean,ep_fs_weights,ep_setupqpt
!!      ftgam,ftgam_init,gamma_free,gamma_init,gamma_interp_setup,gamma_linwid
!!      get_all_gkk2,get_all_gkq,get_all_gkr,get_fs_bands,get_nv_fs_en
!!      get_nv_fs_temp,get_rank_1kpt,get_tau_k,get_veloc_tr,hdr_comm,hdr_free
!!      hdr_io,integrate_gamma,integrate_gamma_tr,integrate_gamma_tr_lova,mka2f
!!      mka2f_tr,mka2f_tr_lova,mka2fqgrid,mkfskgrid,mknesting,mkph_linwid
!!      mkqptequiv,new_integrate_gamma,new_integrate_gamma_tr
!!      new_integrate_gamma_tr_lova,order_fs_kpts,outelph,printvtk,rchkgsheader
!!      read_el_veloc,symkpt,timein,update_occ,wrap2_pmhalf,wrtout,xmpi_bcast
!!
!! NOTES
!!
!! SOURCE
...
subroutine elphon(anaddb_dtset,Cryst,Ifc,filnam)
 ...
 use m_header,          only : hdr_get_nelect_byocc, hdr_free, hdr_comm, hdr_io
 use m_crystal,         only : crystal_t
 use m_ebands,          only : ebands_from_hdr, update_occ, ebands_free, ebands_3dprint
 use m_ifc,             only : ifc_type
 use m_nesting,         only : mknesting
 use m_gamma,           only : gamma_t, gamma_free, gamma_init, gamma_interp_setup, gamma_linwid, &
...
!scalars
 ...
 integer :: a2f_nqshift,rdwr, fform,ie,ie1,ie2,i_start,i_end
...
!arrays
 ...
 integer :: qptrlatt(3,3),a2f_qptrlatt(3,3),kptrlatt_fine(3,3)
...
& '-begin elphon at tcpu',tcpui,'  and twall',twalli,' sec'
 ...
 comm = xmpi_world
 nproc = xcomm_size(comm); me = xcomm_rank(comm)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 elph_ds%elphsmear    = anaddb_dtset%elphsmear     ! smearing for Eliashber functions
...
!the following is for the non master nodes
 call hdr_comm(hdr,master,me,comm)
...
!==================================================
 ...
 ABI_CHECK_ALLOC('out-of-memory in eigenGS')
...
!==================================================
 ...
 if (abs(anaddb_dtset%elph_fermie) > tol10) then
   ...
   call wrtout(std_out,message,'COLL')
   call ebands_from_hdr(Bst,Hdr,nband,eigenGS)
...
&   anaddb_dtset%ep_extrael
   ...
   call ebands_from_hdr(Bst,Hdr,nband,eigenGS,nelect=elph_ds%nelect)
...
!  keep the band dependency btw elph_ds%minFSband and elph_ds%maxFSband
 ...
 ABI_CHECK_ALLOC("allocating elph_ds%k_phon%kptirr")

 ABI_ALLOCATE(elph_ds%k_phon%irredtoGS,(elph_ds%k_phon%nkptirr))
 ABI_CHECK_ALLOC("allocating irredtoGS_phon")
...
!====================================================================
   ...
   ABI_CHECK_ALLOC("allocating indkpt1,wtk_fullbz,wtk_folded")
...
&   Cryst%nsym,Cryst%symrec,timrev,wtk_fullbz,wtk_folded)
   ...
   ABI_CHECK_ALLOC("allocating elph_ds%k_phon%new_kptirr")
   
   ABI_ALLOCATE(elph_ds%k_phon%new_wtkirr,(elph_ds%k_phon%new_nkptirr))
   ABI_CHECK_ALLOC("allocating elph_ds%k_phon%new_wtkirr")
   
   ABI_ALLOCATE(elph_ds%k_phon%new_irredtoGS,(elph_ds%k_phon%new_nkptirr))
   ABI_CHECK_ALLOC("allocating new_irredtoGS_phon")
...
!=====================================================
 if (anaddb_dtset%use_k_fine == 1) then
   ...
   call hdr_comm(hdr1,master,me,comm) 

   ABI_ALLOCATE(eigenGS_fine,(nband,hdr1%nkpt,elph_ds%nsppol))
   ABI_CHECK_ALLOC("allocating eigenGS_fine")
...
!  Be careful. This part has not been tested.
   ...
   call ebands_from_hdr(Bst,hdr1,nband,eigenGS_fine)
...
&   elph_ds%k_fine%nkptirr,elph_ds%k_fine%irredtoGS)
   ...
   ABI_CHECK_ALLOC("allocating elph_ds%k_fine%wtk")
...
!  call elph_k_copy(elph_ds%k_phon, elph_ds%k_fine)
   ...
   ABI_CHECK_ALLOC('out-of-memory in eigenGS_fine')
...
&   sum(elph_ds%k_fine%wtk(:,:,2)),elph_ds%k_fine%nkpt
 ...
 ABI_CHECK_ALLOC("allocating elph_ds%gkk_intweight")
...
!  FIXME
!  MG shiftk is defined neither in the anaddb nor in the hdr data type
...
!  MG Don't know what happens if use_k_fine == 1
   shiftk=one
   do ii=1,hdr%nkpt
     if (all(hdr%kptns(:,ii) == zero)) shiftk=zero
   end do

   if (ebands_3dprint(Bst,Cryst,elph_ds%kptrlatt,1,shiftk,fname) /= 0) then
...
!mapping of k + q onto k' for k and k' in full BZ
 ...
 ABI_CHECK_ALLOC("allocating FSfullpqtofull")
...
!qpttoqpt(itim,isym,iqpt) = qpoint index which transforms to iqpt under isym and with time reversal itim.
 ...
 ABI_CHECK_ALLOC("allocating qpttoqpt")
...
!to be consistent with the dimensions of the rpt, which come from anaddb.
 ...
 ABI_CHECK_ALLOC('out of memory in array elph_ds%gamma_rpt')
...
!  if (.TRUE.) then
   ...
   call gamma_init(Gam,gkk_fname,elph_ds,Cryst,ifc,Bst,FSfullpqtofull,ifc%gprim,qptrlatt,Ifc%nrpt,Ifc%rpt,Ifc%wghatm)

   call gamma_interp_setup(Gam,Cryst,"INIT")

   call gamma_linwid(Gam,Cryst,ifc,elph_ds,anaddb_dtset%nqpath,anaddb_dtset%qpath)
...
!  a2f_qptrlatt = RESHAPE( (/4,0,0,0,4,0,0,0,4/),(/3,3/) )
   ...
   call a2f_init(A2f,Cryst,ifc,Gam,elph_ds%na2f,elph_ds%a2fsmear,elph_ds%n0,&
...
!==========================================================
 ...
 if (elph_tr_ds%ifltransport == 1)then ! LOVA
   ...
   ABI_CHECK_ALLOC('trying to allocate array elph_tr_ds%gamma_rpt_trout')
   elph_tr_ds%gamma_rpt_trout = zero

   ABI_ALLOCATE(elph_tr_ds%gamma_rpt_trin,(2,9,elph_ds%nbranch**2,elph_ds%nsppol,Ifc%nrpt))
   ABI_CHECK_ALLOC('trying to allocate array elph_tr_ds%gamma_rpt_trin')
...
!  moved from integrate_gamma_tr to here
   ...
   ABI_CHECK_ALLOC("trying to allocate array elph_tr_ds%gamma_qpt_tr")

   ABI_ALLOCATE(elph_tr_ds%gamma_rpt_tr,(2,9,elph_ds%nbranch**2,elph_ds%nsppol,Ifc%nrpt,4,elph_ds%n_pair))
   ABI_CHECK_ALLOC('trying to allocate array elph_tr_ds%gamma_rpt_tr')
...
!======================================================
 ...
 ABI_CHECK_ALLOC("allocating a2f_1d,dos_phon")
