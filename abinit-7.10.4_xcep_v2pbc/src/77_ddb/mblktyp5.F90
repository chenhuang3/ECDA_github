...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (XG,MT,SP)
...
!!
!! CHILDREN
!!      cmpar8,ddb_free,ddb_getdims,ddb_malloc,ioddb8_in,ioddb8_out
!!      pawtab_destroy,pawtab_nullify,psddb8,read_blok8,write_blok8,wrtout
!!
!! SOURCE
...
&   msym_tmp,natom,nblok,nkpt,ntypat,ddbun,usepaw_tmp,vrsddb,xmpi_self)
...
&         msize,nkpt,ddbuntmp,blkval2(:,:,:,:),kpnt(:,:,ii))
         choice=2
         call write_blok8(ddb,ii,choice,nband(1),mpert,&
...
!  Also write summary of bloks at the end
   ...
   do iblok=1,nblokt
     if(mgblok(iblok)==1)then
       call write_blok8(ddb,iblok,choice,nband(1),mpert,&
&       msize,nkpt,nunit)
     end if
   end do
