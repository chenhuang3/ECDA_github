...
!!
!! CHILDREN
!!      fftpac,fourdp,fourwf,kgindex,metric,mkffnl,nonlop,out1dm,ph1d3d
...
!scalars
 integer :: bdtot_index,choice,cpopt,dimffnl,i1,i2,i3,ia,iatom,iband
 integer :: icg,ider,idir,ierr,ifft,ifft1,ifft2,ii1,ii2,ii3,ikg,ikpt,ilm,index
 integer :: ipw1,ipw2,ispden,isppol,istwf_k,matblk,me,n1
 integer :: n2,n3,n4,n5,n6,nband_k,nkpg,nnlout,npw_k,option,paw_opt,signs
 integer :: spaceComm,tim_fourwf,tim_nonlop
...
!arrays
 ...
 real(dp) :: lavnlr(dtset%nfft,dtset%nspden),nonlop_dum(1,1),nonlop_dum2(1,1),dummy_lambda(1)
 ...
 real(dp) :: ylmgr_dum(1,1,1),qphon(3)
 real(dp),allocatable :: bigdielmat(:,:,:),bigrdielmat(:,:,:),buffer(:)
 real(dp),allocatable :: cwavef(:,:),dummyt2(:,:),dummyt3(:,:,:)
 real(dp),allocatable :: eig_k(:),ffnl(:,:,:,:),g2cart(:)
 real(dp),allocatable :: kpg_dum(:,:),lavnl3d(:,:,:),occ_k(:)
...
!Test size of FFT grids (1 grid in norm-conserving, 2 grids in PAW)
 if (psps%usepaw==1) then
   message = ' prctfw is able to cope only with norm-conserving pseudo potentials'
   MSG_BUG(message)
 end if
...
!    WARNING this may work only for dtset%nspden == 1
     call fftpac(ispden,mpi_enreg,dtset%nspden,n1,n2,n3,n4,n5,n6,dtset%ngfft,lavnlr,lavnl3d,option)
...
!    lavnlr(:,ispden) = lavnlr(:,ispden)-min+1.0d0
     ...
     call out1dm(filapp,mpi_enreg,dtset%natom,dtset%nfft,dtset%ngfft,dtset%nspden,dtset%ntypat,&
&     lavnlr,&
