...
!!      ab7_mixing_eval_deallocate,ab7_mixing_use_moving_atoms,fourdp,lavnl
!!      mean_fftr,metric,prcref_pma,prctfvw1,prctfvw2,timab,wvl_prcref
!!      wvl_vtrial_abi2big
!!
!! SOURCE
...
!scalars 
 ...
 integer,intent(in) :: moved_atm_inside,my_natom,n1xccc,n3xccc,nfft
 ...
 ! jmb real(dp),intent(out) :: eeig,ek,enl
 real(dp),intent(inout) :: eeig,ek,enl
...
! integer :: i1,i2,i3,ifft2,ifft3,ifft4,ifft5,ii1,ii2,ii3,ii4,ii5
 ...
 real(dp) :: dielng,diemix,fact,ucvol,ucvol_local,vme,vme_inold,vme_um
...
!in case we are using a tfvw based preconditioner compute the local average of the non local potential
 ...
 if(compute_lavnlr == 1) then
   call lavnl(atindx,atindx1,cg,dtset,eigen,&
...
&   istep,kg_diel,kxc,lavnlr,mgfft,moved_atm_inside,mpi_enreg,my_natom,&
...
!  Compute the mean of the old vtrial
   call mean_fftr(vtrial,vmean,nfft,nfftot,dtset%nspden,mpi_comm_sphgrid)
   call mean_fftr(vin_old,vmean_inold,nfft,nfftot,dtset%nspden,mpi_comm_sphgrid)
   call mean_fftr(vout_unmixed,vmean_um,nfft,nfftot,dtset%nspden,mpi_comm_sphgrid)
...
!to avoid charge sloshing in large system
 ...
 if(dtset%iprctfvw /= 0.and.(istep.gt.1).and.(mod(istep,1)==0))  then
   if(dtset%iprctfvw==1) then
     call prctfvw1(atindx,atindx1,cg,deltae,dtset,eeig,&
&     efermi,eigen,ek,enl,etotal,dtset%spinmagntarget,gsqcut,&
...
&     wvl,xccc3d,xred,ylm)
     ...
     call prctfvw2(atindx,atindx1,cg,dtset,eeig,&
&     efermi,eigen,ek,enl,dtset%spinmagntarget,gsqcut,&
&     kg,dtset%mband,mcg,mgfft,dtset%mkmem,mpi_enreg,psps%mpsang,dtset%mpw,dtset%natom,nattyp,nfft,nfftf,ngfftf,&
&     nhat,nhatgr,nhatgrdim,&
&     dtset%nkpt,nkxc,npwarr,dtset%nspden,dtset%nspinor,dtset%nsppol,psps%ntypat,n3xccc,occ,dtset%occopt,&
