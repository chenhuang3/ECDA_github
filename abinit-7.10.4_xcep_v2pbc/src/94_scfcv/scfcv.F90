...
! chen ------------------------
 ...
 use interfaces_53_spacepar, only: dotprod_vn 
...
! pbar(3) and dbar(3) are reduced polarization and displacement field, defined by Eq.(27) and (29) Nat. Phys. suppl. (2009)
 ...
 logical :: has_atom_weight
 integer :: nspin_psolver, myrank, & 
            icoulomb_backup, & 
            oep_mixing_method = 2  ! 1: anderson 
                                   ! 2: pulay 
 real :: start_time , finish_time
 real(dp) :: shift_eigen(dtset%mband*dtset%nkpt*dtset%nsppol)
 character(len=500) :: msg,structure,message_from_dfet,stemp
 integer, dimension(8) :: dateTime
 integer  :: m, new_rho_new_occ=-1, itmp(1), & 
             acfd_exx=1, & 
             nextra_band = 2, & 
             counter_run, & 
             ndim, & 
 ...
 integer  :: n1, n2, n3, io, stat, & 
             exx_method = 2  ! -1  -- real space poison solver
                             ! 0  -- q=0 is set to zero 
                             ! 1  -- truncated with error function 
                             ! 2  -- Spencer and Alavi 


 logical  :: tigger_mode,ex,file_exist ,file_exist2

 real(dp) :: cg_fixphase(2,mcg), & 
             hse_range,  &   ! for HSE functional 
             factor, f_occ, & 
             delta_mu,  & 
             delta_mu_spinup, & 
             delta_mu_spindn, & 
             coeff, histW(4),net_value,el_renorm,nalpha,nbeta, & 
             entropy_tmp,total_zion,  & 
             chen_xcart(3,dtset%natom), & 
             tmp_fcart(3,dtset%natom), & 
             new_q(dtset%nsppol), & 
             chen_q(dtset%nsppol),  &
             tmp_fermi(dtset%nspden), &
             tmp_dfermi_deigen(dtset%nband(1),dtset%nspden), & 
             ...
             delta, & 
             dmu_dEigen, & 
             docc_dEigen, fn,fm,fk, & 
             cij, & 
             yvec(dtset%nfft,dtset%nsppol), & 
             deps_docc(dtset%nfft), & 
             deps_dN(dtset%nfft,dtset%nsppol), & 
             pvec(dtset%nfft), & 
             docc_dvks(dtset%nfft), & 
             atom_weight(dtset%nfft)

 ! for mklocal()
 integer :: ia, fd
 type(wvl_internal_type) :: dum_wvl
 type(wvl_denspot_type)  :: dum_wvl_den
 integer ::  mklocl_option, & 
             dum_qprtrb(3) = 0.0d0          ! for penalty_force
 real(8) ::  tmp_xcart(3,dtset%natom), &
             tmp_ph1d(2,3*(2*dtset%mgfft+1)*dtset%natom), & 
             tmp_xred(3,dtset%natom), & 
             force_pen(3,dtset%natom), pen_coeff, & 
             vd(dtset%nfft,dtset%nsppol), & 
             lap_vd(dtset%nfft,dtset%nsppol)
 real(8) ::  dum_dyfrlo(3,3,dtset%natom), & ! for penalty_force 
             dum_rhog(2,dtset%nfft) , & 
             dum_rhor(dtset%nfft,dtset%nsppol) , &  
             dum_eei, & 
             vpsp_tmp(dtset%nfft), &        ! for penalty_force 
             vpsp_fd1(dtset%nfft), &        ! for penalty_force 
             vpsp_fd2(dtset%nfft), &        ! for penalty_force 
             der_vpsp(dtset%nfft), &        ! for penalty_force 
             dum_vprtrb(2), &               ! for penalty_force 
             dum_lpsstr(6),  &              ! for penalty_force 
             dum_grtn(3,dtset%natom)        ! for penalty_force 
 ...
 ! mpi vars
 integer :: spaceworld, mpi_err, rss
...
!! =================== for KLI-EXX ===================
 integer  :: iter_kli, ib1, ib2, ib3, ib4, mix_type
 integer  :: counter, ib, ipoint
 integer  :: ik, nband_k, iband, icg_shift, icg, ipw,  & 
             norb_extra0 = 4, & 
             n_homo(dtset%nspden) ! highest band to be considered for DFPT, EXX calculations 
 !
 real(dp) :: occ_tmp, docc_dN, dtmp, dtmp2, dvol, hgx, hgy, hgz, alpha, mix_beta, d_resid, & 
             ...
             atom_eps_exx(dtset%nfft), &                   ! XC energy per particle
             cluster_eps_exx(dtset%nfft,dtset%nspden), &                   ! XC energy per particle
             exx, vxc_kli_bar,  & 
 ...
 !
 real(dp):: cwavef(2,dtset%mpw), &          ! wavefunction in recip space
            cwavef_realspace(dtset%nfft)    ! wavefunction in real space
 !
 real(dp) :: doti,dotr, & 
             phir(dtset%nfft,dtset%nspden,dtset%nband(1)), &  ! wf in real space 
             chen_occ(dtset%nspden,dtset%nband(1)),&          ! my occupation number 
             drho_dN(dtset%nfft,dtset%nspden), & 
             chen_eigen(dtset%nspden,dtset%nband(1)),&        ! my eigenvalues
             chen_fermi(dtset%nspden), & 
             dfermi_dvks(dtset%nfft,dtset%nsppol),& 
...
!  Compute the density from the trial potential
   ...
   icoulomb_backup = dtset%icoulomb
   ...
   ! do not use force correction (due to residual) at this point
   dtset%iprcch = 0

   !if (dtset%nbdbuf==0) then 
   !  print *,'dtset%nbdbuf is not set, set it to 2 to avoid highest bands, stop'
   !  call flush(6)
   !  stop
   !endif 
   ! if (dtset%istwfk(1)/=1) then 
   !   print *,'must set dtset%istwfk to 1, stop'
   !   call flush(6)
   !   stop
   ! endif
   ...
   if (mpi_enreg%me==0) then 
     call c_wrtlog('=========== Program start =========')
     write(message,'(a)')'gprimd:';call c_wrtlog(message)
     write(message,'(a,3f14.6)')'gprimd: ',gprimd(1,1:3); call c_wrtlog(message)
     write(message,'(a,3f14.6)')'gprimd: ',gprimd(2,1:3); call c_wrtlog(message)
     write(message,'(a,3f14.6)')'gprimd: ',gprimd(3,1:3); call c_wrtlog(message)
     call c_wrtlog('')
     ...
     write(111)vpsp
     close(111)
     call c_wrtlog("[scfcv] vpsp.dat file is written for DFET code.")
     write(message,'(a,2f14.6)')'[scfcv] vpsp min/max: ',minval(vpsp),maxval(vpsp)
     call c_wrtlog(message)

     ! dump atom coordinates 
     call xred2xcart(dtset%natom, rprimd, chen_xcart, xred)
     open(file='atom_coords.dat',action='write',form='unformatted',unit=111)
     write(111)chen_xcart
     close(111)
     call c_wrtlog("[scfcv] xcart is written to atom_coords.dat.")

     ! dump zion (valence ion)
     open(file='zion.dat',action='write',form='unformatted',unit=111)
     do iatom=1,dtset%natom 
        write(111)dtset%ziontypat(dtset%typat(iatom))
     enddo
     close(111)
   endif
   ...
   !
   ! ziontypat: Charge of the pseudo-ion (=number of valence electrons that are 
   ! needed to screen exactly the pseudopotential).
   !
   total_zion = 0.d0
   do ii=1,dtset%natom
     total_zion = total_zion + psps%ziontypat(dtset%typat(ii))
   end do
   write(message,'(a,f16.6)') '[scfcv] total_zion= ',total_zion
   call c_wrtlog(message)

   ! write information 
   call date_and_time(VALUES=dateTime)    !today
   write(msg, '(A,i2,A,i2,A,i4,A,i2.2,A,i2.2,A,i2.2,A,i3.3)') "[scfcv] start at: ", &
               dateTime(3),"/",  dateTime(2),"/",  dateTime(1)," : ",             &
               dateTime(5), ":", dateTime(6), ":", dateTime(7), ".", dateTime(8)
   call c_wrtlog(msg);
   write (message, '(A,I10,A,I5,A,I5,A,I5)')  &
    '[scfcv] nfft=',dtset%nfft,' n1=',n1,' n2=',n2,' n3=',n3
   call c_wrtlog(message)
   write (message, '(A,f9.3,A,9I2,a,I2,a,f8.4,a,f14.3)') & 
    '[scfcv] ecut=',dtset%ecut,' nkpt=',dtset%kptrlatt, ' occ=', dtset%occopt, & 
...
do while(.true.)
   ...
   write(message,'(a)')' '; call c_wrtlog(message)
   write(message,'(a)')' =================================='; call c_wrtlog(message)
   write(message,'(a)')'   new subsystem/global system run   '; call c_wrtlog(message)
   write(message,'(a)')' =================================='; call c_wrtlog(message)
   write(message,'(a)')' '; call c_wrtlog(message)

   !
   ! Wait for the messages from XCEP to run
   !
   write(message,*)'[scfcv] waiting for file message_from_dfet ...'; call c_wrtlog(message)
   file_exist = .false.
   do while ( .NOT. file_exist) 
     inquire( file='message_from_dfet', exist=file_exist ) 
     call sleep(1)
   enddo
   write(message,*)'[scfcv] waiting for file message_from_dfet2 ...'; call c_wrtlog(message)
   file_exist = .false.
   do while (.not. file_exist) 
   ...
   !========================================
   ! read and remove message_from_dfet
   do while (.true.) 
     open(file='message_from_dfet',action='read',unit=111,form='formatted')
     read(111,*,IOSTAT=stat) message_from_dfet
     if ( stat/=0) then 
       close(111)
       write(message,'(a,i4)')'error on reading message_from_dfet file, try again in 2 seconds. code: ',stat
       call c_wrtlog(message)
       call sleep(2)
     else 
       close(111,status='delete')
       call c_wrtlog('loaded message_from_dfet file, removed and move on')
       exit
     endif 
   enddo 
   call xmpi_barrier(spacecomm)
   call c_wrtlog('File message_from_dfet has been read and removed.')
   !
   if ( message_from_dfet(1:3) .eq. "RUN") then 
     write(message,'(a)')' run subsystem KS DFT now ....';call c_wrtlog(message)
   else 
     write(message,'(a)')' exit subsystem KS DFT now ....';call c_wrtlog(message)
     stop
   endif
   ...
   open(file='../system_info.dat',action='read',unit=111,form='unformatted')
   read(111) exx_method 
   read(111) hse_range 
   close(111)
   ...
   if (file_exist) then 
     ...
     read(111,*)dtset%type_of_system  ! 1: subsystem 
                                      ! 2: global system
     read(111,*)dtset%calc_mode, dtset%conv_unocc_orb
     read(111,*)do_exx
     close(111)
     call c_wrtlog("")
     call c_wrtlog("found control.dat file, read it in.")
     write(message,'(a,i3,a)')"type_of_system: ",dtset%type_of_system, & 
      ' (1: clu/env 2: total system)';call c_wrtlog(message)
     write(message,'(a,i3)')"calc_mode:      ",dtset%calc_mode; call c_wrtlog(message)
     write(message,'(a,i3)')'do_exx:         ',do_exx;call c_wrtlog(message)
     call c_wrtlog("")

     xcep = .false.

     if (dtset%calc_mode==4) then 
        ! calc_mode=4 -> solve KS eqn and get new rho 
        ! from cluster for computing embedding potential 
        system_role = 1 ! cluster
     else if (dtset%calc_mode==5) then 
        ! calc_mode=5 -> solve KS eqn and compute EXX potential job for env
        system_role = 2 ! environment 
     else if (dtset%calc_mode==7) then 
        ! calc_mode=7 -> solve KS eqn and compute dEdvks and EXX energy 
        ! according to XCEP for cluster 
        xcep = .true. 
        do_exx = 1 
        system_role = 1 ! cluster
     else if (dtset%calc_mode == 200) then 
        ! calc_mode=200 -> load new occ from 
        system_role = 1 ! cluster 
     else if (dtset%calc_mode == 10) then 
        ! calc_mode=10 -> ECDA is finished, we are doing geometry relaxation 
        ! read the forces and finish scfcv.f90 file 
        open(file='xcep_force.dat',unit=111,action='read',form='unformatted')
        read(111)fcart
        close(111)
        ! remove average forces
        do i=1,3
          fcart(i,:) = fcart(i,:) - sum(fcart(i,:))/dble(dtset%natom)
        enddo 
        call fcart2fred(fcart,fred,rprimd,dtset%natom)
        ...
        write(111,*)11
        close(111)
        exit ! exit ECDA and scfcv loop
     elseif (dtset%calc_mode==11) then 
        ! For global system, cluster and environment 
        ! return to mover.F90 and update the xred and then re-enter scfcv()
        call c_wrtlog('get message calc_mode=11, go back to gstate.F90 to update xred, then re-enter scfcv()')
        quit = 1
        exit 
     endif
   else 
     call c_wrtlog('no control.dat file, stop! ')
     stop 
   endif


   !==================================================================
   ! perform DFET method (with env treated by OF-DFT)
   if (dtset%calc_mode == 30) then 

     call dfet_zp_envOF(logOF,vw_lam,vw_reg,mxband,nfft,nspin,natom,j_atom, & 
   ...
   !========================================================================
   ! load vd = vks-vpsp-vhart from vd.dat file
   ! compute the forces due to the \partial (penalty ) / \partial (R_i)
   if (dtset%calc_mode==12) then 
     write(message,'(a)')"compute forces of penalty ..."; call c_wrtlog(message)
     open(file='vd.dat',unit=111,action='read',form='unformatted')
     read(111)vd; close(111)
     open(file='lap_vd.dat',unit=111,action='read',form='unformatted')
     read(111)lap_vd; close(111)
     open(file='pen_coeff.dat',unit=111,action='read',form='unformatted')
     read(111)pen_coeff; close(111)
     ...
     do ia = 1,dtset%natom 
       do idir = 1,3
         do fd =  1,2
           ! compute der_vpsp using finite difference 
           call xred2xcart(dtset%natom,rprimd,tmp_xcart,xred)
           if (fd==1) tmp_xcart(idir,ia) = tmp_xcart(idir,ia) + 0.01d0
           if (fd==2) tmp_xcart(idir,ia) = tmp_xcart(idir,ia) - 0.01d0
           call xcart2xred(dtset%natom,rprimd,tmp_xcart,tmp_xred)
           call getph(atindx,dtset%natom,ngfft(1),ngfft(2),ngfft(3),tmp_ph1d,tmp_xred)

           mklocl_option = 1 ! local crystal pseudopotential in real space.
           call mklocl(dtset,dum_dyfrlo,dum_eei,gmet,gprimd,dum_grtn,gsqcut,dum_lpsstr,dtset%mgfft,&
             mpi_enreg,dtset%natom,nattyp,dtset%nfft,ngfft,dtset%nspden,dtset%ntypat,mklocl_option,pawtab, & 
             tmp_ph1d,psps,dum_qprtrb,dum_rhog,dum_rhor,rprimd,ucvol,dum_vprtrb,vpsp_tmp, & 
             dum_wvl,dum_wvl_den,tmp_xred)

           if (fd==1) vpsp_fd1 = vpsp_tmp
           if (fd==2) vpsp_fd2 = vpsp_tmp
         enddo 
         der_vpsp = (vpsp_fd1-vpsp_fd2)/(2*0.01d0)
         ! comute force
         do isp=1,dtset%nsppol 
           force_pen(idir,ia) = force_pen(idir,ia) + pen_coeff*sum((vd(:,isp)-lap_vd(:,isp))*der_vpsp)*dvol
         enddo 
       enddo 
     enddo 

     open(file='force_pen.dat',unit=111,action='write',form='unformatted')
     write(111)force_pen
     close(111)

     call go_back_to_xcep()
     cycle 
   endif 
   ...
   if (dtset%type_of_system==1 .and. (has_atom_weight .eqv. .false.) .and. xcep) then 
     open(file='atom_weight.dat',unit=111,action='read',form='unformatted')
     read(111) atom_weight
     close(111)
     has_atom_weight = .true.
     print *,'atom_weight: ',minval(atom_weight),maxval(atom_weight)
   endif 
   ! global system 
   if (dtset%type_of_system==2) then
     atom_weight = 1.d0
   endif 
   ...
   ! >>>> calc_mode = 600 
   !
   ! load the new q from ECDA
   !===============================================
   if (dtset%calc_mode == 600) then 
     ! load new q from XCEP and then return 
     open(file='new_q.dat',action='read',form='formatted',unit=111)
     read(111,*)new_q(1:dtset%nspden)
     close(111)
     dtset%nelect = sum(new_q)
     if (dtset%nsppol==2)  then 
       dtset%spinmagntarget = new_q(1)-new_q(2)
       write(message,'(a,2f16.8)')'new q from XCEP: ',new_q
     else 
       write(message,'(a,f16.8)')'new q from XCEP: ',new_q
     endif 
     call c_wrtlog(message)
     call go_back_to_xcep() 
     cycle 
   ...
   !======================================================
   if ( dtset%calc_mode == 100 ) then 

     call c_wrtlog("(scfcv) loading new_occ.xcpp ...")
     open(file='new_occ.xcpp',unit=111,action='read',form='formatted')
     !
     ! read in new fermi level 
     !
     read(111,*)message,chen_fermi(1:dtset%nsppol)
     write(message,'(a,2f12.4)')'(scfcv) load fermi level from new_occ.xcpp: ',chen_fermi(1:dtset%nsppol)
     call c_wrtlog(message)
     !
     ...
     do isp=1,dtset%nsppol
       do ikpt=1,dtset%nkpt
         do iband=1,dtset%nband(ikpt+dtset%nkpt*(isp-1))
            ...
            if (isp==1) dtmp  = dtmp + occ(ii)   ! spin up electron number 
            if (isp==2) dtmp2 = dtmp2 + occ(ii)  ! spin dn electron number 
         enddo
       enddo
     ...
     if (dtset%nsppol==2) dtset%spinmagntarget = dtmp - dtmp2 
     write(message,'(a)')'new_occ.dat from XCPP is loaded.'
     call c_wrtlog(message)
     !
     ! make new density 
     !
...
&               rhog,rhor,rprimd,0,ucvol,wvl%wfs,wvl%den,0) 
     ...
     if (dtset%nsppol==2) then 
       ...
       write(message,'(a,f16.6)')'integrated rhor: ',ucvol/dble(nfftf)*sum(rhor(:,1))
       call c_wrtlog(message)
     endif 
     !
     ! dump electron density to ECDA program
     !
   ...
   ! calc_mode = 502
   !
   ! just compute the change of Fermi level w/ cluster and env coupled
   !===================================================================== 
   if (dtset%calc_mode == 502) then 
     call c_wrtlog('calc_mode=502, compute Delta Fermi for given v_perb and wfr from previous the call')
     !
     ! convert wave functions to real space 
     !
     call c_wrtlog('converting wfr to real space ...')
     icg = 0
     ...
     do isp=1,dtset%nspden 
       ...
       do ik=1,dtset%nkpt 
         ...
         do iband=1,dtset%nband(1)-dtset%nbdbuf
         !!do iband=1,dtset%nband(1)
           ...
           call recip_to_real_NEW(1,mpi_enreg,dtset%paral_kgb,1,dtset%mpw,npwarr(1),1,dtset%istwfk(1), & 
                              dtset%nspinor,dtset%mgfft,dtset%nfft,ucvol,dtset%ngfft,&
                              kg,npwarr(1),cwavef,cwavef_realspace,.TRUE.)  
           write(6,'(a,i3,a,2es12.4,a,es12.4)') & 
              '[scfcv] band:',iband,' -> min/max(wf_realspace):', & 
              minval(cwavef_realspace),maxval(cwavef_realspace), ' norm: ', & 
              sum(cwavef_realspace**2)*ucvol/dble(dtset%nfft)

           ! make my own cg whose phase is fixed (wave function is real in space)
           call recip_to_real_NEW(-1,mpi_enreg,dtset%paral_kgb,1,dtset%mpw,npwarr(1),1,dtset%istwfk(1), & 
                              dtset%nspinor,dtset%mgfft,dtset%nfft,ucvol,dtset%ngfft,&
                              kg,npwarr(1),cwavef,cwavef_realspace,.false.)  
           do ipw=1,dtset%mpw
             cg_fixphase(1:2,ipw+icg_shift) = cwavef(1:2,ipw)
           end do
     ...
     ! load perturbation potential
     open(unit=111,action='read',file='vperb.dat',form='unformatted')
     read(111)vperb 
     close(111)
     call c_wrtlog('vperb.dat is loaded')

     ! perturbation on cluster + env 
     ! load the d(Fermi)/d(eigen) computed by XCEP 
     call c_wrtlog('load dfermi_deigen.dat file')
     open(file='dfermi_deigen.dat',unit=111,action='read',form='formatted')
     read(111,*) tmp_fermi(1:dtset%nsppol)
     if (dtset%nsppol==2) then 
       write(message,'(a,2f14.8)')'Fermi level from dfermi_deigen.dat: ',tmp_fermi(1:2)
     else
       write(message,'(a,f14.8)')'Fermi level from dfermi_deigen.dat: ',tmp_fermi(1:1)
     endif 
     call c_wrtlog(message)
     call c_wrtlog('---- d(Fermi)/d(eigen) -----')
     do ib4=1,dtset%nband(1)
       read(111,*)tmp_dfermi_deigen(ib4,1:dtset%nsppol)
       !write(message,'(2f16.8)')tmp_dfermi_deigen(ib4,1:dtset%nsppol)
       !call c_wrtlog(trim(message))
     enddo 
     close(111)
     
     ! compute \Delta Ef & write Delta_Ef to XCEP 
     open(file='my_delta_Ef.dat',unit=111,action='write',form='formatted')
     do isp = 1,dtset%nspden 
       delta_mu = 0.d0 
       do ib=1,dtset%nband(1)
         delta_mu = delta_mu + sum(phir(:,isp,ib)**2*tmp_dfermi_deigen(ib,isp)*vperb(:,isp))*dvol
       enddo
       write(111,*)delta_mu
       write(message,'(a,f14.6,a,i2)')'my delta_mu: ',delta_mu,' spin:',isp
       call c_wrtlog(message)
     enddo 
     close(111) 
   ...
   !====================================================================================================
   ! calc_mode = 500 ==> single system perturbation theory   
   ! calc_mode = 501 ==> perturbation theory consider cluster + env
   !     
   ! Perform density functional perturbation theory 
   !
   ...
   !====================================================================================================
   if (dtset%calc_mode==500 .or. dtset%calc_mode==501) then 
     call c_wrtlog("")
...
&     ylm,ylmgr,ylmdiel,vxctau=vxctau)
     ...
     dtset%tolwfr = 0.d0 ! set the stop criterion for cg solver: cgwf.F90 and lobpcgwf.F90
     ...
     write(message,'(a,2f14.8)')'Fermi levels computed by chen_newocc_ef() => ',chen_fermi
     call c_wrtlog(message)
     ...
     do isp=1,dtset%nspden 
        ...
        do ib=1,dtset%nband(1)
           ...
           call c_wrtlog(message)
        enddo
        ! add a little bit more band for DFPT
        n_homo(isp) = min(dtset%nband(1)-dtset%nbdbuf,n_homo(isp)+nextra_band)
        !! check if we have enough bands, note that for bands from 
        !! dtset%nband(1)-dtset%nbdbuf to dtset%nbdbuf they are often not accurate. 
        !if (n_homo(isp) > dtset%nband(1)-dtset%nbdbuf) then 
        !   print *,'increase your nband! n_homo(isp)+nextra_band>=dtset%nband(1)-dtset%nbdbuf , stop'
        !   call flush(6)
        !   stop
        !endif 
     ...
     rho_perb = 0.d0 
     do isp=1,dtset%nspden 
        write(message,'(a,i3,a,i3)')'spin: ',isp, & 
         '  solving DFPT eq. using chen_dfpt_sternheimer_frac_recip(), n_homo: ',n_homo(isp)
        call c_wrtlog(message)

        !do ib=1,dtset%nband(isp)
        do ib=1,n_homo(isp)
          ! Solve DFPT's sternheimer equation in recipcal space,
          !
          ! NOTE: by solving it in reciprocal space, it will be much more accurate, 
          ! since the back-and-forth FFT will cause errors in solving Ax=b equation. 
          ! Solve it in reciprocal space or you can have wrong answer to the sternheimer equation.
          !
          call chen_dfpt_sternheimer_frac_recip(isp,ib,chen_fermi(isp),chen_eigen(isp,:),chen_occ(isp,:),occ_tol, & 
                n_homo(isp),e_window,cg_fixphase,phir(:,isp,:),vtrial(:,isp),orb_perb,vperb(:,isp), & 
                npwarr,kg,dtset,ucvol,psps,xred,rprimd,rmet,gmet,gprimd,ylm,ylmgr,dtfil,atindx,atindx1,mpi_enreg,nattyp)

          write(message,'(a,i3,a,f10.4,a,2es14.4)')'ib:',ib,' occ:',chen_occ(isp,ib), & 
            '      orb_perb:',minval(orb_perb),maxval(orb_perb)
          call c_wrtlog(message)
        ...
        call c_wrtlog(message)
     enddo
     if (dtset%nspden==1) rho_perb = rho_perb*2.d0 


     ! compute perturbed density (due to fermi level change)
     !
     ! NOTE: In the above already consider the density change due to the change of occupation numbers 
     !       (with fermi level fixed) Next, we only need to consider the density variation due to 
     !       the variation of Fermi level.
     !
     call c_wrtlog('add the contribution of d(fermi)/dvks ...')

     if (dtset%calc_mode==501) then 
       ! perturbation on cluster + env 
       ! directly load \Delta Fermi from file (made by XCEP)
       open(file='delta_fermi.dat',unit=111,action='read',form='formatted')
       read(111,*) delta_mu_spinup
       read(111,*) delta_mu_spindn
       close(111)
       write(message,'(a,f14.6)')'loaded delta_fermi.dat file => delta_fermi(spin up): ',delta_mu_spinup
       call c_wrtlog(message)
       write(message,'(a,f14.6)')'loaded delta_fermi.dat file => delta_fermi(spin dn): ',delta_mu_spindn
       call c_wrtlog(message)
     endif 
     !
     ! change of Fermi level due to the change of the potential
     !
     do isp=1,dtset%nspden 
        select case (dtset%calc_mode)
        case (500) 
          ! perturbation on single system (cluster, env, or the total system)
          delta_mu = 0.d0 
          do ib=1,dtset%nband(isp)
            ! compute d(Fermi)/(eigen)
            call calc_dfermi_deigen(ib,isp,dtset%nband(1),dtset%nspden,dtset%tsmear, & 
                                    chen_fermi,chen_occ,chen_eigen,dmu_dEigen)
            delta_mu = delta_mu + sum(phir(:,isp,ib)**2*dmu_dEigen*vperb(:,isp))*dvol
          enddo
        case (501) 
          if (isp==1) delta_mu = delta_mu_spinup
          if (isp==2) delta_mu = delta_mu_spindn
        endselect

        if (dtset%nspden==1) factor = 2
        if (dtset%nspden==2) factor = 1

        ! compute d(rho)/(Ef)
        drho_dEf = 0.d0 
        do ib=1,dtset%nband(isp)
          f_occ = chen_occ(isp,ib)/factor
          drho_dEf = drho_dEf + f_occ*(1.d0-f_occ)/dtset%tsmear*phir(:,isp,ib)**2
        enddo
        rho_perb(:,isp) = rho_perb(:,isp) + factor*drho_dEf*delta_mu
        ...
        ! check 
        if (abs(sum(rho_perb(:,isp))/dtset%nfft)>1e-3) then 
          call c_wrtlog('[scfcv]: abs(sum(rho_perb(:,isp))/nfft) >1e-3, BUG in DFPT code, FIX IT')
          stop
        endif 
     enddo 
   ...
   !===============================
   ! end of calc_mode = 500 & 501 
   !===============================



   select case (dtset%type_of_system)
   case(1)
     !===========================================
     ! cluster and env, read their KS potential 
     ...
     !==========================================
     ! embedding potentail from XCEP for DFET
   ...
   case(2)
     !=======================================
     ! global system (load KS potential)
     !
     open(unit=111,action='read',file='vks.dat',form='unformatted')
     read(111)bare_vks
     close(111)
     write(message,'(a,2e12.4)')'min/max(vks_alpha):',& 
     minval(bare_vks(:,1)),maxval(bare_vks(:,1));call c_wrtlog(message)
     if (dtset%nsppol==2) then 
       write(message,'(a,2e12.4)')'min/max(vks_beta): ', & 
       minval(bare_vks(:,2)),maxval(bare_vks(:,2));call c_wrtlog(message)
     endif 
     vtrial = bare_vks 
   ...
   computed_forces = 1 ! compute forces due to nonlocal psps term

   write(message,'(a,f12.5)')'dtset%nelect: ',dtset%nelect
   call c_wrtlog(message)
...
&     ylm,ylmgr,ylmdiel,vxctau=vxctau)
   ...
   write(message,'(a,f16.12)')'total_q: ',sum(rhor(:,1))*ucvol/dble(dtset%nfft)
   call c_wrtlog(message)
   dtset%tolwfr = 0.0d0 ! set tolwfr back to zero 
   ...
   if ( mpi_enreg%me==0) then 
     write(msg,'(4(es16.8),f12.4)') & 
      energies%e_kinetic,energies%e_nonlocalpsp, & 
      energies%e_eigenvalues,residm,finish_time-start_time 
     call c_wrtlog(msg)
   endif
...
&     0,computed_forces,optres,pawang,pawfgrtab,pawrhoij,pawtab,&
&     ph1df,red_ptot,psps,rhog,rhor,rprimd,symrec,synlgr,&
&     psps%usepaw,vhartr,vpsp,vxc,wvl%descr,wvl%den,xccc3d,xred)

   write(message,'(a,3es12.4)')'favg: ',favg
   call c_wrtlog(message)
   call c_wrtlog('favg is included in the forces sent to ECDA.')
!   print *,'gresid:',gresid;print *,''
!   print *,'grewtn:',grewtn;print *,''
!   print *,'grhf:  ',grhf;  print *,''
!   print *,'grnl:  ',grnl;  print *,''
!   print *,'grvdw: ',grvdw; print *,''
!   print *,'grxc:  ',grxc;  print *,''
!   call flush(6)

   ! ABINIT zeros out the average of forces, we add the favg back
   do iatom=1,dtset%natom
      tmp_fcart(:,iatom) = fcart(:,iatom) + favg
   enddo 
   open(file='forces.dat',action='write',unit=111,form='unformatted')
   write(111) tmp_fcart
   close(111)
   ...
   ! in case we remove global_system folder
   ! write system's eigenvalue to backup 
   if (dtset%type_of_system==2 .and. myrank==0) then 
     open(file='../global_system_eigen.txt',unit=112,action='write',form='formatted') 
     write(112,'(a,i5,a,i5,a,f12.6)') & 
       "nband: ",dtset%nband(1)," nkpt  ",dtset%nkpt," tsmear_eV: ",dtset%tsmear*27.2114d0
   endif 

   open(file='eigen.dat',unit=111,action='write',form='formatted')
   write(111,'(a,i5,a,i5,a,f12.6)')  &
     "nband: ",dtset%nband(1)," nkpt  ",dtset%nkpt," tsmear_eV: ",dtset%tsmear*27.2114d0
   ii = 0
   do isp=1,dtset%nsppol
     do ikpt=1,dtset%nkpt
       do iband=1,dtset%nband(ikpt+dtset%nkpt*(isp-1))
          ...
          if (dtset%type_of_system==2 .and. myrank==0) & 
            write(112,'(a,3i4,a,f16.8,a,f16.8)') &   ! in case we remove global_system folder 
            'isp_ikpt_iband: ',isp,ikpt,iband," ",occ(ii)," ",eigen(ii) 
       enddo
     end do
   end do
   close(111)
   if (dtset%type_of_system==2 .and. myrank==0) close(112)
   ...
   call c_wrtlog(message)
   open(file='fermi.dat',action='write',unit=111,form='unformatted')
   write(111) chen_fermi
   close(111)
   ...
   do isp=1,dtset%nsppol
     do ikpt=1,dtset%nkpt
       do iband=1,dtset%nband(ikpt+dtset%nkpt*(isp-1))
          ...
          ! mark HOMO and LUMO ==> compute band gap
          if (chen_eigen(isp,iband)  >chen_fermi(isp) .and. & 
              chen_eigen(isp,iband-1)<chen_fermi(isp)) then 
            write(message,'(a,f12.4,a,i3)')'band gap (eV): ', & 
             27.2114*(chen_eigen(isp,iband)-chen_eigen(isp,iband-1)) ,' for spin',isp 
            call c_wrtlog(message)
          endif 
       enddo
     end do
   ...
   do isp=1,dtset%nspden 
     do ik=1,dtset%nkpt 
       do iband=1,dtset%nband(1)
          ...
          dtmp = sum(cwavef_realspace**2)*ucvol/dble(dtset%nfft) ! norm of wf
          ! check norm = 1 (?)
          if (abs(dtmp -1.0d0)>1e-4) then 
            write(6,'(a,i3,a,2es12.4,a,es12.4)') & 
             '|wf| is not 1 for band:',iband,' -> min/max(wf_realspace):', & 
              minval(cwavef_realspace),maxval(cwavef_realspace), ' norm: ', & 
              sum(cwavef_realspace**2)*ucvol/dble(dtset%nfft)
            stop
          endif 
   ...
   dfermi_dvks = 0.d0
   do isp=1,dtset%nspden 
     ! check fermi computed by my routine and abinit's routine 
     if ( abs(chen_fermi(1)-energies%e_fermie)>1e-6) then 
       print *,'abs(chen_fermi(1)-energies%e_fermie)>1e-6'
       print *, 'chen_fermi: ',chen_fermi(1),' energies%fermi:',energies%e_fermie
       stop
     endif 
     do ib=1,dtset%nband(1)
       call calc_dfermi_deigen(ib,isp,dtset%nband(1),dtset%nspden,dtset%tsmear, & 
                               chen_fermi,chen_occ,chen_eigen,dmu_dEigen)
       !!print *,'dmu_dEigen=',dmu_dEigen,' occ:',chen_occ(isp,ib)
       dfermi_dvks(:,isp) = dfermi_dvks(:,isp) + phir(:,isp,ib)**2*dmu_dEigen
     enddo
     write(message,'(a,2f12.4,a,i3)') & 
      'dfermi_dvks: ',minval(dfermi_dvks(:,isp)),maxval(dfermi_dvks(:,isp)),' spin: ',isp
     call c_wrtlog(message)
   enddo
   open(file='dfermi_dvks.dat',action='write',unit=111,form='unformatted')
   write(111) dfermi_dvks
   close(111) 
   call c_wrtlog('file dfermi_dvks.dat is written'//NEW_LINE('a'))

   !
   ! compute drho/dN 
   !
   if (dtset%nspden==1) factor = 2.d0 
   if (dtset%nspden==2) factor = 1.d0
   drho_dN = 0.0d0
   do isp=1,dtset%nspden
      dtmp = 0.0d0 
      do ib=1,dtset%nband(isp)
        occ_tmp = chen_occ(isp,ib)/factor 
        dtmp = dtmp + occ_tmp*(1.d0-occ_tmp)
      enddo 
      ! calcualte drho_dN
      do ib=1,dtset%nband(isp)
         occ_tmp = chen_occ(isp,ib)/factor 
         drho_dN(:,isp) = drho_dN(:,isp) + phir(:,isp,ib)**2 * occ_tmp*(1.d0-occ_tmp)/dtmp
      enddo 
      write(message,'(a,2es12.4,a,i2)')'drho_dN: ',minval(drho_dN(:,isp)),maxval(drho_dN(:,isp)),' spin:',isp
      call c_wrtlog(message)
   enddo 
   open(file='drho_dN.dat',action='write',unit=111,form='unformatted')
   write(111) drho_dN
   close(111)
   call c_wrtlog('file drho_dN.dat is written')
...
&       rhor,rprimd,strsxc,usexcnhat,vhartr,vxc,vxcavg,xccc3d,taug=taug,taur=taur)
     ...
     energies%e_hartree = dvol*sum(vhartr*rhor(:,1))*0.5d0
   else
     ! xc pot
     optxc = 1
     call psolver_rhohxc(energies%e_hartree, energies%e_xc, evxc, &
&      dtset%icoulomb, dtset%ixc,mpi_enreg, nfftf, ngfftf, nhat,psps%usepaw,&
&      dtset%nscforder,dtset%nspden,n3xccc,rhor,rprimd, & 
&      usexcnhat,psps%usepaw,dtset%usewvl,vhartr, vxc, vxcavg,&
&      wvl%descr,wvl%den, wvl%e,xccc3d,dtset%xclevel,dtset%xc_denpos)
   endif 
   write(message,*)'vhart: ',minval(vhartr),maxval(vhartr)
   call c_wrtlog(message)
   write(message,*)'rhor : ',minval(rhor(:,1)),maxval(rhor(:,1))
   call c_wrtlog(message)
   write(message,*)'vpsp:  ',minval(vpsp),maxval(vpsp)
   call c_wrtlog(message)
   ...
   write(111,'(f18.10,a)') energies%e_hartree,' Hartree energy'
   close(111)

   ! write hartree potential 
   open(file='vhartr.dat',action='write',unit=111,form='unformatted')
   write(111)vhartr
   close(111)



   write(message,'(a,f16.12)')'total_q: ',sum(rhor(:,1))*ucvol/dble(dtset%nfft); call c_wrtlog(message)
   if (dtset%nsppol==2) then 
     write(message,'(a,f16.12)')'mag:     ',sum(rhor(:,2)*2.0d0-rhor(:,1))*ucvol/dble(dtset%nfft); call c_wrtlog(message)
   endif 
   ...
   write(message,'(a,f16.8)')'Fermi energy: ', energies%e_fermie;      call c_wrtlog(message)
   write(message,'(a,2f14.6)')'min/max vhartree: ',minval(vhartr),maxval(vhartr); call c_wrtlog(message)
   ...
   do isp=1,dtset%nspden 
     ...
     do ib=1,dtset%nband(1)
       counter = counter + 1
       write(message,'(a,es12.4,a,f14.6,a,i4)') & 
   ...
   !
   if (do_exx>0 .and. mpi_enreg%me==0) then 
     write(message,'(a)')'computing dE_xc/dvks on master node ...'
     call c_wrtlog(message)
     ...
     do isp=1,dtset%nspden  ! loop over spin 
       ...
       do ik=1,dtset%nkpt   ! loop over k points
         print *,'[scfcv] ---- kpoint: ',ik,' ------'
         do iband=1,dtset%nband(1)-dtset%nbdbuf ! loop over bands

           icg_shift=npwarr(1)*(iband-1)+icg
           ! the dimension of cg is 
           ! cg(1:2, npw, nband, nkpt, nspin)
           do ipw=1,dtset%mpw
             cwavef(1:2,ipw)=cg(1:2,ipw+icg_shift)
           end do
           ! conver wf to real space
           call recip_to_real_NEW(1,mpi_enreg,dtset%paral_kgb,1,dtset%mpw,npwarr(1),1,dtset%istwfk(1), & 
                              ...
                              kg,npwarr(1),cwavef,cwavef_realspace,.TRUE.)  
           write(6,'(a,i3,a,2es12.4,a,es12.4)') & 
              '[scfcv] ib:',iband,' -> min/max(wf_real):', & 
              minval(cwavef_realspace),maxval(cwavef_realspace), ' norm: ', & 
              sum(cwavef_realspace**2)*ucvol/dble(dtset%nfft)
           !
           ! make my own cg whose phase is fixed (wave function is real in space)
     ...
     V_slater = 0.d0 
     atom_eps_exx = 0.d0
     cluster_eps_exx = 0.d0

     if (acfd_exx==1) call c_wrtlog("exx is computed based on acfd")
     if (acfd_exx <0) call c_wrtlog("exx is computed in the HF way")
     if (exx_method==-1) call c_wrtlog('exx_method = -1 --> real space, use psolver_hartree')
     if (exx_method==2) then 
       call c_wrtlog('exx_method = 2 --> Spencer and Alavi')
       write(message,'(a,f12.4,a)')'rc: ',1.d0/hse_range,' bohr'; call c_wrtlog(message)
     endif 
     if (exx_method==1) then 
       call c_wrtlog('exx_method = 1 --> HSE functional')
       write(message,'(a,f12.4,a)')'rc: ',1.d0/hse_range,' bohr'; call c_wrtlog(message)
     endif 
     ...
     do isp = 1,dtset%nspden
        ...
        do ib1 = 1,n_homo(isp)  
          ...
          do ib2 = 1,n_homo(isp)
             ! computing kpot = phi_i*phi_j/|r-r'|
             !
             ! call poisson solver 
             ...
             if (exx_method==-1) then 
               ...
               call psolver_hartree(dtmp, (/ hgx, hgy, hgz /), dtset%icoulomb, & 
                  ...
                  mpi_enreg%nproc_fft, dtset%nscforder, nspin_psolver, arr_tmp, kpot, dtset%usewvl)
               dtset%icoulomb = icoulomb_backup
               ...
               call psolver_hartree(dtmp, (/ hgx, hgy, hgz /), dtset%icoulomb, & 
                  mpi_enreg%me_fft, mpi_enreg%comm_fft, dtset%nfft, (/ngfft(1),ngfft(2),ngfft(3)/), & 
                  mpi_enreg%nproc_fft, dtset%nscforder, nspin_psolver, arr_tmp, kpot1, dtset%usewvl)
               dtset%icoulomb = icoulomb_backup

             else
               ! compute EXX in Fourier space 
               !
               ! exx_method = 0: set v(q=0) to 0
               ! exx_method = 1: HSE, cut using error function 
               !
               arr_tmp = phir(:,isp,ib1)*phir(:,isp,ib2)
               call fourdp(1,dum_rhog,arr_tmp,-1,mpi_enreg,nfftf,ngfftf,dtset%paral_kgb,0)
               call chen_exx_recip(1,gmet,gsqcut,1,mpi_enreg,dtset%nfft,dtset%ngfft, & 
                 dtset%paral_kgb,ucvol,dtset%nkpt,qphon,exx_method,dum_rhog,kpot,hse_range)

               arr_tmp = phir(:,isp,ib1)*phir(:,isp,ib2)*atom_weight
               call fourdp(1,dum_rhog,arr_tmp,-1,mpi_enreg,nfftf,ngfftf,dtset%paral_kgb,0)
               call chen_exx_recip(1,gmet,gsqcut,1,mpi_enreg,dtset%nfft,dtset%ngfft, & 
                 dtset%paral_kgb,ucvol,dtset%nkpt,qphon,exx_method,dum_rhog,kpot1,hse_range)
             endif 
             ...
             if (ib2>ib1) then 
               atom_eps_exx = atom_eps_exx + 0.d0
             elseif (ib2==ib1) then 
               atom_eps_exx       = atom_eps_exx - 0.50d0*chen_occ(isp,ib1)*phir(:,isp,ib1)*phir(:,isp,ib2)*kpot*atom_weight
               dExx_docc(isp,ib1) = dExx_docc(isp,ib1) - 0.5d0*dvol*sum(phir(:,isp,ib1)*phir(:,isp,ib2)*kpot*atom_weight)
               cluster_eps_exx(:,isp) = cluster_eps_exx(:,isp) - 0.50d0*chen_occ(isp,ib1)*phir(:,isp,ib1)*phir(:,isp,ib2)*kpot
             elseif (ib2<ib1) then
               atom_eps_exx       = atom_eps_exx - chen_occ(isp,ib1)*phir(:,isp,ib1)*phir(:,isp,ib2)*kpot*atom_weight
               dExx_docc(isp,ib1) = dExx_docc(isp,ib1) - dvol*sum(phir(:,isp,ib1)*phir(:,isp,ib2)*kpot*atom_weight)
               cluster_eps_exx(:,isp) = cluster_eps_exx(:,isp) - chen_occ(isp,ib1)*phir(:,isp,ib1)*phir(:,isp,ib2)*kpot
             endif

             ! deps(r)/docc 
             if (ib1>ib2)  cij = 2.d0  ! c = 1+sgn(e1-e2)
             if (ib1==ib2) cij = 1.d0  ! c = 1+sgn(e1-e2)
             if (ib1<ib2)  cij = 0.d0  ! c = 1+sgn(e1-e2)
             deps_docc = deps_docc - 0.5d0*cij*phir(:,isp,ib2)*phir(:,isp,ib1)*kpot
          enddo ! iband2
          ...
          do ib=1,dtset%nband(isp)
            ...
            dtmp = dtmp + occ_tmp*(1.d0-occ_tmp)
          enddo 
          occ_tmp = chen_occ(isp,ib1)/factor 
          docc_dN = occ_tmp*(1.d0-occ_tmp)/dtmp
          deps_dN(:,isp) = deps_dN(:,isp) + deps_docc*docc_dN
 
          !
          ! compute dExx/dVks for XCEP 
          !
          call system_mem_usage(rss)
          write(message,'(a,f12.4,a,f12.4,a)')'memory used (RSS): ',rss/1000.0, ' MByte', rss/1.0e6,' GB'
          call c_wrtlog(message)
          write(message,'(a,i3,a)')'running chen_orb_shift_recip_NEW() for band: ',ib1,' ...'
          call c_wrtlog(message)
          orb_shift_vxc = 0.d0

          !!
          !! NOTE: it is STRONLGY recommend that you solve the orbital shift equaiton (i.e., Sternheimer equation)
          !! in the reciprocal space. If you do it in real space (might be easier to do the prototype), 
          !! the back-and-forth FFT will cause small errors in the gradient and finally will give you the wrong solution 
          !! to the Ax=b equation. I spent nearly three days to trace down this problem. So solve Sternheimer equation 
          !! in reciprocal space! 
          !!
          call chen_orb_shift_recip_NEW(isp,ib1,chen_eigen(isp,:),chen_fermi(isp),cg_fixphase,phir(:,isp,:),vtrial(:,isp), & 
            orb_shift,-Avec,orb_shift_vxc,npwarr,kg,dtset,ucvol,psps,xred,& 
            rprimd,rmet,gmet,gprimd,ylm,ylmgr,dtfil,atindx,atindx1,mpi_enreg,nattyp)

          yvec(:,isp) = yvec(:,isp) + orb_shift*phir(:,isp,ib1)

          write(message,'(a,2f12.4)')'yvec (due to wfr): ', & 
            minval(yvec(:,isp)),maxval(yvec(:,isp))
          call c_wrtlog(message)
          ...
          do ib2=1,dtset%nband(isp) 
             ! compute dmu/d(eigen)
             call calc_dfermi_deigen(ib2,isp,dtset%nband(1),dtset%nspden, & 
               dtset%tsmear,chen_fermi,chen_occ,chen_eigen,dmu_dEigen)
             !write(message,'(a,f12.4)')'dmu_dEigen: ',dmu_dEigen
             !call c_wrtlog(message)
             ...
             if (dtset%nsppol==1) fk = chen_occ(isp,ib1)/2.d0
             if (dtset%nsppol==2) fk = chen_occ(isp,ib1)
             docc_dEigen = fk*(1.d0-fk)/dtset%tsmear*(dmu_dEigen-delta)
             
             if (dtset%nsppol==1) then 
               yvec(:,isp) = yvec(:,isp) + 2.d0*dExx_docc(isp,ib1)*docc_dEigen*phir(:,isp,ib2)**2
             else 
               yvec(:,isp) = yvec(:,isp) + dExx_docc(isp,ib1)*docc_dEigen*phir(:,isp,ib2)**2
             endif 
          enddo 

          write(message,'(a,2f12.4)')'yvec (added occ):  ',minval(yvec(:,isp)),maxval(yvec(:,isp))
          call c_wrtlog(message)
          
        enddo ! ib1
 
        write(message,'(a,2f12.4)')'final yvec => ',minval(yvec(:,isp)),maxval(yvec(:,isp))
        call c_wrtlog(message)
        call c_wrtlog('')
        write(message,'(a,2f12.4)')'deps_dN => ',minval(deps_dN(:,isp)),maxval(deps_dN(:,isp))
        call c_wrtlog(message)
        call c_wrtlog('')

     enddo  ! loop over spin 

     exx = sum(atom_eps_exx)*dvol
     
     write(message,'(a,f16.8,a)')'exx: ',exx, ' hartree'       
     call c_wrtlog(message)
     open(file='yvec.dat',unit=111,action='write',form='unformatted')
     write(111) exx
     write(111) cluster_eps_exx
     write(111) yvec
     close(111)
     open(file='deps_dN.dat',unit=111,action='write',form='unformatted')
     write(111) deps_dN
     close(111)
     call c_wrtlog('dumped yvec.dat and deps_dN files.')
...
subroutine KLI_solution()
  ...
  do isp=1,dtset%nspden
    ...
    write(message,'(a,es13.5,a,i3,a)') & 
      '(KLI) vxc_bar - uxc_bar (homo): ',shift_val,' (spin: ',isp,')'
    call c_wrtlog(message)
