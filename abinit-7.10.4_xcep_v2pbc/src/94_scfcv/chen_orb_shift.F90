...
!
subroutine chen_orb_shift(iband,eigen,fermi,wfr,vlocal,orb_shift,uxc,vxc,npwarr,& 
 ...
 do cg_step = 1,10000
   ...
   if (dot_product(cg_resid,cg_resid) < 1e-6) then 
     write(6,*)'(chen_cgwf_orb_shift) CG converged to 1e-8.'
     exit
   endif
...
!     dtset%nspinor,dtset%mgfft,dtset%nfft,ucvol,dtset%ngfft,kg,npwarr(1),fftg_out,wfr(:,iband))  
!
...
!   print *,dot_product(cg_Ap,wfr(:,iband))*ucvol/dble(dtset%nfft)
!   stop
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
&    kinpw,eval,mpi_enreg,dtset%natom,1,npwarr(1),dtset%nspinor, & 
   ...
   do ib=1,dtset%nband(1)
     ! energy window 
     if ( eigen(ib)<eigen(iband)+e_window ) then 
       cg_Ap = cg_Ap + wfr(:,ib)*sum(dvol*wfr(:,ib)*cg_p)*(eigen(iband)-eigen(ib)+e_thr)
 ...
 ! Project the cg_x to the |eigen> to get the final orbital shift
 ! then orb_shift is the solution for Ax=b
 ...
 !
 ! make cg_x be orthorgonal to wfr 
 !
