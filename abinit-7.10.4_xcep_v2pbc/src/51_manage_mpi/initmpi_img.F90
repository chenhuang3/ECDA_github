...
!    Communicator over all images
     if (mpi_enreg%me<=iprocmax) then
       ...
       ABI_DEALLOCATE(ranks)
       mpi_enreg%me_img=xcomm_rank(mpi_enreg%comm_img)
...
!  call xcomm_group(mpi_enreg%comm_cell,group_cell,ierr)
...
!  call MPI_COMM_COMPARE(xmpi_self,mpi_enreg%comm_cell,irank,ierr)
