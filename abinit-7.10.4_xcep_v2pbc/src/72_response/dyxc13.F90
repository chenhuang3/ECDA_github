...
!!
!! CHILDREN
!!      atm2fft3,dotprod_vn,mkcor3,mkvxc3,timab,xmpi_sum
!!
!! SOURCE
...
!Local variables-------------------------------
!scalars
 integer :: cplex,iat1,iatom1,iatom2,idir1,idir2,ierr,ifft,my_natom,mpi_comm_atom
...
!    PAW: 1st-order core charge in reciprocal space
     if (usepaw==1) then
       call atm2fft3(atindx,cplex,gmet,gprimd_dummy,gsqcut,idir1,iatom1,&
&       mgfft,mqgrid,natom,1,nfft,ngfft,ntypat,ph1d,qgrid,&
&       qphon,typat,ucvol,usepaw,xred,atmrho1=xcccwk1,optn2_in=1,pawtab=pawtab)
...
!    Compute the corresponding potential
     ...
     call mkvxc3(cplex,ixc,kxc,mpi_enreg,nfft,ngfft,dum_nhat,0,dum_nhat,0,nkxc,&
...
!          PAW: 1st-order core charge in reciprocal space
           if (usepaw==1) then
             call atm2fft3(atindx,cplex,gmet,gprimd_dummy,gsqcut,idir2,iatom2,&
&             mgfft,mqgrid,natom,1,nfft,ngfft,ntypat,ph1d,qgrid,&
&             qphon,typat,ucvol,usepaw,xred,atmrho1=xcccwk2,optn2_in=1,pawtab=pawtab)
...
!          Get the matrix element j1,j2
 ...
 if (paral_atom) then
   ...
   call xmpi_sum(dyfrx1,mpi_comm_atom,ierr)
   call xmpi_sum(blkflgfrx1,mpi_comm_atom,ierr)
