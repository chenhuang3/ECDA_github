...
!!
!! COPYRIGHT
!! Copyright (C) 2002-2014 ABINIT group (MVeithen)
...
!!
!! CHILDREN
!!      fftpac,fourwf,mkffnl,mkkpg,nonlop,sphereboundary,status,xmpi_sum
!!
!! SOURCE
...
!scalars
 ...
 real(dp) :: arg,dot1i,dot1r,dot2i,dot2r,doti,dotr,lagi,lagr,sumi,sumr
 real(dp) :: weight
!arrays
 ...
 real(dp) :: phkxredin(2,1),phkxredout(2,1),tmp(2),xred_der(3),ylmgr_dum(1,1,1)
...
!    Compute (k+G) and (k+q+G) vectors (only if useylm=1)
     ...
     if (nqpg>0) then
       call mkkpg(kg_k,qpg_k,kpq,nqpg,npw1_k)
     end if
...
&         natom_der,nattyp_der,1,dtset%ngfft,nkpg,nqpg,&
