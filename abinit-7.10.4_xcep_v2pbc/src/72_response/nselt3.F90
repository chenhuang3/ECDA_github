...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DRH, XG, DCA, GMR, MM, AR, MV, MB)
...
!!
!! CHILDREN
!!      dotprod_vn,hartrestr,mkcor3,mkvxcstr3,nstwf4,stresssym,vlocalstr,wrtout
!!      xmpi_barrier
!!
!! SOURCE
...
!arrays
 ...
 real(dp),intent(in) :: ylm(mpw*mkmem,mpsang*mpsang*psps%useylm)    !vz_d
 real(dp),intent(in) :: ylm1(mpw1*mk1mem,mpsang*mpsang*psps%useylm) !vz_d
 real(dp),intent(in) :: ylmgr(mpw*mkmem,3,mpsang*mpsang*psps%useylm)!vz_d
 real(dp),intent(in) :: ylmgr1(mpw1*mk1mem,3,mpsang*mpsang*psps%useylm)!vz_d
...
&     occ_k,ph1d,psps,rmet,ucvol,wtk_k,xred,ylm_k,ylmgr_k)
...
!    Shift array memory
 ...
 if(xmpi_paral==1)then
   ...
   call wrtout(std_out,' nselt3: loop on k-points and spins done in parallel','COLL')
 end if
