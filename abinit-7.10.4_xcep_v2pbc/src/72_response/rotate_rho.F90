...
!!
!! COPYRIGHT
!! Copyright (C) 2013-2014 ABINIT group (MVerst)
...
!!  paral_kgb = flag for use of full kpt + G vector + band parallelism
...
!args
 integer,intent(in) :: cplex, nfft, nspden, itirev, paral_kgb
...
! fft input rhor1 to reciprocal space: uses work* as a buffer
   call fourdp(cplex,workg,rhor1(:,ispden),-1,mpi_enreg,nfft,ngfft,paral_kgb,0)
...
!    Pull out full or spin up density, now symmetrized
   call fourdp(cplex,workg_eq,rhor1_eq(:,ispden),1,mpi_enreg,nfft,ngfft,paral_kgb,0)
