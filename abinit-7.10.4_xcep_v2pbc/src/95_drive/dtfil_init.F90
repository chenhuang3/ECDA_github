...
! Also, unit number 21, 22 and 23 are used in nstdy3, for the 3 dot wavefunctions. 
...
!   std_in=5, ab_in=5, std_out=6, ab_out=7, tmp_unit=9, tmp_unit2=10
 ...
 integer,parameter :: unt_nlcc_derivs=900, unt_pspdat=400
...
!******************************************************************
 ...
 dtfil%unpos  =unpos
 dtfil%undelI =undelI
 dtfil%unt_nlcc_derivs =unt_nlcc_derivs
 dtfil%unt_pspdat =unt_pspdat
...
!According to getwfkfine and irdkssfine, build _WFK file name, referred as filwfkfine
...
!Build name of files from dtfil%filnam_ds(4)
 ...
 dtfil%fnameabo_ps_wfk=trim(dtfil%filnam_ds(4))//'_PS_WFK'
 dtfil%fnameabo_ae_onsite_wfk=trim(dtfil%filnam_ds(4))//'_AE_ONSITE_WFK'
 dtfil%fnameabo_ps_onsite_wfk=trim(dtfil%filnam_ds(4))//'_PS_ONSITE_WFK'
 ...
 dtfil%fnameabo_pawstat=trim(dtfil%filnam_ds(4))//'_PAWSTAT'
...
!'_PAW' -> dtfil%unpaw
 ...
 if(xmpi_paral==1)then ! parallel case : the index of the processor must be appended
   ...
   ixx=1
   if (xmpi_mpiio == 1 .and. dtset%accesswff == IO_MODE_MPI ) ixx=3
   do ii=ixx,7
...
!'_PAWQ' -> dtfil%unpawq
 ...
 if(xmpi_paral==1) then
   ...
   if (xmpi_mpiio==1.and.dtset%accesswff==IO_MODE_MPI)then
     idtmpfil(1)=0              !_1wf1
     idtmpfil(2)=0              ! s1wf2
     idtmpfil(7)=0              !  WFGS
     idtmpfil(8)=0              !  WFKQ
