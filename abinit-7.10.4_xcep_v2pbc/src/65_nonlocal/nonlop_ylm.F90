...
!!    Only choices 0,1,2,3,4,6,8,23,24,51,52,53,54 are compatible with signs=1 (not choices==7)
...
!scalars
 ...
 integer :: iatm,ic,ii,ierr,ilmn,iln,ispinor,itypat,jc,mincat,mu,mua,mub,n1,n2,n3,nd2gxdt,ndgxdt
 integer :: ndgxdtfac,nincat,nkpgin_,nkpgout_,nlmn,nua1,nua2,nub1,nub2,optder
 real(dp) :: enlk
...
!arrays
 ...
 real(dp),allocatable :: enlout_tmp(:,:)
 ...
 real(dp),allocatable :: sij_typ(:),strnlk(:),strnlk_out(:)
 real(dp),allocatable :: work0(:),work1(:),work2(:,:),work3(:,:)
!no_abirules
...
&     choice==23.or.choice==24.or.choice==5.or.choice==51.or.choice==52.or.&
     choice==53.or.choice==53.or.choice==54.or.choice==6.or.choice==7.or.choice==8)
   else if (paw_opt==3) then
     check=(choice==0.or.choice==1.or.choice==2.or.choice==5.or.choice==54)
   else
     check = .false.
   end if
...
&   choice==5.or.choice==7.or.choice==51.or.choice==52.or.choice==53)
...
!Initialize output arrays
 if (signs==1) then
   ...
   enlk=zero;fnlk=zero;strnlk=zero
   enlout(:)=zero
 end if
 if (signs==2) then
...
!Eventually re-compute (k+G) vectors (and related data)
 ...
 if ((choice==2).or.((choice==3.or.choice==23.or.choice==6.or.choice==54).and.signs==1)) nkpgin_=3
...
!          Contraction from <p_i|c> to Sum_j[Dij.<p_j|c>] (and derivatives)
           call opernlc_ylm(atindx1,cplex,cplex_dgxdt,cplex_enl,cplex_fac,dgxdt,dgxdtfac,dgxdtfac_sij,&
...
!        ==============================================================
         if (signs==1) then
           if (.not.present(cprjin_left)) then
             call opernld_ylm(choice_b,cplex,cplex_fac,d2gxdt,dgxdt,dgxdtfac,dgxdtfac_sij,&
...
!          Prepare the phase factors if they were not already computed
           ...
           call opernlb_ylm(choice_b,cplex,cplex_dgxdt,cplex_fac,&
&           dgxdtfac,dgxdtfac_sij,dimffnlout,ffnlout_typ,gxfac,gxfac_sij,ia3,&
...
!Convert ddk contribution from cartesian to reduced (reciprocal space) coordinates
 if ((choice==5.or.choice==53).and.signs==1.and. &
& (paw_opt==0.or.paw_opt==1.or.paw_opt==2.or.paw_opt==3)) then
   ABI_ALLOCATE(work0,(3))
   ia1 = 0 
   ia2 = 1 ; if(choice==54) ia2=3*natom
   do mu=1,ia2
     work0(:) = gprimd(:,1)*enlout(ia1+1)+gprimd(:,2)*enlout(ia1+2)+gprimd(:,3)*enlout(ia1+3)
     enlout(ia1+1:ia1+3) = gprimd(1,:)*work0(1)+gprimd(2,:)*work0(2)+gprimd(3,:)*work0(3)
     ia1=ia1+3
   end do
   ABI_DEALLOCATE(work0)
 end if

!Convert ddk contribution from cartesian to reduced (reciprocal space) coordinates
 if ((choice==54).and.signs==1.and. &
&    (paw_opt==0.or.paw_opt==1.or.paw_opt==2.or.paw_opt==3)) then
   ABI_ALLOCATE(work0,(3))
   ABI_ALLOCATE(work1,(3))
   ia1 = 0; ia2=3*natom
   do mu=1,ia2
     work0(:) = gprimd(:,1)*enlout(ia1+1)+gprimd(:,2)*enlout(ia1+3)+gprimd(:,3)*enlout(ia1+5)
     work1(:) = gprimd(:,1)*enlout(ia1+2)+gprimd(:,2)*enlout(ia1+4)+gprimd(:,3)*enlout(ia1+6)
     enlout(ia1+1) = gprimd(1,1)*work0(1)+gprimd(2,1)*work0(2)+gprimd(3,1)*work0(3)
     enlout(ia1+3) = gprimd(1,2)*work0(1)+gprimd(2,2)*work0(2)+gprimd(3,2)*work0(3)
     enlout(ia1+5) = gprimd(1,3)*work0(1)+gprimd(2,3)*work0(2)+gprimd(3,3)*work0(3)
     enlout(ia1+2) = gprimd(1,1)*work1(1)+gprimd(2,1)*work1(2)+gprimd(3,1)*work1(3)
     enlout(ia1+4) = gprimd(1,2)*work1(1)+gprimd(2,2)*work1(2)+gprimd(3,2)*work1(3)
     enlout(ia1+6) = gprimd(1,3)*work1(1)+gprimd(2,3)*work1(2)+gprimd(3,3)*work1(3)
     ia1=ia1+6
   end do
   ABI_DEALLOCATE(work0)
...
!Convert ddk 2nd derivative from cartesian to reduced (reciprocal space) coordinates
 if (choice==8.and.signs==1.and. &
& (paw_opt==0.or.paw_opt==1.or.paw_opt==2)) then
   ABI_ALLOCATE(work2,(3,3))
   ABI_ALLOCATE(work3,(3,3))
   work2(1,1)=enlout(1) ; work2(1,2)=enlout(6) ; work2(1,3)=enlout(5)
   work2(2,1)=enlout(6) ; work2(2,2)=enlout(2) ; work2(2,3)=enlout(4)
   work2(3,1)=enlout(5) ; work2(3,2)=enlout(4) ; work2(3,3)=enlout(3)
   do mu=1,3
     work3(:,mu) = gprimd(:,1)*work2(mu,1)+gprimd(:,2)*work2(mu,2)+gprimd(:,3)*work2(mu,3)
   end do
   do mu=1,3
     work2(:,mu) = gprimd(:,1)*work3(mu,1)+gprimd(:,2)*work3(mu,2)+gprimd(:,3)*work3(mu,3)
   end do
   do mu=1,3
     work3(:,mu) = gprimd(1,:)*work2(mu,1)+gprimd(2,:)*work2(mu,2)+gprimd(3,:)*work2(mu,3)
   end do
   ...
   enlout(1) = work2(1,1) ; enlout(2) = work2(2,2) ; enlout(3) = work2(3,3)
   enlout(4) = work2(3,2) ; enlout(5) = work2(1,3) ; enlout(6) = work2(2,1)
...
!Convert elastic tensor from reduced to cartesian coordinates
 if (choice==6.and.signs==1.and.(paw_opt==0.or.paw_opt==1.or.paw_opt==2)) then
   ...
   do mu=1,6+3*natom
     work0(1:6)=enlout_tmp(mu,1:6)
     call strconv(work0,gprimd,work1)
     enlout_tmp(mu,1:6)=work1(1:6)
   end do
   strnlk_out = zero
