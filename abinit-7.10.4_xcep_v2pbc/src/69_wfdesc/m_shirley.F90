...
MODULE m_shirley
 ...
 use m_ebands,         only : pack_eneocc, ebands_init, ebands_ncwrite, ebands_free, edos_t, edos_init, edos_free
...
&                             load_paw_hamiltonian,finalize_hamiltonian, gs_hamiltonian_type
 use m_wfs,            only : wfd_get_ur, wfd_t, wfd_barrier, wfd_get_cprj, wfd_barrier, wfd_change_ngfft, wfd_print,&
...
&    two*dpc*ovlp_size**2*b2Mb," Mb; Matrix size = ",ovlp_size
   ...
   ABI_CHECK_ALLOC("Allocation error!")
   O(spin)%mat = czero
...
&          .and. ALL(multable(2:4,inv_k1_sym,k2_sym)==g_gamma) )
 ...
 if (Wfd%usepaw==1) then
   ...
   ABI_DT_FREE(Pwij)
   call pawcprj_destroy(Cp_k1)
   ABI_DT_FREE(Cp_k1)
   call pawcprj_destroy(Cp_k2)
   ABI_DT_FREE(Cp_k2)
 end if
...
!scalars
 ...
 integer :: blc1,blc2,istwf_k,useylm_,iat,iatom,nspinor
 real(dp) :: arg
...
!arrays
 ...
 real(dp),allocatable :: kpg_k(:,:),ph3d(:,:,:),phkxred(:,:),vnl_psi(:,:),vectin(:,:),s_psi(:,:)
...
!************************************************************************
 ...
 ! Prepare the GS Hamiltonian for this k-point.
 call load_paw_hamiltonian(Ham_k,spin,Paw_ij)
 call finalize_hamiltonian(Ham_k,npw_k,istwf_k,k4intp)
...
&  Psps%ntypat,Psps%pspso,Psps%qgrid_ff,Cryst%rmet,Psps%usepaw,useylm_,ylm_k,ylmgr_dum)
 ...
 ABI_MALLOC(phkxred,(2,Cryst%natom))
...
#endif
 integer :: nloalg(5),intp_npwarr(intp_nk),intp_istwfk(intp_nk)
 integer,allocatable :: nlmn_sort(:)
...
&    Wsh%Kdata(k1)%kg_k,Wsh%nloalg,Wsh%prtvol,Wsh%pawprtvol,Wsh%comm)
 ...
 do spin=1,nsppol
   ...
   ABI_CHECK_ALLOC("Out of memory in vloc_ij")
   vloc_ij = czero
...
&    Cryst%ntypat,Cryst%typat,Cryst%xred,Wsh%nfft,Wsh%mgfft,Wsh%ngfft,Cryst%rprimd,nloalg)
   ...
   ABI_CHECK_ALLOC("Out of memory in hk_ij")

   ABI_MALLOC(sk_ij,(sh_size,sh_size*usepaw))
   ABI_CHECK_ALLOC("Out of memory in sk_ij")
   ...
   do ikpt=my_kstart,my_kstop
     ...
     if (.not.do_full_diago) then
       ...
       ABI_CHECK_ALLOC("Allocation of eig_vec failed. Full diago will be performed")
     end if
     !do_full_diago = .True.
...
&        Psps,Cryst%rprimd,copy_vtrial,Cryst%xred,onband_diago,diag_ene,diag_vec,diag_Cprj,xmpi_self,ierr)
...
&  intp_nband,intp_nk,intp_npwarr,nsppol,nspinor,Dtset%tphysel,Dtset%tsmear,Dtset%occopt,intp_occ,intp_wtk)
 ...
 if (Wsh%my_rank == 0 .and. len_trim(sh_fname)>0) then
   NCF_CHECK(ncfile_create(ncf,sh_fname,NF90_CLOBBER),"Creating Shirley file")
   call crystal_ncwrite(Cryst,ncf%ncid)
   call ebands_ncwrite(oBands,dummy_nshiftk_orig1,dummy_shiftk_orig,&
&    dummy_nshiftk1,dummy_shiftk,dummy_ngkpt,dummy_kptrlatt,ncf%ncid)
   NCF_CHECK(ncfile_close(ncf),"Closing Shirley file")
 end if
#endif
...
!************************************************************************
 ...
 ABI_CHECK(xcomm_size(Wsh%comm)==1,"ovlp2_init is not parallelized")
 ...
 ABI_CHECK_ALLOC("out of memory ur34_big")
...
&  Wsh%prtvol,Wsh%pawprtvol,Wsh%comm)
 ...
 ABI_CHECK_ALLOC("out-of-memory in eh_ur")
 eh_ur = czero

 ABI_MALLOC(eh_ug,(npw_gamma*nspinor,eh_size))
 ABI_CHECK_ALLOC("out-of-memory in eh_ug")
 eh_ug = czero
