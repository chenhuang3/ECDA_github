...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, MF, GZ, DRH, MT)
...
& k3xc,electronpositron,taug,taur,vxctau,exc_vdw_out) ! optional arguments
...
!scalars
 ...
 integer :: nspden_apn,nspden_eff,nspden_updn,nspgrad,nvxcgrho,order,mgga
...
!Check options
 if (all(nspden /= [1,2,4])) then
   write(message,'(a,i0)')'The only allowed values of nspden are 1, 2, or 4, while the argument nspden=',nspden
   MSG_BUG(message)
 end if
...
!Are we using a functional from libxc? Is the functional a MGGA?
 ...
 if (dtset%ixc<0) then
   ...
   if (mgga==1 .and. dtset%usekden==0) then
     write(message, '(3a)' )&
...
&     'is not present. Please set "usekden 1" in the input file.'
 ...
 if(mgga==1.and.nspden==4)then
   MSG_ERROR('meta-GGA XC kernel is not yet implemented for non-collinear magnetism case!')
 end if
...
!Initializations
 ...
 if (dtset%xclevel==0.or.dtset%ixc==0) then
...
!  Normalize enxc, strsxc and vxc
   ...
   vxc=vxc*divshft
   if(present(vxctau)) vxctau=vxctau*divshft
