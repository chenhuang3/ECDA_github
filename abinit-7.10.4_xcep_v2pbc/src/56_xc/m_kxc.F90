...
!!
!! COPYRIGHT
!!  Copyright (C) 1998-2013 ABINIT group (DCA, MF, XG, GMR, LSI, YMN, Rhaltaf, MS)
...
!!   end if
!!   if (new_ixc<0) then
!!     call libxc_functionals_init(new_ixc,nspden)
!!   end if
!!  #endif
!!
...
!!   end if
!!   if (old_ixc<0) then
!!     call libxc_functionals_init(old_ixc,nspden)
!!   end if
!!  #endif
!!
!! PARENTS
...
!    Calculate $K(G,G'',q)=\frac{1}{nfft_tot}\sum_{r} exp(-i(q+G_{2}).r_{2} kxcr(r_{1}r_{2}) exp(i(q+G_{1}).r_{1} $
     ...
     do igp=1,npw
       ...
       call mkvxc3(cplex,ixc,kxcr,MPI_enreg_seq,nfft_tot,ngfft,dum,0,dum,0,nkxc,nspden,&
