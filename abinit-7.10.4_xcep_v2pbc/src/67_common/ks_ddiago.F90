...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2014 ABINIT group (MG)
...
!!      init_distribfft_seq,init_hamiltonian,initmpi_seq,initylmg,kpgsph,metric
!!      mkffnl,mkkin,mkkpg,pawcprj_alloc,pawcprj_destroy,pawcprj_getdim
!!      pawcprj_reorder,ph1d3d,sphereboundary,transgrid,wrtout,xheev,xheevx
!!      xhegv,xhegvx,xmpi_barrier
!!
!! SOURCE
...
&                                pawcprj_getdim, pawcprj_reorder
...
!Initialize the Hamiltonian datatype on the coarse FFT mesh.
 if (PRESENT(Electronpositron)) then
   call init_hamiltonian(gs_hamk,Psps,pawtab,nspinor,nspden,natom,Psps%ntypat,typat,xred,nfftc,mgfftc,ngfftc,&
...
!Allocate the arrays phkxred and ph3d, compute phkxred and eventually ph3d.
 ...
 if (nloalg(1)>0) then ! Allocation as well as precomputation
   if (MPI_enreg_seq%paral_kgb/=1) then
     call ph1d3d(1,natom,kg_k,gs_hamk%matblk,natom,npw_k,n1,n2,n3,gs_hamk%phkxred,gs_hamk%ph1d,ph3d)
   else
...
!% call finalize_hamiltonian(gs_hamk,npw_k,istwfk,kpoint)
...
!call wrtout(std_out,msg,"COLL")
 ...
 ABI_MALLOC(ghg_mat,(cplex_ghg,npw_k*nspinor,npw_k*nspinor))
 ABI_CHECK_ALLOC(msg)

 write(msg,'(a,f0.3,a)')" Out-of-memory in gtg_mat. Memory required by the PAW overlap operator: ",size_mat," [Mb]."
 ABI_MALLOC(gtg_mat,(cplex_ghg,npw_k*nspinor,npw_k*nspinor*Psps%usepaw))
 ABI_CHECK_ALLOC(msg)

 ABI_DT_MALLOC(Cwaveprj,(natom,nspinor*(1+cpopt)*gs_hamk%usepaw))
 if (cpopt==0) then  ! Cwaveprj is ordered, see nonlop_ylm.
   ABI_MALLOC(dimcprj_srt,(natom))
   call pawcprj_getdim(dimcprj_srt,natom,gs_hamk%nattyp,ntypat,typat,pawtab,'O')
   call pawcprj_alloc(Cwaveprj,0,dimcprj_srt)
...
&   gvnlc,kg_k,kinpw,lambda,MPI_enreg_seq,natom,ndat,npw_k,nspinor,&
...
!========================================================
 if (Psps%usepaw==1) then
   ...
   call pawcprj_alloc(Cprj_k,0,dimcprj_srt)
   ABI_FREE(dimcprj_srt)
