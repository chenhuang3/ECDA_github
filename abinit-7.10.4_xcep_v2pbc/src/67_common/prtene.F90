...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, LBoeri, MT)
...
!   etotaldc = etotaldc + energies%e_ewald + energies%e_vdw_dftd2
...
&       '    Spherical terms = ',energies%e_paw
     ...
     call wrtout(iout,message,'COLL')
     if (dtset%vdw_xc==5.and.ipositron/=1) then
       write(message, '(a,es21.14)' ) &
&       '    Vd Walls DFT-D2 = ',energies%e_vdw_dftd2
       call wrtout(iout,message,'COLL')
     end if
