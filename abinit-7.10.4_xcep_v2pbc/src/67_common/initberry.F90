...
!!
!! COPYRIGHT
!! Copyright (C) 2004-2014 ABINIT group (MVeithen).
...
!scalars
 ...
 real(dp) :: gprimdlc(3,3),rmetllc(3,3),gmetllc(3,3),ucvol_local   !!HONG
...
! ucvol = volume of the unit cell in Bohr**3
 ...
 logical :: calc_epaw3_force,calc_epaw3_stress,fieldflag,use_symrec
!arrays
...
!HONG need to use dtset%red_efieldbar in the whole code
!Compute the reciprocal lattice coordinates of the electric field  
 if (fieldflag) then   !!HONG
   
   call  metric(gmetllc,gprimdlc,-1,rmetllc,rprimd,ucvol_local)

   if (dtset%berryopt == 4 .or. dtset%berryopt == 6 .or. dtset%berryopt == 7) then   !!HONG
...
&     '  red_efieldbar(1:3) = ',dtset%red_efieldbar(1:3),ch10
   ...
   if (dtset%berryopt == 6 .or. dtset%berryopt ==7 ) then    !!HONG
...
&     '  red_dfield(1:3) = ',dtset%red_dfield(1:3),ch10
   ...
   if (  dtset%berryopt == 14 ) then   !!HONG
...
&     '  red_efieldbar(1:3) = ',dtset%red_efieldbar(1:3),ch10
   ...
   if ( dtset%berryopt == 16 ) then    !!HONG
...
&     '  red_efieldbar(1:3) = ',dtset%red_efieldbar(1:3),ch10
   ...
   if ( dtset%berryopt ==17) then    !!HONG
...
!------------------------------------------------------------------------------
 ...
 if (usepaw == 1 .and. dtefield%has_expibi == 1) then
   call expibi(dtefield%expibi,dtefield%dkvecs,gprimd,natom,rprimd,xred)
...
!build mpi_enreg%kpt_loc2ibz_sp, dtefield%cgqindex and dtefield%nneigh
 if ((fieldflag).and.(xmpi_paral== 1)) then
   ikpt_loc = 0
   do isppol = 1, nsppol
     do ikpt = 1, nkpt
...
!        skip idir values for which efield_dot(idir) = 0
         if (abs(dtefield%efield_dot(idir)) < tol12 .and. (dtset%berryopt == 4 .or. &
&         dtset%berryopt == 6 .or. dtset%berryopt == 7 .or. dtset%berryopt == 14 &
&         .or. dtset%berryopt == 16 .or. dtset%berryopt == 17)) cycle   !!HONG
