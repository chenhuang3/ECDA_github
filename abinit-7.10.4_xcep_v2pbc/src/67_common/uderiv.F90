...
!!
!! COPYRIGHT
!! Copyright (C) 2001-2014 ABINIT group (NSAI).
!!
!! INPUTS
...
!scalars
 ...
 integer :: master,maxband,mcg_disk,me,minband,nband_diff,nband_k
...
! *************************************************************************
 ...
 if(min(2,(1+mpi_enreg%paral_spinor)*nspinor)==2)then
   message = ' uderiv : does not yet work for nspinor=2'
   MSG_ERROR(message)
 end if
...
!  open ddk 1WF file
   ...
   if (wffddk%accesswff /= IO_MODE_NETCDF) then
     ...
     MSG_ERROR("Missing ETSF I/O support in uderiv")
   else 
     MSG_ERROR("Unknown accesswff")
   end if
