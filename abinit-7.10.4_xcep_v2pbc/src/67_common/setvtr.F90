...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (XG, GMR, FJ, MT, EB)
...
!!      psolver_rhohxc,rhohxc,rhohxcpositron,timab,vdw_dftd2,wvl_psitohpsi
!!      wvl_vtrial_abi2big,xred2xcart
!!
!! SOURCE
...
&  electronpositron,taug,taur,vxctau) ! optionals arguments
...
&     wvl%descr, wvl%den, xred)
   ...
   if (dtset%vdw_xc==5.and.ngrvdw==dtset%natom) then
     call vdw_dftd2(energies%e_vdw_dftd2,grvdw,dtset%ixc,dtset%natom,ntypat,1,0,dtset%typat,rprimd,&
&     dummy_out,dtset%vdw_tol,xred,psps%znucltypat)
   end if
 else
...
!--------------------------------------------------------------
 if (psps%usepaw==1 .and. psps%usewvl==0 .and. dtset%icoulomb==0) then

   call timab(552,1,tsec)

   optatm=1;optdyfr=0;opteltfr=0;optgr=0;optstr=0;optv=1;optn=n3xccc/nfft;optn2=1
   call atm2fft(atindx1,xccc3d,vpsp,dummy_out,dummy_out,energies%e_localpsp,dummy_out,dummy_in,&
&   gmet,gprimd,dummy_out,dummy_out,gsqcut,mgfft,psps%mqgrid_vl,dtset%natom,nattyp,nfft,ngfft,ntypat,&
&   optatm,optdyfr,opteltfr,optgr,optn,optn2,optstr,optv,pawtab,ph1d,psps%qgrid_vl,dtset%qprtrb,&
&   dummy_in,dummy6,dummy6,ucvol,psps%usepaw,dummy_in,dummy_in,dummy_in,dtset%vprtrb,psps%vlspl,&
...
&     'has to be verified in the PAW formalism.'
     ...
     call atm2fft(atindx1,dummy_out,vpsp,dummy_out,dummy_out,energies%e_localpsp,dummy_out,dummy_in,&
&     gmet,gprimd,dummy_out,grtn,gsqcut,mgfft,psps%mqgrid_vl,dtset%natom,nattyp,nfft,ngfft,ntypat,&
&     optatm,optdyfr,opteltfr,optgr,optn,optn2,optstr,optv,pawtab,ph1d,psps%qgrid_vl,dtset%qprtrb,&
&     rhojellg,dummy6,dummy6,ucvol,psps%usepaw,dummy_in,dummy_in,dummy_in,dtset%vprtrb,psps%vlspl,&
&     mpi_comm_fft=mpi_enreg%comm_fft,me_g0=mpi_enreg%me_g0,&
&     paral_kgb=mpi_enreg%paral_kgb,distribfft=mpi_enreg%distribfft)
...
&   1, istep, dtset%iscf, dtset%ixc, mpi_enreg%me_wvl, dtset%natom, dtset%nfft, mpi_enreg%nproc_wvl, dtset%nspden, &
...
!EB Add the zeeman field to vtrial
 vzeeman(:) = zero
...
!EB write (*,*) 'setvtr --> vtrial before zeeman:',vtrial(1,1),vtrial(1,2),vtrial(1,3),vtrial(1,4)
 if(dtset%nspden==4)then
   do ispden=2,dtset%nspden
     do ifft=1,nfft
       vtrial(ifft,ispden) = vtrial(ifft,ispden) + vzeeman(ispden) + Vmagconstr(ifft,ispden)
     end do !ifft
   end do !ispden
...
!    TODO : mjv: check that magnetic constraint works also for nspden 2 or add input variable condition
     vtrial(ifft,2) = vtrial(ifft,2) + vzeeman(1) + Vmagconstr(ifft,2) ! ispden=2 is rho_up only EB: to be tested
   end do !ifft
 end if

!EB  write (*,*) 'setvtr --> vtrial after zeeman:',vtrial(1,1),vtrial(1,2),vtrial(1,3),vtrial(1,4)
...
!  Already available in the Psolver case through psolver_rhohxc().
   if (ipositron/=1) then
     ...
     call dotprod_vn(1,rhor,energies%e_hartree,doti,nfft,nfftot,1,1,vhartr,ucvol_local,mpi_comm_sphgrid=mpi_comm_sphgrid)
...
!  Compute local psp energy eei
   call dotprod_vn(1,rhor,energies%e_localpsp,doti,nfft,nfftot,1,1,vpsp,ucvol_local,mpi_comm_sphgrid=mpi_comm_sphgrid)
 end if
...
!  Compute double-counting XC energy enxcdc
   if (ipositron/=1) then
     if (dtset%usepaw==0.or.usexcnhat/=0) then
       call dotprod_vn(1,rhor,energies%e_xcdc,doti,nfft,nfftot,dtset%nspden,1,vxc,ucvol_local,mpi_comm_sphgrid=mpi_comm_sphgrid)
       if(with_vxctau)then
         call dotprod_vn(1,taur,e_xcdc_vxctau,doti,nfft,nfftot,dtset%nspden,1,&
&         vxctau(:,:,1),ucvol_local,mpi_comm_sphgrid=mpi_comm_sphgrid)
         energies%e_xcdc=energies%e_xcdc+e_xcdc_vxctau
       end if
     else
       ABI_ALLOCATE(rhowk,(nfft,dtset%nspden))
       rhowk=rhor-nhat
       call dotprod_vn(1,rhowk,energies%e_xcdc,doti,nfft,nfftot,dtset%nspden,1,vxc,ucvol_local,mpi_comm_sphgrid=mpi_comm_sphgrid)
