...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2014 ABINIT group (MT, FDahm)
...
!    npw_max_loc = max(npw_max_loc,bandfft_kpt(ikpt)%npw_tot)
!    end do
   else
...
!    do ikpt=1,dtset%nkpt
!    npw_max_nonloc = max(npw_max_nonloc,bandfft_kpt(ikpt)%ndatarecv)
!    end do
   else
