...
!!
!! COPYRIGHT
!!  Copyright (C) 2012-2014 ABINIT group (LNguyen,FDahm)
...
!Scalapack initalization
 if (eigen_group_size>0) then
   ...
   rank=xcomm_rank(comm_scalapack)

   ! We create abi_communicator using a cartesian grid, and store its complement
   commsize = MIN(eigen_group_size, xcomm_size(comm_scalapack))
   sizecart = (/commsize, xcomm_size(comm_scalapack)/commsize/)
   periodic = (/.true.,.true./)
   reorder = .false.
...
!******************************************************************
 ...
 if (abi_communicator/=xmpi_comm_null) then
   call xcomm_free(abi_communicator)
