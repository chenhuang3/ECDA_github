...
!!      pawcprj_alloc,pawcprj_destroy,pawfgrtab_destroy,pawfgrtab_init
!!      pawfgrtab_print,printxsf,wfd_change_ngfft,wfd_sym_ur,wrap2_zero_one
!!      wrtout,xmpi_bcast,xmpi_sum_master
!!
!! SOURCE
...
subroutine exc_plot(Bsp,Bs_files,Wfd,Kmesh,Cryst,Psps,Pawtab,Pawrad,paw_add_onsite,spin_opt,which_fixed,eh_rcoord,nrcell,ngfftf)
 ...
 use m_wfs,               only : wfd_t, wfd_change_ngfft, wfd_get_cprj, wfd_sym_ur
...
&                           Pawtab,Pawrad,Pawfgrtab)
 ...
 ABI_CHECK_ALLOC(" out of memory in rcl tables")
 ...
 ABI_CHECK_ALLOC("out of memory in exc_phi")
 exc_phi=czero
 !
 !got=0; 
 ...
 if (Wfd%usepaw==1) then
   call pawcprj_destroy(Cp_v)
   ABI_DT_FREE(Cp_v)
   call pawcprj_destroy(Cp_c)
   ABI_DT_FREE(Cp_c)
 ...
 ABI_CHECK_ALLOC(" out of memory in exc_phi2")
