...
!!      find_qmesh,gsph_extend,gsph_init,init_transitions,kmesh_init
!!      kmesh_print,make_mesh,print_gsphere,reportgap,update_occ,vcoul_init
!!      wfk_read_eigenvalues,wrtout
!!
!! SOURCE
...
& Kmesh_dense,Qmesh_dense,KS_BSt_dense,QP_bst_dense,Gsph_x,Gsph_c,Vcp_dense,Hdr_wfk_dense,ngfftf,grid,comm)
 ...
 use m_ebands,        only : ebands_init, ebands_print, ebands_copy, ebands_free, update_occ, apply_scissor, ReportGap
...
!************************************************************************
 ...
 nelect_hdr_dense = hdr_get_nelect_byocc(Hdr_wfk_dense)
...
&    'The calculation will be done with nbands= ',nbnds_kss_dense
 ...
 if (Gsph_x%ng > Gsph_c%ng ) then
   call vcoul_init(Vcp_dense,Gsph_x,Qmesh_dense,Kmesh_dense,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,Gsph_x%ng,&  
&    nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 else
   call vcoul_init(Vcp_dense,Gsph_c,Qmesh_dense,Kmesh_dense,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,Gsph_c%ng,&  
&    nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 end if
...
&  Hdr_wfk_dense%occopt,occfact,Kmesh_dense%wt)
 ...
 call update_occ(KS_BSt_dense,Dtset%spinmagntarget,prtvol=Dtset%prtvol)

 call ebands_print(KS_BSt_dense,"Interpolated band structure read from the WFK file",unit=std_out,prtvol=Dtset%prtvol)

 call ReportGap(KS_BSt_dense,header="Interpolated KS band structure",unit=std_out,mode_paral="COLL")
 ...
 CASE (BSE_HTYPE_RPA_KS)
   if (ABS(BSp%soenergy)>tol6) then
     ...
     call apply_scissor(QP_BSt_dense,BSp%soenergy)
   else
 ...
 call ReportGap(QP_BSt_dense,header=" Interpolated QP band structure",unit=std_out,mode_paral="COLL")
