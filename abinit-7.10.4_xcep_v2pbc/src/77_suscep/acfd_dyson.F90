...
!!      getlambda,kxc_alda,kxc_eok,kxc_local,kxc_rpa,timab,wrtout,zheev
!!
!! SOURCE
...
!Check input parameters.
 ...
 if (nspden > 2) then
   message = ' acfd_dyson does not work yet for nspden > 2.'
   MSG_ERROR(message)
 end if
...
!DEBUG
!write(std_out,*)' acfd_dyson : call kxc_rpa '
!call flush(6)
!ENDDEBUG
       ...
       select case (ikhxc)
         ...
         case (ikhxc_ALDA, ikhxc_BPG, ikhxc_EOK2)
           ...
           do ikxc = 1,nkxc
             ...
             call kxc_local(ispxc(ikxc),kg_diel,khxc,kxcgdiff,nfft,ngfft,npwdiel,nspden,1)
           end do
...
&           'is only implemented for the RPA at the present stage.'
       ...
       case default
         write (message,'(a,i0,a)')' idyson = ',idyson,' is not a valid method to solve the Dyson equation.'
         MSG_ERROR(message)
     end select
