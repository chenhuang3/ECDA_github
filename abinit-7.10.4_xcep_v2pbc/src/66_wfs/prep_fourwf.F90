...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (FBottin,MT,GZ,FDahm)
...
!!      prep_wavef_sym_undo,timab,xmpi_alltoallv
!!
!! SOURCE
...
!arrays
 ...
 real(dp),allocatable :: buff_wf(:,:),cwavef_alltoall(:,:),cwavef_fft(:,:), cwavef_fft_tr(:,:)
...
!      filling of sorted send buffers before exchange
       do ipw = 1 ,ndatarecv
         buff_wf(1:2, indices_pw_fft(ipw) ) = cwavef_alltoall(1:2,ipw)
       end do
...
&       use_gpu_cuda=use_gpu_cuda_)
       ...
       call fourwf(1,rhoaug,cwavef_alltoall,dummy,wfraug,gbound_,gbound_,&
...
&         cwavef_alltoall(1:2,ipw + ndatarecv*(iibandpp-1))
       end do
     end do
