...
&    "Only the ferromagnetic subgroup will be used "
 ...
 ABI_MALLOC(symrec_fm,(3,3,nsym_afm))
 ABI_MALLOC(fm2symrec,(nsym_afm))
 ...
 ABI_MALLOC(ksym_table,(4,2,nsym_afm))

 call symq3(nsym_afm,esymm%kpt,ksym_table,symrec_fm,dummy_timrev,prtvol=0)
...
&          "gave ",esymm%herring_test(irp),", while it should be 0 or +- ",Ptg%nsym 
 ...
 if (esymm%has_chtabs) then
   ...
   if (nacc_deg/=0) then
     ...
     MSG_WARNING(msg)
     esymm%err_status=ESYM_ACCDEG_ERROR  ! TODO this should signal to the caller that we have to decompose the calculated representation.
     esymm%err_msg =msg(1:500)
   end if
 end if
...
! *************************************************************************
 ...
 if (allocated(esymm%Calc_irreps)) then
   call destroy_irrep(esymm%Calc_irreps)
 end if

 if (allocated(esymm%trCalc_irreps)) then
   call destroy_irrep(esymm%trCalc_irreps)
 end if

 if (allocated(esymm%Ref_irreps)) then
   call destroy_irrep(esymm%Ref_irreps)
 end if
