...
!End of the abilint section
 ...
 real(dp), allocatable, dimension(:,:), target :: cg_alltoall,gsc_alltoall,ghc_alltoall,gvnlc_alltoall
...
! ************************************************************************* 
 ...
 if (dtset%paral_kgb == 1) then
   ...
   ABI_ALLOCATE(cg_alltoall, (2, npw_filter*nspinor*nband_filter))
   ABI_ALLOCATE(gsc_alltoall, (2, npw_filter*nspinor*nband_filter))
   ABI_ALLOCATE(ghc_alltoall, (2, npw_filter*nspinor*nband_filter))
   ABI_ALLOCATE(gvnlc_alltoall, (2, npw_filter*nspinor*nband_filter))
...
#define cg_loadbalanced ghc
   ...
   ! Transpose input cg into cg_alloall. cg_alltoall is now (npw_filter, nband_filter)
   call timab(timer_alltoall, 1, tsec)
   call xmpi_alltoallv(cg_loadbalanced,sendcountsloc,sdisplsloc,cg_alltoall,&
...
&   index_wavef_band)
   cg_alltoall(:,:) = cg_alltoall(:,index_wavef_band)

   cg_filter => cg_alltoall
   gsc_filter => gsc_alltoall
   ghc_filter => ghc_alltoall
   gvnlc_filter => gvnlc_alltoall
 else
   npw_filter = npw
   nband_filter = nband
...
&       sij_opt,vlocal,fock,cpopt,cwaveprj(:,iactive:),.true.)
 ...
 ! transpose back
 if(dtset%paral_kgb == 1) then
   cg_filter(:,index_wavef_band) = cg_filter(:,:)
   ghc_filter(:,index_wavef_band) = ghc_filter(:,:)
   if(paw) then
     gsc_filter(:,index_wavef_band) = gsc_filter(:,:)
   else
     gvnlc_filter(:,index_wavef_band) = gvnlc_filter(:,:)
   end if
   ...
   call xmpi_alltoallv(cg_alltoall,recvcountsloc,rdisplsloc,cg,&
&   sendcountsloc,sdisplsloc,mpi_enreg%comm_band,ierr)
   call xmpi_alltoallv(ghc_filter,recvcountsloc,rdisplsloc,ghc,&
&   sendcountsloc,sdisplsloc,mpi_enreg%comm_band,ierr)
   if(paw) then
...
&     sendcountsloc,sdisplsloc,mpi_enreg%comm_band,ierr)
   ...
   if(mpi_enreg%paral_kgb == 1) then
     ABI_DEALLOCATE(cg_alltoall)
     ABI_DEALLOCATE(gsc_alltoall)
     ABI_DEALLOCATE(ghc_alltoall)
