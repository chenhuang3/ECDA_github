...
!!
!! COPYRIGHT
!! Copyright (C) 2013 ABINIT group (AL)
...
!!
!! SOURCE
subroutine make_invovl(ikpt, ham, dimffnl, ffnl, ph3d, mpi_enreg)
 use defs_basis
...
!End of the abilint section
 ...
 real(dp),intent(in) :: ffnl(ham%npw,dimffnl,ham%lmnmax,ham%ntypat)
 real(dp),intent(in) :: ph3d(2,ham%npw,ham%matblk)
 type(mpi_type) :: mpi_enreg

 real(dp) :: atom_projs(2, ham%npw, ham%lmnmax)
 real(dp) :: temp(ham%npw)
...
! *************************************************************************
 ...
 invovl => invovl_kpt(mpi_enreg%my_kpttab(ikpt))
 ...
 ABI_ALLOCATE(projs, (2, ham%npw, invovl%nprojs))
 ...
 do itypat = 1, ham%ntypat
   ...
   do ia = 1, ham%nattyp(itypat)
     ...
     if(ia == 1) then
       ...
       call abi_xgemm(blas_transpose,'N', nlmn, nlmn, (3-cplx)*ham%npw, cone, atom_projs(:,:,1), (3-cplx)*ham%npw, &
&                     atom_projs(:,:,1), (3-cplx)*ham%npw, czero, gram_proj(:,:,1), nlmn,x_cplx=cplx)
 ...
 do iproc = 1, mpi_enreg%nproc_fft
   ...
   call abi_xgemm(blas_transpose,'N', invovl%nprojs, slice_size, (3-cplx)*ham%npw, cone, projs(:,:,1), (3-cplx)*ham%npw, &
&                      projs(:, :, shift+1), (3-cplx)*ham%npw, czero, gramwork(:,:,1), invovl%nprojs,x_cplx=cplx)
...
!!
!! SOURCE
 subroutine apply_invovl(ham, cwavef, sm1cwavef, cwaveprj, npw, ndat, mpi_enreg, dimffnl, ffnl, ikpt,&
&nspinor, kg, ph3d, dtset, dimcprj)
 use defs_basis
 use defs_abitypes
 use m_hamiltonian, only : gs_hamiltonian_type
 use m_profiling_abi
 use defs_abitypes
 use m_xmpi
 use m_pawcprj, only : pawcprj_type, pawcprj_alloc, pawcprj_destroy, pawcprj_axpby
 use m_bandfft_kpt
...
!End of the abilint section
 ...
 integer,intent(in) :: dimffnl, ikpt, nspinor
 real(dp), intent(inout) :: cwavef(2, npw*nspinor*ndat) ! TODO should be in, fix nonlop
 type(mpi_type) :: mpi_enreg
 real(dp) :: ffnl(npw,dimffnl,ham%lmnmax,ham%ntypat)
 integer :: kg(3,npw)
 real(dp) :: ph3d(2,npw,ham%matblk)
 ...
 character :: blas_transpose
 integer :: dimcprj(ham%natom)
 type(pawcprj_type) :: cwaveprj_in(ham%natom,nspinor*ndat)

 integer, parameter :: tim_nonlop = 13
 ! dummies
 real(dp) :: enlout(ndat), kpg_k(1,1), lambda_block(1), gvnlc(1,1)
 integer, parameter :: nnlout = 0, idir = 0, nkpg = 0, signs = 2

 type(invovl_kpt_type), pointer :: invovl
 integer, parameter :: timer_apply_inv_ovl_opernla = 1630, timer_apply_inv_ovl_opernlb = 1631, timer_apply_inv_ovl_inv_s = 1632
...
! *************************************************************************
 ...
 call pawcprj_alloc(cwaveprj_in,0,dimcprj)
 ...
 if (mpi_enreg%paral_kgb==1) then
   call prep_nonlop(ham%atindx1,choice,cpopt,cwaveprj_in,ham%dimekb1,&
&           ham%dimekb2,dimffnl,ham%ekb,enlout,&
&           ham%gmet,ham%gprimd,idir,ikpt,ham%indlmn,ham%istwf_k,&
&           ham%kpoint,lambda_block,ham%lmnmax,ham%matblk,ndat,ham%mgfft,&
&           mpi_enreg,ham%mpsang,ham%mpssoang,&
&           ham%natom,ham%nattyp,ham%ngfft,nkpg,ham%nloalg,nnlout,npw,&
&           nspinor,dtset%nspinor,ham%ntypat,paw_opt,ham%phkxred,ham%ph1d,signs,&
&           ham%sij,sm1cwavef,tim_nonlop,ham%ucvol,ham%useylm,cwavef,gvnlc,&
&           use_gpu_cuda=dtset%use_gpu_cuda, already_transposed=.true.)
 else
   call nonlop(ham%atindx1,choice,cpopt,cwaveprj_in,&
&           ham%dimekb1,ham%dimekb2,dimffnl,dimffnl,ham%ekb,&
&           enlout,ffnl,ffnl,ham%gmet,ham%gprimd,idir,&
&           ham%indlmn,ham%istwf_k,kg,kg,kpg_k,kpg_k,ham%kpoint,ham%kpoint,&
&           lambda_block,ham%lmnmax,ham%matblk,ham%mgfft,mpi_enreg,ham%mpsang,&
&           ham%mpssoang,ham%natom,ham%nattyp,ndat,&
&           ham%ngfft,nkpg,nkpg,ham%nloalg,nnlout,npw,npw,nspinor,dtset%nspinor, &
&           ham%ntypat,0,paw_opt,&
&           ham%phkxred,ham%phkxred,ham%ph1d,ph3d,ph3d,&
&           signs,ham%sij,sm1cwavef,&
&           tim_nonlop,ham%ucvol,ham%useylm,cwavef,gvnlc,&
&           use_gpu_cuda=dtset%use_gpu_cuda)
 ...
 paw_opt = 3
 if (mpi_enreg%paral_kgb==1) then
   call prep_nonlop(ham%atindx1,choice,cpopt,cwaveprj,ham%dimekb1,&
&           ham%dimekb2,dimffnl,ham%ekb,enlout,&
&           ham%gmet,ham%gprimd,idir,ikpt,ham%indlmn,ham%istwf_k,&
&           ham%kpoint,lambda_block,ham%lmnmax,ham%matblk,ndat,ham%mgfft,&
&           mpi_enreg,ham%mpsang,ham%mpssoang,&
...
&           use_gpu_cuda=dtset%use_gpu_cuda, already_transposed=.true.)
   ...
   call nonlop(ham%atindx1,choice,cpopt,cwaveprj,&
&           ham%dimekb1,ham%dimekb2,dimffnl,dimffnl,ham%ekb,&
&           enlout,ffnl,ffnl,ham%gmet,ham%gprimd,idir,&
&           ham%indlmn,ham%istwf_k,kg,kg,kpg_k,kpg_k,ham%kpoint,ham%kpoint,&
&           lambda_block,ham%lmnmax,ham%matblk,ham%mgfft,mpi_enreg,ham%mpsang,&
...
&           use_gpu_cuda=dtset%use_gpu_cuda)
 ...
 call pawcprj_destroy(cwaveprj_in)
