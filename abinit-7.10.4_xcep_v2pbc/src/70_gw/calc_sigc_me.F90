...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (FB, GMR, VO, LR, RWG, MG, RShaltaf)
...
!!      coeffs_gausslegint,cwtime,destroy_paw_pwij,epsm1_pole_symmetrizer
!!      epsm1_pole_symmetrizer_inplace,epsm1_symmetrizer
!!      epsm1_symmetrizer_inplace,esymm_symmetrize_mels,findqg0,get_bz_item
!!      get_epsm1,get_gftt,get_pole_epsm1,get_uug,gsph_fft_tabs,gw_eet_sigma
!!      gw_eet_sigma_cd,init_paw_pwij,littlegroup_print,paw_cross_rho_tw_g
!!      paw_rho_tw_g,paw_symcprj,pawcprj_alloc,pawcprj_copy,pawcprj_destroy
!!      ppm_get_qbz,rho_tw_g,rotate_fft_mesh,setup_ppmodel,sigma_distribute_bks
!!      timab,wfd_change_ngfft,wfd_get_cprj,wfd_get_many_ur,wfd_get_ur
!!      wfd_paw_get_aeur,wfd_sym_ur,wrtout,xmpi_max,xmpi_sum
!!
!! SOURCE
...
& gwc_ngfft,rho_ngfft,rho_nfftot,rhor,use_aerhor,aepaw_rhor,sigcme_tmp)
 ...
 use m_wfs,           only : wfd_get_ur, wfd_t, wfd_get_cprj, wfd_barrier, wfd_change_ngfft, wfd_paw_get_aeur, &
...
&                            epsm1_pole_symmetrizer, epsm1_pole_symmetrizer_inplace, get_pole_epsm1
 ...
 use m_eet,           only : gw_eet_sigma, gw_eet_sigma_cd
...
!scalars
 ...
 type(sigma_parameters),target,intent(in) :: Sigp
...
!arrays
 ...
 integer,allocatable :: npoles_missing(:)
 ...
 real(gwpc), allocatable :: epsm1_pole_qbz(:,:,:) 
...
&       ' Verify the frequencies in the SCR file. '
   ...
   ABI_MALLOC(ac_epsm1cqwz2,(Sigp%npwc,Sigp%npwc,Er%nomega_i))
   ABI_MALLOC(ac_integr,(Sigp%npwc,Sigp%npwc,Sr%nomega_i))
 end if
 !
 ...
 ABI_MALLOC(aherm_sigc_ket,(Sigp%npwc*nspinor,nomega_sigc))
 ABI_MALLOC( herm_sigc_ket,(Sigp%npwc*nspinor,nomega_sigc))
 ...
 ABI_MALLOC(ket1,(Sigp%npwc*nspinor,nomega_tot))
 ABI_MALLOC(ket2,(Sigp%npwc*nspinor,nomega_tot))
...
!   end if
 ...
 if ( ANY( mod10==(/SIG_GW_AC, SIG_GW_CD, SIG_QPGW_CD/) )) then
   if (.not.(mod10==SIG_GW_CD.and.Er%mqmem==0)) then
     if (pole_screening) then
       ABI_MALLOC(epsm1_pole_qbz,(Sigp%npwc,Sigp%npwc,Er%ncoeff))
     end if
     ABI_MALLOC(epsm1_qbz,(Sigp%npwc,Sigp%npwc,Er%nomega))
     ABI_CHECK_ALLOC("out-of-memory in epsm1_qbz")
   end if
 end if
 ...
 if (mod10==SIG_QPGW_CD.and.(.not.pole_screening)) then
   ABI_MALLOC(epsm1_trcc_qbz,(Sigp%npwc,Sigp%npwc,Er%nomega))
   ABI_CHECK_ALLOC("out-of-memory in epsm1_trcc_qbz")
   ABI_MALLOC(epsm1_tmp,(Sigp%npwc,Sigp%npwc))
 end if
 ...
 do spin=1,Wfd%nsppol
   ...
   if (ALL(proc_distrb(:,:,spin)/=Wfd%my_rank).and.dtset%gw_eet==-1) CYCLE
...
&          ur_ae_sum,ur_ae_onsite_sum,ur_ps_onsite_sum)
 ...
 do ik_bz=1,Kmesh%nbz
   ...
   if (ALL(proc_distrb(:,ik_bz,spin)/=Wfd%my_rank).and.dtset%gw_eet==-1) CYCLE
...
!$OMP PARALLEL DO
   ...
   if (dtset%gw_eet/=-1) then ! MG Where are the GW wavefunctions? It won't work in parallel
     ...
     if (mod10==2) then
       call gw_eet_sigma_cd(Sigp,Sr,Dtset,Cryst,Wfd,Kmesh,Qmesh,Gsph_Max,Gsph_c,Psps,Vcp,QP_BSt,Er, &
&                        spin,iq_bz,ik_bz,jk_bz,ik_ibz,jk_ibz,itim_q,isym_q,iq_ibz,&
...
&                        nomega_tot,nomega_sigc,fact_sp,nspinor,sigcme_tmp,sigc,nbopt, &
&                        tim_fourdp2,wtqp,wtqm,extrapolar_distrb,can_symmetrize,epsm1_qbz,npoles_missing)
     else
...
&                        tim_fourdp2,wtqp,wtqm,extrapolar_distrb,can_symmetrize)
     endif
     nbmax=nbopt
...
#else
       call wfd_sym_ur(Wfd,Cryst,Kmesh,ib,ik_bz,spin,usr_bz,trans="C",with_umklp=.FALSE.,ur_kibz=ur_ibz)
#endif
...
#else
           call get_uug(Sigp%npwc,gwc_nfftot,ndat1,gwc_ngfft,use_padfft,igfftcg0,gw_gbound,&
             usr_bz,wfr_bdgw(:,jb),rhotwg_ki(:,jb))
#endif
...
&            Cprj_ksum(:,:),Cprj_kgw(:,i2:i2+spad),Pwij_qg,rhotwg_ki(:,jb))
           if (Dtset%pawcross==1) then ! Add paw cross term
             call paw_cross_rho_tw_g(nspinor,Sigp%npwc,nfftf,rho_ngfft,1,use_padfftf,igfftfcg0,gboundf,&
...
&                ' Use another Plasmon Pole Model when gwcalctyp=28.'
             ...
             ABI_MALLOC(botsq_conjg_transp,(PPm%dm2_botsq,Sigp%npwc))
...
&               method=Dtset%cd_frqim_method)
         ...
         if (Sigp%gwcomp==1) then ! TODO spinor not implemented
           call calc_sig_ppm_comp(Sigp%npwc,nomega_tot,rhotwgp,botsq,otq,DBLE(Sr%egw(kb,jk_ibz,spin)-en_high),&
&            Sigp%zcut,theta_mu_minus_e0i,sigc_ket,PPm%model,Sigp%npwc,PPm%dm2_botsq,PPm%dm2_otq)
         ...
         do irow=1,Sigcij_tab(spin)%col(kb)%size1
           ...
           if (Sigp%gwcomp==1) then ! Evaluate Extrapolar term TODO this does not work with spinor
             ...
             !if ( (.not.done_once(jb,kb,spin)) .and. (gwpara/=2 .or. Wfd%my_rank==modulo(jb+kb*(ib2-ib1+1),Wfd%nproc) ) ) then
             !  done_once(jb,kb,spin)=.TRUE.
             if ( extrapolar_distrb(jb,kb,ik_bz,spin) == Wfd%my_rank ) then
               extrapolar_distrb(jb,kb,ik_bz,spin) = xmpi_undefined_rank ! Do it once as it does not depend on the ib index summed over.
#if 1
...
&              (wtqp+wtqm)*DBLE(sigctmp(:,iab)) + (wtqp-wtqm)*j_gw*AIMAG(sigctmp(:,iab))
 ...
 if (Psps%usepaw==1) then
   ...
   call pawcprj_destroy(Cprj_ksum)
