...
interface
 subroutine cchi0(use_tr,Dtset,Cryst,qpoint,Ep,Psps,Kmesh,QP_BSt,Gsph_epsG0,Gsph_wfn,&  
  ...
  type(gsphere_t),intent(in) :: Gsph_wfn
  type(kmesh_t),intent(in) :: Kmesh
...
interface
 subroutine prep_calc_ucrpa(sigmak_ibz,ikcalc,itypatcor,minbnd,maxbnd,Cryst,QP_BSt,Sigp,Gsph_x,Vcp,Kmesh,Qmesh,lpawu,&  
  ...
  integer,intent(in) :: gwx_ngfft(18)
  integer,intent(in) :: ngfftf(18)
  complex(dpc), intent(out) :: M1_q_m(cryst%nattyp(itypatcor),2*lpawu+1, &
  &         2*lpawu+1,sigp%npwx,Qmesh%nibz)
  type(paw_pwaves_lmn_t),intent(in) :: Paw_onsite(Cryst%natom)
  type(paw_pwff_type),intent(in) :: Paw_pwff(Psps%ntypat*Psps%usepaw)
  type(pawfgrtab_type),intent(inout) :: Pawfgrtab(Cryst%natom*Psps%usepaw)
  type(pawtab_type),intent(in) :: Pawtab(Psps%ntypat)
  ...
  &         2*lpawu+1,2*lpawu+1,sigp%npwx,Qmesh%nibz)
...
interface
 subroutine read_plowannier(cryst,bandinf,bandsup,coeffW_BZ,itypatcor,Kmesh,lcor,luwindow,nsppol,pawang,prtvol,ucrpa_bands)
  use defs_basis
  use m_pawang
  use m_bz_mesh
  use m_crystal
...
interface
 subroutine tddft_bootstrap(iqibz,Vcp,npwe,nI,nJ,nomega,omega,chi0,nsteps,tolerance,&  
  ...
  complex(dpc),intent(out) :: epsm_lf(nomega,my_nqlwl)
  complex(dpc),intent(out) :: epsm_nlf(nomega,my_nqlwl)
