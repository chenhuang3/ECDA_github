...
subroutine calculate_c_pbe(n1,n2,n3,nspin,rho,qvec,dvol,xc_pot,xc_energy)
...
!======================================function body=====================================
  ...
  !   write(*,*) "USING NEW SPIN LIBXC!!!!"
     ...
     do k = 1,n3
         do j = 1,n2
             do i = 1,n1
                 ...
                 depsxc(i,j,k,1) =  vctmp(1)

                 If(nspin==1) then
                     !depsxc(i,j,k,5) =  2.d0 * (vcsigmatmp(1) + vxsigmatmp(1))
                     depsxc(i,j,k,5) =  2.d0 * vcsigmatmp(1)
                 Else
                     ...
                     depsxc(i,j,k,2) =  vctmp(2)
                     depsxc(i,j,k,3) =  2.d0*(vcsigmatmp(1)) - vcsigmatmp(2)
                     depsxc(i,j,k,4) =  2.d0*(vcsigmatmp(3)) - vcsigmatmp(2)
                     depsxc(i,j,k,5) =  vcsigmatmp(2)
                 Endif
             enddo
         enddo
     enddo
     ...
     !now energy
     xc_energy = SUM(rhotot*excpbe)*dvol
...
end subroutine calculate_c_pbe
