...
if test ! -s "./configure.ac" -o ! -s "config/specs/tests.conf"; then
  echo "[tstclean]   Cowardly refusing to remove something from here!" >&2
  exit 1
fi

# Remove temporary directories and files
echo "[tstclean]   Removing temporary directories and files"
rm -rf tmp*
find . -depth -name 'tmp-*' -exec rm -rf {} \;

# Remove autotools files
echo "[tstclean]   Removing autotools files"
...
echo "[tstclean]   Removing files produced by the configure script"
rm -f libtool
