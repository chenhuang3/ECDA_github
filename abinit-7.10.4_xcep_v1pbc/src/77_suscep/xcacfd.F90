...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2014 ABINIT group (XG,MF)
...
!!      kxc_eok,kxc_local,kxc_pgg,prtsusd,suscep_dyn,wrtout
!!
!! SOURCE
...
subroutine xcacfd(dielar,dtfil,dtset,eigen,freq,gbound_diel,gmet,&
...
!scalars
 ...
 integer :: ndyson,susopt,temp_unit
...
!Check input parameters.
 ...
 if (nspden > 1) then
   MSG_ERROR('xcacfd does not work yet for nspden > 1.')
 end if
 ...
 select case (ikhxc)
   ...
   case (ikhxc_EOK2)
   case default
     write (message,'(a,i0,a)')'  ikhxc = ',ikhxc,' is not a valid xc kernel.'
     MSG_ERROR(message)
 end select
...
&   ' xcacfd: Use idyson = ',idyson,' (solve the Dyson equation as a ',trim(idyson_name(idyson)),').',ch10,&
...
&   ' xcacfd: Calculating the PGG kernel...'
   ...
   if (nspden > 1) then
     message = ' The PGG and BPG kernels do not work yet for nspden > 1.'
     MSG_ERROR(message)
   end if
...
&   ' xcacfd: Calculating the linear energy optimized kernel (EOK1)...'
   ...
   do ii = 1,2*nspden-1
     call kxc_local(ii,kg_diel,khxc,kxcg(:,:,ii),nfft,ngfft,npwdiel,nspden,1)
   end do
...
!The following file will be used to store data from prtsusd.
 ...
 temp_unit = get_unit()
 open (unit = temp_unit,file = dtfil%fnametmp_sustr,status = 'unknown')
