...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (XG,MF,AR,MB)
...
!!
!! CHILDREN
!!      fftpac,sphereboundary,susk_dyn,susk_dyn_pgg,suskmm_dyn,symg,symrhg
!!      timab,zhpev
!!
!! SOURCE
...
&  susopt,sus_diag_dyn,susmat_dyn,symafm,symrel,tnons,ucvol,wtk)
...
!scalars
 ...
 integer :: ier,ifft,ifreq,ii,ikg,ikpt,index,ipw1,ipw2,isp
! integer :: isp1,isp2
 integer :: istwf_k,isym,j1,j2,j3,jj,k1,k2,k3,mcg_disk
 integer :: nband_k,nband_k_use,ndiel1,ndiel2,ndiel3,ndiel4,ndiel5,ndiel6
 integer :: npw_k,npwsp,t1,t2,testocc
 real(dp) :: ai,ar,diegap,dielam,emax,invnsym
 real(dp) :: phi1,phi12,phi2,phr1,phr12,phr2,weight
 !character(len=500) :: message
!arrays
 integer,allocatable :: gbound(:,:),kg_k(:,:),sym_g(:,:),tmrev_g(:)
 real(dp) :: tsec(2)
 real(dp),allocatable :: cg_disk(:,:),eig_diel(:)
 real(dp),allocatable :: occ_deavg(:),occ_freq(:,:,:),phdiel(:,:,:)
...
!ENDDEBUG
 ...
 if( occopt>=3 )then
   MSG_ERROR('suscep_dyn: occopt>= 3 does not yet work, stopping')
 end if

 if(nspinor==2)then
   MSG_ERROR('suscep_dyn: does not yet work for nspinor=2')
 end if
...
!    ENDDEBUG
     ...
     if(proc_distrb_cycle(mpi_enreg%proc_distrb,ikpt,1,nband_k,isp,mpi_enreg%me)) then
...
!    DEBUG
!    write(std_out,*) 'suscep_dyn: mark 1'
!    call flush_unit(std_out
!    ENDDEBUG
...
&         isp,istwf_k,kg_diel,kg_k,mband,mcg_disk,mgfftdiel,mpi_enreg,mpw,&
...
!      &     isp,istwf_k,kg_diel,kg_k,mband,mcg,mgfftdiel,mpi_enreg,&
...
!      &     gbound_diel,icg,ikpt,&
!      &     isp,istwf_k,kg_diel,kg_k,mband,mcg_disk,mgfftdiel,mpi_enreg,&
!      &     nband_k,ndiel4,ndiel5,ndiel6,neglect_pawhat,nfftdiel,ngfftdiel,nkpt,&
!      &     npwdiel,npw_k,nspden,nspinor,nsppol,occ,occopt,occ_deavg,rhoextrap,sumdocc,&
!      &     susmat,ucvol,wtk)
!      end if
...
!    DEBUG
!    write(std_out,*)' %suscep_dyn: extrapolation: show density part'
!    call flush_unit(std_out)
!    ENDDEBUG
...
!      Complex density: option 10, fill real part into rhoextrr_cmplx(even,:)
       ...
       call fftpac(1,mpi_enreg,1,2*ndiel1,ndiel2,ndiel3,ndiel4,ndiel5,ndiel6,&
...
!      DEBUG
!      write(std_out,*) '%suscep_dyn: symmetrization mark 4'
!      call flush_unit(std_out)
!      ENDDEBUG
...
!      DEBUG
!      write(std_out,*) '%suscep_dyn: symmetrization mark 5'
!      call flush_unit(std_out)
!      ENDDEBUG
...
!DEBUG
!write(std_out,*) '%suscep_dyn: symmetrization mark 7'
!call flush_unit(std_out)
!ENDDEBUG
...
!DEBUG
!write(std_out,*) '%suscep_dyn: symmetrization mark 8'
!call flush_unit(std_out)
!ENDDEBUG
