...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2014 ABINIT group (RS, MG, MS)
...
!!
!! CHILDREN
!!      abi_io_redirect,copy_scrhdr,cqratio,crystal_free,crystal_from_hdr
!!      cspint,decompose_epsm1,destroy_epsilonm1_results,destroy_mpi_enreg
!!      find_qmesh,flush_unit,fourdp,free_scrhdr,get_ppm_eigenvalues,get_rhor
!!      getem1_from_ppm_one_ggp,getng,gsph_free,gsph_init,herald,im_screening
!!      init_er_from_file,initmpi_seq,int2char4,isfile,kmesh_free,kmesh_init
!!      kmesh_print,merge_scrhdr,metric,mkdump_er,ppm_free,ppm_init
...
!!      sort_dp,test_charge,timein,vcoul_free,vcoul_init,write_pole_screening
!!      write_screening,wrtout,xmpi_init
!!
!! SOURCE
...
program mrgscr
 ...
 use m_io_tools,            only : prompt, get_unit, file_exists, flush_unit
...
!scalars
 ...
 integer :: ifile,ifound,ierr,ii,ios,iqiA,iqibz,iqf !,idx1,idx2
 ...
 integer :: ifrq,npwe4mI,npwe4mJ,indx_q0,imax
 ...
 real(dp) :: dwre,dwim,tcpu,tcpui,twall,twalli
 real(gwp) :: phase
#ifdef HAVE_ALGO_LEVMAR
 ...
 character(len=fnlen) :: fname_out,fname,fname_dump,fname_rho,prefix,fname_eigen
...
!integer,pointer :: ip2fp(:,:)
 ...
 real(dp)  ,allocatable :: qlwl(:,:),real_omega(:),imag_omega(:)
 ...
 !type(ScrHdr_type),target,allocatable :: Hscr_file(:) ! Cannot use allocatable as pathscale5 miscompiles the code
 type(ScrHdr_type),target :: Hscr_file(MAX_NUMFILES)
...
!Initialize MPI
 ...
 ! parse command line arguments (only if nprocs==1)
...
!      * Merge the headers creating the full list of q-points.
       call merge_ScrHdr(Hscr_file(1:nfiles),Hscr_merge)
...
!      Useful to do the merge point-by-point thus avoiding the allocation of the entire epsm1 array.
       ABI_MALLOC(merge_table,(Hscr_merge%nqibz,2))
...
!        Check if point has been found, multiple points not allowed.
       ...
       unt_out=get_unit()
       open(unit=unt_out,file=trim(fname_out),status='new',form='unformatted',iostat=ios)
       if (ios/=0) then
...
!      TODO Use new format but first of all fix problem with hdr_check
       ...
       ABI_MALLOC(epsm1,(npwe4m,npwe4m,nomega4m,1))
     ...
     case(2) ! Merge frequencies
       ...
       msg = "Advanced user option, consistency in fform etc. will not be checked."
       MSG_WARNING(msg)
...
!      Then modify entries for new frequency grid
       ...
       ABI_FREE(Hscr_merge%omega)
       ABI_MALLOC(Hscr_merge%omega,(nfreq_tot))
...
&         Hscr_merge%qibz(3,iqibz)<tol6) then
         ...
         ABI_FREE(Hscr_merge%uwing)
         ABI_FREE(Hscr_merge%lwing)
         ABI_MALLOC(Hscr_merge%uwing,(npwe4mJ,nfreq_tot,Hscr_merge%nqlwl))
         ABI_MALLOC(Hscr_merge%lwing,(npwe4mI,nfreq_tot,Hscr_merge%nqlwl))
         do ifrq=1,nfreq_tot
...
&       2.0*gwpc*npwe4mI*npwe4mJ*nomega4m*b2Mb," [Mb]."
       ABI_MALLOC(epsm1,(npwe4mI,npwe4mJ,nomega4m,1))
       ABI_CHECK_ALLOC('out of memory in epsm1')
...
&           2.0*gwpc*npwe4mI*npwe4mJ*nomega4m*b2Mb,' [Mb].'
           ABI_MALLOC(epsm1_temp,(npwe4mI,npwe4mJ,nomega4m,1))
           ABI_CHECK_ALLOC('out of memory in epsm1_temp')
...
!  write(std_out,'(a)')         '  or interpolate the screening in k-space       (= 8) ?'
   ...
   select case(choice)
     case(1) ! Recover subset of q-points --------------------------------------------------
       ...
       unt_tmp = get_unit()
       open(unit=unt_tmp,file=trim(fname_out),status='new',form='unformatted',iostat=ios)
...
!      Had to change dimensions and arrays associated to nqibz.
       ...
       ABI_FREE(Hscr_recovery%qibz)
       ABI_MALLOC(Hscr_recovery%qibz,(3,nq_selected))
...
!      TODO Use new format but first of all fix problem with hdr_check
       ...
       ABI_MALLOC(epsm1,(npwe_asked,npwe_asked,nomega_asked,1))
       ABI_CHECK_ALLOC('out of memory in epsm1')
...
!          === Initialize the G-sphere ===
           ...
           ABI_MALLOC(epsm1,(Hscr0%npwe,Hscr0%npwe,Hscr0%nomega,1))
           ABI_CHECK_ALLOC('out of memory in epsm1')
...
!          Specify grid
           ...
           ABI_MALLOC(omega,(nfreqre*nfreqim))
           idx = 1
           ...
           ABI_MALLOC(epsm1_pole,(Hscr0%npwe,Hscr0%npwe,Hscr0%ncoeff,1))
           ABI_MALLOC(fval,(nfreqre*nfreqim))
...
&       mgfft,nfft,ngfft,MPI_enreg%nproc_fft,Cryst%nsym,1,&
&       MPI_enreg%paral_kgb,Cryst%symrel)
...
&         MPI_enreg,rhor,get_pawden=.TRUE.)
       else
         call get_rhor(fname_rho,accesswff,Hscr0%Hdr%nspden,nfft,ngfft,paral_kgb0,MPI_enreg,rhor)
       end if
...
&       ' Would you like to calculate the eigenvalues of eps^{-1}_GG''(omega) [Y/N] ? '
       ...
       if (ans=='Y'.or.ans=='y') then
         ...
         do iqibz=1,Er%nqibz
           ...
           unt_dump=get_unit()
           open(file=trim(fname_eigen),unit=unt_dump,status='replace',form='formatted',iostat=ios)
         ...
         if (ans=='Y'.or.ans=='y') then
           ...
           do iqibz=1,Er%nqibz
             ...
             do ppmodel=1,2
               ...
               unt_dump=get_unit()
               open(file=trim(fname_eigen),unit=unt_dump,status='new',form='formatted',iostat=ios)
...
!              Prepare file for data on imaginary omega axis
               ...
               unt_dump2=get_unit()
               open(file=trim(fname_dump2),unit=unt_dump2,status='replace',form='formatted',iostat=ios)
...
!              Prepare ratios and density for the f-sum rule
               ABI_MALLOC(qratio,(orig_npwe,orig_npwe))
               ABI_CHECK_ALLOC("out-of-memory in qratio")

               ABI_MALLOC(rhoggp,(Er%npwe,Er%npwe))
               ABI_CHECK_ALLOC("out-of-memory in rhoggp")
...
!      === Initialize the G-sphere ===
       ...
       select case(choice)
         ...
         case(1) ! Fit just one element
           ...
           ABI_MALLOC(epsm1,(Hscr_file(1)%npwe,Hscr_file(1)%npwe,Hscr_file(1)%nomega,1))
           ABI_CHECK_ALLOC('out of memory in epsm1')
...
!          Allocate
           ABI_MALLOC(coeff,(npoles*3+1))
           ABI_MALLOC(startcoeff,(npoles*3+1))
           ABI_MALLOC(refval,(Hscr_file(1)%nomega))
           ABI_MALLOC(imfval,(Hscr_file(1)%nomega))
           ABI_MALLOC(imfval2,(Hscr_file(1)%nomega))
           ABI_MALLOC(refval2,(Hscr_file(1)%nomega))
...
!          Write header and screening
           ...
           do iqibz=1,Hscr_file(1)%nqibz
             ...
             ABI_MALLOC(epsm1_pole,(Hscr_file(1)%npwe,Hscr_file(1)%npwe,npoles*3+1,1))
...
!              if (mod(ig1,10)==1)write(std_out,'(2(a,i0))')' Done ig1 = ',ig1,' of ',Hscr_file(1)%npwe
             end do   ! ig1
!$OMP END DO
             ABI_FREE(refval)
             ABI_FREE(imfval)
...
!          allocate and evaluate new frequency grid
           ABI_MALLOC(omega_new,(new_nfreqre+nfreqim))
