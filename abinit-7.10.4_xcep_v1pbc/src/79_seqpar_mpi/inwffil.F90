...
!!
!! CHILDREN
!!      copy_mpi_enreg,destroy_mpi_enreg,hdr_check,hdr_free,hdr_io,hdr_io_etsf
...
!Arguments ------------------------------------
 ...
 character(len=*),intent(in) :: wffnm,wft1nm
...
!Init mpi_comm
 ...
 me=xcomm_rank(spaceComm)  !Init me
...
!  Initialize hdr0 (sent to all procs), thanks to reading of wff1
   ...
   if (fform_dum == 0) then
     write(message,"(4a)")&
&      "hdr_io returned fform == 0 while trying to read the wavefunctions from file: ",trim(wffnm),ch10,&
&      "This usually means that the file does not exist or that you don't have enough privileges to read it"
     MSG_ERROR(message)
   end if

   call wrtout(std_out,'inwffil: examining the header of disk file '//TRIM(wffnm),'COLL')
...
!    wait for wavelets.
   ...
   write(message,'(2a)')'-inwffil : will read wavefunctions from disk file ',trim(wffnm)
...
!  Compute reciprocal space metric gmet for unit cell of disk wf
   ...
   if ((mpi_enreg%paral_kgb==1).or.(mpi_enreg%paral_hf==1)) then
     ...
     message="tmpfil"
     call kpgio(ecut0,dtset%exchn2n3d,gmet0,istwfk0,kg0,message, &
&     kptns0,nkpt0,nband0,nkpt0,'PERS',mpi_enreg0,&
&     mpw0,npwarr0,npwtot0,nsppol0,tmp_unit)
...
!(in the parallel case, it allows to change localrdwf=0 in localrdwf=1)
 ...
 if(xmpi_paral == 1 .or. mpi_enreg%paral_kgb == 1 .or. mpi_enreg%paral_hf == 1) then
   if(localrdwf==0 .and. mkmem==0)then
     MSG_BUG('localrdwf==0 and mkmem==0 are not allowed together (yet)')
   end if
 end if
...
!  The input wavefunctions have been transferred from disk to core memory
   ...
   ABI_DEALLOCATE(cg_disk)
   !end if
   !if(mpi_enreg%paral_kgb == 1 .or. xmpi_paral == 1 .or. mpi_enreg%paral_hf == 1)then
   !  if(localrdwf==0) then
   !    ABI_DEALLOCATE(cg_disk)
   !  end if
   !end if
...
&   mpw0,mpw,nband_eff,nband,ngfft0,ngfft,nkpt0,nkpt,npwarr0,npwarr,&
