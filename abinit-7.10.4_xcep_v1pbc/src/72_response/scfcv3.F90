...
!!      bec3,dfptdb_close,dfptdb_open_write,dfptdb_write,die3,ebp3,edie3,etot3
!!      fourdp,getcut,initberry3,ioarr,metric,newfermie1,newvtr3,nselt3,nstdy3
!!      nstpaw3,paw_an_destroy,paw_an_init,paw_an_nullify,paw_an_reset_flags
!!      paw_ij_destroy,paw_ij_init,paw_ij_nullify,paw_ij_reset_flags
!!      pawcprj_alloc,pawcprj_destroy,pawdenpot,pawdij,pawdijfr,pawmknhat
!!      pawnhatfr,pawrhoij_alloc,pawrhoij_destroy,qmatrix,rhofermi3,rhotov3
!!      scprqt,status,symdij,timab,vtorho3,wffclose,wrtout,xmpi_barrier
!!      xmpi_sum
!!
!! SOURCE
...
&  wtk_rbz,wvl_den,xccc3d1,xred,ylm,ylm1,ylmgr,ylmgr1,conv_retcode)
 ...
 use m_paw_an,   only : paw_an_type, paw_an_init, paw_an_destroy, paw_an_nullify, paw_an_reset_flags
...
!scalars
 ...
 integer :: accessfil,afford,choice,cplex_rhoij,dbl_nnsclo,fformr,fformv
 ...
 integer :: rdwrpaw,spaceComm,sz1,sz2,usexcnhat
...
!arrays
 ...
 real(dp),allocatable :: fcart(:,:),nhat1(:,:),nhat1gr(:,:,:),nhatfermi(:,:),nvresid1(:,:)
 real(dp),allocatable :: qmat(:,:,:,:,:,:),rhorfermi(:,:),susmat(:,:,:,:,:)
 real(dp),allocatable :: vhartr1(:),vxc1(:,:)
...
!  1st-order arrays/variables related to the PAW spheres
   ...
   has_dijfr=0;if (ipert/=dtset%natom+1) has_dijfr=1
...
!    ----------------------------------------------------------------------
     ...
     if (psps%usepaw==1.and.ipert/=dtset%natom+1.and.ipert/=dtset%natom+5) then
!      if (psps%usepaw==1.and.(ipert/=dtset%natom+1.and.ipert/=dtset%natom+5) &
!      &     .and.((usexcnhat==0) &
!      &       .or.(dtset%xclevel==2.and.(dtfil%ireadwf/=0.or.dtset%ird1den/=0.or. &
!      &                                  dtset%get1den/=0.or.initialized/=0)) &
...
&         mpi_comm_atom=mpi_enreg%comm_atom)
       end if

       call rhofermi3(atindx,atindx1,cg,cgq,cplex,cprj,cprjq,dimcprj,&
...
&     dtset%nspden,psps%ntypat,nzlmopt,option,paw_an1,paw_an,paw_ij1,pawang,&
...
!  ----------------------------------------------------------------------
   ...
   call vtorho3(atindx,atindx1,cg,cgq,cg1,cg1_active,cplex,cprj,cprjq,cprj1,&
&   dbl_nnsclo,dimcprj,dim_eig2rf,doccde_rbz,docckqde,dtefield,dtfil,dtset,edocc,&
...
&   rhor1,rmet,rprimd,symaf1,symrc1,symrl1,ucvol,usecprj,useylmgr1,wffddk,&
...
&       psps%ntypat,nzlmopt,option,paw_an1,paw_an,paw_ij1,pawang,dtset%pawprtvol,&
...
!    If criteria in scprqt say to quit, then exit the loop over istep
     ...
     ! TODO
     ! Better error handling is the SCF cycle goes banana:
...
!Deallocate the no more needed arrays
 ...
 if(psps%usepaw==0.and.(ipert==dtset%natom+3 .or. ipert==dtset%natom+4)) then
...
&     kxc,mgfftf,mkmem,mkqmem,mk1mem,mpert,mpi_enreg,mpw,mpw1,nattyp,nband_rbz,ncpgr,nfftf,ngfftf,nhat,nhat1,&
...
&     pawtab,phnons1,ph1d,ph1df,psps,rhog,rhor,rhor1,rmet,rprimd,symaf1,symrc1,symrl1,ucvol,usecprj,&
&     usexcnhat,useylmgr1,vhartr1,vpsp1,vtrial,vtrial1,vxc,wtk_rbz,&
...
&     cplex*nfftf,pawrhoij1,rdwr,rdwrpaw,wvl_den)
   end if
...
#if 0
   write(std_out,*)"pertcase netcdf",pertcase
   call dfptdb_open_write(dfpt_db,"hello.nc",dtset%natom,dtset%nspinor,dtset%nspden,dtset%usepaw)
   call dfptdb_write(dfpt_db,qphon,ipert,idir,cplex,nfftf,nspden,ngfftf(1:3),rhor1,vtrial1,pawrhoij1,ierr)
   call dfptdb_close(dfpt_db)
#endif
 end if
...
& dtset%berryopt==14.or.dtset%berryopt==16.or.dtset%berryopt==17) then
 ...
 if(psps%usepaw==1) then
   call paw_an_destroy(paw_an1)
   call paw_ij_destroy(paw_ij1)
 end if
