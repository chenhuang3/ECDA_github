...
!!
!! COPYRIGHT
!! Copyright (C) 2013-2014 ABINIT group (MT,FJ,MD)
...
MODULE m_paral_pert
 ...
 use m_paw_an,    only : paw_an_type, paw_an_destroy, paw_an_redistribute
 use m_paw_ij,    only : paw_ij_type, paw_ij_destroy, paw_ij_redistribute
 use m_pawfgrtab, only : pawfgrtab_type, pawfgrtab_destroy, pawfgrtab_redistribute
 use m_pawrhoij,  only : pawrhoij_type, pawrhoij_destroy, pawrhoij_redistribute
...
!New communicators for the perturbed cell
 ...
 mpi_enreg%me_cell   =xcomm_rank(mpi_enreg%comm_cell)
...
! *************************************************************************
 ...
 nproc_in=xcomm_size(mpicomm_in)
 me_in=xcomm_rank(mpicomm_in)
 my_natom_in=size(my_atmtab_in)
 nproc_out=xcomm_size(mpicomm_out)
 me_out=xcomm_rank(mpicomm_out)
