...
! Arrays
  integer,pointer  :: iatfix(:,:)         ! Atoms to fix
  real(dp)         :: neb_spring(2)       ! Spring constants for the NEB method
  real(dp),pointer :: bfgs_xprev(:,:,:)   ! BFGS storage (prev positions)
  real(dp),pointer :: bfgs_fprev(:,:,:)   ! BFGS storage (prev forces)
  real(dp),pointer :: gbfgs_hess(:,:)     ! global-BFGS storage (Hessian matrix)
  real(dp),pointer :: lbfgs_hess(:,:,:)   ! local-BFGS storage (Hessian matrix)
  real(dp),pointer :: qmin_vel(:,:,:)     ! Quick-min algo storage (velocities)
  real(dp),pointer :: rk4_xcart1(:,:,:)   ! 4th-order Runge-Kutta storage
  real(dp),pointer :: rk4_fcart1(:,:,:)   ! 4th-order Runge-Kutta storage
  real(dp),pointer :: rk4_fcart2(:,:,:)   ! 4th-order Runge-Kutta storage
  real(dp),pointer :: rk4_fcart3(:,:,:)   ! 4th-order Runge-Kutta storage
 end type mep_type
!!***
...
!************************************************************************
 ...
 if (associated(mep_param%bfgs_xprev)) then
   ABI_DEALLOCATE(mep_param%bfgs_xprev)
 end if
 if (associated(mep_param%gbfgs_hess))     then
   ABI_DEALLOCATE(mep_param%gbfgs_hess)
 end if
 if (associated(mep_param%bfgs_fprev)) then
   ABI_DEALLOCATE(mep_param%bfgs_fprev)
 end if
 if (associated(mep_param%lbfgs_hess))     then
   ABI_DEALLOCATE(mep_param%lbfgs_hess)
 end if
 if (associated(mep_param%qmin_vel))     then
   ABI_DEALLOCATE(mep_param%qmin_vel)
 end if
 if (associated(mep_param%rk4_xcart1)) then
   ABI_DEALLOCATE(mep_param%rk4_xcart1)
 end if
 if (associated(mep_param%rk4_fcart1)) then
   ABI_DEALLOCATE(mep_param%rk4_fcart1)
 end if
 if (associated(mep_param%rk4_fcart2)) then
   ABI_DEALLOCATE(mep_param%rk4_fcart2)
 end if
 if (associated(mep_param%rk4_fcart3)) then
...
!!
!! FUNCTION
!!  Nullify the content of a datastructure of type mep_type.
!!
!! INPUTS
!!
!! OUTPUT
...
subroutine mep_nullify(mep_param)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'mep_nullify'
!End of the abilint section
...
!************************************************************************
 ...
 nullify(mep_param%bfgs_xprev)
 nullify(mep_param%bfgs_fprev)
 nullify(mep_param%gbfgs_hess)
 nullify(mep_param%lbfgs_hess)
 nullify(mep_param%qmin_vel)
 nullify(mep_param%rk4_xcart1)
 nullify(mep_param%rk4_fcart1)
 nullify(mep_param%rk4_fcart2)
 nullify(mep_param%rk4_fcart3)

end subroutine mep_nullify
!!***
