...
!!
!! COPYRIGHT
!! Copyright (C) 2004-2014 ABINIT group (CS,GZ,FB,MG)
...
&         processor%comm_rows_elpa, processor%comm_cols_elpa)
  ...
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (call to solve_evp)!')
  end if
#endif
...
!End of the abilint section
  ...
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (call to choleski)!')
  end if
  call invert_trm_complex(na, b, na_rows, nblk, mpi_comm_rows, mpi_comm_cols, elpa_success)
  if (.not.elpa_success) then
    MSG_ERROR('Problem in ELPA use (call to invert_trm)!')
  end if
#else
...
& w,Slk_vec%buffer_cplx,1,1,Slk_vec%descript%tab,work,lwork,rwork,lrwork,info)
 ...
 ABI_MALLOC(rwork,(lrwork))
 ABI_CHECK_ALLOC("out-of-memoru work-rwork")
...
&   " scaLAPACK matrix cannot be read with a single MPI-IO call."
 ...
 if (nelw==buffer_size) then ! Dump Slk_mat% immediately.
   ...
   ABI_CHECK_ALLOC("out-of-memory buffer1_cplx")
   do elw=1,nelw
     iloc = elw2slk(1,elw)
     jloc = elw2slk(2,elw)
...
!@matrix_scalapack
 ...
 do sweep=1,2
   if (sweep==2) then
      ...
      ABI_CHECK_ALLOC("out of memory in myel2loc")
   end if
   my_nel=0
...
!@matrix_scalapack
 ...
 ABI_CHECK_ALLOC("out-of-memory elw2slk")
 elw2slk=0
!
!Cannot use MPI_type_CREATE_INDEXED_BLOCK since it is not correctly implemented in several MPI libraries.
!etype has to be set to MPI_BYTE, since the displacement in MPI structures is always in byte.
...
!@matrix_scalapack
 ...
 case ("C") 
   ...
   ABI_CHECK_ALLOC("out of memory in seen")
   nseen=0
   !
