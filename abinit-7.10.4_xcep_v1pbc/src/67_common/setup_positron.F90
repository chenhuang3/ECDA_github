...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (GJ,MT)
...
!!      initro,ioarr,pawcprj_alloc,pawcprj_copy,pawcprj_destroy,pawmknhat
!!      pawrhoij_alloc,pawrhoij_copy,pawrhoij_destroy,wrtout
!!
!! SOURCE
...
&          xccc3d,xred,ylm,ylmgr)
 ...
 use m_pawrhoij, only : pawrhoij_type, pawrhoij_copy, pawrhoij_destroy, pawrhoij_alloc
...
!Compatibility tests
 if (dtset%positron==0) then
   message = ' Not valid for dtset%positron=0 !'
   MSG_BUG(message)
 end if
...
&       forold_dum,fred_tmp,gresid_dum,grewtn,grhf_dum,grvdw,grxc_dum,gsqcut,indsym,&
&       kg,kxc,maxfor_dum,mcg,mgfft,mpi_enreg,my_natom,n3xccc0,nattyp,nfft,ngfft,ngrvdw,nhat,nkxc,npwarr,&
&       dtset%ntypat,nvresid,occ,optfor,optres,paw_ij,pawang,pawfgr,&
&       pawfgrtab,pawrhoij,pawtab,ph1dc,ph1d,psps,rhog,rhor,rprimd,optstr,&
&       strsxc,str_tmp,symrec,synlgr_dum,ucvol,&
&       vhartr,vpsp,vxc,wvl,wvl_den,xccc3d,xred,ylm,ylmgr,0.0_dp)
...
&       hdr,mpi_enreg,nfft,electronpositron%pawrhoij_ep,rdwr,rdwrpaw,wvl_den)
...
&           mpi_comm_atom=mpi_enreg%comm_atom,mpi_atmtab=mpi_enreg%my_atmtab)
           ...
           call pawrhoij_destroy(pawrhoij_tmp)
...
&           mpi_comm_atom=mpi_enreg%comm_atom,mpi_atmtab=mpi_enreg%my_atmtab,&
&           mpi_comm_fft=mpi_enreg%comm_fft,paral_kgb=dtset%paral_kgb,&
