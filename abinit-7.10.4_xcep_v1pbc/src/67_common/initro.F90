...
&     ' initro : indeed, coreel=',znucl(itypat)-zion(itypat),', nval=',int(zion(itypat)),&
...
!    Set contribution of unbalanced components to zero
     if (izero==1) then
       call zerosym(rhog,2,n1,n2,n3,mpi_comm_fft=mpi_enreg%comm_fft,distribfft=mpi_enreg%distribfft)
     end if
