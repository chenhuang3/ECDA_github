...
!!
!! COPYRIGHT
!!  Copyright (C) 2012-2014 ABINIT group (TRangel)
...
!!
!! CHILDREN
!!      ind_positions_,mkcore_inner,mkgrid_fft,pawrad_destroy,pawrad_init
...
& ucvol,vxc,xccc3d,xred)
 ...
 use m_pawrad,   only : pawrad_type, pawrad_init, pawrad_destroy
 use m_pawtab,   only : pawtab_type
 use m_xmpi,     only : xcomm_size,xmpi_sum,xcomm_rank
...
!mpi
 nproc =xcomm_size(mpi_enreg%comm_fft); nproc_fft= ngfft(10)
 me    =xcomm_rank(mpi_enreg%comm_fft);    me_fft= ngfft(11)
