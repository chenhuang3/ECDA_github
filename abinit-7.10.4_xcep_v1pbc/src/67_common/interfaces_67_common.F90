...
interface
 subroutine calc_efg(gprimd,my_natom,natom,nfft,ngfft,nspden,nsym,ntypat,paral_kgb,&  
...
interface
 subroutine cgq_builder(berryflag,cg,cgq,dtbfield,dtefield,dtset,ikpt,ikpt_loc,isppol,mcg,mcgq,&  
...
interface
 subroutine forstr(atindx1,cg,diffor,dtefield,dtset,eigen,electronpositron,energies,favg,fcart,&  
  ...
  &  kg,kxc,maxfor,mcg,mgfftf,mpi_enreg,my_natom,n3xccc,nattyp,&  
  ...
  &  strsxc,strten,symrec,synlgr,ucvol,vhartr,vpsp,&  
...
interface
 subroutine init_eb_field_vars(dtbfield,dtefield,dtset,gmet,gprimd,kg,&  
  ...
  integer,intent(in) :: symrec(3,3,dtset%nsym)
  real(dp),intent(in) :: xred(3,dtset%natom)
 end subroutine init_eb_field_vars
end interface
...
interface
 subroutine mag_out(dtbfield,mpi_enreg,nkpt,rprimd,wtk)
  use defs_basis
  use defs_abitypes
  use m_bfield
  implicit none
...
interface
 subroutine make_grad_berry(cg,cgq,cprj_k,detovc,dimffnl,dimlmn,dimlmn_srt,direc,dtefield,ffnl,grad_berry,&  
  &  gs_hamk,iband,icg,ikpt,isppol,kg_k,mband,mcg,mcgq,mkgq,mpi_enreg,mpw,natom,nkpt,&  
  &  nloalg,npw,npwarr,nspinor,nsppol,pwind,ph3d,pwind_alloc,pwnsfac,pwnsfacq)
  use defs_basis
  use defs_abitypes
  use m_efield
  use m_pawcprj
...
interface
 subroutine mkrho(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,paw_dmft,phnons,&  
  &  rhog,rhor,rprimd,tim_mkrho,ucvol,wfs,wvl_den,&  
  &  option) !optional
  use defs_basis
  use defs_abitypes
  use defs_wvltypes
...
interface
 subroutine newkpt(ceksp2,cg,debug,ecut1,ecut2,ecut2_eff,eigen,exchn2n3d,fill,&  
  ...
  &  mpi_enreg1,mpi_enreg2,mpw1,mpw2,nband1,nband2,&  
...
interface
 subroutine odamix(deltae,dtset,elast,energies,etotal,&  
  ...
  &  red_ptot,psps,rhog,rhor,rprimd,strsxc,taug,taur,ucvol,usepaw,&  !!HONG
...
interface
 subroutine posdoppler(cg,cprj,Crystal,dimcprj,dtfil,dtset,electronpositron,&  
  &  filpsp,gammastate_core,kg,mcg,mcprj,mpi_enreg,my_natom,&  
  ...
  type(dataset_type),intent(in) :: dtset
  type(electronpositron_type),pointer :: electronpositron
  real(dp),intent(in) :: gammastate_core
  type(mpi_type),intent(inout) :: mpi_enreg
  type(pawang_type),intent(in) :: pawang
  integer,intent(in) :: ngfft(18)
...
interface
 subroutine poslifetime(dtset,electronpositron,gammastate_core,gprimd,my_natom,mpi_enreg,n3xccc,nfft,ngfft,nhat,&  
  ...
  type(dataset_type), intent(in) :: dtset
  type(electronpositron_type),pointer :: electronpositron
  real(dp),intent(out) :: gammastate_core
  type(mpi_type),intent(inout) :: mpi_enreg
  type(pawang_type), intent(in) :: pawang
...
interface
 subroutine store_bfield_cprj(atindx1,cprj,dtbfield,ikpt,isppol_index,&  
  ...
  implicit none
  integer, intent(in) :: ikpt
  integer, intent(in) :: isppol_index
  integer, intent(in) :: mband
  integer, intent(in) :: mcprj
  integer, intent(in) :: mkmem
  integer, intent(in) :: natom
...
interface
 subroutine stress(atindx1,berryopt,dtefield,eei,efield,ehart,eii,gsqcut,ixc,kinstr,&  
  ...
  &  nfft,ngfft,nlstr,nspden,nsym,ntypat,paral_kgb,pawtab,ph1d,&  
  &  prtvol,qgrid,red_efieldbar,rhog,rprimd,strten,strsxc,symrec,&  
  &  typat,usepaw,vdw_tol,vdw_xc,&  
...
interface
 subroutine update_eb_field_vars(atindx,atindx1,cg,dimcprj,dtbfield,dtefield,dtfil,dtset,&  
  ...
  real(dp), intent(in) :: ylm(dtset%mpw*dtset%mkmem,psps%mpsang*psps%mpsang*psps%useylm)
  real(dp), intent(in) :: ylmgr(dtset%mpw*dtset%mkmem,3,psps%mpsang*psps%mpsang*psps%useylm)
 end subroutine update_eb_field_vars
end interface
...
interface
 subroutine update_orbmag(cg,cgq,cprj,dimffnl,dtbfield,ffnl,filstat,gs_hamk,&  
  ...
  integer,intent(in) :: kg(3,mpw*mkmem)
  real(dp),intent(inout) :: kinpw(npw_k)
  integer,intent(in) :: npwarr(nkpt)
  type(pawtab_type),intent(in) :: pawtab(ntypat)
