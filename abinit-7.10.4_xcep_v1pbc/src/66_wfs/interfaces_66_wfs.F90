...
interface
 subroutine getghc(cpopt,cwavef,cwaveprj,dimffnl,ffnl,filstat,ghc,gsc,gs_ham,&  
  ...
  real(dp),intent(in) :: ffnl(npw,dimffnl,gs_ham%lmnmax,gs_ham%ntypat)
  ...
  real(dp),intent(in) :: kinpw(npw)
  real(dp),intent(inout) :: ph3d(2,npw,gs_ham%matblk)
  real(dp),intent(inout) :: vlocal(gs_ham%n4,gs_ham%n5,gs_ham%n6,gs_ham%nvloc)
...
interface
 subroutine getgsc(cg,cprj,dimcprj,dimffnl,ffnl,gs_ham,gsc,ibg,icg,igsc,ikpt,isppol,&  
  ...
  real(dp),intent(in) :: ffnl(npw_k,dimffnl,gs_ham%lmnmax,gs_ham%ntypat)
