...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2014 ABINIT group (SM,VR,FJ,MT)
...
!!
!! CHILDREN
!!      hdr_io,int_ang,nderiv_gen,pawcprj_alloc,pawcprj_destroy
!!      pawcprj_diskinit_r,pawcprj_get,pawcprj_mpi_allgather,pawrad_deducer0
!!      simp_gen,timab,wffclose,wffopen,xmpi_exch,xmpi_sum,xmpi_sum_master
!!
!! SOURCE
...
&                      pawcprj_destroy,pawcprj_mpi_allgather
...
!Local variables-------------------------------
!scalars
 integer :: accesswff,basis_size,bdtot_index,cplex,etiq,fformopt,iatom,ib,ibg,ibsp
...
!PAW file
 iorder_cprj=0
 call pawcprj_diskinit_r(atindx1,natom,iorder_cprj,mkmem,natom,0,dimcprj,my_nspinor,dtfil%unpaw)
...
!        In that case parallelisation on nspinor not implemented yet;put this line just in case
       ...
       if (mkmem*nsppol/=1) then
         call pawcprj_destroy(cprj_k_loc)
