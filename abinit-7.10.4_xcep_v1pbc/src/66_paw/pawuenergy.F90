...
!           2: E_int=-U/2.(Nup.(Nup-1)+Ndn.(Ndn-1))
 ...
 real(dp) :: eks_opt3,edcdc_opt3,edcdctemp,edctemp,eldautemp,jpawu,mnorm,mx,my,mz
...
!Full localized limit
 if(pawtab%usepawu==1.or.dmftdc==1) then
   ...
   if (paw_ij%ndij/=4.or.option_interaction==2) then
     edcdctemp=edcdctemp+half*jpawu*(n_upup**2+n_dndn**2)
     edctemp  =edctemp  -half*jpawu*(n_upup*(n_upup-one)+n_dndn*(n_dndn-one))
   else if (paw_ij%ndij==4.and.option_interaction==1) then
!    write(message,'(a)') "  warning: option_interaction==1 for test         "
!    call wrtout(std_out,message,'COLL')
     edcdctemp=edcdctemp+quarter*jpawu*n_tot**2
     edctemp  =edctemp  -quarter*jpawu*(n_tot*(n_tot-two))
