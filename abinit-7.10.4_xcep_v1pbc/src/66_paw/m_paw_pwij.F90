...
!!
!! COPYRIGHT
!! Copyright (C) 2008-2014 ABINIT group (MG,GKA)
...
MODULE m_paw_pwij
 ...
 use m_paw_numeric,    only : jbessel_4spline, paw_spline
 use m_splines,        only : splfit
 use m_pawang,         only : pawang_type, realgaunt
 use m_pawrad,         only : pawrad_type, pawrad_init, pawrad_destroy, pawrad_copy, simp_gen
...
!scalars
 ...
 integer :: dummy_nsppol,lmn_size,lmn2_size,nq_spl
 real(dp) :: ucvol
...
! *********************************************************************
 ...
 do itypat=1,Psps%ntypat
   ...
   ABI_MALLOC(Pwij(itypat)%mqpgij,(2,npw,lmn2_size))
   ABI_CHECK_ALLOC('Out of memory in %mqpgij')
...
!*************************************************************************
 ...
 CASE (PWIJ_ARNAUD)
   ...
   meshsz = Pawrad(itypat)%mesh_size ; mmax=meshsz
   ...
   do jln=1,nln
     ...
     do iln=1,jln
       ...
       rrdphi_ij = rr*dphi_ij ! will be used for first derivative
       ...
       if (ir/=mmax) then  ! mmax=meshsz
         mmax=ir
         call pawrad_init(Tmpmesh,mesh_size=Pawrad(itypat)%mesh_size,mesh_type=Pawrad(itypat)%mesh_type, &
