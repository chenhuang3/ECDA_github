...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (MG, FJ, MT)
...
!!
!! PARENTS
!!      calc_sigx_me,prep_calc_ucrpa
!!
!! CHILDREN
!!      destroy_distribfft,fourdp,free_my_atmtab,get_my_atmtab
!!      init_distribfft_seq,initmpi_seq,pawgylm,set_mpi_enreg_fft,timab
!!      unset_mpi_enreg_fft,xmpi_sum,zerosym
!!
!! SOURCE
...
!scalars
 ...
 integer,optional,intent(in) :: me_g0,mpi_comm_fft,paral_kgb
...
!scalars
 ...
 integer :: lmax,lmin,lm_size,mm,my_comm_atom,my_comm_fft,optgr0,optgr1,paral_kgb_fft,cplex
 integer :: ilmn,jlmn,lmn_size,lmn2_size
 real(dp) :: re_p,im_p
 logical :: compute_grad,compute_nhat,my_atmtab_allocated,paral_atom
...
!arrays
 ...
 real(dp) :: rdum(1),cpf(2),cpf_ql(2),tsec(2)
...
!  Do FFT
   ...
   do isp1=1,MIN(2,nspinor**2)
     ...
     call zerosym(work,cplex,ngfft(1),ngfft(2),ngfft(3),mpi_comm_fft=my_comm_fft,distribfft=my_distribfft)
     call fourdp(cplex,work,nhat12(:,:,isp1),+1,mpi_enreg_fft,nfft,ngfft,paral_kgb_fft,0)
   end do
   ABI_DEALLOCATE(work)
