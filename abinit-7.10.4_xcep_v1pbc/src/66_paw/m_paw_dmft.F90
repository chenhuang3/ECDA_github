...
! Initialise spaceComm, myproc, and nproc
 ...
 if(nproc>2.and.use_dmft/=0) then
         !  Do not comment these lines: it guarantees will the automatic tests
         !  paral/tAI that the parallelism in DMFT/HI or QMC will work.
...
&   ch10," ******************************************"
 ...
 if(use_dmft>=1) then
   if(dmft_solv==0) then
     ...
     write(message, '(a,a)') ch10,' DMFT uses the Continuous Time Quantum Monte Carlo solver'
  endif
  call wrtout(std_out,message,'COLL') 
  call wrtout(ab_out,message,'COLL') 
 endif
...
!=========================================================
   write(message, '(16x,2(2x,a))') "  Use of linear frequencies for DMFT calculation"
   call wrtout(std_out,message,'COLL')
...
! *********************************************************************
 ...
 open(unit=unitsaveocc,file=trim(tmpfil),status='unknown',form='formatted')
 rewind(unitsaveocc)
