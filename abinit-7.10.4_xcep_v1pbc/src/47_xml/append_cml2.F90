...
!!
!! CHILDREN
!!      atomdata_from_symbol,findmarkup,getattribute,wrtout
!!
!! SOURCE
...
!  inside the first 'molecule' block.
   ...
   index_upper=indices_molecule(3)
   markuplen=8
   call findmarkup(builtin,index_lower,index_upper,indices_symmetry,'symmetry',markuplen,strln,string_cml)
...
&       'atom',markuplen,strln,string_cml)
       ...
       if(hasattrib_fractional)then
         ...
         message = 'Atomic positions are given in fractional coordinates in CML file'
         MSG_COMMENT(message)
         do iatom=1,natom
           call findmarkup(builtin,index_lower,index_upper,indices_markup,'atom',markuplen,strln,string_cml)

           call getattribute('elementType',11,hasattrib,indices_markup,strln,string_cml,string2)
           if(hasattrib)then
             read(string2,*)elementtype(iatom)
           else
             write(message,'(a,i4,4a)')&
...
&             '        it is likely that ABINIT is not yet able to read it.'
         ...
         message = 'Atomic positions are given in cartesian coordinates in CML file'
         MSG_COMMENT(message)
         do iatom=1,natom
           call findmarkup(builtin,index_lower,index_upper,indices_markup,'atom',markuplen,strln,string_cml)
