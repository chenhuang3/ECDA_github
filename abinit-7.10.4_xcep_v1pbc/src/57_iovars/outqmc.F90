...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, JYR, MKV, MT, FJ)
...
&     ' Parallelization over k-points is not currently supported'
 ...
 do io=10,99
   ...
   if(ierr/=0.or.io>99)then
     message = ' outqmc : ERROR - problem finding free unit number.'
     MSG_ERROR(message)
   end if
...
!Open CASINO pwfn.data file
 ...
 if(ierr/=0)then
   message = "ERROR - Unable to open the pwfn.data file"
   MSG_ERROR(message)
 end if
...
!Write the required quantities to pwfn.data
 ...
 select case(dtset%nspden)
   ...
   case default
     close(io,status='delete')
     message = ' outqmc : ERROR - Unrecognized value of nspden.'
     MSG_ERROR(message)
 end select
