...
interface
 subroutine inkpts(bravais,chksymbreak,iout,iscf,istwfk,jdtset,&  
  &  kpt,kptopt,kptnrm,kptrlatt,kptrlen,lenstr,msym,&  
  ...
  real(dp),intent(out) :: shiftk_orig(3,8)
...
interface
 subroutine invars1m(dmatpuflag,dtsets,iout,lenstr,mband_upper_,&  
  ...
  &  ndtset,ndtset_alloc,string,npsp,zionpsp)
  use defs_basis
  use defs_abitypes
  implicit none
...
interface
 subroutine memory(n1xccc,getcell,idtset,icoulomb,intxc,ionmov,iout,iprcch,iprcel,&  
  ...
  &  prtvol,pspheads,typat,ucvol,usepaw,useylm,use_gpu_cuda,xclevel)
  use defs_basis
  use defs_abitypes
  use defs_datatypes
  implicit none
