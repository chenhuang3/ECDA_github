...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (XG, RC)
...
!!      gensymspgr,ingeo_img,ingeobld,intagm,mati3inv,metric,mkrdim
!!      randomcellpos,symanal,symatm,symfind,symlatt,symrelrot,symzat,wrtout
...
& msym,natom,nimage,npsp,npspalch,nspden,nsppol,nsym,ntypalch,ntypat,pawspnorb,&
...
!scalars
 integer :: bckbrvltt,brvltt,chkprim,iatom,idir,ii,ipsp,irreducible,isym,itypat
...
!  Alternate SIESTA input variable for acell
   call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'LatticeConstant',tread,'LEN')
   if(tread==1)then
     if(tacell==1)then
...
!  Might initialize scalecart from CML file
   if(tacell==0)then
     call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'_acell',tacell,'LEN')
     if(tacell==1) acell(1:3)=dprarr(1:3)
   end if
 end if
...
!  Might initialize angdeg from CML file
   if(tangdeg==0 .and. nimage==1)then
     call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),'_angdeg',tangdeg,'DPR')
     angdeg(:)=dprarr(1:3)
   end if
   if(tangdeg==1)then
...
!Might initialize nsym from CML file
 if(tread==0)then
   call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'_nsym',tread,'INT')
 end if
...
!  Might initialize symrel from CML file
   if(tread==0)then
     call intagm(dprarr,intarr,jdtset,marr,9*nsym,string(1:lenstr),'_symrel',tread,'INT')
   end if
...
!  Might initialize tnons from CML file
   if(tread==0)then
     call intagm(dprarr,intarr,jdtset,marr,3*nsym,string(1:lenstr),'_tnons',tread,'DPR')
   end if
...
!  Might initialize spgroup from CML file
   if(tread==0)then
     call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'_spgroup',tread,'INT')
     if(tread==1) spgroup=intarr(1)
   end if
...
!    Check whether the symmetry operations are consistent with the lattice vectors
     call chkorthsy(gprimd,nsym,rmet,rprimd,symrel)
