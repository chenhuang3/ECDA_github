...
!!
!! COPYRIGHT
!! Copyright (C) 2009-2014 ABINIT group (M.Giantomassi, L. Reining, V. Olevano, F. Sottile, S. Albrecht, G. Onida)
...
!!
!! CHILDREN
!!      bs_parameters_free,chkpawovlp,crystal_free,denfgr,destroy_hur
!!      destroy_mpi_enreg,destroy_paw_pwff,double_grid_free,ebands_free
!!      energies_init,exc_build_ham,exc_den,exc_diago_driver,exc_haydock_driver
!!      fourdp,get_gftt,getph,gsph_free,hdr_free,init_distribfft_seq
!!      init_paw_pwff,initmpi_seq,kmesh_free,make_hur_commutator,metric
!!      mkdenpos,mkrdim,nhatgrid,paw_an_destroy,paw_an_init,paw_an_nullify
!!      paw_gencond,paw_ij_destroy,paw_ij_init,paw_ij_nullify,pawdenpot,pawdij
!!      pawfgr_destroy,pawfgr_init,pawfgrtab_destroy,pawfgrtab_init,pawinit
!!      pawmknhat,pawnabla_init,pawprt,pawpuxinit,pawrhoij_alloc,pawrhoij_copy
!!      pawrhoij_destroy,pawtab_print,print_ngfft,prtrhomxmn,pspini,psps_print
!!      rdqps,rotate_fft_mesh,screen_free,screen_init,screen_nullify
!!      setsymrhoij,setup_bse,setup_bse_interp,setvtr,symdij,test_charge,timab
!!      update_occ,vcoul_free,wfd_free,wfd_init,wfd_mkrho,wfd_print
...
subroutine bethe_salpeter(acell,codvsn,Dtfil,Dtset,Pawang,Pawrad,Pawtab,Psps,rprim,xred)
 ...
 use m_ebands,          only : update_occ, ebands_free, ebands_copy, get_valence_idx
 ...
 use m_wfs,             only : wfd_init, wfd_free, wfd_print, wfd_t, wfd_test_ortho,&
&                              wfd_read_wfk, wfd_wave_free, wfd_rotate, wfd_reset_ur_cprj
 ...
 use m_pawfgrtab,       only : pawfgrtab_type, pawfgrtab_destroy, pawfgrtab_init
 use m_pawrhoij,        only : pawrhoij_type, pawrhoij_alloc, pawrhoij_copy,&
&                              pawrhoij_destroy, symrhoij
 use m_pawdij,          only : pawdij, symdij
 use m_pawfgr,          only : pawfgr_type, pawfgr_init, pawfgr_destroy
 use m_paw_commutator,  only : HUr_commutator, destroy_Hur, make_Hur_commutator
...
!scalars
 ...
 integer :: ii,band,spin,ik_ibz,mqmem,iwarn
 integer :: has_dijU,has_dijso,gnt_option,iomode
 integer :: ik_bz,mband
 integer :: accessfil,choice
...
& Gsph_x%gvec,Dtset%nloalg,Dtset%prtvol,Dtset%pawprtvol,comm,opt_ecut=opt_ecut)
 ...
 if (.FALSE.) then ! This test has been disabled (too expensive!)
...
&   Gsph_x_dense%gvec,Dtset%nloalg,Dtset%prtvol,Dtset%pawprtvol,comm,opt_ecut=opt_ecut)
   ...
   call wfd_read_wfk(Wfd_dense,wfk_fname_dense,iomode)
   
   if (.FALSE.) then ! This test has been disabled (too expensive!)
     call wfd_test_ortho(Wfd_dense,Cryst,Pawtab,unit=std_out,mode_paral="COLL")
   end if
 end if
...
&   nfftf,ngfftf,Cryst%ucvol,Wfd%paral_kgb,Cryst,Pawtab,MPI_enreg_seq,nbsc,m_lda_to_qp,prev_rhor,prev_Pawrhoij)
 ...
 ! Hereafter bands from 1 up to lomo-1 and all bands abolve humo+1 should not be accessed!
...
& Wfd,W,Hdr_bse,nfftot_osc,ngfft_osc,Psps,Pawtab,Pawang,Paw_pwff)
 ...
 if (Bsp%inclvkb/=0 .and. Wfd%usepaw==1 .and. Dtset%usepawu/=0) then !TODO here I need KS_Paw_ij
   ...
   call make_Hur_commutator(Wfd%nsppol,Wfd%pawprtvol,Cryst,Pawtab,Pawang,Pawrad,KS_Paw_ij,Hur)
 end if
