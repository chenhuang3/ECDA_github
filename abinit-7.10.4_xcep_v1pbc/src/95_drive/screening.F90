...
!!
!! COPYRIGHT
!! Copyright (C) 2001-2014 ABINIT group (GMR, VO, LR, RWG, MT, MG, RShaltaf)
...
&                            gw_uses_wfk_file, use_wfk_file
 use m_header,        only : hdr_free
 use m_mpinfo,        only : destroy_mpi_enreg
 use m_crystal,       only : crystal_free, crystal_t
 use m_psps,          only : psps_print
 use m_ebands,        only : update_occ, ebands_copy, get_valence_idx, get_occupied, apply_scissor, &
&                            ebands_free, bst_metallic_scheme
 ...
 use m_screening,     only : make_epsm1_driver, mkem1_q0
 ...
 use m_wfs,           only : wfd_init, wfd_free,  wfd_nullify, wfd_print, wfd_t, wfd_rotate, wfd_test_ortho,&
...
#define ABI_FUNC 'screening'
 ...
 use interfaces_95_drive, except_this_one => screening
...
!scalars
 ...
 integer :: choice,cplex,dim_kxcg,dim_wing,ount
 integer :: fileID,fform_chi0,fform_em1,gnt_option,iat,ider,idir,ierr,band
 ...
 real(dp) :: compch_fft,compch_sph,domegareal,e0,ecore,ecut_eff,ecutdg_eff,nelect
 real(dp) :: gsqcutc_eff,gsqcutf_eff,gsqcut_shp,omegaplasma,ucvol,vxcavg,gw_gsq,length,r_s
...
!arrays
 ...
 integer,allocatable :: ktabr(:,:),ktabrf(:,:),l_size_atm(:)
 integer,allocatable :: ks_vbik(:,:),ks_occ_idx(:,:),qp_vbik(:,:),nband(:,:)
 integer,allocatable :: nq_spl(:),nlmn_atm(:)
 ...
 real(dp),allocatable :: ec_rpa(:),rspower(:)
 ...
 real(dp),allocatable :: grewtn(:,:),grvdw(:,:),kxc(:,:),qmax(:)
 ...
 character(len=fnlen) :: gw_fname,wfk_fname
...
&   'P Number of bands treated by each node ~',nbcw/nprocs
   if (nprocs>1.or.Ep%awtr==1.or.Dtset%spmeth>0) then
     call wrtout(ab_out,msg,'COLL')
   end if
...
!  Calculate new occ. factors and fermi level.
   call update_occ(QP_BSt,Dtset%spinmagntarget)
...
&   Cryst%natom,Cryst%natom,Dtset%nspden,Cryst%ntypat,nzlmopt,option,Paw_an,Paw_an,&
...
& ks_vhartr,vpsp,ks_vtrial,ks_vxc,vxcavg,wvl,xccc3d,Cryst%xred,taug=taug,taur=taur)
...
!* Allocate chi0, wings and array for chi0_sumrule check.
 ...
 ABI_MALLOC(chi0,(Ep%npwe*Ep%nI,Ep%npwe*Ep%nJ,Ep%nomega))
 ABI_CHECK_ALLOC("Out of memory in chi0")
 if (Dtset%chkgwcomp==1)  then
   ABI_MALLOC(deltaI,(Ep%npwe*Ep%nI,Ep%npwe*Ep%nJ,1))
 end if
!
...
&   ' q-point number ',label,'        q = (',(Qmesh%ibz(ii,iqibz),ii=1,3),') [r.l.u.]',ch10,bar
   ...
   if (is_qeq0==1) then  ! Special treatment of the long wavelength limit.
...
!    add_chi0_intraband=.TRUE.
     ...
     if (add_chi0_intraband .and. bst_metallic_scheme(QP_BSt)) then

       ABI_MALLOC(chi0intra,(Ep%npwe*Ep%nI,Ep%npwe*Ep%nJ,Ep%nomega))
       ABI_CHECK_ALLOC("Out of memory in chi0intra")
...
&       ngfft_gw,chi0intra,chi0intra_head,chi0intra_lwing,chi0intra_uwing)
     ...
     if (.FALSE.) then
       ...
       ABI_MALLOC(eps_head,(3,3,Ep%nomega))
...
!    * Open file and write header for deltaI files.
     if (is_first_qcalc) then
       if (open_file(dtfil%fnameabo_delI,msg,unit=unt_delI,status='unknown',form='formatted') /= 0) then
         MSG_ERROR(msg)
       end if
...
!  If the vertex is being included for the spectrum, calculate the kernel now and pass it on
   if (Dtset%gwgamma==0) then
     ...
     ABI_MALLOC(kxcg,(Ep%npwe,dim_kxcg))
...
!    r_s is Wigner-Seitz radius and alpha=(4/(9*Pi))^(1/3)
     ...
     call kxc_ADA(Dtset,Cryst,ikxc,ngfftf,nfftf,Wfd%nspden,&
...
!    This might be parallelized but I have to use xmpi_sum in cchi0 and cchi0q0
