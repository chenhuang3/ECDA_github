...
interface
 subroutine dtfil_init_time(dtfil,iapp,mpi_enreg)
  use defs_abitypes
  implicit none
...
interface
 subroutine gstate(acell,amu,codvsn,cpui,dtfil,dtset,iexit,initialized,&  
  ...
  real(dp),intent(inout) :: amu(psps%ntypat)
...
interface
 subroutine loper3(atindx,atindx1,blkflg,codvsn,cpus,dimcprj,dim_eigbrd,dim_eig2nkq,doccde,&  
  ...
  type(hdr_type),intent(out),optional :: hdr0
  type(hdr_type),intent(out),optional :: hdr_fine
  type(mpi_type), intent(inout) :: mpi_enreg
  ...
  real(dp),intent(out),optional :: eigen0_pert(:)
  real(dp),pointer,optional :: eigen1_pert(:,:,:)
  real(dp),pointer,optional :: eigenq_fine(:,:,:)
  real(dp),intent(out),optional :: eigenq_pert(:)
  ...
  real(dp),intent(out),optional :: occ_rbz_pert(:)
