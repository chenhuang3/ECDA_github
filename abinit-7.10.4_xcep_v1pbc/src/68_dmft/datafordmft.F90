...
!scalars
 ...
 integer :: nbandi,nbandf,nnn,nprocband,nsploop,option,opt_renorm,spaceComm
...
!Prepare temporary PAW file if mkmem==0
 iorder_cprj=0
 call pawcprj_diskinit_r(cryst_struc%atindx1,dtset%natom,iorder_cprj,dtset%mkmem,dtset%natom,0,dimcprj,my_nspinor,unpaw)
...
!*****************   in forlb.eig
 if(me.eq.0.and.abs(dtset%pawprtvol)>=3) then
   ...
   write(2010,*) mband,nsppol,nkpt,my_nspinor,nbandi,nbandf
...
&   matlu_temp,lda_occup%matlu,natom,option,tol3,ierrr)
   if(ierrr==-1) then
...
!Local variables ------------------------------------
 ...
 integer :: iat,iatom,jj1,ilmn,m1,nband_k
 integer :: m1_t2g,ll
 real(dp) :: chinorm
...
!  Header for calc_uCRPA.F90
   if  (COUNT(pawtab(:)%lpawu.NE.-1).EQ.1) then
     do  itypat=1,ntypat
       if(t2g) then
         if(pawtab(itypat)%lpawu.ne.-1) write(2011,*) "l= ",ll,itypat
       else
         if(pawtab(itypat)%lpawu.ne.-1) write(2011,*) "l= ",pawtab(itypat)%lpawu,itypat
       end if
     end do
   else
     write(2011,*) "More than one correlated species"
   end if

   write(2011,*) "Bands ",paw_dmft%dmftbandi,paw_dmft%dmftbandf
