...
!! SOURCE
 ...
 type, public :: energy_type ! for each typat
  ...
  real(dp), pointer :: e_dc(:)  => null()! => vee

  real(dp), pointer :: e_hu(:) => null()

  real(dp), pointer :: e_hu_ldau(:) => null()

  real(dp), pointer :: e_hu_mig(:) => null()

  real(dp), pointer :: e_hu_qmc(:) => null()
...
!*********************************************************************
 ...
 nullify(energies_dmft%e_hu_qmc)
...
! *********************************************************************
 ...
 if ( associated(energies_dmft%e_hu_mig) )  then
  ...
  if ( associated(energies_dmft%e_hu_qmc) )  then
...
&       ", E_dc =",energies_dmft%e_dc(iatom)
 ...
 write(message,'(a,5x,2a,5x,a,8(a,5x,a,2x,f15.11),a,5x,a)') ch10 &
&      ,"-----------------------------------------------",ch10 &
...
& ln_matlu,add_matlu,zero_matlu,shift_matlu,nullify_matlu,copy_matlu,trace_matlu,print_matlu
...
&              paw_ij(iatom)%noccmmp(1,im1,im1,idijeff)
     ...
     call pawuenergy(iatom,eldaumdc,eldaumdcdc,3,pawtab(cryst_struc%typat(iatom)),paw_ij(iatom),&
