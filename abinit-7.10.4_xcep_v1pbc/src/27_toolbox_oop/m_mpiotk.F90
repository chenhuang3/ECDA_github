...
!!
!! FUNCTION
!!  This module provides helper functions for MPI-IO access. 
!!
!! COPYRIGHT
...
!arrays
 ...
 integer,pointer :: my_basead(:),my_subsizes(:,:),my_starts(:,:)
...
!************************************************************************
 ...
 if (sc_mode==xmpio_single) then
   call setup_fsuba_dp2D(sizes,subsizes,starts,chunk_bsize,my_basead,my_subsizes,my_starts,my_ncalls,ncalls,xmpi_self,ierr) 
 else if (sc_mode==xmpio_collective) then
   call setup_fsuba_dp2D(sizes,subsizes,starts,chunk_bsize,my_basead,my_subsizes,my_starts,my_ncalls,ncalls,comm,ierr) 
 else 
   MSG_ERROR("Wrong sc_mode")
...
!************************************************************************
 ...
 if (sc_mode==xmpio_single) then
  ABI_CHECK(comm==xmpi_self,"comm != xmpi_self")
 else if (sc_mode==xmpio_collective) then
   continue
 else 
   MSG_ERROR("Wrong sc_mode")
