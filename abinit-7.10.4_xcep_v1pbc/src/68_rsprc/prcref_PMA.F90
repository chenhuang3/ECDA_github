...
!!      prcrskerker2,prctfw3,rhohxc,testsusmat,xcart2xred,xmpi_sum,zerosym
!!
!! SOURCE
...
!arrays
 ...
 real(dp),allocatable :: dummy_in(:),dummy_out(:)
...
!  Not compatible with non-collinear magnetism
   if(dtset%nspden==4)then
     MSG_ERROR('iprcch=3 does not work for nspden=4!')
   end if
...
!    Note: atindx1 should be passed to atm2fft (instead of atindx) but it is unused...
     call atm2fft(atindx,xccc3d,vpsp,dummy_out,dummy_out,eei,dummy_out,dummy_in,gmet,&
&     gprimd,dummy_out,dummy_out,gsqcut,mgfft,psps%mqgrid_vl,dtset%natom,nattyp,&
&     nfft,ngfft,ntypat,optatm,optdyfr,opteltfr,optgr,optn,optn2,optstr,optv,&
&     pawtab,ph1d,psps%qgrid_vl,qprtrb,dummy_in,dummy_out,dummy_out,ucvol,&
