...
!!  mpi_comm_atom=the mpi communicator over atoms ; PAW only (optional argument)
...
!scalars
 ...
 integer :: i_vstore,ierr,ifft,ii,index,isp,jj,mpi_comm_atom_,niter,npulay,tmp
...
!ENDDEBUG
 ...
 mpi_comm_atom_=xmpi_self;if(present(mpi_comm_atom)) mpi_comm_atom_=mpi_comm_atom
...
!Compute the new residual resid_new, from f_fftgr/f_paw(:,:,i_vrespc(1))
 ...
 if (usepaw==1.and.pawoptmix==1) then
   ...
   call xmpi_sum(resid_new(1),mpi_comm_atom_,ierr)
 end if
 vresid = resid_new(1)
...
&   nfft,n_fftgr,n_fftgr,nspden,opt_denpot,f_fftgr,f_fftgr)
   if (usepaw==1.and.pawoptmix==1) then
     ...
     call xmpi_sum(prod_resid(1),mpi_comm_atom_,ierr)
   end if
...
!    MPI reduction
     ...
     call xmpi_sum(mpibuff,mpi_comm_atom_,ierr)
...
!  Compute "A" matrix
   ...
   if (usepaw==1.and.pawoptmix==1) then
     ...
     call xmpi_sum(amat_paw,mpi_comm_atom_,ierr)
   end if
   do ii=1,niter
