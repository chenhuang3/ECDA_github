...
& gwx_ngfft,gwc_ngfft,Hdr_wfk,Hdr_out,Cryst,Kmesh,Qmesh,KS_BSt,Gsph_Max,Gsph_x,Gsph_c,Vcp,Er,Sigp,comm)
 ...
 use m_gwdefs,        only : GW_Q0_DEFAULT, SIG_GW_AC, sigma_parameters, sigma_is_herm, sigma_needs_w,&
&                            gw_uses_wfk_file
...
&                            get_bz_item, has_IBZ_item, find_qmesh
 use m_ebands,        only : ebands_init, enclose_degbands, get_valence_idx, update_occ, ReportGap, &
...
!scalars
 ...
 integer :: mod10,mod100,mqmem,mpsang_kss,mband,ng_kss,nsym_kss,ikcalc2bz,ierr,gap_err,ng
 ...
 character(len=fnlen) :: fname,fcore,string,wfk_fname
...
!arrays
 ...
 real(dp),pointer :: energies_p(:,:,:),energies_kss(:,:,:)
...
&  gprimd,Dtset%prtvol,test_npwkss,test_gvec_kss,ierr)
 ...
 if (gw_uses_wfk_file) then
   ...
   call testkss(Dtfil%fnameabi_kss,Dtset%accesswff,nsym_kss,mband,&
&    ng_kss,mpsang_kss,gvec_kss,energies_kss,Hdr_kss,comm)

   ABI_FREE(energies_kss)
   call hdr_vs_dtset(Hdr_kss,Dtset)

   remove_inv=(nsym_kss/=Hdr_kss%nsym)
   call hdr_free(Hdr_kss)
 end if
...
!* spinmagntarget is passed to fermi.F90 to fix the problem with newocc in case of magnetic metals
 ...
 call ReportGap(KS_BSt,unit=std_out)
...
#if 0
     ! old version
     ! * If not specified, calculate all k-points in the IBZ and all bands.
   ...
   if(dtset%ucrpa>0) then
     ...
     do spin=1,Sigp%nsppol
       do ifo=1,3
         ...
         found = .FALSE.; jj = 0
         do while (.not. found .or. jj < nk_found) 
           jj = jj + 1
           found = (kpos(jj) == ik)
         end do
         if (.not. found) then
     ...
     do ii=1,Sigp%nkptgw
       ...
       do spin=1,Sigp%nsppol
         Sigp%minbnd(ik,spin) = val_indeces(ik,spin)
         Sigp%maxbnd(ik,spin) = val_indeces(ik,spin) + 1
       end do
     end do
#endif
   end if
...
!ENDDEBUG
 ...
 if (Gsph_x%ng > Gsph_c%ng) then
   call vcoul_init(Vcp,Gsph_x,Qmesh,Kmesh,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,&
&    Gsph_x%ng,nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 else
   call vcoul_init(Vcp,Gsph_c,Qmesh,Kmesh,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,&
&    Gsph_c%ng,nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
 end if
#else
   call vcoul_init(Vcp,Gsph_Max,Qmesh,Kmesh,Dtset%rcut,Dtset%icutcoul,Dtset%vcutgeo,&
&    Sigp%npwx,nqlwl,qlwl,Cryst%rprimd,ngfftf,comm)
#endif
