...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (GMR, VO, LR, RWG, MG, RShaltaf)
...
!!      destroy_hur,destroy_kb_potential,destroy_paw_pwij,flush_unit
!!      get_bz_item,get_gftt,get_uug,gsph_fft_tabs,gsph_free,gsph_in_fftbox
!!      gw_eet_chi0,hilbert_transform,hilbert_transform_headwings
!!      init_kb_potential,init_paw_pwij,littlegroup_print,make_hur_commutator
!!      make_transitions,paw_cross_ihr_comm,paw_cross_rho_tw_g,paw_rho_tw_g
!!      paw_symcprj,pawcprj_alloc,pawcprj_copy,pawcprj_destroy,print_gsphere
!!      read_plowannier,rho_tw_g,setup_spectral,symmetrize_afm_chi0
...
&  nfftot_gw,ngfftf,nfftf_tot,chi0,chi0_head,chi0_lwing,chi0_uwing,Ltg_q,chi0_sumrule,Wfd,Wfdf)
 ...
 use m_wfs,             only : wfd_get_ur, wfd_t, wfd_distribute_bbp, wfd_get_cprj, &
&                              wfd_barrier, wfd_change_ngfft,wfd_paw_get_aeur, wfd_sym_ur
 ...
 use m_paw_commutator,  only : HUr_commutator, destroy_Hur, make_Hur_commutator, paw_ihr_comm, paw_cross_ihr_comm
...
!scalars
 ...
 integer :: band1,band2,iat,idle,ig,ig1,ig2,itim_k,ik_bz,ik_ibz,io,iqlwl,isym_k
 ...
 integer :: comm,ierr,my_wl,my_wr,iomegal,iomegar,gw_mgfft,dummy,nbmax
...
& nsppol,pawang,dtset%prtvol,ucrpa_bands)
 endif
! End of reading forlb.ovlp
 ...
 if (Psps%usepaw==0) then
   ...
   if (Dtset%usepawu/=0) then
     call make_Hur_commutator(nsppol,Dtset%pawprtvol,Cryst,Pawtab,Pawang,Pawrad,Paw_ij,Hur)
   end if
 end if
 !
 ...
 if (Ep%gwcomp==1) then
   ...
   if (Psps%usepaw==1) then ! Prepare the onsite contributions on the GW FFT mesh.
     ...
     call init_paw_pwij(Pwij_fft,nfft,(/zero,zero,zero/),gw_gfft,Cryst%rprimd,Psps,Pawtab,Paw_pwff)
   end if
 end if
 !
...
&  ' Using symmetries to sum only over the IBZ_q  = ',Ep%symchi
 ...
 if (dtset%gw_eet/=-1) then
   ...
   got=0
   ABI_MALLOC(bbp_ks_distrb_eet,(nbvw,Kmesh%nbz,nsppol))
   do spin=1,nsppol
     do ik_bz=1,Kmesh%nbz
       if (Ep%symchi==1) then
         if (Ltg_q%ibzq(ik_bz)/=1) CYCLE  ! Only IBZ_q
       ...
       do band1=1,nbvw
         if (Wfd%nproc==1) then
           bbp_ks_distrb_eet(band1,ik_bz,spin)=0
         else
           idle = imin_loc(got)
           got(idle) = got(idle) + 1
           bbp_ks_distrb_eet(band1,ik_bz,spin)=idle-1
         end if
       end do ! band1
     enddo
   enddo
...
&     0,Ep%zcut,zero,my_wl,my_wr,kkweight)
   ...
   ABI_MALLOC(sf_chi0,(Ep%npwe,Ep%npwe,my_wl:my_wr))
   ABI_CHECK_ALLOC('out-of-memory in sf_chi0q0')
   sf_chi0=czero_gw
 ...
 do spin=1,nsppol
   ...
   if (ALL(bbp_ks_distrb(:,:,:,spin) /= Wfd%my_rank).and.dtset%gw_eet==-1) CYCLE
   ...
   do ik_bz=1,Kmesh%nbz ! Loop over k-points in the BZ.
     ...
     if (ALL(bbp_ks_distrb(:,:,ik_bz,spin) /= Wfd%my_rank).and.dtset%gw_eet==-1) CYCLE
...
&                       nfft,ngfft_gw,use_padfft,igffteps0,gw_gbound,gw_mgfft,spin, &
...
!       write(6,*) "ik,band1",ik_bz,band1
!       do band2=1,Ep%nbnds
!         write(6,*) band2,bbp_ks_distrb(band1,band2,ik_bz,spin)
!       enddo
       if (ALL(bbp_ks_distrb(band1,:,ik_bz,spin) /= Wfd%my_rank)) CYCLE

!       write(6,*) "ik,band1 (after cycle)",ik_bz,band1
...
&            Cprj1_bz,Cprj2_bz,Pwij,rhotwg)
           ...
           rhotwx = paw_ihr_comm(spin,nspinor,npw_k,istwf_k,Kmesh%ibz(:,ik_ibz),Cryst,Pawtab,ug1,ug2,kg_k,Cprj1_ibz,Cprj2_ibz,HUr)
...
!               if(dtset%prtvol>=10)write(6,*)"calculation is in progress",band1,band2,ucrpa_bands(1),ucrpa_bands(2)
                 do iat=1, cryst%nattyp(itypatcor)
                   do m1=1,2*lcor+1
                     fac1=fac1+ real(coeffW_BZ(iat,spin,band1,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band1,ik_bz,m1)))
                     fac2=fac2+ real(coeffW_BZ(iat,spin,band2,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band2,ik_bz,m1)))
                     do m2=1,2*lcor+1
                       fac=fac - real(coeffW_BZ(iat,spin,band1,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band1,ik_bz,m1))* &
&                                 coeffW_BZ(iat,spin,band2,ik_bz,m2)*conjg(coeffW_BZ(iat,spin,band2,ik_bz,m2)))
                       fac3=fac3 + real(coeffW_BZ(iat,spin,band1,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band1,ik_bz,m1))* &
&                                 coeffW_BZ(iat,spin,band2,ik_bz,m2)*conjg(coeffW_BZ(iat,spin,band2,ik_bz,m2)))
...
!                 write(6,'(a,6i4,e15.5,a)') "*****FAC*********",ik_bz,ik_bz,band1,band2,m1,m2,fac," q==0"
               ...
               if        (band1<=ucrpa_bands(2).AND.band1>=ucrpa_bands(1)) then
                 do iat=1, cryst%nattyp(itypatcor)
                   do m1=1,2*lcor+1
                     fac2=fac2-real(coeffW_BZ(iat,spin,band1,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band1,ik_bz,m1)))
                   enddo
                 enddo
               ...
               if        (band2<=ucrpa_bands(2).AND.band2>=ucrpa_bands(1)) then
                 do iat=1, cryst%nattyp(itypatcor)
                   do m1=1,2*lcor+1
                     fac3=fac3-real(coeffW_BZ(iat,spin,band2,ik_bz,m1)*conjg(coeffW_BZ(iat,spin,band2,ik_bz,m1)))
                   enddo
                 enddo
...
&            Ltg_q,Gsph_epsG0,Ep%npwepG0,rhotwg,chi0_sumrule)
 ...
 CASE (1,2) ! Spectral method.
  ...
  if (xcomm_rank(comm) == 0) then
    tmp_unt=get_unit()
    open(tmp_unt,file='Spectral_chi0_2-10.dat',status='replace')
    do ig1=2,MIN(3,Ep%npwe)
      do ig2=2,MIN(3,Ep%npwe)
        do io=my_wl,my_wr
          write(tmp_unt,'(3ES30.20E3)'),omegasf(io)*Ha_eV,REAL(sf_chi0(ig1,ig2,io)),AIMAG(sf_chi0(ig1,ig2,io))
        end do
        write(tmp_unt,'(a)')
        write(tmp_unt,'(a)')
      end do
...
!$OMP END PARALLEL WORKSHARE
 ...
 if (Psps%usepaw==1) then ! deallocation for PAW.
   call pawcprj_destroy(Cprj1_bz )
   ABI_DT_FREE(Cprj1_bz)
   call pawcprj_destroy(Cprj2_bz )
   ABI_DT_FREE(Cprj2_bz)
   call pawcprj_destroy(Cprj1_ibz )
   ABI_DT_FREE(Cprj1_ibz)
   call pawcprj_destroy(Cprj2_ibz )
