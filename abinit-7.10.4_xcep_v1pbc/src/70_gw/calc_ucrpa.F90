...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (TApplencourt,BA)
...
!Arguments ------------------------------------
 ...
 complex(dpc), intent(in) :: rhot1_q_m(cryst%nattyp(itypatcor),2*lpawu+1,2*lpawu+1,npw,Qmesh%nibz)
 complex(dpc), intent(in) :: M1_q_m(cryst%nattyp(itypatcor),2*lpawu+1,2*lpawu+1,npw,Qmesh%nibz)
...
!scalars
 ...
 integer :: ierr,ik_bz,ik_ibz,iq_ibz,i,iG1,iG2,iG,iiG,iomega,iomega1
 integer :: nkibz,nbband,nkbz,nprocs,nqalloc,nqibz,mbband
...
!  |_| |_|\___/|_|  |_| |_| |_|\___|       \_____|
 ...
 if (verbose) then
   ...
   call wrtout(std_out,message,'COLL')
   open(unit=2021,file='normeG',form='formatted',status='unknown')
...
!Read "l"
 ...
 rewind(2012)
 read(2012,*) message, mbband 
 read(2012,*) message, ib1, ib2
...
! end do
 ...
 if(real(M1_q_m(1,1,1,1,1))>0) then
 endif
 ...
 do iat=1,Cryst%nattyp(itypatcor)
   ...
   if (verbose) then 
     ...
     do iG=1,npw
       do m1=1,mbband
         do m2=1,mbband
           write(2111,*) normG(iG),m1,m2,rhot_q_m(m1,m2,iG,:)
         enddo
       enddo
     end do
       write(2111,*) q_coord
...
!  !==Check if calculation is correct
   ...
   call wrtout(std_out,message,'COLL')
   call checkk(V_m,1,mbband,tol,1,0,uu,jj,"bare interaction")
...
!  ==========================================================
 ...
 !  call wrtout(ab_out,message,'COLL'); call wrtout(std_out,message,'COLL')
   ...
   if(nsym>1.and..not.lscr_one) then
     write(message,*) "==Read the dielectric matrix=="
     call wrtout(ab_out,message,'COLL'); call wrtout(std_out,message,'COLL')
     call read_screening(fname,npwe,nqibz,nomega,scr,IO_MODE_MPI,Wfd%comm)
   endif
...
!  
   ...
   ABI_ALLOCATE(rhot_q_m_npwe,(mbband,mbband,npwe,nqibz))
   ...
   do iG1=1,npwe
     ...
     do m1=1,mbband
       trrho(iG1,:)= trrho(iG1,:)+rhot_q_m(m1,m1,iG1,:)
     enddo
   enddo
!   write(6,*) "trrho"
!   do iG1=1,npwe
      ...
      ! call wrtout(std_out,message,'COLL')
       ...
       do iq_ibz=1,nqibz
         ...
         if(nsym==1.or.lscr_one) then
           ...
           call read_screening(fname,npwe,1,nomega,scr,IO_MODE_MPI,Wfd%comm,iqiA=iq_ibz)
#else
           call read_screening(fname,npwe,1,nomega,scr,IO_MODE_FORTRAN,Wfd%comm,iqiA=iq_ibz)
#endif
           write(message,*) "Check the hermiticity"
           call wrtout(std_out,message,'COLL')
           tol = 0.01_dp
           ...
           do m2=1,mbband
             do m3=1,mbband
              ...
              !     sum q sum G1 f(g1,q) sum G2 f(G2,q)G(G1,G2,q)
                     do iG1=1,npwe
                          nc=nc+conjg(rhot_q_m_npwe(m3,m1,iG1,iq_ibz))*sum(rhot_q_m_npwe(m2,m4,:,iq_ibz)*scr(:,iG1,iomega,iqalloc))
                     end do
...
!           endif
    ...
    ! tolerance of the symetry of screened U.
      tol=1E-2
      call checkk(U_m,1,mbband,tol,0,iomega,uu,jj,"UminusVbare")
      U_m=V_m+U_m
      write(message,*)  "UCRPA interaction"
      call wrtout(std_out,message,'COLL')
      call checkk(U_m,1,mbband,tol,1,iomega,uomega(iomega),jomega(iomega),"cRPA interaction")
   enddo
