...
subroutine wvl_psitohpsi(alphamix,eexctX, eexcu, ehart, ekin_sum, epot_sum, eproj_sum, eSIC_DC, &
     & itrp, iter, iscf, ixc, me, natom, nfft, nproc, nspden, rpnrm, scf, &
 ...
 use BigDFT_API, only: psitohpsi, KS_POTENTIAL
#endif
...
!
 if(scf) then
   if (present(vxc)) then
     ...
     call wvl_vxc_abi2big(nfft,nspden,2,vxc,wvl%den)
   end if
...
!    end do
     call wvl_vtrial_abi2big(nfft,nspden,2,vtrial,wvl%den)
   end if
 end if !if scf
