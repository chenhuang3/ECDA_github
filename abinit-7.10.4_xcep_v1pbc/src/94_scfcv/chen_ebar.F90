...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2014 ABINIT group (DCA, XG, GMR, LSI, AR, MB)
...
!!
!! CHILDREN
!!      fftpac,fourwf,prep_fourwf,prtrhomxmn,sphereboundary,symrhg,timab,wrtout
!!      wvl_mkrho,xmpi_sum
!!
!! SOURCE
...
! 
! compute   ebar = sum(e_i*rho_i)/rho
!

subroutine chen_ebar(cg,dtset,gprimd,irrzon,kg,mcg,mpi_enreg,npwarr,occ,eigen,paw_dmft,phnons,&
...
! end if
 ...
 ioption = 0 ! Just accumulate electron density however weighted by eigenvalues 
...
!in wavelet.
 if (dtset%usewvl == 1) then
   select case(ioption)
     case (0)
       call wvl_mkrho(dtset, irrzon, mpi_enreg, phnons, rhor, wfs, wvl_den)
       return
     case (1)
...
!!! chen !!!!!!!!!!!!!!!!!!!!!!!
!! add the eigen values here            
                   weight = weight*eigen(iband+bdtot_index)
!!!!!!!!!!!!!!!!!!!! end of code 
                   weight_i=weight
                 end if
...
!              End loop on iband
           ...
           if (.not.(bandfft_kpt(ikpt_this_proc)%flag1_is_allocated==1.and.&
...
&               iblock,ikpt,istwf_k,dtset%mgfft,mpi_enreg,&
