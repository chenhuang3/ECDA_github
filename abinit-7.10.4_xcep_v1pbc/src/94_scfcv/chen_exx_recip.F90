!
! This subroutine is based on the hartre.F90, it computes the 
! 
!   \int dr' phi_i(r') phi_j(r') / |r-r'| Erfc(r-r')   (due to the the HSE exchange)
!
! in reciprocal space 
!
! Date : 9/24/2016 by Chen Huang
!
...
!!
!! PARENTS
!!      fock_getghc,ftfvw2,loop3dte,nres2vres,prctfvw2,rhohxc,rhotov3
!!      setup_positron,tddft
!!
!! CHILDREN
...
subroutine chen_exx_recip(cplex,gmet,gsqcut,izero,mpi_enreg,nfft,ngfft,paral_kgb,ucvol,nkpt, & 
                          qphon,exx_method,rhog,vhartr,hse_range)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 integer :: exx_method 
 real(8) :: rcut, xx, ucvol, divgq0
 integer :: nkpt 
 real(8),parameter :: chen_pi = 3.14159265359d0 
...
!!!!!!!!!!!!!!!!!! chen !!!!!!!!!!!!!!!!!!!!!!!!
! note that the G is defined a in different way compared to the paper 
! Screened hybrid density functionals applied to solids -- J. Chem. Phys. 124, 154709 (2006) 
! in that paper the q = 2 pi G, where G is used in ABINIT here and q is used in
! that paper, therefore, the exponent exp(-q*q/4/hse_range**2) would be changed
! to exp(-gs*pi*pi/hse_range**2), because we will replace the q with (2 pi G)

           select case(exx_method) 
           case (0) 
             den = 0.0d0
           case (1) 
             ! Error funciton, follow paper: 
             ! Screened hybrid density functionals applied to solids -- J. Chem. Phys. 124, 154709 (2006)
             den = pi/hse_range**2
           case (2)
             ! Spencer and Alavi 
             ! https://journals.aps.org/prb/pdf/10.1103/PhysRevB.77.193110
             den = 2.d0*pi/hse_range**2
           end select

           work1(re,1+i23)=rhog(re,1+i23)*den
           work1(im,1+i23)=rhog(im,1+i23)*den
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!           
...
! to exp(-gs*pi*pi/hse_range**2), because we will replace the q with (2 pi G)
         ...
         case (0) 
           den = piinv/gs
         case (1)  ! Error function 
           ! follow paper: 
           ! Screened hybrid density functionals applied to solids -- J. Chem. Phys. 124, 154709 (2006)
           den = piinv/gs*(1.d0-exp(-pi*pi*gs/hse_range**2))
         case (2) 
           ! Spencer and Alavi 
           ! https://journals.aps.org/prb/pdf/10.1103/PhysRevB.77.193110
           den = piinv/gs*(1.d0-cos(sqrt(gs)*2.d0*pi/hse_range))
         end select
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 if (izero==1) then
   ...
   if (qeq0==1) then !q=0
     call zerosym(work1,2,n1,n2,n3,mpi_comm_fft=mpi_enreg%comm_fft,distribfft=mpi_enreg%distribfft)
...
&     mpi_comm_fft=mpi_enreg%comm_fft,distribfft=mpi_enreg%distribfft)
   end if
 end if
