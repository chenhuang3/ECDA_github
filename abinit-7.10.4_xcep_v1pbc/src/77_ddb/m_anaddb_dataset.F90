...
!!
!! COPYRIGHT
!!  Copyright (C) 2014 ABINIT group (XG,JCC,CL,MVeithen,XW,MJV)
...
!scalars
 ...
 type(anaddb_dataset_type),intent(inout) :: anaddb_dtset !vz_i
...
!number to control the spline interpolation in RTA
 ...
 if(anaddb_dtset%ep_nspline < 0 .or. anaddb_dtset%ep_nspline > 1000) then
   write(message, '(a,i0,a,a,a,a,a)' )&
&   'ep_nspline is',anaddb_dtset%ep_nspline,&
&   ', but this should not be ',ch10,&
...
&         'Action: correct the second list','of wavevectors in the input file.'
 ...
 ! TODO
 ! Add support for ndivsmall here
 if (anaddb_dtset%nqpath > 0) then
