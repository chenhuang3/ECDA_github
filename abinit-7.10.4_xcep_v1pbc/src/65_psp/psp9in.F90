...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2014 ABINIT group (XG, AF)
...
!!
!! CHILDREN
!!      atomxc,cc_derivatives,close_xml_t,get_n_semicore_shells,open_xml_file
!!      parse,psp5lo,psp5nl,schro_eq,smoothvlocal,spline,splint,vhrtre,wrtout
!!
!! SOURCE
...
!no_abirules
 ...
 real(dp), parameter :: Rmax_kb_default = 60.0_dp ! Radius of the sphere where
                                                  ...
                                                  !   will be computed
 character(len=10)  :: functl   
 character(len=10)  :: author  
 integer :: ipsang,ir,jj,maxn_pots,npts
 integer :: il,index,irelt,nrwf,nnodes,nprin,l,ii, ikb, jkb,lhigh
 ...
 real(dp) :: sum, vl, eps, r, avgv, vphi
 ...
 real(dp),allocatable :: rphi2(:,:), vii(:,:), vij(:)
...
!                   First argument: index in the radial grid.
!                   Second argument: angular momentum.
!                   Third argument: number of KB projectors for angular momentum
...
! generate the pseudopotential
 chgvps = 0.0_dp
 do il = 1, psxml%npots_down + psxml%npots_up
   chgvps = chgvps + psxml%pot(il)%occupation
   write(std_out,*)' psp9in : occupation = ', psxml%pot(il)%occupation
 end do
...
!
! Rescale rho so that it contains the 'true' charge in the pseudopotential
! calculation, as rho in the XML file is rescaled to the
! charge of a neutral atom. 
! This is important if the semicore is explicitly included in valence
!
 do ir = 1, npts
...
! ENDF DEBUG   
 ...
 ABI_ALLOCATE( wfllatm,(nrmax,mpsang,2))
 ABI_ALLOCATE( erefkb,(nkbmx,0:lmax))
 ABI_ALLOCATE( rphi2,(npts,nkbmx))
 ABI_ALLOCATE( vii,(mpsang,nkbmx))
 ABI_ALLOCATE( vij,(nkbmx))
 ABI_ALLOCATE( nproj_tmp,(mpssoang))

 erefkb(:,:) = huge(1.0_dp)

! Find the number of projectors per angular momentum shell
 call get_n_semicore_shells( psxml, nproj_tmp )
 nproj_tmp(:) = nproj_tmp(:) + 1

!!DEBUG
!  write(std_out,*)' psp9in : nproj_tmp(:) = ', nproj_tmp(:)
!!DEBUG
...
&     erefkb(ikb,l), wfllatm(1,il,ikb) )
!    Normalization of the eigenstates inside a sphere of radius Rmax=nrwf
     dnrm = 0.0_dp
     do ir = 1, nrwf
       dnrm =dnrm + drdi(ir) * wfllatm(ir,il,ikb)**2
     end do
     dnrm = sqrt(dnrm)
     do ir = 1, nrwf
       wfllatm(ir,il,ikb) = wfllatm(ir,il,ikb)/dnrm
     end do
!!DEBUG
!     write(std_out,'(a,i5)')' # il      = ', il
!     write(std_out,'(a,i5)')' # ikb     = ', ikb
!     write(std_out,'(a,i5)')' # l       = ', l
!     write(std_out,'(a,i5)')' # nnodes  = ', nnodes
!     write(std_out,'(a,i5)')' # nprin   = ', nprin
!     do ir = 1, npts
!       write(std_out,'(3f20.12)')ratm(ir), wfllatm(ir,il,ikb)
!     end do
!     write(std_out,'(a,i5)')
!!ENDDEBUG
   end do
...
!Interpolate the radial wave functions to compute the first projector ---
 do il = 1, mpsang
   ...
   if( use_read_wfnl ) then
     ...
     funrad(1:npts) = wfllatm(1:npts,il,1)
   end if
...
!!ENDDEBUG
 ...
 ABI_ALLOCATE(ekb_tmp,(mpssoang,2))
 ABI_ALLOCATE(ffspl_tmp,(mqgrid,2,mpssoang,2))
...
!  Compute KB form factors and fit splines
   ...
   call psp5nl(al,ekb_tmp(:,1),ffspl_tmp(:,:,:,1),lmax,mmax,mpsang,&
&   mqgrid,qgrid,rad,vloc,vpspll,wfll)

 end if

 lhigh = -1
 do ipsang = 1, min(lmax+1,mpssoang)
 ...
 if (lhigh>-1) then
!Interpolate the radial wave functions to compute the second projector ---
...
!        For the first KB projector of a given shell, nothing has to be done.
         do ir = 1, npts
           rphi2(ir,1) = wfllatm(ir,il,1)          
         end do
!        We compute the denominator of Eq. (28) in the paper by 
!        X. Gonze, R. Stumpf, and M. Scheffler, Phys. Rev. B 44, 8503 (1991)
         avgv        = 0.0_dp
         vii(il,ikb) = 0.0_dp
         do ir = 2, npts
           r    = ratm(ir)
!          vps is stored in Ry. Here we divide by 2.0_dp to transform to Ha
           vl   = (vps(ir,il-1)/2.0_dp) - vlocal(ir)
           phi  = rphi2(ir,ikb)
           vphi = vl * phi
...
!         write(std_out,*)
!         write(std_out,*)'# il     = ', il-1
!         write(std_out,*)'# ikb    = ', ikb
!         write(std_out,*)'# npts   = ', npts
!         write(std_out,*)'# mpsang = ', mpsang
!         write(std_out,*)'# lnmax  = ', lnmax
!         write(std_out,*)'# vii    = ', vii(il,ikb)
!! ENDDEBUG
         

       elseif( ikb .gt. 1 ) then
!        If there are more than one KB projector per shell,
!        then we have to transform the atomic wave functions into a new
!        set of functions for which the matrix of the potential is diagonal.
!        See paragraph after Eq. (4) in the paper by
!        P. E. Blochl, Phys. Rev. B 41, 5414 (1990).
!        This is also written in Eq. (5) of the paper by Siesta,
!        J. M. Soler et al., J. Phys.: Condens. Matter 14, 2745 (2002)
         do jkb = 1, nproj(il) - 1
           sum = 0.0_dp
!          In this loop we compute the numerator at the right hand side term
!          of Eq. (5) of the Siesta paper.
           do ir = 1, npts
!            vps is stored in Ry. Here we divide by 2.0_dp to transform to Ha
             vl = (vps(ir,il-1)/2.0_dp) - vlocal(ir)
             sum = sum + wfllatm(ir,il,ikb) * vl * rphi2(ir,jkb) * drdi(ir)
           end do
           vij(jkb) = sum
!! DEBUG
!           write(std_out,*)'# jkb = ', jkb
!           write(std_out,*)'# vij = ', vij(jkb)
!! ENDDEBUG
         end do
         do ir = 1, npts
           sum = 0.0_dp
...
!          of the Siesta paper.
!          Note that the denominator was computed and saved as vii before
           do jkb = 1, ikb-1
             sum = sum + vij(jkb) * rphi2(ir,jkb)/(vii(il,jkb)+1.0d-20)
           end do
!          Finally, we perform the differentiation in Eq. (5) of the technical
!          paper of Siesta
           rphi2(ir,ikb) = wfllatm(ir,il,ikb) - sum
         end do
!! DEBUG
!           write(std_out,*)'# ikb   = ', ikb
!           write(std_out,*)'# rphi2 = ', rphi2(2,ikb)
!! ENDDEBUG
...
!     write(std_out,*)'# il = ', il-1
!     do ir = 2, npts
!       write(std_out,'(2f20.12)')ratm(ir), funrad(ir)
!     end do
!! ENDDEBUG
...
!  Compute KB form factors and fit splines for second wf if any
   call psp5nl(al,ekb_tmp(:,2),ffspl_tmp(:,:,:,2),lhigh,mmax,mpsang,&
&   mqgrid,qgrid,rad,vloc,vpspll,wfll)

 end if   ! Endif the second projector has to be computed
...
!!    DEBUG
!     write(std_out,*)' psp9in : lmnmax,ii,indlmn(1,ii)=',lmnmax,ii,indlmn(1,ii)
!     write(std_out,*)' psp9in : ii,kk =',ii,kk
!     write(std_out,*)' psp9in : indlmn(1,ii) =', indlmn(1,ii)
!     write(std_out,*)' psp9in : indlmn(2,ii) =', indlmn(2,ii)
!     write(std_out,*)' psp9in : indlmn(3,ii) =', indlmn(3,ii)
!     write(std_out,*)' psp9in : indlmn(4,ii) =', indlmn(4,ii)
!     write(std_out,*)' psp9in : indlmn(5,ii) =', indlmn(5,ii)
!     write(std_out,*)' psp9in : indlmn(6,ii) =', indlmn(6,ii)
!     write(std_out,*)' psp9in : ekb(kk) = ', ekb(kk)
!!    ENDDEBUG
