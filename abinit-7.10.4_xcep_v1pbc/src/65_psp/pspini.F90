...
!!
!! CHILDREN
!!      pawtab_set_flags,pspatm,pspcor,psps_print,timab,wrtout,xmpi_sum
!!
!! SOURCE
...
!scalars
 ...
 logical :: has_kij,has_tproj,has_tvale,has_vhnzc,has_vhtnzc,has_nabla,has_shapefncg,has_wvl
...
!Determine whether new optional data requests have changed
 ...
 if (psps%usepaw==1) then
   ...
   has_vhnzc=(dtset%orbmag>0)
   has_vhtnzc=(dtset%orbmag>0)
   has_nabla=.false.
   ...
   if (has_vhnzc)    paw_options(3)=1
   if (has_vhtnzc)   paw_options(4)=1
...
&         pawrad_dum,pawtab_dum,psps,vlspl,dvlspl,xcccrc,xccc1d)
         ...
         comm_mpi_=xmpi_self;if (present(comm_mpi)) comm_mpi_=comm_mpi
         call pspatm(dq,dtset,dtfil,ekb,epsatm(ipsp),ffspl,indlmn,ipsp,&
...
!-------------------------------------------------------------
! Following section is only reached when debugging is activated
!-------------------------------------------------------------
 if(dtset%prtvol==-level) then
   call psps_print(psps,std_out,dtset%prtvol)
 end if
