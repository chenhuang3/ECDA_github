...
!!
!! PARENTS
!!      dyfro3,eltfrxc3,extraprho,forces,fresidrsp,prcref,prcref_PMA,respfn
!!      setvtr,stress
!!
!! CHILDREN
...
!scalars
 ...
 integer,optional,intent(in) :: is2_in,me_g0,mpi_comm_fft,paral_kgb
...
!scalars
 ...
 real(dp) :: aa,alf2pi2,bb,cc,cutoff,dbl_ig1,dbl_ig2,dbl_ig3,dd,dg1,dg2,d2g,dgsqds,d2gsqds,diff
 real(dp) :: dn_at,d2n_at,d2n_at2,dq,dq2div6,dqdiv6,dqm1,dv_at,ee,ff,gauss1,gauss2,gg,gmag,gsq,gsquar,n_at
 real(dp) :: ph12i,ph12r,ph1i,ph1r,ph2i,ph2r,ph3i,ph3r,sfi,sfr,term,term1,term2,term4,tmpni,tmpnr
...
!                    Also get nc''(q)
                     if (opteltfr==1) then
                       gg=aa*pawtab(itypat)%tcorespl(jj,2)+bb*pawtab(itypat)%tcorespl(jj+1,2)
                     end if
                   else if (optn2==2) then
...
!                    Also get nc''(q)
                     if (opteltfr==1) then
                       gg=aa*pawtab(itypat)%tvalespl(jj,2)+bb*pawtab(itypat)%tvalespl(jj+1,2)
                     end if
                   else if (optn2==3) then
...
!              Compute G*dG/Deps_{\gamme\delta}
               dg2=0.5_dp*dgsqds(ig1,ig2,ig3,is2_in)
...
!                Compute G*dG/Deps_{\alpha\beta}
                 dg1=0.5_dp*dgsqds(ig1,ig2,ig3,is1)
!                Compute G^2*d2G/Deps_{alphabetagammadelta}
                 d2g=(0.25_dp*d2gsqds(ig1,ig2,ig3,is1,is2_in))
...
!    Create fake mpi_enreg to wrap fourdp
     ...
     if (present(mpi_comm_fft)) then
       ...
       my_comm_fft=xmpi_self;paral_kgb_fft=0;
...
!    Transform back to real space; divide by unit cell volume
     xnorm=one/ucvol
     if (optv==1) then
       call zerosym(workv,2,n1,n2,n3,mpi_comm_fft=my_comm_fft,distribfft=my_distribfft)
       call fourdp(1,workv,atmvloc,1,mpi_enreg_fft,nfft,ngfft,paral_kgb_fft,0)
       atmvloc(:)=atmvloc(:)*xnorm
       ABI_DEALLOCATE(workv)
     end if
     if (optn==1) then
       call zerosym(workn,2,n1,n2,n3,mpi_comm_fft=my_comm_fft,distribfft=my_distribfft)
...
!Additional treatment in case of parallelization
 if (present(mpi_comm_fft)) then
   if ((xcomm_size(mpi_comm_fft)>1).and.&
&   (optgr==1.or.optstr==1.or.optdyfr==1.or.opteltfr==1)) then
     call timab(48,1,tsec)
     if (optv==1) then
       if (optgr==1)then
         call xmpi_sum(grv_indx,mpi_comm_fft,ierr)
       end if
       if (optstr==1)then
         call xmpi_sum(strv,mpi_comm_fft,ierr)
       end if
       if (optdyfr==1)then
         call xmpi_sum(dyfrv_indx,mpi_comm_fft,ierr)
       end if
     end if
     if (optn==1) then
       if (optgr==1)then
         call xmpi_sum(grn_indx,mpi_comm_fft,ierr)
       end if
       if (optstr==1)then
         call xmpi_sum(strn,mpi_comm_fft,ierr)
       end if
       if (optdyfr==1)then
         call xmpi_sum(dyfrn_indx,mpi_comm_fft,ierr)
       end if
     end if
