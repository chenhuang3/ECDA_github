...
!!  Paw_pwff(Cryst%ntypat*Wfd%usepaw)<Paw_pwff_type>=Form factor used to calculate the onsite matrix 
...
&  nfftot_osc,ngfft_osc,Psps,Pawtab,Pawang,Paw_pwff,rhxtwg_q0,is_resonant,fname)
 ...
 use m_wfs,          only : wfd_t, wfd_get_ur, wfd_get_cprj, wfd_change_ngfft, wfd_ihave_ur, wfd_ihave_cprj, &
&                           wfd_sym_ur
...
&     ' Calculating coupling blocks. Memory required: ',bsize_my_block*b2Mb,' Mb. '
   ...
   if (BSp%prep_interp) then
     ...
     ABI_MALLOC(acoeffs,(t_start (my_rank):t_stop(my_rank)))
     ABI_CHECK_ALLOC('Not enough memory for acoeffs')

     ABI_MALLOC(bcoeffs,(t_start(my_rank):t_stop(my_rank)))
     ABI_CHECK_ALLOC('Not enough memory for bcoeffs')

     ABI_MALLOC(ccoeffs,(t_start(my_rank):t_stop(my_rank)))
     ABI_CHECK_ALLOC('Not enough memory for ccoeffs')
   end if
     ...
     do ikp_bz=1,BSp%nkbz ! Loop over kp  
       ...
       do ik_bz=1,ikp_bz ! Loop over k
         ...
         if (Wfd%usepaw==1.and.ik_bz/=ikp_bz) then
           ...
           call init_paw_pwij(Pwij_q,npweps,Qmesh%bz(:,iq_bz),Gsph_c%gvec,Cryst%rprimd,Psps,Pawtab,Paw_pwff)
         end if
...
200    call xmpi_barrier(comm)
 ...
 if (Wfd%usepaw==1) then
   call pawcprj_destroy(Cp_vk)
   ABI_DT_FREE(Cp_vk)
   call pawcprj_destroy(Cp_ck)
   ABI_DT_FREE(Cp_ck)
   call pawcprj_destroy(Cp_ckp)
   ABI_DT_FREE(Cp_ckp)
   call pawcprj_destroy(Cp_vkp)
   ABI_DT_FREE(Cp_vkp)
   call pawcprj_destroy(Cp_tmp1)
   ABI_DT_FREE(Cp_tmp1)
   call pawcprj_destroy(Cp_tmp2)
   ABI_DT_FREE(Cp_tmp2)
   call pawcprj_destroy(Cp_tmp3)
   ABI_DT_FREE(Cp_tmp3)
   call pawcprj_destroy(Cp_tmp4)
   ABI_DT_FREE(Cp_tmp4)
 end if
