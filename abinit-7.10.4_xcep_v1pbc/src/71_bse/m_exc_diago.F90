...
!!
!! COPYRIGHT
!! Copyright (C) 2009-2014 ABINIT and EXC groups (L.Reining, V.Olevano, F.Sottile, S.Albrecht, G.Onida, M.Giantomassi)
...
!************************************************************************
 ...
 nprocs  = xcomm_size(comm); my_rank = xcomm_rank(comm)
 ...
 ABI_MALLOC(exc_ene,(exc_size))
 ABI_CHECK_ALLOC('out of memory: excitonic eigenvalues')
 ...
 CASE (.FALSE.)
   ...
   ABI_MALLOC(exc_mat,(exc_size,exc_size))
   ABI_CHECK_ALLOC('out of memory: excitonic hamiltonian')
...
!scalars
 ...
 integer :: mi,it,nprocs,master,my_rank !itp
...
!************************************************************************
 ...
 ABI_CHECK_ALLOC('out of memory: full excitonic hamiltonian')
...
&  ' Memory-space requested: ',bsize_ham*b2Gb,' Gb. '
 ...
 ABI_MALLOC(exc_ene,(exc_size))
 ABI_CHECK_ALLOC('out of memory: exc_ene')
...
!ENDDEBUG
 ...
 ! ======================================================
 ABI_MALLOC(exc_rvect,(exc_size,exc_size))
 ABI_CHECK_ALLOC("out of memory: excitonic eigenvectors")
...
&  " Largest imaginary part:     ",temp*Ha_eV,      " [eV] ",ch10
 ...
 ABI_MALLOC(ovlp,(nstates,nstates))
 ABI_CHECK_ALLOC('out of memory in ovlp matrix')
...
!scalars
 ...
 integer :: mi,it,nprocs,master,my_rank
...
!************************************************************************
 ...
 CASE (.FALSE.)
   ...
   ABI_MALLOC(fmat,(exc_size,exc_size))
   ABI_CHECK_ALLOC('out of memory: fmat')
...
&      " Excitonic matrix cannot be read with a single MPI-IO call."
   ...
   ABI_MALLOC(tmp_cbuffer,(my_nel))
   ABI_CHECK_ALLOC(" out of memory tmp_cbuffer")
