...
MODULE m_bse_io
 ...
 use netcdf
 use m_ncfile,         only : ab_define_var
#endif
 ...
 use m_header,         only : hdr_free, hdr_mpio_skip, hdr_skip, hdr_io
...
!scalars
 ...
 call hdr_io(fform_1002,Hdr,rdwr2,funt)
...
!arrays
 ...
 call hdr_io(fform,Hdr,rdwr1,funt)
...
! *********************************************************************
 ...
 ABI_CHECK_ALLOC("out of memory in cbuff_dpc")
 !
...
!arrays
 ...
 if (nsppol==1) then ! Construct resonant block from the upper triangle stored on file.
   ...
   spad=nreh(1)
   do itp=1,nreh(2)
     read(funt,ERR=10) cbuff_dpc(1:nreh(1))
...
!arrays
 ...
 ABI_MALLOC(vec_list,(hsize,nvec))
 ABI_CHECK_ALLOC("out of memory in vec_list")
...
!1. Create netCDF file
 ncerr = nf90_create(path=trim(filname),cmode=NF90_CLOBBER, ncid=ncid)
 !MT aug 2013: keep the following on THREE lines in order to help abilint script
 NCF_CHECK(ncerr," create netcdf OME file")
 
!2. Define dimensions
 ncerr = nf90_def_dim(ncid,"xyz",3,xyz_id)
 NCF_CHECK(ncerr," define dimension nband")

 ncerr = nf90_def_dim(ncid,"cmplx",2,cmplx_id)
 NCF_CHECK(ncerr," define dimension nband")

 ncerr = nf90_def_dim(ncid,"nband",maxb-minb+1,nband_id)
 NCF_CHECK(ncerr," define dimension nband")

 ncerr = nf90_def_dim(ncid,"nkbz",nkbz,nkbz_id)
 NCF_CHECK(ncerr," define dimension nkbz")

 ncerr = nf90_def_dim(ncid,"nq",nq,nq_id)
 NCF_CHECK(ncerr," define dimension nq")

 ncerr = nf90_def_dim(ncid,"nsppol",nsppol,nsppol_id)
 NCF_CHECK(ncerr," define dimension nsppol")
...
!5 Write scalars (minb and maxb)
 ...
 ncerr = nf90_put_var(ncid, minb_id, minb)
 NCF_CHECK(ncerr," write variable minb")

 ncerr = nf90_put_var(ncid, maxb_id, maxb)
 NCF_CHECK(ncerr," write variable maxb")
     ...
     do ik=1,nkbz
       do ib=minb,maxb
         do jb=minb,maxb
           ...
           ncerr = nf90_put_var(ncid, ome_id,&
&             complex2, &
&             start = start6,&
&             count = count6)
           NCF_CHECK(ncerr," write variable OME")
       end do
       end do
     end do
   end do
...
!7 Close file
 ...
 NCF_CHECK(ncerr," close netcdf optical matrix elements file")
