...
!Initialize the file
 ...
 temp_unit = get_unit()
 open (unit=temp_unit,file=fnameabo_app_1dm,status='unknown',form='formatted')
 rewind(temp_unit)
...
&     '#                  (Hartree unit)   (electron/red. unit)'
     ...
     do islice=1,nslice
       ...
       mean_pot(islice)=zero
       lin_den(islice)=zero
       if (idim/=3.or.fftn3_distrib(islice)==mpi_enreg%me_fft) then
         do ib=1,nb
           if (idim==3.or.fftn3_distrib(ib)==mpi_enreg%me_fft) then
             do ia=1,na
               if(idim==1) ifft = islice + nslice*( ia    -1 + na    *(ffti3_local(ib)    -1) )
               if(idim==2) ifft = ia     + na    *( islice-1 + nslice*(ffti3_local(ib)    -1) )
               if(idim==3) ifft = ia     + na    *( ib    -1 + nb    *(ffti3_local(islice)-1) )
