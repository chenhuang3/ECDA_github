...
& npw,nspinor,occ_k,1,0,tim_rwwf,wff1)
 ...
 if (ikpt<=nkpt_max) then
   write(msg,'(a,i6,a,i6,a,i5)')' initwf : disk file gives npw=',npw,' nband=',nband_disk,' for kpt number=',ikpt
...
&   '  Bands are skipped or filled with random numbers.'
 ...
 if (ikpt<=nkpt_max) then
   write(msg,'(a,i6,a)')' initwf :',nband_disk,' bands have been initialized from disk'
   call wrtout(std_out,msg,'PERS')
 end if
