...
MODULE m_wfs
 ...
 use m_fstrings,       only : toupper, starts_with, int2char10
 ...
 use m_header,         only : hdr_init_lowlvl, hdr_init, hdr_free, hdr_update, &
&                             hdr_read_from_fname, hdr_io_etsf, hdr_io, hdr_copy
 ...
 use m_pawcprj,        only : pawcprj_type, pawcprj_alloc, pawcprj_destroy, pawcprj_copy, paw_overlap
...
!! SOURCE
 ...
 type,public :: kdata_t
   ...
   integer,allocatable :: ig1_inver(:,:)
   ! ig1_inver(ng1,8)

   integer,allocatable :: ig2_inver(:,:)
   ! ig2_inver(ng2,8)

   integer,allocatable :: ig3_inver(:,:) 
   ! ig3_inver(ng3,8)
   ...
   ! TODO Remove this array, not uses anymore
   ! igfft0(npw)
...
!! SOURCE
 ...
 type,public :: iskg_tabs_t
   ...
   complex(dpc),allocatable :: ph_mskpgt(:)
   ! ph_mskpgt(npw_k)
...
! types
 ...
 public :: wfd_dur_isk
 public :: wfd_mkheader            ! Constructu the header of the WFK file.
 public :: check_sym_ug
!!***
...
!scalars
 ...
 integer :: mpw_,npw_k,dimffnl,useylmgr,nkpg,iatom,ipw,ist
...
&  Psps%pspso,Psps%qgrid_ff,Cryst%rmet,Psps%usepaw,Psps%useylm,Kdata%ylm,ylmgr_k)
 ...
 ABI_MALLOC(Kdata%gc2kg,(ng1,ng2,ng3))
 Kdata%gc2kg=0
 do ipw=1,npw_k
 ...
 ABI_MALLOC(Kdata%ig1_inver,(ng1,8))
 ABI_MALLOC(Kdata%ig2_inver,(ng2,8))
 ABI_MALLOC(Kdata%ig3_inver,(ng3,8))
...
!************************************************************************
 ...
 !@kdata_t
 if (allocated(Kdata%gc2kg)) then
   ABI_FREE(Kdata%gc2kg)
 end if
 if (allocated(Kdata%ig1_inver)) then
   ABI_FREE(Kdata%ig1_inver)
 end if
 if (allocated(Kdata%ig2_inver)) then
   ABI_FREE(Kdata%ig2_inver)
 end if
 if (allocated(Kdata%ig3_inver)) then
   ABI_FREE(Kdata%ig3_inver)
 end if
...
!************************************************************************
 ...
 call alloc_copy(Kdata_in%gc2kg,Kdata_out%gc2kg)
 call alloc_copy(Kdata_in%ig1_inver,Kdata_out%ig1_inver)
 call alloc_copy(Kdata_in%ig2_inver,Kdata_out%ig2_inver)
 call alloc_copy(Kdata_in%ig3_inver,Kdata_out%ig3_inver)
...
!!
!! PARENTS
!!      bethe_salpeter,m_shirley,m_wfs,screening,sigma
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
&         two*(Cryst%gmet(1,2)*g1*g2+Cryst%gmet(1,3)*g1*g3+Cryst%gmet(2,3)*g2*g3)
 ...
 ug_size=nspinor*mpw*COUNT(bks_mask)
 write(msg,'(a,f12.1,a)')' Memory needed for Fourier components u(G) = ',2*gwpc*ug_size*b2Mb,' [Mb]'
 ...
 write(msg,'(a,f12.1,a)')' Memory needed for real-space u(r) = ',2*gwpc*ur_size*b2Mb,' [Mb]'
 call wrtout(std_out,msg,'PERS')
...
!!      exc_build_ham,exc_den,m_eet,m_shirley,m_wfs,prep_calc_ucrpa,wfd_mkrho
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
&  '  Number of FFT points treated by me .... ',Wfd%nfft,ch10
 ...
 ug_size = Wfd%nspinor * mpw * count(Wfd%Wave(:,:,:)%has_ug >= WFD_ALLOCATED)
 write(msg,'(a,f12.1,a)')' Memory allocated for Fourier components u(G) = ',2*gwpc*ug_size*b2Mb,' [Mb]'
 call wrtout(std_out,msg,'PERS')

 ur_size = Wfd%nspinor * Wfd%nfft *count(Wfd%Wave(:,:,:)%has_ur >= WFD_ALLOCATED)
 write(msg,'(a,f12.1,a)')' Memory allocated for real-space u(r) = ',2*gwpc*ur_size*b2Mb,' [Mb]'
 call wrtout(std_out,msg,'PERS')

 if (Wfd%usepaw==1) then
   cprj_size = Wfd%nspinor * sum(Wfd%nlmn_atm) * count(Wfd%Wave(:,:,:)%has_cprj >= WFD_ALLOCATED)
...
!************************************************************************
 ...
 if (starts_with(my_what,["A", "C"])) then
   if (allocated(Wave%Cprj)) then
     call pawcprj_destroy(Wave%Cprj)
...
!************************************************************************
 ...
 if (comm_spin /= xmpi_comm_null) then
   ans = .True.; rank_spin = xcomm_rank(comm_spin); nproc_spin = xcomm_size(comm_spin)
 end if
...
!************************************************************************
 ...
 call xcomm_group(Wfd%comm,world_group,ierr)
 !
 ...
 do spin=1,Wfd%nsppol
   ...
   if (how_many>0) then
     call xgroup_incl(world_group,how_many,proc_ranks,spin_group,ierr)
     call xcomm_create(Wfd%comm,spin_group,spin_comm,ierr)
     Wfd%bks_comm(0,0,spin) = spin_comm
     call xgroup_free(spin_group)
   else
...
if (.FALSE.) then
 ...
 do spin=1,Wfd%nsppol
   do ik_ibz=1,Wfd%nkibz
     ...
     if (how_many>0) then
       call xgroup_incl(world_group,how_many,proc_ranks,kpt_group,ierr)
       call xcomm_create(Wfd%comm,kpt_group,kpt_comm,ierr)
       Wfd%bks_comm(0,ik_ibz,spin) = kpt_comm
       call xgroup_free(kpt_group)
     else
       if (Wfd%debug_level>0) then
 ...
 do spin=1,Wfd%nsppol
   do ik_ibz=1,Wfd%nkibz
     do band=1,Wfd%nband(ik_ibz,spin)
       ...
       call wfd_who_has_ug(Wfd,band,ik_ibz,spin,how_many,proc_ranks)
       if (how_many>0) then
         call xgroup_incl(world_group,how_many,proc_ranks,bks_group,ierr)
         call xcomm_create(Wfd%comm,bks_group,Wfd%bks_comm(band,ik_ibz,spin),ierr)
         call xgroup_free(bks_group)
       else
         if (Wfd%debug_level>0) then
...
!!
!! FUNCTION
!!  This routines distributes a set of band indices taking into account the
...
!!
!! FUNCTION
!!  This routines returns an iterator used to loop over bands, k-points and spin indices
...
! *************************************************************************
 ...
 if (Wfd%gamma_centered) then
   ...
   call xmpi_sum(cg,comm,ierr) ! Collect on each node.
  end if
...
!scalars
 ...
 integer :: rank,ncpus,idle,b1_stop
 character(len=500) :: msg
!arrays
...
!************************************************************************
 ...
 ABI_MALLOC(whocan_k ,(Wfd%mband,Wfd%nproc))
 ABI_CHECK_ALLOC("out of memory in whocan_k")
 ABI_MALLOC(whocan_kp,(Wfd%mband,Wfd%nproc))
 ABI_CHECK_ALLOC("out of memory in whocan_kp")
...
!!      exc_build_block,exc_build_ham,m_shirley,m_wfs,prep_calc_ucrpa,sigma
!!      wfd_pawrhoij,wfd_vnlpsi
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!!      calc_sigc_me,calc_sigx_me,calc_vhxc_me,cchi0,cchi0q0,cchi0q0_intraband
!!      check_completeness,classify_bands,cohsex_me,exc_build_block
!!      exc_build_ham,exc_plot,m_haydock,m_shirley,m_wfs,prep_calc_ucrpa
!!      screening,sigma,wfd_mkrho
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!!
!! PARENTS
!!      cchi0q0_intraband,check_completeness
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!************************************************************************
 ...
 if (.not.PRESENT(bks_ids)) then 
   ...
   MSG_WARNING(" spin optional argument not coded, have to introduce MPI communicators")
   call xmpi_barrier(Wfd%bks_comm(bks_ids(1),bks_ids(2),bks_ids(3)))
 end if
...
!!      exc_build_block,exc_plot,m_haydock,m_shirley
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!************************************************************************
 ...
 CASE (1)
   ...
   do ir=1,Wfd%nfft
     ...
     !if (irot < 1 .or. irot > Wfd%nfft) then
     !  write(std_out,*)"irot",irot
     !  MSG_ERROR("Wrong irot")
     !end if
...
!!****f* m_wfs/wfd_map_gvec
!! NAME
!!  wfd_map_gvec
!!
!! FUNCTION
!!  Compute the mapping between input gvec and the G-vectors used in Wfd
!!
!! INPUTS
!!   Wfd<wfd_t>
!!   ik_ibz = Index of the k-point in the IBZ
!!   ng=Number of G vectors in gvec
!!   gvec(3,ng) = G-vectors
!!
!! OUTPUT
!!   gvec2wfd(ng) = Mapping gvec -> Wfd%Kdata(ik_ibz)%kg_k
!!                  Set to 0 if gvec(:,ig) not in kg_k
!!   nmiss = Number of G in gvec that are not found
!!
!! PARENTS
!!      m_wfs
!!
...
subroutine wfd_map_gvec(Wfd,ik_ibz,ng,gvec,gvec2wfd,nmiss)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'wfd_map_gvec'
!End of the abilint section
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: ik_ibz,ng
 integer,intent(out) :: nmiss
 type(wfd_t),intent(in) :: Wfd
!arrays
 integer,intent(in) :: gvec(3,ng)
 integer,intent(out) :: gvec2wfd(ng)
...
!************************************************************************
 ...
 do ig=1,ng
   ...
   if (found) then    ! Store it if found.
     ...
     gvec2wfd(ig) = 0 ! Not found
     nmiss = nmiss+1
   end if
 end do

end subroutine wfd_map_gvec
!!***
...
!************************************************************************
 ...
 nprocs = xcomm_size(Wfd%comm)
 my_rank = xcomm_rank(Wfd%comm)
 ...
 if (iam_master) then
   call wfk_open_write(WfkFile,Hdr,wfk_fname,formeig0,iomode,get_unit(),xmpi_self,write_hdr=.TRUE.,write_frm=.FALSE.)
 end if

 ! Other nodes wait here before opening the same file.
 call xmpi_barrier(Wfd%comm)
 if (.not.iam_master) then
   call wfk_open_write(WfkFile,Hdr,wfk_fname,formeig0,iomode,get_unit(),xmpi_self,write_hdr=.FALSE.,write_frm=.FALSE.)
 end if
 ...
 do spin=1,Wfd%nsppol
   do ik_ibz=1,Wfd%nkibz
     ...
     ABI_MALLOC(cg_k, (2,cgsize))

     if (ABI_ALLOC_STAT_ABI /=0) then
        MSG_ERROR("out of memory in cg_k")
...
!!
!! PARENTS
!!      bethe_salpeter,m_wfs,screening,sigma
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!************************************************************************
 ...
 if (Wfd%prtvol>0) then
   call hdr_io(fform,Hdr,4,std_out)
 endif
...
&       " nband_wfd to be read cannot be greater than nband_disk while: ",nband_wfd,nband_disk
     ...
     !call wfk_open_read(Wfk,wfk_fname,formeig0,iomode,wfk_unt,Wfd%comm)
     ...
     if (Wfd%my_rank==Wfd%master) then
       if (Wfd%nsppol==2) then
         write(ab_out,'(i3,a,10f7.2/50(10x,10f7.2/))') ik_ibz,tag_spin(spin),(eig_k(ib)*Ha_eV,ib=1,nband_wfd)
       else
         write(ab_out,'(i3,7x,10f7.2/50(10x,10f7.2/))')ik_ibz,(eig_k(ib)*Ha_eV,ib=1,nband_wfd)
       end if
     end if
     !
     ...
     call wfd_map_gvec(Wfd,ik_ibz,npw_disk,kg_k,gf2wfd,nmiss)
...
!!      classify_bands,m_wfs,prep_calc_ucrpa
!!
!! CHILDREN
!!      fft_ug,get_bz_item
!!
...
!************************************************************************
 ...
 if (Wfd%usepaw==1) then
   ...
   ABI_MALLOC(l_size_atm,(Cryst%natom))
   do iatom=1,Cryst%natom
     l_size_atm(iatom) = Pawtab(Cryst%typat(iatom))%lcut_size
   end do
...
!!  Wfd<wfd_t>=
!!  [got(Wfd%nproc)]=The number of tasks already assigned to the nodes.
!!  [bbks_mask(Wfd%mband,Wfd%mband,Wfd%nkibz,Wfd%nsppol)]= mask used to select the (b,b',k,s) indices.
!!
!! OUTPUT
...
!!****f* m_wfs/wfd_mkheader
!! NAME
!! wfd_mkheader
!!
!! FUNCTION
!!  Build the header of the WFK file.
!!
!! INPUTS
!!  Wfd<wfd_t>=Initialized wavefunction descritptor. 
...
!!  Pawtab(ntypat*usepaw) <type(pawtab_type)>=paw tabulated starting data
!!  Pawrhoij(natom*usepaw) <type(pawrhoij_type)>=rhoij quantities for each atom
!!  iHdr <type(hdr_type)>=Initial version of the header.
!!  pawecutdg=cut-off for fine "double grid" used in PAW calculations (unused for NCPP)
!!  ngfftdg(18)=Dense FFT mesh.
!!
!! OUTPUT 
!!  oHdr <type(hdr_type)>=output header (consistent with the info reported in Wfd)
!!
!! PARENTS
!!
!! CHILDREN
...
subroutine wfd_mkheader(Wfd,Crystal,Bands,Psps,Pawtab,Pawrhoij,iHdr,pawecutdg,ngfftdg,oHdr)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'wfd_mkheader'
!End of the abilint section
...
!scalars
 ...
 type(ebands_t),intent(in) :: Bands
 type(pseudopotential_type),intent(in) :: Psps
 type(hdr_type),intent(in) :: iHdr
 type(hdr_type),intent(out) :: oHdr
!arrays
 integer,intent(in) :: ngfftdg(18)
 type(pawtab_type),intent(in) :: Pawtab(Crystal%ntypat*Psps%usepaw)
 type(pawrhoij_type),intent(in) :: Pawrhoij(Crystal%natom*Psps%usepaw)

!Local variables ------------------------------
!scalars
 integer :: pawcpxocc
 type(wvl_internal_type) :: dumWvl

!************************************************************************

 ! pawcpxocc=1 if rhoij is REAL,2 if COMPLEX
 pawcpxocc = 0; if (Psps%usepaw==1) pawcpxocc = Pawrhoij(1)%cplex

 ! Check if basic dimensions agree!
 ABI_CHECK(Bands%nkpt == Wfd%nkibz, "Different number of k-points!")
 ABI_CHECK(Bands%nsppol == Wfd%nsppol, "Different number of spins!")
 ABI_CHECK(Bands%nspinor == Wfd%nspinor, "Different number of spinors!")

 ! These values must be consistent since they are used to access data when we read the WFK file.
 ABI_CHECK(all(Bands%istwfk == Wfd%istwfk), "Mismatch in istwfk")
 ABI_CHECK(all(Bands%npwarr ==  Wfd%npwarr), "Mismach npwarr")
 ABI_CHECK(all(Bands%nband == reshape(Wfd%nband, [Wfd%nkibz*Wfd%nsppol])), "Mismatch in nband")
 ABI_CHECK(all(abs(Bands%kptns-Wfd%kibz) < tol12), "Mismatch in k-points")

 call hdr_init_lowlvl(oHdr,Bands,Psps,Pawtab,dumWvl,abinit_version,&
&  iHdr%pertcase,Crystal%natom,Crystal%nsym,Wfd%nspden,Wfd%ecut,pawecutdg,Wfd%ecutsm,Wfd%dilatmx,&
&  iHdr%intxc,iHdr%ixc,iHdr%stmbias,Wfd%usewvl,pawcpxocc,Wfd%ngfft,ngfftdg,iHdr%so_psp,&
&  iHdr%qptn,Crystal%rprimd,Crystal%xred,Crystal%symrel,Crystal%tnons,Crystal%symafm,Crystal%typat)

 ! Copy pawrhoij datastructure.
 if (Psps%usepaw == 1) then
   call pawrhoij_copy(Pawrhoij,oHdr%Pawrhoij)
 end if

end subroutine wfd_mkheader
!!***

!----------------------------------------------------------------------

!!****f* m_wfk/check_sym_ug
!! NAME
!! check_sym_ug
!!
!! FUNCTION
...
!************************************************************************
 ...
 do spin=1,Wfd%nsppol
   do ik_bz=1,Kmesh%nbz
     ...
     ABI_MALLOC(ug_kbz,(npw_k*Wfd%nspinor))
...
!************************************************************************
 ...
 ABI_MALLOC(box2,(g2bd(1,1):g2bd(1,2), g2bd(2,1):g2bd(2,2), g2bd(3,1):g2bd(3,2)))
 box2=0
 do ig2=1,ISkg%npw_k
   ii = ISkg%kg_k(1,ig2)
   jj = ISkg%kg_k(2,ig2)
 ...
 !     negative indices are used to signal that cc has to be taken. (useful when istwf_k>2)
 !
 ABI_MALLOC(ISkg%gt_k2isk,(npw_k))
 ISkg%gt_k2isk=0
 ABI_MALLOC(ISkg%ph_mskpgt,(npw_k))
 ...
 !ABI_MALLOC(tmp_igfft,(ISkg%npw_k))
 !ABI_MALLOC(kg_mask,(ISkg%npw_k))
 !call kgindex(tmp_igfft,ISkg%kg_k,kg_mask,MPI_enreg_seq,ngfft,ISkg%npw_k)
 !ABI_CHECK(ALL(kg_mask),"FFT para not yet implemented")
 !ABI_FREE(tmp_igfft)
 !ABI_FREE(kg_mask)

 ABI_MALLOC(ISkg%gbound,(2*mgfft+8,2))
...
! integer
 if (allocated(ISkg%gbound)) then
   ABI_FREE(ISkg%gbound)
 end if
 if (allocated(ISkg%gt_k2isk)) then
   ABI_FREE(ISkg%gt_k2isk)
 end if
 if (allocated(ISkg%kg_k)) then
   ABI_FREE(ISkg%kg_k)
 end if

! complex
 if (allocated(ISkg%ph_mskpgt)) then
   ABI_FREE(ISkg%ph_mskpgt)
 end if
...
!!****f* m_wfs/wfd_get_ug_isk
!! NAME
!!  wfd_get_ug_isk
!!
!! FUNCTION
!!  Symmetrize a wave function in reciprocal space
!!
!! INPUTS
!!  Wfd<wfd_t>=the wavefunction descriptor.
...
!!
!! OUTPUT
!!   ug_isk(ISkg%npw_k*Wfd%nspinor) = The symmetrized wavefunction in real space.
!!
!! PARENTS
!!
!! CHILDREN
...
subroutine wfd_get_ug_isk(Wfd,ISkg,band,ik_ibz,spin,ug_isk)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'wfd_get_ug_isk'
!End of the abilint section
...
!arrays
 complex(gwpc),intent(out) :: ug_isk(ISkg%npw_k*Wfd%nspinor)

!Local variables ------------------------------
!scalars
 integer :: isym,npw_k,ig1,isg
 character(len=500) :: msg
...
!************************************************************************
 ...
 CASE (1)
   ...
   do ig1=1,npw_k
     ...
     if (ISkg%itim==2) ug_isk(isg) = GWPC_CONJG(ug_isk(isg))
   end do
...
!************************************************************************
 ...
 if (isym==1) then ! Avoid the rotation.
   ...
   if (ISkg%itim==2) ur_isk = GWPC_CONJG(ur_isk)
   RETURN
 end if
 !
 ...
 CASE (1)
   if (use_rspace) then
     ...
     ! irottb(:,isym) --> Table for rotated FFT points
     ur_isk = ur_kibz(Wfd%irottb(:,isym) ) * ph_mkt
     if (ISkg%itim==2) ur_isk = GWPC_CONJG(ur_isk)
   else
     ! Symmetrization in done in G-space:
     !  1) Rotation in G space
     ...
     call fft_ug(ISkg%npw_k,Wfd%nfft,Wfd%nspinor,ndat1,Wfd%mgfft,Wfd%ngfft,&
&      ISkg%istwf_k,ISkg%kg_k,ISkg%gbound,ug_isk,ur_isk)
...
!scalars
 ...
 character(len=500) :: msg
!arrays
 integer,ABI_CONTIGUOUS pointer :: kg_k(:,:),gbound_k(:,:)
...
!************************************************************************
 ...
 if (.not. do_sym) then
   ...
   ABI_MALLOC(grad_dir,(npw_k*Wfd%nspinor))
   do dir=1,3
     ...
     call fft_ug(npw_k,Wfd%nfft,Wfd%nspinor,ndat1,Wfd%mgfft,Wfd%ngfft,&
&       istwf_k,kg_k,gbound_k,grad_dir,grad_ur(:,dir))
   end do
  ...
  CASE (1)
    ...
    do ig=1,npw_k
      ...
      if (ISkg%itim==2) ug_isk = GWPC_CONJG(ug_isk)
    end do

    ABI_MALLOC(grad_dir,(npw_isk))
    ...
    do dir=1,3
      ...
      call fft_ug(npw_isk,Wfd%nfft,Wfd%nspinor,ndat1,Wfd%mgfft,Wfd%ngfft,&
&        ISkg%istwf_k,ISkg%kg_k,ISkg%gbound,grad_dir,grad_ur(:,dir))
    end do
    ABI_FREE(ug_isk)
    ABI_FREE(grad_dir)
...
subroutine wfd_dur_isk(Wfd,Cryst,Kmesh,band,ik_bz,spin,npw_k,grottbm1,dur)  !,ISkg,ik_ibz
