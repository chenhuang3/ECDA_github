...
& iproc, kernel, mpi_comm, ngfft, nproc, nscforder)
 ...
 use BigDFT_API, only : coulomb_operator,nullify_coulomb_operator, deallocate_coulomb_operator, mpi_environment
...
!If iaction == 0, we free the kernel.
 if (iaction == 0) then
   if (associated(pkernel%kernel)) then
     ...
     call deallocate_coulomb_operator(pkernel, ABI_FUNC)
   end if
   if (associated(kernelseq%kernel)) then
     write(message, "(A)") "Psolver_kernel() : deallocating kernelseq..."
     call wrtout(std_out, message,'COLL')

     call deallocate_coulomb_operator(kernelseq, ABI_FUNC)
   end if
   data_size = (/ -1, -1, -1 /)
   return
 end if
