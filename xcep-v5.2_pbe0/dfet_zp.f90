...
subroutine dfet_zp(mxband,nfft,nspin,natom,j_atom,ucvol,logf,myrank,do_exx,do_xcep, & 
   ...
   if (dfet_pen_type==1) pulay_beta = 0.1d0
   if (dfet_pen_type==2) pulay_beta = 0.5d0
   if (dfet_mix_type==2) pulay_beta = 0.1d0
   ...
   if(myrank==0) then 
     ...
     if (dfet_mix_type==2) then 
       write(logf,*)'*** density mixing ***'
       write(logf,'(a,L2)')'do_precond: ',do_precond
     endif 
     if (dfet_mix_type==1) then 
     ...
     write(logf,'(a,i3)')    'pen_type:   ',dfet_pen_type
     ...
     if (dfet_pen_type==2) then 
       write(logf,'(a,i3)')    'OFenv -> vwreg_type: ',vwreg_type
       write(logf,'(a,es12.4)')'OFenv -> vw_reg:     ',vw_reg
     endif 
   ...
   if (dfet_mix_type==2) then 
     write(logf,'(a,2es12.4)')'(dfet_zp): initial den_diff: ', & 
     minval(den_diff),maxval(den_diff)
   endif 
   call print_header()
   ...
   ! density mixing (compute uemb for den_diff)
   if (dfet_mix_type==2) then 
     do s=1,nspin 
       call yukawa(n1,n2,n3,1,ucvol,qvec,zp_alpha,den_diff(:,s),vh_tot)
       u(:,s) = zp_coeff*vh_tot
     enddo  
   endif 
   ...
   do while (.true.) 
     ...
     if (dfet_mix_type==2) then 
       ...
       den_diff_old = den_diff
       den_diff = sum_rho-ref_rho

       ! convergence information
       dtmpv1 = sum((den_diff(:,1)-den_diff_old(:,1))**2*ucvol/dble(nfft))
       if (nspin==2) dtmpv2 = sum((den_diff(:,2)-den_diff_old(:,2))**2*ucvol/dble(nfft))

       ! mix it with old one to avoid blow up that is due to large zp_coeff
       den_diff = den_diff_old*(zp_coeff-1.d0)/zp_coeff + den_diff/zp_coeff
       
       ! pulay mixing 
       !=================
       do s=1,nspin
         if (.NOT. do_precond) then 
           call pulay_mix(nfft,iter,den_diff_old(:,s),den_diff(:,s), & 
                          pulay_work(:,:,:,s),npulay,pulay_beta)
         else 
           ! precondition (focus on long-wave length)
           call pulay_mix_kerker_dfet(n1,n2,n3,nfft,iter,den_diff_old(:,s),den_diff(:,s), & 
             pulay_work(:,:,:,s),npulay,pulay_beta,qvec,ucvol)
         endif 
       enddo 
       den_diff_old = den_diff  ! backup old den_diff

       ! compute new uemb based on den_diff
       select case(dfet_pen_type)
       case (1)  
         do s=1,nspin 
           call yukawa(n1,n2,n3,1,ucvol,qvec,zp_alpha,den_diff(:,s),vh_tot)
           u(:,s) = zp_coeff*vh_tot
     ...
     if (dfet_mix_type ==1) then 
       ...
       do s=1,nspin
         ...
         case (2)
           u_new(:,s) = rho_diff - u(:,s)/zp_coeff + u(:,s)
         endselect
       enddo 
...
subroutine  print_header 
   ...
   if (do_envOF) write(logf,'(a)')'*** env is treated by OF-DFT ***'
   ...
   if (dfet_mix_type==1) then 
     if (nspin==1) then 
       if (do_envOF) then 
         ...
         ! non OF treatment for env 
         write(715,'(a)')'iter  max(d_rho)   |d_u|      min(u)    max(u)     e_coul     time(s)  q_clu  q_env'
       endif 
     else
       ! spin polarized 
     ...
     if (myrank==0) then 
      if (nspin==1) then 
        if (do_envOF) then 
          ...
          write(logf,'(2a)')'iter max(d_rho) |d_u|      min(u)   max(u)    e_coul       J_clu      J_env   time(s)'
        endif 
      else
        ! spin polarized 
   ...
   if (dfet_mix_type==2) then 
     if (nspin==1) then 
       write(715,'(a)')'iter  max(d_rho)    |d_rho|    min(u)      max(u)      e_coul     time(s) '
     else
       write(715,'(a,a)')'iter   max(d_rho)  |d_rho(up)| |d_rho(dn)|', & 
       ' min/max(u(up))          min/max(u(dn))          e_coul     time(s) '
     endif 
     call flush(715)
     if (myrank==0) then 
      if (nspin==1) then 
        write(logf,'(2a)')'iter  max(d_rho)    |d_rho|     min(u)      max(u)      e_coul',&
                         '        J_clu       J_env   time(s)'
      else
        write(logf,'(a,a)')'iter   max(d_rho)  |d_rho(up)| |d_rho(dn)|', & 
        ' min/max(u(up))          min/max(u(dn))          e_coul        J_clu       J_env   time(s)'
      endif 
      call flush(logf)
     endif
     ...
     if (myrank==0) then
       if (nspin==2) then 
         ...
         if (do_envOF) then 
           ...
           write(logf,'(i3,es10.2,a,es9.2,a,3es10.2,2f11.5,f8.1)') & 
            iter,d_rho,' ',dtmpv1,' ',minval(u),maxval(u),e_coul,ee_clu,ee_env, & 
            end_time-start_time
         endif 
       endif 
