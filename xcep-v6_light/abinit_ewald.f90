...
!   zion(natom): charge on each atom
!
! OUTPUT
!
...
subroutine abinit_ewald(eew,gmet,grewtn,natom,rmet,ucvol,xred,zion)
...
!Arguments ------------------------------------
 ...
 real(dp),parameter :: two_pi = pi*2.0d0
!scalars 
 integer,intent(in) :: natom
 real(dp),intent(in) :: ucvol
 real(dp),intent(out) :: eew
!arrays 
 real(dp),intent(in) :: gmet(3,3),rmet(3,3),xred(3,natom),zion(natom)
 real(dp),intent(out) :: grewtn(3,natom)

!Local variables-------------------------------
!scalars
 integer :: ia,ib,ig1,ig2,ig3,ir1,ir2,ir3,newg,newr,ng,nr
 real(dp) :: arg,c1i,ch,chsq,derfc_arg,direct,drdta1,drdta2,drdta3,eta,fac,abi_derfc
...
!                Sum real and imaginary parts (avoid complex variables)
                 summr=summr+zion(ia)*cos(arg)
                 summi=summi+zion(ia)*sin(arg)
               end do
...
!                c1r=(phr*summr-phi*summi)*(term*zion(ia))
                 c1i=(phi*summr+phr*summi)*(term*zion(ia))
...
!                  derfc is the real(dp) complementary error function
                   ...
                   sumr=sumr+zion(ia)*zion(ib)*term
                   term=zion(ia)*zion(ib)*&
...
!Finally assemble Ewald energy, eew
 ...
 write(6,'(a,g20.12,a)')'abinit_ewald() -> ewald: ',eew,' Ha'
