...
subroutine solve_total_oep_prec(iter_scf,nfft,cell_nfft,qvec,nspin,reg_type, & 
  ...
  write(logf,'(a)') NEW_LINE('a')//'    >>> enter solve_total_oep_prec() <<<'
  ...
  if (init<0) then
    ...
    cg_z = cg_resid 
    do isp=1,nspin
      call precond(n1,n2,n3,ucvol,reg_lambda,qvec,cg_z(:,isp))
    enddo
    init = 1
  else 
    cg_x = vxc
  ...
  do while (.true.) 
    ...
    cg_beta      = sum(cg_z*cg_resid)/sum(cg_z_old*cg_resid_old) !! Fletcher–Reeves
...
contains 
  ...
  subroutine conv_test(conv)
    ...
    if (iter_cg==40 .or. dtmp<1e-10) then 
...
end subroutine solve_total_oep_prec
